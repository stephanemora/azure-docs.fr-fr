---
title: Vue d’ensemble des machines virtuelles Linux | Microsoft Docs
description: Aide relative à la vue d’ensemble des machines virtuelles Linux au sein du portail Azure
services: virtual-machines-linux
author: sewatson
manager: lwelicki
ms.service: virtual-machines-linux
ms.topic: article
ms.date: 04/27/2017
ms.author: sewatson
ms.openlocfilehash: ff439083ca9c648ceec95b89b041bb5b60785c91
ms.sourcegitcommit: 0cc25b792ad6ec7a056ac3470f377edad804997a
ms.translationtype: HT
ms.contentlocale: fr-FR
ms.lasthandoff: 02/25/2020
ms.locfileid: "77603005"
---
# <a name="linux-virtual-machines-documentation"></a>Documentation sur les machines virtuelles Linux

Les Machines virtuelles Linux Azure fournissent une infrastructure sécurisée et virtualisée à la demande et à grande échelle avec Red Hat, Ubuntu ou la distribution Linux de votre choix. Découvrez comment créer, configurer, gérer et mettre à l’échelle des machines virtuelles Linux avec nos guides de démarrage rapide, nos didacticiels et nos exemples.

## <a name="5-minute-quickstarts"></a>Démarrages en 5 minutes

Découvrez comment déployer un serveur web NGINX au sein d’une machine virtuelle exécutant Ubuntu :

- [Portail Azure](/azure/virtual-machines/virtual-machines-linux-quick-create-portal?toc=%2fazure%2fvirtual-machines%2flinux%2ftoc.json)
- [Azure PowerShell](/azure/virtual-machines/virtual-machines-linux-quick-create-powershell?toc=%2fazure%2fvirtual-machines%2flinux%2ftoc.json)
- [Azure CLI](/azure/virtual-machines/virtual-machines-linux-quick-create-cli?toc=%2fazure%2fvirtual-machines%2flinux%2ftoc.json)

## <a name="step-by-step-tutorials"></a>Didacticiels pas à pas

Découvrez comment déployer, gérer et mettre à l’échelle des machines virtuelles Linux sur Azure.

- [Créer et gérer des machines virtuelles Linux](/azure/virtual-machines/linux/tutorial-manage-vm)
- [Créer et gérer des disques de machine virtuelle](/azure/virtual-machines/linux/tutorial-manage-disks)
- [Automatiser la configuration de machine virtuelle](/azure/virtual-machines/linux/tutorial-automate-vm-deployment)
- [Créer des images de machine virtuelle personnalisées](/azure/virtual-machines/linux/tutorial-custom-images)
- [Créer des machines virtuelles hautement disponibles](/azure/virtual-machines/linux/tutorial-availability-sets)
- [Créer un groupe de machines virtuelles identiques](/azure/virtual-machines/linux/tutorial-create-vmss)
- [Équilibrer la charge des machines virtuelles](/azure/virtual-machines/linux/tutorial-load-balancer)
- [Gérer les machines virtuelles et les réseaux virtuels](/azure/virtual-machines/linux/tutorial-virtual-network)
- [Sauvegarder les machines virtuelles](/azure/virtual-machines/linux/tutorial-backup-vms)
- [Surveiller les machines virtuelles](/azure/virtual-machines/linux/tutorial-monitoring)
- [Gérer la sécurité sur les machines virtuelles](/azure/virtual-machines/linux/tutorial-azure-security)
- [Créer une infrastructure CI/CD avec Jenkins, Docker et GitHub](/azure/jenkins/tutorial-jenkins-github-docker-cicd)

## <a name="free-pluralsight-video-training"></a>Formation vidéo gratuite Pluralsight

- [Administrateur Azure](https://go.microsoft.com/fwlink/?linkid=2012827)

## <a name="samples"></a>Exemples 

Déployez votre première application dans Azure.

- [Azure CLI](/azure/virtual-machines/virtual-machines-linux-cli-samples)
- [Azure PowerShell](/azure/virtual-machines/virtual-machines-linux-powershell-samples)

## <a name="more"></a>En savoir plus

- [Consulter la documentation pour en savoir plus](/azure/virtual-machines/linux/index)
- [Découvrir tous les services Azure](https://aka.ms/j3wr7y)