...
- **Distribution renvoyée des réponses** : réponses que l’API Recherche web a renvoyées dans la réponse.  
  
- **Distribution du serveur réponse** : serveur d’applications ayant servi les demandes de votre API. Les valeurs possibles sont Bing.com (pour le trafic provenant d’appareils de bureau et portables) et Bing.com-mobile (pour le trafic provenant d’appareils mobiles). Le serveur est déterminé à l’aide de la valeur d’en-tête User-Agent de la demande. Si la demande n’inclut pas l’en-tête User-Agent, Bing tente de dériver le serveur à partir d’autres signaux.  
