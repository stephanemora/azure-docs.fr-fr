...
|:-------------------------------|:---------------------------------------------|:--------------------------------------------------------------|
|Afficher la définition                 | Stockée dans un pool SQL dédié              | Stockée dans un pool SQL dédié
|Contenu de la vue                    | Générée chaque fois que la vue est utilisée.   | Prétraitée et stockée dans un pool SQL dédié pendant sa création. Mise à jour à mesure que des données sont ajoutées aux tables sous-jacentes.
...
Les vues matérialisées implémentées dans le pool SQL dédié offrent également les avantages suivants :
...
Les modifications de schéma et de requête dans les pools SQL dédiés sont généralement réduites au minimum pour prendre en charge les opérations ETL standard et la création de rapports.  Les utilisateurs peuvent utiliser des vues matérialisées pour le réglage des performances des requêtes, si le coût engendré par les vues peut être compensé par le gain de performance des requêtes.

En comparaison avec d’autres options de réglage, comme la mise à l’échelle et la gestion des statistiques, la création et la gestion d’une vue matérialisée est une modification de production de moindre impact et son gain de performance potentiel est également plus élevé.
...
Le pool SQL dédié est un système de traitement de requêtes distribuées.  Les données d'une table SQL sont réparties sur 60 nœuds à l'aide de l'une des trois [stratégies de distribution](sql-data-warehouse-tables-distribute.md?toc=/azure/synapse-analytics/sql-data-warehouse/toc.json&bc=/azure/synapse-analytics/sql-data-warehouse/breadcrumb/toc.json) disponibles (hachage, tourniquet ou réplication).   
...
Vous pouvez exécuter cette requête pour générer la liste des vues matérialisées dans un pool SQL dédié :
...
Ces deux fonctionnalités du pool SQL dédié sont utilisées à des fins de réglage des performances des requêtes. La mise en cache du jeu de résultats est utilisée pour obtenir une concurrence élevée et une réponse rapide des requêtes répétitives sur des données statiques.  
...
Revérifiez le plan d’exécution de la requête d’origine.  À présent, le nombre de jointures passe de 17 à 5 et il n’y a pas de lecture aléatoire.  Sélectionnez l’icône de filtrage de l’opération dans le plan, sa liste de sortie indique que les données sont lues à partir des vues matérialisées et non des tables de base.  

 ![Plan_Output_List_with_Materialized_Views](./media/performance-tuning-materialized-views/output-list.png)

Avec les vues matérialisées, la même requête s’exécute plus rapidement, sans aucune modification de code.  

## <a name="next-steps"></a>Étapes suivantes

Pour obtenir des conseils supplémentaires en matière de développement, consultez [Présentation du développement d’un pool SQL dédié](sql-data-warehouse-overview-develop.md).
