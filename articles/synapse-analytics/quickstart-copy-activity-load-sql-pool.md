---
title: Démarrage rapide pour charger des données dans un pool SQL en utilisant l’activité de copie
description: Utiliser Azure Synapse Analytics pour charger des données dans un pool SQL
...
ms.locfileid: "93280759"
---
# <a name="quickstart-load-data-into-sql-pool-using-copy-activity"></a>Démarrage rapide : Charger des données dans un pool SQL en utilisant l’activité de copie
...
Dans ce guide de démarrage rapide, vous allez apprendre à *charger des données depuis Azure SQL Database dans Azure Synapse Analytics*. Vous pouvez procéder de même pour copier des données à partir d’autres types de banques de données. Et le même flux s’applique également à la copie de données entre d’autres sources et récepteurs.
...
* Azure Synapse Analytics : Vous utilisez un pool SQL en tant que magasin de données récepteur. Si vous ne disposez pas d’une instance Synapse Analytics, consultez [Créer un pool SQL](quickstart-create-sql-pool-portal.md) pour savoir comment procéder.
...
Dans Azure Synapse Analytics, un service lié vous permet de définir vos informations de connexion à d’autres services. Dans cette section, vous allez créer les deux types de services liés suivants : Azure SQL Database et Azure Data Lake Storage Gen2.
...
Un pipeline contient le flux logique pour l’exécution d’un ensemble d’activités. Dans cette section, vous allez créer un pipeline contenant une activité de copie qui ingère des données d’Azure SQL Database dans un pool SQL.
...
1. Sélectionnez **Pool SQL Analytics** comme magasin de données, puis sélectionnez **Continuer**.
