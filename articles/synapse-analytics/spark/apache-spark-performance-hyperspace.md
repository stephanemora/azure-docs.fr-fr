...
## <a name="hybrid-scan-for-mutable-datasets"></a>Analyse hybride de jeux de données mutables

Souvent, si vos données sources sous-jacentes ont été complétées par de nouveaux fichiers ou si des fichiers existants ont été supprimés, votre index devient obsolète et Hyperspace décide de ne pas l'utiliser. Cependant, vous voudrez parfois simplement utiliser l'index sans avoir à l’actualiser à chaque fois. Cette erreur peut avoir plusieurs causes :

- Vous ne souhaitez pas actualiser continuellement votre index, mais plutôt le faire périodiquement afin de mieux comprendre vos charges de travail.
- Vous n'avez ajouté/supprimé que quelques fichiers et ne voulez pas attendre la fin d'une autre actualisation.

Pour vous permettre de continuer à utiliser un index obsolète, Hyperspace propose l'analyse hybride, une nouvelle technique qui permet d'utiliser des index obsolètes ou périmés (par exemple, les données sources sous-jacentes ont été complétées par de nouveaux fichiers ou des fichiers existants ont été supprimés) sans actualiser les index.

Pour ce faire, lorsque vous définissez la configuration appropriée pour activer l’analyse hybride, Hyperspace modifie le plan de requête pour tirer parti des modifications de la façon suivante :
* Les fichiers ajoutés peuvent être fusionnés avec les données d’index en utilisant Union ou BucketUnion (pour la jointure). Une sélection aléatoire des données ajoutées peut également être appliquée avant la fusion, si nécessaire.
* Les fichiers supprimés peuvent être traités en injectant la condition Filter-NOT-IN sur la colonne lineage des données d'index, de sorte que les lignes indexées des fichiers supprimés puissent être exclues au moment de la requête.

Vous pouvez vérifier la transformation du plan de requête dans les exemples suivants.

> [!NOTE]
> Actuellement, l’analyse hybride est uniquement prise en charge pour les données non partitionnées.

### <a name="hybrid-scan-for-appended-files---non-partitioned-data"></a>Analyse hybride des fichiers ajoutés - données non partitionnées

Les données non partitionnées sont utilisées dans l'exemple suivant. Dans cet exemple, nous pensons que l'index Join peut être utilisé pour la requête et que BucketUnion est introduit pour les fichiers ajoutés.
...
L’analyse hybride est désactivée par défaut. Par conséquent, vous verrez que puisque nous avons ajouté de nouvelles données, l'Hyperspace décidera de ne *pas* utiliser l'index.

Dans la sortie, vous ne verrez aucune différence de plan (donc, aucune surbrillance).
...
### <a name="enable-hybrid-scan"></a>Activer l’analyse hybride

Dans un plan avec des index, vous pouvez voir le partitionnement de hachage Exchange requis uniquement pour les fichiers ajoutés afin de pouvoir toujours utiliser les données d’index « aléatoires » avec les fichiers ajoutés. BucketUnion est utilisé pour fusionner les fichiers ajoutés « aléatoires » avec les données de l'index.
...
## <a name="incremental-index-refresh"></a>Actualisation incrémentielle de l’index

Lorsque vous êtes prêt à mettre à jour vos index mais que vous ne souhaitez pas reconstruire votre index entier, Hyperspace prend en charge la mise à jour des index de manière incrémentielle à l’aide de l'API `hs.refreshIndex("name", "incremental")`. Il ne sera donc plus nécessaire de reconstruire l'index entier à partir de zéro en utilisant des fichiers d'index créés précédemment et en mettant à jour les index uniquement avec les données nouvellement ajoutées.

Bien entendu, veillez à utiliser l’API `optimizeIndex` complémentaire (voir ci-dessous) régulièrement pour vous assurer que vous ne constatez aucune baisse de performances. Nous vous recommandons de procéder à une optimisation au moins une fois tous les 10 appels `refreshIndex(..., "incremental")`, en supposant que les données que vous avez ajoutées/supprimées représentent < 10 % du jeu de données d’origine. Par exemple, si votre jeu de données d’origine est de 100 Go et que vous avez ajouté/supprimé des données par incréments/décréments de 1 Go, vous pouvez appeler `refreshIndex` 10 fois avant d’appeler `optimizeIndex`. Notez que cet exemple est simplement utilisé à des fins d’illustration et que vous devez l’adapter à vos charges de travail.

Dans l’exemple ci-dessous, notez l’ajout d’un nœud de tri Sort dans le plan de requête lorsque des index sont utilisés. Cela est dû au fait que des index partiels sont créés sur les fichiers de données ajoutés, provoquant l’introduction d’un nœud `Sort` par Spark. Notez également qu’avec `Shuffle`, par exemple, Exchange est toujours éliminé du plan, ce qui vous donne l’accélération appropriée.
...
## <a name="optimize-index-layout"></a>Optimiser la disposition de l’index

Après avoir appelé des actualisations incrémentielles plusieurs fois sur des données récemment ajoutées (par exemple, si l’utilisateur écrit dans des données par petits lots ou dans le cas de scénarios de diffusion en continu), le nombre de fichiers d’index a tendance à augmenter, ce qui affecte les performances de l’index (problème causé par un nombre élevé de petits fichiers). Hyperspace fournit l’API `hyperspace.optimizeIndex("indexName")` permettant d’optimiser la disposition de l’index et de réduire le problème des fichiers volumineux.

Dans le plan ci-dessous, notez que Hyperspace a supprimé un nœud Sort supplémentaire du plan de requête. L’optimisation permet d’éviter le tri des compartiments d’index qui contiennent un seul fichier. Toutefois, cela est vrai uniquement si TOUS les compartiments d’index comportent au maximum 1 fichier par compartiment, après `optimizeIndex`.
...
### <a name="optimize-modes"></a>Modes d’optimisation

Le mode par défaut pour l’optimisation est le mode « rapide » dans lequel les fichiers inférieurs à un seuil prédéfini sont choisis pour l’optimisation. Pour optimiser l’effet de l’optimisation, Hyperspace autorise un autre mode d’optimisation « complet », comme indiqué ci-dessous. Ce mode sélectionne TOUS les fichiers d’index pour l’optimisation, quelle que soit leur taille de fichier, et crée la meilleure disposition possible de l’index. Ce mode est également plus lent que le mode d’optimisation par défaut, car davantage de données sont traitées ici.
