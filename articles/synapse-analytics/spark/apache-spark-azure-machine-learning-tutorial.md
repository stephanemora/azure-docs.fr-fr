...
Pour envoyer une expérience à distance, convertissez votre jeu de données en instance ```TabularDatset``` Azure Machine Learning. [TabularDataset](/python/api/azureml-core/azureml.data.tabulardataset) représente les données sous forme de tableau en analysant les fichiers fournis.
