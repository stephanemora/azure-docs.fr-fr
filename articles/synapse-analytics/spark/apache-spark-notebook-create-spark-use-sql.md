---
title: 'Démarrage rapide : Créer un pool Apache Spark (préversion) dans Azure Synapse Analytics'
...
ms.locfileid: "81420433"
---
# <a name="quickstart-create-an-apache-spark-pool-preview-in-synapse-analytics-using-web-tools"></a>Démarrage rapide : Créer un pool Apache Spark (préversion) dans Synapse Analytics à l’aide d’outils web

Dans ce guide de démarrage rapide, vous allez découvrir comment créer un pool Apache Spark (préversion) dans Azure Synapse Analytics à l’aide des outils web. Vous apprendrez ensuite à vous connecter au pool Apache Spark et à exécuter des requêtes Spark SQL sur des fichiers et des tables. Apache Spark permet une analytique des données et des calculs sur cluster rapides à l’aide du traitement en mémoire. Pour plus d’informations sur Spark sur Synapse Analytics, consultez [Vue d’ensemble : Apache Spark sur Azure Synapse Analytics](apache-spark-overview.md).

> [!IMPORTANT]
> La facturation des instances Spark est calculée au prorata des minutes écoulées, que vous les utilisiez ou non. Veillez à arrêter votre instance Spark une fois que vous avez fini de l’utiliser, ou définissez un délai d’expiration réduit. Pour plus d’informations, consultez la section **Nettoyer les ressources** de cet article.

Si vous ne disposez pas d’abonnement Azure, créez un [compte gratuit](https://azure.microsoft.com/free/) avant de commencer.

## <a name="create-an-apache-spark-pool"></a>Créer un pool Apache Spark

Cet article explique comment créer un pool Apache Spark à l’aide des outils web.
...
1. Dans la vue du portail Azure de l’espace de travail Synapse Analytics que vous souhaitez utiliser, sélectionnez **Lancer Synapse Studio**.
2. Une fois que Synapse Analytics Studio a démarré, sélectionnez **Développer**. Ensuite, pointez sur l’entrée **Notebooks**. Sélectionnez les points de suspension ( **...** ).
3. À partir de là, sélectionnez **Nouveau notebook**. Un notebook est créé et ouvert avec un nom généré automatiquement.
  ![Nouveau notebook](./media/apache-spark-notebook-create-spark-use-sql/spark-get-started-new-notebook.png "Nouveau notebook")
...
9. Maintenant, exécutez la cellule en appliquant l’une des méthodes suivantes :
   ...
   ![Créer un objet de trame de données](./media/apache-spark-notebook-create-spark-use-sql/spark-get-started-create-data-frame-object.png "Sortie du travail Spark")
...
10. Si l’instance du pool Apache Spark n’est pas en cours d’exécution, elle est démarrée automatiquement. Vous pouvez voir l’état de l’instance du pool Apache Spark sous la cellule que vous exécutez, ainsi que dans le panneau d’état au bas du notebook. En fonction de la taille du pool, le démarrage doit prendre de deux à cinq minutes. Une fois l’exécution du code terminée, les informations sous la cellule affichent le temps nécessaire à l’exécution. La sortie s’affiche dans la cellule de sortie.
    ...
    ![Sortie de l’exécution d’une cellule](./media/apache-spark-notebook-create-spark-use-sql/run-cell-with-output.png "Sortie du travail Spark")
...
12. Entrez le code ci-dessous dans une autre cellule, puis exécutez-le pour créer une table Spark, un fichier CSV et un fichier Parquet, tous avec des copies des données :
    ...
    ![Vue de la sortie dans l’explorateur de stockage](./media/apache-spark-notebook-create-spark-use-sql/spark-get-started-default-storage.png "Vue de la sortie dans l’explorateur de stockage")

    ![Vue de la sortie dans l’explorateur de stockage](./media/apache-spark-notebook-create-spark-use-sql/spark-get-started-default-storage2.png "Vue de la sortie dans l’explorateur de stockage")
...
1. Collez le code suivant dans une cellule vide, puis exécutez le code. La commande liste les tables sur le pool.
   ...
   Quand vous utilisez un notebook avec votre pool Apache Spark Synapse Analytics, vous disposez d’un `sqlContext` prédéfini que vous pouvez utiliser pour exécuter des requêtes à l’aide de Spark SQL. `%%sql` indique au notebook qu’il faut utiliser le `sqlContext` prédéfini pour exécuter la requête. La requête extrait les 10 premières lignes d’une table système qui est fournie avec tous les pools Apache Spark Synapse Analytics par défaut.
...
2. Exécutez une autre requête pour afficher les données dans `demo_df`.
    ...
    Par défaut, la vue de résultats affiche une grille, mais il existe un sélecteur de vue sous la grille qui permet de basculer entre les vues de grille et de graphique.

    ![Sortie de requête dans Synapse Analytics Spark](./media/apache-spark-notebook-create-spark-use-sql/spark-get-started-query.png "Sortie de requête dans Synapse Analytics Spark")
...
9. Sélectionnez **Appliquer**.
   ...
   ![Sortie du graphique dans Synapse Analytics Spark](./media/apache-spark-notebook-create-spark-use-sql/spark-get-started-query-chart-output.png "Sortie du graphique dans Synapse Analytics Spark")
...
Synapse Analytics enregistre vos données dans Azure Data Lake Storage. Vous pouvez laisser une instance Spark s’arrêter en toute sécurité quand elle n’est pas utilisée. Vous êtes facturé pour un pool Apache Spark Synapse Analytics tant qu’il est en cours d’exécution, même s’il n’est pas en cours d’utilisation. Étant donné que les frais pour le pool sont bien plus élevés que les frais de stockage, économique, mieux vaut laisser les instances Spark s’arrêter quand elles ne sont pas utilisées.
...
Dans ce guide de démarrage rapide, vous avez découvert comment créer un pool Apache Spark Synapse Analytics et exécuter une requête Spark SQL de base.
...
>[!NOTE]
> Une partie de la documentation Apache Spark officielle repose sur l’utilisation de la console Spark, qui n’est pas disponible sur Azure Synapse Spark. Utilisez à la place les expériences [notebook](../spark/apache-spark-notebook-create-spark-use-sql.md) ou [IntelliJ](../spark/intellij-tool-synapse.md).
