...
- [Tableau de prise en charge](backup-support-matrix-mabs-dpm.md) pour le serveur de sauvegarde Microsoft Azure/sauvegarde DPM
- [Tableau de prise en charge](backup-support-matrix-mars-agent.md) pour la sauvegarde avec l’agent MARS
...
**Action** | **Support**
--- | ---
Activer la sauvegarde quand vous créez une machine virtuelle Azure Windows | Pris en charge pour :  Windows Server 2016 (Datacenter/Datacenter Core) ; Windows Server 2012 R2 DataCenter ; Windows Server 2008 R2 SP1
...
Sauvegarder des disques managés après l’activation d’un verrou de groupe de ressources | Non pris en charge.<br/><br/> La sauvegarde ne peut pas supprimer les anciens points de ressources et les sauvegardes commenceront à échouer une fois la limite maximale de points de restauration atteinte.
...
Ajustement automatique de l’horloge | Non pris en charge.<br/><br/> Sauvegarde Azure ne prend pas en compte l’ajustement automatique de l’horloge pour le passage à l’heure d’été lors de la sauvegarde de machines virtuelles.<br/><br/>  Modifiez la stratégie manuellement en fonction des besoins.
[Fonctionnalités de sécurité pour la sauvegarde hybride](https://docs.microsoft.com/azure/backup/backup-azure-security-feature) |  La désactivation des fonctionnalités n’est pas prise en charge.



## <a name="operating-system-support-windows"></a>Prise en charge des systèmes d’exploitation (Windows)

Le tableau récapitule les systèmes d’exploitation pris en charge lors de la sauvegarde de machines virtuelles Azure Windows.

**Scénario** | **Système d’exploitation pris en charge** 
--- | ---
Sauvegarder avec l’extension de l’agent de machine virtuelle Azure | Client Windows : Non pris en charge<br/><br/> Serveur Windows : Windows Server 2008 R2 ou version ultérieure 
...
Sauvegarder via DPM/MABS | Systèmes d’exploitation pris en charge pour la sauvegarde avec [MABS](backup-mabs-protection-matrix.md) et [DPM](https://docs.microsoft.com/system-center/dpm/dpm-protection-matrix?view=sc-dpm-1807).
...
Fréquence maximale de sauvegarde dans le coffre (extension de machine virtuelle Azure) | 1 par jour
...
Le tableau récapitule la prise en charge de la sauvegarde pendant les tâches de gestion des machines virtuelles, comme l’ajout ou le remplacement de disques de machine virtuelle.
...
Restaurer sur un compte de stockage avec stockage redondant interzone (ZRS) | Non pris en charge
...
Restaurer une machine virtuelle dans un autre réseau virtuel |   Pris en charge.<br/><br/> Le réseau virtuel doit se trouver dans le même abonnement et la même région.
...
Sauvegarder des machines virtuelles dans des [groupes à haute disponibilité](https://docs.microsoft.com/azure/virtual-machines/windows/regions-and-availability#availability-sets) |  Pris en charge.<br/><br/> Vous ne pouvez pas restaurer une machine virtuelle dans un groupe à haute disponibilité en utilisant l’option de création rapide d’une machine virtuelle. Au lieu de cela, quand vous restaurez la machine virtuelle, vous devez restaurer le disque et l’utiliser pour déployer une machine virtuelle, ou bien restaurer un disque et l’utiliser pour remplacer un disque existant. 
...
Taille de disque de données | La taille maximale d’un disque individuel est de 4 095 Go.<br/><br/> Si vous exécutez la dernière version de la sauvegarde de machine virtuelle Azure (appelée « restauration instantanée »), les tailles de disque jusqu’à 4 To sont prises en charge. [Plus d’informations](backup-instant-restore-capability.md)
Type de stockage | HDD Standard, SSD Standard, SSD Premium <br/><br/> SSD Standard est pris en charge si vous exécutez la dernière version de la sauvegarde de machine virtuelle Azure (appelée « restauration instantanée »). [Plus d’informations](backup-instant-restore-capability.md)
...
**Composant** | **Support**
--- | ---
Nombre de cartes réseau | Jusqu’au nombre maximal de cartes réseau prises en charge pour une taille spécifique de machine virtuelle Azure.<br/><br/> Les cartes réseau sont créées lors de la création de la machine virtuelle pendant le processus de restauration.<br/><br/> Le nombre de cartes réseau sur la machine virtuelle restaurée correspond au nombre de cartes réseau présentes sur la machine virtuelle quand vous avez activé la protection. La suppression de cartes réseau après cette activation n’affecte pas leur nombre.
Équilibreur de charge externe/interne |    Pris en charge. <br/></br> [Découvrez plus d’informations](backup-azure-arm-restore-vms.md#restore-vms-with-special-network-configurations) sur la restauration de machines virtuelles avec des paramètres réseau spéciaux.
Plusieurs adresses IP réservées |   Pris en charge. <br/></br> [Découvrez plus d’informations](backup-azure-arm-restore-vms.md#restore-vms-with-special-network-configurations) sur la restauration de machines virtuelles avec des paramètres réseau spéciaux.
Machines virtuelles avec plusieurs cartes réseau  |  Pris en charge. <br/></br> [Découvrez plus d’informations](backup-azure-arm-restore-vms.md#restore-vms-with-special-network-configurations) sur la restauration de machines virtuelles avec des paramètres réseau spéciaux.
Machines virtuelles avec des adresses IP publiques    |  Pris en charge.<br/><br/> Vous devez associer une adresse IP publique existante à la carte réseau, ou bien créer une adresse et l’associer à la carte réseau une fois la restauration effectuée.
...
Traffic Manager | Pris en charge<br/><br/>. Si la machine virtuelle sauvegardée est dans Traffic Manager, vous devez ajouter manuellement la machine virtuelle restaurée au même gestionnaire Traffic Manager. 
...
Points de terminaison de service VNet  |  Pris en charge.<br/><br/> Les paramètres du compte de stockage pour le pare-feu et le réseau virtuel doivent autoriser l’accès depuis tous les réseaux. 



## <a name="vm-securityencryption-support"></a>Prise en charge de la sécurité/du chiffrement des machines virtuelles
