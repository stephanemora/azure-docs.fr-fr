...
- **Une erreur temporaire se produit pendant une commande de requête SQL**
...
Les programmes clients qui rencontrent occasionnellement une erreur temporaire sont plus solides lorsqu’ils contiennent une logique de nouvelle tentative. Si votre programme communique avec SQL Database par le biais d’un intergiciel (middleware) tiers, demandez au fournisseur s’il contient une logique de nouvelle tentative pour les erreurs temporaires.
...
- Ne retentez pas directement une instruction SQL `SELECT` qui a échoué avec une erreur temporaire. Au lieu de cela, établir une nouvelle connexion et relancer l’instruction `SELECT`.
- Si une instruction SQL `UPDATE` échoue avec une erreur temporaire, établissez une nouvelle connexion avant de retenter l’instruction UPDATE. La logique de nouvelle tentative doit s’assurer que la transaction de base de données est complètement terminée ou que la transaction entière a été annulée.
...
Pour en savoir plus sur la période de blocage des clients qui utilisent ADO.NET, consultez [Regroupement de connexions SQL Server (ADO.NET)](https://msdn.microsoft.com/library/8xx3tyca.aspx).
...
Si votre programme client se connecte à SQL Database à l’aide de la classe .NET Framework **System.Data.SqlClient.SqlConnection**, utilisez .NET 4.6.1 ou une version ultérieure (ou .NET Core) afin de pouvoir utiliser la fonctionnalité de nouvelle tentative de connexion. Pour plus d’informations sur la fonctionnalité, consultez [cette page web](https://docs.microsoft.com/dotnet/api/system.data.sqlclient.sqlconnection).
...
Plus précisément, les valeurs que vous choisissez doivent vérifier la formule suivante : Connection Timeout = ConnectRetryCount * ConnectionRetryInterval
...
Sur Windows, l’utilitaire [PortQry.exe](https://www.microsoft.com/download/details.aspx?id=17148) peut s’avérer utile. Voici un exemple d’exécution qui a demandé l’état d’un port sur un serveur SQL Database et qui a été exécuté sur un ordinateur portable :
...
Enterprise Library 6 (EntLib60) offre des classes .NET gérées afin de faciliter la journalisation. Pour plus d’informations, consultez [5 - Un jeu d’enfants : utilisation du bloc d’application de journalisation](https://msdn.microsoft.com/library/dn440731.aspx).

<a id="h-diagnostics-examine-logs-errors" name="h-diagnostics-examine-logs-errors"></a>

### <a name="diagnostics-examine-system-logs-for-errors"></a>Diagnostics : examinez les journaux d’activité d’erreur système
...
- Livre électronique : [Guide du développeur de Microsoft Enterprise Library, 2e édition](https://www.microsoft.com/download/details.aspx?id=41145).
...
- Téléchargement NuGet de [Bibliothèque d’entreprise - Bloc applicatif de gestion des erreurs 6.0 temporaires de Microsoft](https://www.nuget.org/packages/EnterpriseLibrary.TransientFaultHandling/).
