---
title: Règles de pare-feu Azure SQL Database et Data Warehouse | Microsoft Docs
description: Apprenez à configurer un pare-feu Azure SQL Database et SQL Data Warehouse avec des règles de pare-feu au niveau du serveur pour gérer les accès, et à configurer des règles de pare-feu au niveau de la base de données SQL Database.
...
> [!IMPORTANT]
> SQL Data Warehouse prend en charge uniquement les règles de pare-feu au niveau du serveur et ne prend pas en charge les règles de pare-feu au niveau de la base de données.
...
Pour définir une règle de pare-feu au niveau du serveur dans le portail Azure, vous pouvez accéder à la page de présentation de votre base de données Azure SQL Database ou de votre serveur SQL Database.
...
> [!TIP]
> Pour obtenir des exemples PowerShell dans le contexte d’un démarrage rapide, consultez [Créer une base de données à l’aide de PowerShell](sql-database-powershell-samples.md) et [Utiliser PowerShell pour créer une base de données unique SQL Azure et configurer une règle de pare-feu de serveur SQL Database](scripts/sql-database-create-and-configure-database-powershell.md).
...
|[az sql server firewall-rule show](/cli/azure/sql/server/firewall-rule#az-sql-server-firewall-rule-show)|Serveur|Affiche les détails d’une règle de pare-feu|
|[az sql server firewall-rule update](/cli/azure/sql/server/firewall-rule##az-sql-server-firewall-rule-update)|Serveur|Met à jour une règle de pare-feu|
|[az sql server firewall-rule delete](/cli/azure/sql/server/firewall-rule#az-sql-server-firewall-rule-delete)|Serveur|Supprime une règle de pare-feu|
...
> [!TIP]
> Pour obtenir un exemple d’interface de ligne de commande Azure dans le contexte d’un démarrage rapide, consultez [Créer une base de données - Interface de ligne de commande Azure](sql-database-cli-samples.md) et [Utiliser Azure CLI pour créer une base de données unique Azure SQL et configurer une règle de pare-feu](scripts/sql-database-create-and-configure-database-cli.md).
