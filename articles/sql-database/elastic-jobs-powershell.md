...
- Installez la préversion 4.8.1 du module **AzureRM.Sql** pour obtenir les applets de commande de travail élastique les plus récentes. Exécutez les commandes suivantes dans PowerShell avec un accès administrateur.
...
- En plus du module **AzureRM.Sql**4.8.1-préversion, ce tutoriel nécessite également le module PowerShell *sqlserver*. Pour plus d’informations, consultez [Installer le module SQL Server PowerShell](https://docs.microsoft.com/sql/powershell/download-sql-server-ps-module).
...
L’applet de commande **New-AzureRmSqlElasticJobAgent** requiert la préexistence d’une base de données Azure SQL, de sorte que les paramètres *ResourceGroupName*, *ServerName*, et *DatabaseName* doivent tous pointer vers des ressources existantes.
