...
Azure SQL Database vous permet de surveiller facilement l’utilisation de votre base de données, d’y ajouter ou d’en supprimer des ressources (CPU, mémoire, E/S), de détecter ses problèmes potentiels et de recevoir des suggestions pour améliorer ses performances. Azure SQL Database possède de nombreuses fonctionnalités qui peuvent résoudre automatiquement les problèmes de vos bases de données si vous voulez que la base de données s’adapte à votre charge de travail et optimise automatiquement les performances. Cependant, quelques problèmes personnalisés pourraient requérir plus particulièrement votre attention. Cet article explique quelques meilleures pratiques et outils que vous pouvez utiliser pour détecter les problèmes de performance.

La vérification du bon fonctionnement de votre base de données passe par deux grandes catégories d’activités :
- [La surveillance des performance de votre base de données](#monitoring-database-performance) pour vous assurer que les ressources affectées à votre base de données peuvent gérer votre charge de travail. Si vous constatez que vous atteignez les limites de vos ressources, vous devrez identifier les requêtes les plus consommatrices de ressources et les optimiser, ou ajouter des ressources en mettant à niveau le niveau de service.
- [La détection des problèmes de performance](#troubleshoot-performance-issues), pour déterminer pourquoi un problème potentiel s’est produit, identifier la cause profonde du problème et l’action qui permettra de le résoudre.
...
L’analyse des performances d’une base de données SQL dans Azure démarre par l’analyse de l’utilisation des ressources par rapport au niveau de performances que vous avez choisi pour votre base de données. Vous devez surveiller les ressources suivantes :
 - **Utilisation du processeur** : vous devez vérifier si le processeur est utilisé à 100 % sur une longue période. Cela peut indiquer que vous devez probablement mettre à niveau votre base de données ou votre instance, ou identifier et régler les requêtes qui utilisent la majeure partie de la puissance de calcul.
 - **Statistiques d’attente** : vous devez vérifier pourquoi vos requêtes attendent certaines ressources. Par exemple, Queriesmig attend que les données doivent être récupérées ou enregistrées dans les fichiers de la base de données, car une limite de ressources est atteinte, etc.
 - **Utilisation de l’E/S** : vous devez vérifier si vous avez atteint les limites d’E/S du stockage sous-jacent.
 - **Utilisation de la mémoire** : la quantité de mémoire disponible pour votre base de données ou votre instance est proportionnelle au nombre de VCORE, et vous devez vérifier si ce dernier est suffisant pour votre charge de travail. L’espérance de vie de page est l’un des paramètres qui peuvent indiquer si vos pages sont supprimées rapidement de la mémoire.

Azure SQL Database **fournit des conseils qui peuvent vous aider à dépanner et résoudre les éventuels problèmes de performances**. Vous pouvez déceler les opportunités d’amélioration et d’optimisation des performances des requêtes sans modifier les ressources, et ce, en passant en revue les [recommandations de paramétrage des performances](sql-database-advisor.md). Des index manquants et des requêtes optimisées de manière incorrecte sont souvent à l’origine de performances de base de données limitées. Vous pouvez appliquer ces recommandations de paramétrage pour améliorer les performances de votre charge de travail. Vous pouvez également laisser la base de données Azure SQL [optimiser automatiquement les performances de vos requêtes](sql-database-automatic-tuning.md) en appliquant toutes les recommandations identifiées et en vérifiant qu’elles améliorent les performances de la base de données.
...
- Des outils tels que SQL Server Management Studio fournissent de nombreux rapports utiles, tels qu’un [Tableau de bord des performances](https://docs.microsoft.com/sql/relational-databases/performance/performance-dashboard?view=sql-server-2017) où vous pouvez surveiller l’utilisation des ressources et identifier les requêtes consommant le plus de ressources ou un [Magasin de données des requêtes](https://docs.microsoft.com/sql/relational-databases/performance/monitoring-performance-by-using-the-query-store#Regressed), qui vous permet d’identifier les requêtes dont les performances diminuent.
- Utilisez [Query Performance Insight](sql-database-query-performance.md) dans le [Portail Microsoft Azure](https://portal.azure.com) pour identifier les requêtes consommant le plus de ressources. Cette fonctionnalité est uniquement disponible dans la base de données unique et les pools élastiques.
...
- **Problèmes d’exécution** : si le plan de requête est optimal, il atteint probablement des limites de ressources dans la base de données comme le débit d’écriture des logs ou il peut utiliser des index défragmentés qui doivent être reconstruits. Un grand nombre de requêtes simultanées qui dépensent les ressources peuvent également être à l’origine de problèmes d’exécution. Les problèmes **liés à l’attente** sont dans la plupart des cas liés aux problèmes d’exécution, car les requêtes qui ne s’exécutent pas efficacement sont probablement en attente de certaines ressources.

Les causes ou **problèmes liés à l’attente** pourraient être :
- **Blocage** : une requête peut contenir le verrouillage de certains objets de la base de données alors que d’autres essaient d’accéder aux mêmes objets. Vous pouvez facilement identifier les requêtes de blocage à l’aide de DMV ou d’outils de surveillance.
- **Problèmes d’IO** : les requêtes peuvent attendre que les pages soient écrites dans les fichiers de données ou les fichiers journaux. Dans ce cas, vous verrez les statistiques d’attente `INSTANCE_LOG_RATE_GOVERNOR`, `WRITE_LOG` ou `PAGEIOLATCH_*` dans le DMV.
- **Problèmes de TempDB**: si vous utilisez beaucoup de tables temporaires ou si vous voyez beaucoup de déversements de TempDB dans vos plans vos requêtes, vous pourriez avoir un problème avec le débit de TempDB. 
- **Problèmes liés à la mémoire**: il se peut que vous n’ayez pas assez de mémoire pour votre charge de travail, de sorte que votre espérance de vie des pages pourrait diminuer, ou que vos requêtes reçoivent moins de mémoire que nécessaire. Dans certains cas, l’intelligence intégrée dans Query Optimizer résoudra ces problèmes.
