...
1. [Tutoriel sur l’outil de division-fusion de bases de données élastiques](sql-database-elastic-scale-configure-deploy-split-and-merge.md)
2. [Configuration de la sécurité de la division-fusion](sql-database-elastic-scale-split-merge-security-configuration.md)
3. [Configuration de la sécurité de la division-fusion](sql-database-elastic-scale-split-merge-security-configuration.md)
...
- **Fusionner pour réduire**
  ...
  Les besoins en capacité varient en raison du caractère saisonnier de l'entreprise. L'outil vous permet de réduire à un nombre inférieur d'unités lorsque l'activité ralentit. La fonction « fusionner » du service de division-fusion de l'infrastructure élastique traite cette exigence.
...
- **Partition cible (diviser)**
  ...
  Une fois que vous avez fourni les informations sur la source de l’opération de division, vous devez définir l’emplacement de copie des données en fournissant le serveur Azure SQL Database et le nom de la base de données correspondant à la cible.
...
- Les partitions doivent exister et être enregistrées dans la carte de partitions pour qu’une opération de division et de fusion sur ces partitions puisse être effectuée.
...
Le service de division-fusion utilise Azure Diagnostics basé sur Azure SDK 2.5 pour la surveillance et les diagnostics. Vous contrôlez la configuration des diagnostics, comme indiqué ici : [Activation des diagnostics dans Azure Cloud Services et Virtual Machines](../cloud-services/cloud-services-dotnet-diagnostics.md). Le package de téléchargement comprend deux configurations de diagnostic : un pour le rôle Web et un pour le rôle de travail. Elle inclut les définitions permettant d'enregistrer les compteurs de performances, les journaux IIS, les journaux des événements Windows et les journaux des événements de l'application de division-fusion.
...
1. Suivez les étapes de la rubrique [Déployer un service de division-fusion](sql-database-elastic-scale-configure-deploy-split-and-merge.md).
...
4. Mettez à jour votre déploiement de service cloud existant pour la division-fusion dans votre abonnement Azure avec le nouveau package et votre fichier de configuration de service mis à jour.

Il est inutile de configurer une nouvelle base de données de métadonnées de division-fusion pour la mise à niveau. La nouvelle version mettra automatiquement à niveau votre base de données de métadonnées existante vers la nouvelle version.
