...
|---------|---------|---------|---------|
|[La restauration d’une sauvegarde manuelle sans SOMME DE CONTRÔLE peut échouer](#restoring-manual-backup-without-checksum-might-fail)|Mai 2020|Solution de contournement| |
...
### <a name="restoring-manual-backup-without-checksum-might-fail"></a>La restauration d’une sauvegarde manuelle sans SOMME DE CONTRÔLE peut échouer

Dans certains cas, une sauvegarde manuelle de bases de données effectuée sur une instance gérée sans SOMME DE CONTRÔLE peut échouer. Vous devez alors réessayer de restaurer la sauvegarde jusqu’à ce que l’opération réussisse.

**Solution de contournement** : Effectuez des sauvegardes manuelles des bases de données sur une instance gérée avec la SOMME DE CONTRÔLE activée.
