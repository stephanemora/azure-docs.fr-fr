...
description: Créez un serveur et une base de données Azure SQL Database dans le Portail Azure, puis interrogez cette base de données.
...
Une base de données SQL Azure possède un ensemble défini de [ressources de calcul et de stockage](sql-database-service-tiers-dtu.md). Vous créez la base de données dans un [serveur SQL Database](sql-database-features.md) au sein d’un [groupe de ressources Azure](../azure-resource-manager/resource-group-overview.md).
