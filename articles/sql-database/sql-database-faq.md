...
## <a name="is-the-vcore-based-model-available-to-sql-database-managed-instance"></a>Le modèle basé sur des cœurs virtuels est-il disponible pour SQL Database Managed Instance ?
...
## <a name="how-do-i-select-the-right-sku-when-converting-an-existing-database-to-the-new-service-tiers"></a>Comment puis-je sélectionner la référence (SKU) appropriée quand je convertis une base de données existante vers les nouveaux niveaux de service ?
...
## <a name="do-the-new-vcore-based-service-tiers-offer-the-compute-sizes-compatible-with-all-existing-compute-sizes"></a>Les nouveaux niveaux de service basés sur des cœurs virtuels offrent-ils des tailles de capacité de calcul compatibles avec toutes les tailles de capacité de calcul existantes ?

Les nouveaux niveaux de service basés sur des vCores offrent des options de performances comparables pour l’ensemble des pools élastiques et des bases de données utilisant 100 DTU ou plus.  Nous continuerons d’ajouter plusieurs tailles de calcul au fil du temps pour prendre en charge les charges de travail utilisant moins de 100 DTU.

## <a name="are-there-any-database-feature-differences-between-the-existing-dtu-based-and-new-vcore-based-service-tiers"></a>Y a-t-il des différences en matière de fonctionnalités de base de données entre les niveaux de service existants basés sur des DTU et les nouveaux niveaux de service basés sur des cœurs virtuels ?
...
## <a name="will-the-new-vcore-based-tiers-support-point-in-time-restore-pitr-for-35-days-as-today"></a>Les nouveaux niveaux de service basés sur des cœurs virtuels prendront-ils en charge la récupération jusqu`à une date et heure pour 35 jours, comme c’est le cas actuellement ?
...
## <a name="can-i-convert-a-database-from-a-vcore-based-service-tier-to-a-dtu-based-one"></a>Puis-je convertir une base de données d’un niveau de service basé sur des cœurs virtuels en niveau de service basé sur des DTU ?
...
## <a name="how-long-does-it-take-to-change-the-service-tier-or-compute-size-of-a-single-database-or-move-a-database-in-and-out-of-an-elastic-pool"></a>Combien de temps faut-il pour modifier le niveau de service ou la taille de la capacité de calcul d’une base de données ou pour déplacer une base de données dans ou en dehors d’un pool élastique ?

Le changement de niveau de service d’une base de données et le déplacement dans ou vers un pool exige que la base de données soit copiée sur la plateforme en tant qu’opération d’arrière-plan. Le changement de niveau de service peut prendre de quelques minutes à plusieurs heures en fonction de la taille de la base de données. Dans les deux cas, les bases de données restent en ligne et disponibles pendant le déplacement. Pour plus d’informations sur la modification de bases de données uniques, consultez [Modifier le niveau de service d’une base de données](sql-database-single-database-scale.md).

## <a name="when-should-i-use-a-single-database-vs-elastic-pools"></a>Quand convient-il d’utiliser une base de données unique plutôt que des pools élastiques ?

En général, les pools élastiques sont conçus pour un [modèle d’application logiciel en tant que service (SaaS)](sql-database-design-patterns-multi-tenancy-saas-applications.md) standard, où il existe une base de données par client ou par locataire. L’achat de bases de données individuelles et le surprovisionnement destiné à répondre aux pics de demandes variables de chaque base de données ne sont généralement pas rentables. Avec les pools, vous gérez les performances collectives du pool, et les bases de données se mettent à l’échelle automatiquement. Le moteur intelligent d’Azure recommande un pool pour les bases de données quand un modèle d’utilisation l’exige. Pour plus d’informations, consultez [Conseils pour les pools élastiques](sql-database-elastic-pool.md).

## <a name="how-does-the-usage-of-sql-database-using-the-dtu-based-purchasing-model-show-up-on-my-bill"></a>Comment l’utilisation de SQL Database via le modèle d’achat basé sur des DTU apparaît-elle sur ma facture ?
...
## <a name="how-does-elastic-pool-usage-using-the-dtu-based-purchasing-model-show-up-on-my-bill"></a>Comment l’utilisation du pool élastique via le modèle d’achat basé sur des DTU apparaît-elle sur ma facture ?
...
## <a name="how-are-elastic-pool-billed-for-the-dtu-based-purchasing-model"></a>Comment sont facturés les pools élastiques pour le modèle d’achat basé sur des DTU ?
...
- Il est facturé toutes les heures. La fréquence de mesure est identique à celle des tailles de calcul associées aux bases de données uniques.
- Si un pool élastique est redimensionné, il n’est pas facturé selon la nouvelle quantité de ressources tant que l’opération de redimensionnement n’est pas terminée. Le processus est le même que lors de la modification de la taille de calcul des bases de données uniques.
...
## <a name="how-does-the-vcore-based-usage-show-up-in-my-bill"></a>Comment l’utilisation basée sur des cœurs virtuels apparaît-elle sur ma facture ?
...
> [!NOTE]
> Vous ne pouvez pas changer le nom du compte de l’administrateur du serveur après sa création.
...
## <a name="what-is-an-expected-replication-lag-when-geo-secondary-is-created-in-the-same-region-as-the-primary-database"></a>Quel est le décalage de réplication attendu quand la géoréplication secondaire est créée dans la même région que la base de données primaire ?
...
## <a name="if-there-is-a-network-failure-between-two-regions-how-does-the-retry-logic-work-when-geo-replication-is-set-up"></a>En cas de défaillance réseau entre deux régions, comment la logique de nouvelle tentative fonctionne-t-elle quand la géoréplication est configurée ?
...
## <a name="what-tools-are-available-to-monitor-the-replication-lag-between-the-primary-database-and-geo-secondary"></a>Quels outils sont disponibles pour surveiller le décalage de réplication entre la base de données primaire et la géoréplication secondaire ?
