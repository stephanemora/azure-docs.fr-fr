...
* [SQL Server sur les machines virtuelles Azure](https://azure.microsoft.com/services/virtual-machines/sql-server/): SQL Server installé et hébergé dans le cloud sur des machines virtuelles Windows Server s’exécutant sur Azure, également appelé infrastructure en tant que service (IaaS).
  SQL Server sur les machines virtuelles Azure est le même logiciel SQL Server que vous installeriez sur votre serveur local. La seule différence réside dans le fait que ce serveur est désormais une machine virtuelle hébergée dans Azure. SQL Server sur Azure est optimisé pour la migration d’applications SQL Server existantes et toutes les versions et éditions de SQL Server sont disponibles. Il vous permet d’héberger autant de bases de données que nécessaire, prend en charge l’exécution des transactions entre les bases de données et offre un contrôle total de votre SE Windows et instance SQL Server.
