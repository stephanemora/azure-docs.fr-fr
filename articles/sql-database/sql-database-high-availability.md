...
- Une couche de données avec état comprenant les fichiers de base de données (.mdf/.ldf) stockés dans le service Stockage Blob Azure. Le service Stockage Blob Azure garantit qu'aucun enregistrement placé dans un fichier de base de données ne subira de perte de données. Stockage d’objets Blob Azure a intégré Disponibilité/redondance des données qui permet de s’assurer que chaque enregistrement dans le fichier journal ou page dans le fichier de données est conservé même si le processus SQL Server tombe en panne.
