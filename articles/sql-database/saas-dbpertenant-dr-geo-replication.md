...
1. Il surveille le service de base de données SQL pour déterminer le moment du basculement des bases de données. Lorsqu’une base de données de locataire est basculée, le script met à jour le catalogue pour enregistrer l’état de récupération de la base de données du locataire et marquer le locataire en ligne.
