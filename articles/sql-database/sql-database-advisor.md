---
title: Recommandations en performances des conseillers de base de données pour les bases de données uniques et mises en pool
description: Azure SQL Database fournit des recommandations pour les bases de données uniques et mises en pool qui peuvent améliorer les performances des requêtes dans une base de données Azure SQL.
...
- La vignette **Réglage automatique** affiche la **[configuration de réglage automatique](sql-database-automatic-tuning-enable.md)** de votre base de données (options de réglage qui sont appliquées automatiquement à votre base de données). Cliquez sur cette vignette pour ouvrir la boîte de dialogue de configuration de l'automatisation.
- La vignette **Requêtes de base de données** affiche le résumé des performances de requête de votre base de données (utilisation DTU globale et requêtes les plus consommatrices de ressources). La page **[Analyse des performances des requêtes](sql-database-query-performance.md)** s’affiche si vous cliquez sur cette vignette.

## <a name="performance-recommendation-options"></a>Options de recommandations en matière de performances

Les options de recommandations en matière de performances disponibles pour les bases de données uniques et mises en pool dans Azure SQL Database sont :
...
Pour appliquer les recommandations en matière de performances, consultez [Application des recommandations](sql-database-advisor-portal.md#applying-recommendations). Pour afficher l’état des recommandations, consultez [Surveillance des opérations](sql-database-advisor-portal.md#monitoring-operations).
