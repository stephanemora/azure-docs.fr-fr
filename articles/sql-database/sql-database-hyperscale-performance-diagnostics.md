---
title: Diagnostics de résolution des problèmes de performances Hyperscale dans Azure SQL Database | Microsoft Docs
description: Cet article explique comment résoudre les problèmes de performances Hyperscale dans SQL Database.
...
|RBIO_RG_REPLICA        | Se produit quand le taux de génération de journal sur le nœud de calcul d’une base de données Hyperscale est limité en raison d’une consommation de journal retardée par le ou les nœuds de réplica secondaire accessibles en lecture.         |
...
Nous avons ajouté des lectures de serveur de pages à un ensemble de vues de gestion dynamique (DMV) et d’événements étendus pour permettre de mieux distinguer les lectures distantes à partir d’un serveur de pages des lectures logiques.
...
- ActualPageServerReads/ActualPageServerReadAheads sont ajoutés au code XML du plan de requête pour les plans réels.
...
- Sur le réplica de calcul principal, une écriture de journal est représentée dans le fichier file_id 2 de sys.dm_io_virtual_file_stats. Une écriture de journal sur le réplica de calcul principal est une écriture dans la zone d’atterrissage du journal, qui est le stockage Azure Premium distant.
- Sur le réplica secondaire, les enregistrements de journal ne sont pas renforcés sur le réplica secondaire à l’occasion d’une validation ; le journal est appliqué par le service Xlog aux réplicas distants. Sachant que les écritures de journal ne se produisent pas réellement sur les réplicas secondaires et qu’elles servent uniquement au suivi.

## <a name="additional-resources"></a>Ressources supplémentaires

- Pour connaître les limites de ressources vCore pour une base de données Hyperscale unique, consultez [Limites vCore pour le niveau de service Hyperscale](sql-database-vcore-resource-limits-single-databases.md#hyperscale-service-tier-for-provisioned-compute)
