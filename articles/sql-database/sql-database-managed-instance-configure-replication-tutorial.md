---
title: Configurer la réplication transactionnelle entre deux instances managées et SQL Server
...
> [!div class="checklist"]
> - Configurer une instance managée comme un serveur de publication de réplication 
> - Configurer une instance managée comme un serveur de distribution de réplication 
> - Configurer une instance SQL Server comme un abonné 

![Réplication entre une instance managée de serveur de publication SQL, une instance managée de serveur de distribution SQL et un abonné SQL Server](media/sql-database-managed-instance-failover-group-tutorial/sqlmi-to-sql-replication.png)
...
Pour plus d’informations, consultez les articles concernant les [instances managées Azure SQL Database](sql-database-managed-instance-index.yml), les [fonctionnalités](sql-database-managed-instance.md) et la [réplication transactionnelle SQL](sql-database-managed-instance-transactional-replication.md).

Pour configurer la réplication entre un serveur de publication d’instance managée et un abonné à une instance managée, consultez [Configurer la réplication transactionnelle entre deux instances managées](replication-with-sql-database-managed-instance.md). 
...
- Le nom de l’instance managée du serveur de distribution doit être `sql-mi-distributor` (avec quelques caractères en plus pour la randomisation), et il doit se trouver _dans le même réseau virtuel que l’instance managée du serveur de publication_.
   ...
   ![Utiliser le réseau virtuel du serveur de publication pour le serveur de distribution](media/sql-database-managed-instance-configure-replication-tutorial/use-same-vnet-for-distributor.png)

Pour plus d’informations sur la création d’une instance managée, consultez [Créer une instance managée dans le portail](sql-database-managed-instance-get-started.md).
...
Pour plus d’informations sur le déploiement d’une machine virtuelle SQL Server sur Azure, consultez [Guide de démarrage rapide : Créer une machine virtuelle SQL Server](../virtual-machines/windows/sql/quickstart-sql-vm-create-portal.md).
...
Une fois l’appairage VPN effectué, testez la connectivité en lançant SQL Server Management Studio (SSMS) sur votre serveur SQL Server et en vous connectant aux deux instances managées. Pour plus d’informations sur la connexion à une instance managée à l’aide de SSMS, consultez [Utiliser SSMS pour se connecter à une instance managée](sql-database-managed-instance-configure-p2s.md#use-ssms-to-connect-to-the-managed-instance). 

![Tester la connectivité aux instances managées](media/sql-database-managed-instance-configure-replication-tutorial/test-connectivity-to-mi.png)

## <a name="5---create-private-dns-zone"></a>5 - Créer une zone DNS privée

Une zone DNS privée autorise le routage DNS entre les instances managées et le serveur SQL Server. 

### <a name="create-private-dns-zone"></a>Créer une zone DNS privée
...
1. Fournissez un nom arbitraire pour votre zone DNS, par exemple `repldns.com`. 
   ...
   ![Créer une zone DNS privée](media/sql-database-managed-instance-configure-replication-tutorial/create-private-dns-zone.png)
...
### <a name="create-a-record"></a>Créer un enregistrement A
...
1. Indiquez le nom de votre machine virtuelle SQL Server, ainsi que l’adresse IP interne privée. 
   ...
   ![Configurer un enregistrement A](media/sql-database-managed-instance-configure-replication-tutorial/configure-a-record.png)
...
1. Cochez la case en regard de l’option **Activer l’inscription automatique**. 
   ...
   ![Créer une liaison de réseau virtuel](media/sql-database-managed-instance-configure-replication-tutorial/configure-vnet-link.png)
...
## <a name="6---create-azure-storage-account"></a>6 - Créer un compte Stockage Azure
...
1. Lancez SQL Server Management Studio (SSMS) sur votre serveur SQL Server. 
...
1. Dans la page **Sécurité de l’agent**, sélectionnez **Paramètres de sécurité**. Fournissez les informations de connexion SQL Server qui doivent être utilisées pour l’Agent d’instantané et pour vous connecter au serveur de publication. Sélectionnez **OK** pour fermer la page **Sécurité de l’Agent d’instantané**. Sélectionnez **Suivant**. 
   ...
   ![Configurer la sécurité de l’Agent d’instantané](media/sql-database-managed-instance-configure-replication-tutorial/snapshot-agent-security.png)
...
L’utilisation d’une barre oblique au lieu d’une barre oblique inverse dans le chemin de fichier du partage de fichiers peut provoquer cette erreur. Le chemin suivant est correct : `\\replstorage.file.core.windows.net\replshare` Ce chemin peut provoquer l’erreur de système d’exploitation 55 : `'\\replstorage.file.core.windows.net/replshare'`
