...
1. Dans la colonne **Détails**, cliquez sur **Afficher** pour obtenir plus de détails sur un rôle.
   ...
   ![Capture d’écran du volet Afficher les détails du rôle avec l’onglet Autorisations.](./media/role-assignments-portal/select-role-permissions.png)
...
1. Recherchez et sélectionnez les identités managées.
   ...
   ![Capture d’écran du volet Sélectionner des identités managées.](./media/role-assignments-portal/select-managed-identity.png)
...
1. Cliquez sur **Ajouter une condition** si vous voulez affiner davantage les attributions de rôles en fonction des attributs d’objet blob de stockage. Pour plus d’informations, consultez [Ajouter ou modifier des conditions d’attribution de rôle Azure](conditions-role-assignments-portal.md).
   ...
   ![Capture d’écran de la page Ajouter une attribution de rôle avec l’onglet Ajouter une condition.](./media/shared/condition.png)

1. Cliquez sur **Suivant**.

## <a name="step-6-assign-role"></a>Étape 6 : Attribuer le rôle

1. Sous l’onglet **Vérifier + attribuer**, vérifiez les paramètres d’attribution de rôle.

   ![Capture d’écran de la page Attribuer un rôle avec l’onglet Vérifier + attribuer.](./media/role-assignments-portal/review-assign.png)
...
1. Cliquez sur **Vérifier + attribuer** pour attribuer le rôle.
   ...
   Après quelques instants, le principal de sécurité est attribué au rôle dans l’étendue sélectionnée.
    ...
    ![Capture d’écran de la liste d’attributions de rôle après l’attribution du rôle.](./media/role-assignments-portal/rg-role-assignments.png)
...
#### <a name="classic"></a>[Classique](#tab/classic/)

## <a name="step-1-identify-the-needed-scope-classic"></a>Étape 1 : Identifier l’étendue nécessaire (classique)

[!INCLUDE [Scope for Azure RBAC introduction](../../includes/role-based-access-control/scope-intro.md)] Pour plus d’informations, consultez [Comprendre l’étendue](scope-overview.md).

![Diagramme montrant les niveaux d’étendue d’Azure RBAC pour l’expérience classique.](../../includes/role-based-access-control/media/scope-levels.png)
...
1. Cliquez sur la ressource spécifique pour cette étendue.
    ...
    ![Capture d’écran de la page de vue d’ensemble d’un groupe de ressources pour une expérience classique.](./media/shared/rg-overview.png)

## <a name="step-2-open-the-add-role-assignment-pane-classic"></a>Étape 2 : Ouvrir le volet Ajouter une attribution de rôle (classique)
...
1. Cliquez sur **Contrôle d’accès (IAM)**.
    ...
    ![Capture d’écran de la page Contrôle d’accès (IAM) d’un groupe de ressources pour l’expérience classique.](./media/shared/rg-access-control.png)
...
1. Cliquez sur **Ajouter** > **Ajouter une attribution de rôle**.
   ...
   ![Capture d’écran du menu Ajouter > Ajouter une attribution de rôle pour l’expérience classique.](./media/shared/add-role-assignment-menu.png)

1. Sur la page Ajouter une attribution de rôle, cliquez sur **Utiliser l’expérience classique**.

   ![Capture d’écran de la page Ajouter une attribution de rôle avec le lien Utiliser l’expérience classique pour l’expérience classique.](./media/role-assignments-portal/add-role-assignment-page-use-classic.png)

    Le volet Ajouter une attribution de rôle s’ouvre.

   ![Capture d’écran de la page Ajouter une attribution de rôle avec les options Rôle, Attribuer l’accès à et Sélectionner pour l’expérience classique.](./media/role-assignments-portal/add-role-assignment-page.png)

## <a name="step-3-select-the-appropriate-role-classic"></a>Étape 3 : Sélectionner le rôle approprié (classique)
...
1. Dans la liste **Rôle**, recherchez le rôle que vous souhaitez affecter, ou faites-le défiler.
   ...
   ![Capture d’écran de la liste Sélectionner un rôle dans Ajouter une attribution de rôle pour l’expérience classique.](./media/role-assignments-portal/add-role-assignment-role.png)

1. Cliquez ici pour sélectionner le rôle.

## <a name="step-4-select-who-needs-access-classic"></a>Étape 4 : Sélectionner les utilisateurs qui ont besoin d’un accès (classique)
...
1. Dans la liste **Attribuer l’accès à**, sélectionnez le type de principal de sécurité auquel attribuer l’accès.
   ...
   ![Capture d’écran de la sélection d’un principal de sécurité dans Ajouter une attribution de rôle pour l’expérience classique.](./media/role-assignments-portal/add-role-assignment-type.png)
...
1. Dans la section **Sélectionner**, recherchez le principal de sécurité en entrant une chaîne ou en faisant défiler la liste.
   ...
   ![Capture d’écran de la sélection d’un utilisateur dans Ajouter une attribution de rôle pour l’expérience classique.](./media/role-assignments-portal/add-role-assignment-user.png)

1. Cliquez sur le principal de sécurité trouvé pour le sélectionner.

## <a name="step-5-assign-role-classic"></a>Étape 5 : Attribuer un rôle (classique)
...
1. Dans l’onglet **Attributions de rôle**, vérifiez que vous voyez l’attribution de rôle dans la liste.
    ...
    ![Capture d’écran de la liste d’attribution de rôle après l’attribution d’un rôle pour l’expérience classique.](./media/role-assignments-portal/rg-role-assignments.png)
