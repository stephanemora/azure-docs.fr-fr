...
* Fichier de données **bankmarketing_train. csv**. [Téléchargez-le](https://automlsamplenotebookdata.blob.core.windows.net/automl-sample-notebook-data/bankmarketing_train.csv).
...
Pour cette expérience, **VotingEnsemble** est considéré comme le meilleur modèle d’après la métrique **AUC_weighted**. En utilisant le Machine Learning automatisé dans le portail Azure, nous pouvons déployer ce modèle en tant que service web pour effectuer des prédictions sur les nouvelles données. 
...
1. Sélectionnez **Déployer**. Le déploiement prend environ 20 minutes.
...
+ Découvrez comment [utiliser un service web](how-to-consume-web-service.md).
