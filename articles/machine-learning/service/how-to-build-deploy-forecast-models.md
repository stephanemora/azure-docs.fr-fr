...
1. Les comptes et l’application suivants doivent être configurés et installés :
   ...
   - Application Azure Machine Learning Workbench installée

   Si ces trois éléments ne sont pas encore été créés ou installés, suivez le [Guide de démarrage rapide d’Azure Machine Learning et l’installation de Workbench](../service/quickstart-installation.md). 
...
> [Se procurer le bloc-notes Jupyter](https://aka.ms/aml-packages/forecasting/notebooks/financial_forecasting)
...
Vous pouvez désormais calculer les erreurs de prévision sur le jeu de test. Vous avez la possibilité d’utiliser l’erreur absolue moyenne en pourcentage (MAPE, mean absolute percentage error) ici. MAPE est l’erreur absolue moyenne par rapport à des valeurs de ventes réelles. La fonction ```calc_error``` fournit quelques fonctions intégrées pour les métriques d’erreur couramment utilisés. Vous pouvez également définir votre fonction d’erreur personnalisée et la transmettre à l’argument ```err_fun```.
...
Il est intéressant de voir la répartition de ces erreurs sur les 249 séries chronologiques dans les données. Pour afficher la répartition des erreurs, utilisez l’argument `by` dans `calc_error` pour calculer les erreurs de chaque série. Ensuite, créez un diagramme à surfaces pour les visualiser.
...
En général, le modèle Naïf semble formuler de meilleures prévisions malgré certaines valeurs hors norme qui sont moins précises. 

## <a name="build-machine-learning-models"></a>Créer des modèles Machine Learning
Outre les modèles à variable unique classiques, le package Azure Machine Learning de la prévision vous permet également de créer des modèles d’apprentissage automatique. O
...
Le modèle d’apprentissage automatique a été en mesure de tirer parti des caractéristiques ajoutées et des similitudes existantes entre les séries pour obtenir une meilleure précision en matière de prévision.

**Validation croisée et balayage de paramètre**
...
Les pipelines ne sont pas prévus pour le déploiement. L’attribution d’un score au service web déployé reforme le modèle et génère des prévisions sur de nouvelles données. 
...
+ Prendre connaissance de la [vue d’ensemble du package et savoir comment l’installer](https://aka.ms/aml-packages/forecasting).
