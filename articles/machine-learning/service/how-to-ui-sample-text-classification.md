---
title: 'Exemple d’interface visuelle #7 : classifier des critiques de livres'
titleSuffix: Azure Machine Learning
description: Découvrez comment créer un modèle Machine Learning pour classifier des critiques de livres dans différentes catégories.
...
Cet exemple montre comment utiliser des modules d’analyse de texte pour générer un pipeline de classification de texte dans l’interface visuelle d’Azure Machine Learning.
...
[![Graphique de l’expérience](./media/how-to-ui-sample-text-classification/nlp-modules-overall.png)](./media/how-to-ui-sample-text-classification/nlp-modules-overall.png#lightbox)

## <a name="data"></a>Données

Dans cette expérience, nous utilisons le jeu de données **Wikipédia SP 500**. Le jeu de données est dérivé de Wikipédia (https://www.wikipedia.org/) basé sur des articles de chaque entreprise cotée au S&P 500. Avant son chargement dans l’interface visuelle d’Azure Machine Learning, le jeu de données a été traité comme suit :
...
Votre choix de l’algorithme dépend souvent de la configuration requise pour le cas d’usage. Étant donné que l’objectif de cette expérience est de prédire la catégorie d’entreprise, un modèle de classifieur multiclasse constitue un bon choix. Compte tenu du fait que le nombre de caractéristiques est important et que celles-ci sont éparses, nous utilisons un **modèle de régression logistique multiclasse** pour cette expérience.
