...
ms.locfileid: "46989683"
---
#<a name="load-data-with-the-azure-machine-learning-data-prep-sdk"></a>Charger des données avec le kit SDK Azure Machine Learning Data Prep

Le [kit SDK Azure Machine Learning Data Prep](https://docs.microsoft.com/python/api/overview/azure/dataprep?view=azure-dataprep-py) vous permet de charger différents types de données d’entrée. Vous pouvez spécifier le type de fichier de données et ses paramètres, ou utiliser la fonctionnalité de lecture intelligente du kit SDK pour détecter automatiquement le type d’un fichier.

## <a name="read-lines"></a>Lire des lignes
...
## <a name="read-csv"></a>Lire des fichiers CSV
...
## <a name="read-excel"></a>Lire des fichiers Excel
Le kit SDK Azure Machine Learning Data Prep comprend une fonction `read_excel` pour charger des fichiers Excel.
```
...
Vous avez chargé la première feuille du fichier Excel. Vous pouvez obtenir le même résultat en spécifiant explicitement le nom de la feuille que vous souhaitez charger. Si vous voulez plutôt charger la deuxième feuille, vous pouvez indiquer son nom en tant qu’argument.
```
...
Comme vous pouvez le constater, le tableau dans la deuxième feuille comportait des en-têtes et trois lignes vides. Vous devez modifier les arguments de la fonction en conséquence.
```
...
## <a name="read-fixed-width-files"></a>Lire des fichiers de largeur fixe
Pour les fichiers de largeur fixe, vous pouvez spécifier une liste de décalages. La première colonne est toujours supposée démarrer au décalage 0.
...
S’il n’y a aucun en-tête dans les fichiers, vous devez traiter la première ligne en tant que données. En passant `PromoteHeadersMode.NONE` à l’argument header keyword, vous pouvez éviter la détection de l’en-tête et obtenir les données correctes.
...
Pour lire des données à partir d’un serveur SQL Server, créez un objet de source de données contenant les informations de connexion.
...
Après avoir créé un objet de source de données, vous pouvez passer à la lecture des données.
```
...
## <a name="read-from-azure-data-lake-storage"></a>Lire à partir d’Azure Data Lake Storage
...
Vous pouvez utiliser l’interface Azure CLI pour créer un principal de service et le certificat correspondant. Ce principal de service particulier est configuré en tant que Lecteur, avec une portée limitée exclusivement au compte Azure Data Lake Storage « dpreptestfiles »
```
...
##### <a name="configure-an-azure-data-lake-storage-account-for-the-service-principal"></a>Configurez un compte Azure Data Lake Storage pour le principal de service
Pour configurer l’ACL du système de fichiers Azure Data Lake Storage, utilisez l’objectId de l’utilisateur ou, pour cet exemple, le principal de service :
```
az ad sp show --id "8dd38f34-1fcb-4ff9-accd-7cd60b757174" --query objectId
```
##### <a name="configure-read-and-execute-access-for-the-azure-data-lake-storage-file-system"></a>Configurez l’accès en lecture et en exécution pour le système de fichiers Azure Data Lake Storage.
Étant donné que le modèle HDFS ACL sous-jacent ne prend pas en charge l’héritage, vous devez configurer l’ACL individuellement pour les dossiers et les fichiers.
```
