...
Dans cet article, découvrez comment utiliser AMLPCV, le **package Azure Machine Learning de la vision par ordinateur** (Azure Machine Learning Package for Computer Vision), pour former, tester et déployer un modèle de classification d’images. 
...
Le contexte de stockage est utilisé pour déterminer l’endroit où les différents fichiers de sortie, tels que les images augmentées ou les fichiers de modèle DNN, vont être stockés. Pour plus d’informations sur les contextes de stockage, consultez la [documentation de StorageContext](https://review.docs.microsoft.com/en-us/python/api/cvtk.core.context.storagecontext?view=azure-python&branch=smoke-test). 

Normalement, le contenu de stockage ne nécessite pas de configuration explicite. Toutefois, afin d’éviter une limite de taille de projet imposée à 25 Mo par Azure Machine Learning Workbench, définissez le répertoire de sortie du package Azure Machine Learning de la vision par ordinateur sur un emplacement situé en dehors du projet Azure Machine Learning ("../../../../cvtk_output"). N’oubliez pas de supprimer le répertoire "cvtk_output" lorsque vous n’en avez plus besoin.
