...
Le processus CI des notebooks Python récupère le code de la branche de collaboration (par exemple, ***maître** _ ou _*_développement_**) et effectue les activités suivantes :
* Linting de code
* Test des unités
...
1. Les ingénieurs des données fusionnent le code source de leurs branches de fonctionnalités dans la branche de collaboration, par exemple ***maître** _ ou _*_développement_**. 
...
Le pipeline Azure de CD est constitué de plusieurs index représentant les environnements. Chaque index contient des [déploiements](/azure/devops/pipelines/process/deployment-jobs) et des [tâches](/azure/devops/pipelines/process/phases?tabs=yaml) qui effectuent les étapes suivantes :
...
Les index du pipeline peuvent être configurés avec des [approbations](/azure/devops/pipelines/process/approvals?tabs=check-pass) et des [portes](/azure/devops/pipelines/release/approvals/gates) qui offrent un contrôle supplémentaire sur la façon dont le processus de déploiement évolue dans la chaîne d’environnements.

### <a name="deploy-a-python-notebook"></a>Déployer un notebook Python

L’extrait de code suivant définit un [déploiement](/azure/devops/pipelines/process/deployment-jobs) de pipeline Azure qui copie un notebook Python sur un cluster Databricks :
...
Les artefacts produits par l’intégration continue sont automatiquement copiés vers l’agent de déploiement et sont disponibles dans le dossier `$(Pipeline.Workspace)`. Dans ce cas, la tâche de déploiement fait référence à l’artefact `di-notebooks` contenant le notebook Python. Ce [déploiement](/azure/devops/pipelines/process/deployment-jobs) utilise l’[extension Databricks Azure DevOps](https://marketplace.visualstudio.com/items?itemName=riserrad.azdo-databricks) pour copier les fichiers du notebook dans l’espace de travail Databricks.
...
Il contient un nombre d’index ***Déployer** _ égal au nombre d’environnements cibles. Chaque index _ *_Déployer_** contient deux [déploiements](/azure/devops/pipelines/process/deployment-jobs) qui s’exécutent en parallèle, et un [travail](/azure/devops/pipelines/process/phases?tabs=yaml) qui s’exécute après les déploiements pour tester la solution sur l’environnement.
