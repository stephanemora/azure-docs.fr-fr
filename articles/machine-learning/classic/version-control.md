...
description: Appliquer les bonnes pratiques d’Application Lifecycle Management dans Machine Learning Studio (classique)
...
ms.locfileid: "112583443"
---
# <a name="application-lifecycle-management-in-machine-learning-studio-classic"></a>Application Lifecycle Management dans Machine Learning Studio (classique)
...
Machine Learning Studio (classique) est un outil permettant de développer les expériences de machine learning qui seront utilisées sur la plateforme cloud Azure. Cet outil est similaire au service cloud évolutif et à l’environnement IDE de Visual Studio, tous deux regroupés au sein d’une même plate-forme. Vous pouvez incorporer dans Machine Learning Studio (classique) des pratiques de gestion du cycle de vie des applications (ALM, Application Lifecycle Management) standard, notamment le versioning de plusieurs ressources ainsi que l’exécution et le déploiement automatisés. Cet article traite de certaines de ces options et approches.
...
Selon le Modèle d’exécution de l’expérience Machine Learning Studio (classique), un instantané immuable de l’expérience est envoyé au planificateur de travaux à chaque fois que vous cliquez sur le bouton **Exécuter** dans l’éditeur d’expérience. Pour afficher cette liste d’instantanés, cliquez sur le bouton **Historique des exécutions** dans la barre de commandes de la vue de l’éditeur d’expérience.
...
Une fois ouvert, vous pouvez enregistrer l’instantané de l’expérience en tant que nouvelle expérience et le modifier. Si l’instantané de votre expérience contient des ressources telles que des modèles formés, des transformations, des jeux de données présentant des versions mises à jour, l’instantané conserve les références à la version d’origine lors de la création de l’instantané. Si vous enregistrez l’instantané verrouillé en tant que nouvelle expérience, Machine Learning Studio (classique) détecte l’existence d’une version plus récente de ces ressources et les met automatiquement à jour dans la nouvelle expérience.
...
Les instantanés de l’historique d’exécution conservent une version immuable de l’expérience dans Machine Learning Studio (classique) chaque fois qu’elle est soumise à des fins d’exécution. Vous pouvez également enregistrer une copie locale de l’expérience et l’archiver dans votre système de contrôle de code source préféré, par exemple Team Foundation Server, puis recréer une expérience à partir de ce fichier local. Pour ce faire, vous pouvez utiliser les applets de commande PowerShell [*Export-AmlExperimentGraph*](https://github.com/hning86/azuremlps#export-amlexperimentgraph) et [*Import-AmlExperimentGraph*](https://github.com/hning86/azuremlps#import-amlexperimentgraph)[de Machine Learning Studio (classique)](https://aka.ms/amlps).
...
Vous pouvez déployer deux types de services web à partir d’une expérience Machine Learning Studio (classique). Le service web classique est étroitement lié à l’expérience, ainsi qu’à l’espace de travail. Le nouveau service web s’appuie sur l’infrastructure Azure Resource Manager ; il n’est plus associé à l’expérience d’origine, ni à l’espace de travail.
...
Un aspect important d’ALM consiste à automatiser l’exécution et le déploiement de l’application. Pour ce faire, vous pouvez utiliser le [module PowerShell](https://aka.ms/amlps) dans Machine Learning Studio (classique). Voici un exemple d’étapes de bout en bout qui correspondent à un processus standard d’exécution/de déploiement ALM automatisé à l’aide du [module PowerShell Learning Studio (classique)](https://aka.ms/amlps). Chaque étape est liée à une ou plusieurs applets de commande PowerShell, que vous pouvez utiliser pour exécuter l’étape en question.
...
* Téléchargez le module [PowerShell Machine Learning Studio (classique)](https://aka.ms/amlps) et commencez à automatiser vos tâches ALM.
...
* Découvrez plus d’informations sur le [déploiement de services web Machine Learning](deploy-a-machine-learning-web-service.md).