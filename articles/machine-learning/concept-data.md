...
4. Créer des moniteurs de jeux de données pour le jeu de données de sortie de votre modèle afin de détecter la dérive de données. 
...
Pour accéder à vos données dans votre compte de stockage, Azure Machine Learning propose des magasins de données et des jeux de données. Les magasins de données répondent à la question : comment puis-je me connecter en toute sécurité à mes données qui se trouvent dans mon Stockage Azure ? Les magasins de données fournissent une couche d’abstraction sur votre service de stockage. Cela contribue à la sécurité et à la facilité d’accès à votre stockage, car les informations de connexion sont conservées dans le magasin de données et ne sont pas exposées dans des scripts. 

Les jeux de données répondent à la question : comment puis-je obtenir des fichiers de données spécifiques dans mon magasin de données ? Les jeux de données pointent vers les fichiers spécifiques dans votre stockage sous-jacent que vous souhaitez utiliser pour votre expérience de Machine Learning. Ensemble, les magasins de données et les jeux de données offrent un workflow de distribution de données sécurisé, scalable et reproductible pour vos tâches de Machine Learning.
