...
Pour utiliser la génération de schéma, incluez le package `inference-schema` open source version 1.1.0 ou ultérieure dans votre fichier de dépendances. Pour plus d’informations sur ce package, consultez [https://github.com/Azure/InferenceSchema](https://github.com/Azure/InferenceSchema). Pour générer un swagger conforme pour une consommation de service web automatisé, la fonction run() du script de scoring doit avoir la forme d’API suivante :
* Un premier paramètre de type « StandardPythonParameterType », nommé **inputs** et imbriqué.
* Un deuxième paramètre facultatif de type « StandardPythonParameterType », nommé **GlobalParameter**.
