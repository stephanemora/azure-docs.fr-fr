...
+ Pour déployer des ressources dans un réseau virtuel ou un sous-réseau, votre compte d’utilisateur doit disposer d’autorisations pour les actions suivantes dans le contrôle d’accès en fonction du rôle Azure (Azure RBAC) :
    ...
    - « Microsoft.Network/virtualNetworks/*/read » sur la ressource de réseau virtuel.
...
> * Si le ou les comptes de stockage Azure pour l’espace de travail sont également sécurisés dans un réseau virtuel, ils doivent se trouver dans le même réseau virtuel ou sous-réseau que l’instance ou le cluster de capacité de calcul Azure Machine Learning. Configurez les paramètres de votre pare-feu de stockage pour permettre la communication avec le réseau virtuel et le sous-réseau dans lequel réside le calcul. Veuillez noter que le fait de cocher la case « Autoriser les services Microsoft approuvés à accéder à ce compte » ne suffit pas à autoriser la communication depuis le calcul.
