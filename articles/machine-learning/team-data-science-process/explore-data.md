---
title: Exploration des données dans le processus TDSP (Team Data Science Process)
description: Explorez les données dans des environnements de stockage différents généralement utilisés dans le processus TDSP (Team Data Science Process).
services: machine-learning
author: marktab
manager: marktab
editor: marktab
ms.service: machine-learning
ms.subservice: team-data-science-process
ms.topic: article
ms.date: 01/10/2020
ms.author: tdsp
ms.custom: seodec18, previous-author=deguhath, previous-ms.author=deguhath
ms.openlocfilehash: ca993fd6a449b2655b13da337875fa3f9f27cade
ms.sourcegitcommit: 2ec4b3d0bad7dc0071400c2a2264399e4fe34897
ms.translationtype: HT
ms.contentlocale: fr-FR
ms.lasthandoff: 03/27/2020
ms.locfileid: "76721266"
---
# <a name="explore-data-in-the-team-data-science-process"></a>Exploration des données dans le processus TDSP (Team Data Science Process)

L’exploration des données est une étape du [processus TDSP (Team Data Science Process)](overview.md).

Les articles suivantes décrivent la façon d’explorer les données dans trois environnements de stockage différents généralement utilisés dans le processus de sciences des données :

* Explorez les données de [conteneurs d’objets blob Azure](explore-data-blob.md) à l’aide du package Python [Pandas](https://pandas.pydata.org/) .
* Explorez [SQL Server](explore-data-sql-server.md) à l’aide de SQL et d’un langage de programmation comme Python.
* Explorez les données de [tables Hive](explore-data-hive-tables.md) à l’aide de requêtes Hive.

En outre, la vidéo [Prétraitement des données dans Azure Machine Learning Studio](https://azure.microsoft.com/documentation/videos/preprocessing-data-in-azure-ml-studio/) décrit les modules couramment utilisés pour le nettoyage et la transformation des données.
