...
## <a name="training-and-experimentation-issues"></a>Problèmes de formation et d’expérimentation
...
### <a name="horovod-has-been-shut-down"></a>Horovod a été arrêté
Dans la plupart des cas, si vous rencontrez le message d’erreur « AbortedError : Horovod a été arrêté », cette exception signifie qu’une exception sous-jacente dans l’un des processus a entraîné l’arrêt de Horovod. Chaque rang dans le travail MPI obtient son propre fichier journal dédié dans Azure ML. Ces journaux sont nommés `70_driver_logs`. Dans le cas d’une formation distribuée, les noms de journaux sont suivis du suffixe `_rank` pour faciliter leur différenciation. Pour trouver l’erreur exacte qui a provoqué l’arrêt de Horovod, parcourez tous les fichiers journaux et recherchez `Traceback` à la fin des fichiers driver_log. L’un de ces fichiers indiquera l’exception sous-jacente réelle. 

### <a name="sr-iov-availability-on-ncv3-machines-in-amlcompute-for-distributed-training"></a>Disponibilité de SR-IOV sur des machines NCv3 dans AmlCompute pour une formation distribuée
Azure Compute a déployé une mise à niveau [SR-IOV](https://azure.microsoft.com/updates/sriov-availability-on-ncv3-virtual-machines-sku/) des machines NCv3, que les clients peuvent exploiter avec l’offre de calcul gérée d’Azure ML (AmlCompute). Les mises à jour permettront la prise en charge de l’ensemble de la pile MPI et de l’utilisation du réseau RDMA InfiniBand pour améliorer les performances de formation distribuée sur plusieurs nœuds, en particulier pour le Deep Learning.

Affichez la [planification de mise à jour](https://azure.microsoft.com/updates/sr-iov-availability-schedule-on-ncv3-virtual-machines-sku/) pour voir à quel moment la prise en charge sera déployée pour votre région.
...
## <a name="azure-machine-learning-compute-issues"></a>Problèmes relatifs à Capacité de calcul Azure Machine Learning
Problèmes connus liés à l’utilisation de Capacité de calcul Azure Machine Learning (AmlCompute).

### <a name="trouble-creating-amlcompute"></a>Problèmes de création d’une capacité AmlCompute
...
### <a name="import-error-cannot-import-name-timedelta-from-pandas_libstslibs"></a>Erreur d'importation : impossible d'importer le nom « Timedelta » à partir de « pandas._libs.tslibs »

Si cette erreur apparaît lorsque vous utilisez le Machine Learning automatisé, exécutez les deux lignes suivantes dans votre notebook :
```
