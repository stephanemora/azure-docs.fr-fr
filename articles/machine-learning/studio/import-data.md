---
title: Importer les données de formation
...
* **Expérience Machine Learning Studio (classique)**  : utiliser les données qui ont été enregistrées en tant que jeu de données dans la version classique de Machine Learning Studio
...
> [!NOTE]
> Un certain nombre d’exemples de jeux de données sont disponibles dans la version classique de Machine Learning Studio et vous pouvez les utiliser comme données de formation. Pour plus d’informations, voir [Utilisation des exemples de jeux de données dans Azure Machine Learning Studio (classique)](use-sample-datasets.md).

## <a name="prepare-data"></a>Préparer les données

La version classique de Machine Learning Studio est conçue pour travailler avec des données tabulaires ou rectangulaires, comme des données texte délimitées ou structurées à partir d’une base de données, bien que dans certains cas des données non rectangulaires puissent être utilisées.

Il est préférable que vos données soient relativement propres avant leur importation dans la version classique de Studio. Par exemple, vous souhaitez prendre en charge des problèmes tels que des chaînes sans guillemets.

Toutefois, des modules sont disponibles dans la version classique de Studio, qui permettent d’effectuer certaines manipulations de données dans votre expérience après importation de vos données. En fonction des algorithmes d’apprentissage automatique que vous allez utiliser, vous devrez décider comment gérer les problèmes structurels des données tels que des valeurs manquantes et des données fragmentées. Certains modules existent pour vous aider à régler ces problèmes. Rechercher dans la section **Transformation des données** de la palette des modules ceux qui exécutent ces fonctions.

À tout stade de votre expérience, vous pouvez voir ou télécharger les données générées par un module en cliquant sur le port de sortie. En fonction du module, différentes options de téléchargement peuvent être disponibles. Vous pouvez également afficher les données dans votre navigateur web dans la version classique de Studio.
...
Si vous importez des données dans un format tel que ARFF qui inclut des métadonnées, la version classique de Studio utilise celles-ci pour définir le titre et le type de données de chaque colonne.

Si vous importez des données dans des formats tels que TSV ou CSV qui n’incluent pas ces métadonnées, la version classique de Studio déduit le type de données de chaque colonne en échantillonnant les données. Si les données n’ont pas non plus de titre de colonne, la version classique de Studio fournit des noms par défaut.
...
Les types de données reconnus par la version classique de Studio sont les suivants :
...
Les modules de la version classique de Machine Learning Studio prennent en charge les jeux de données d’une taille maximale de 10 Go de données numériques denses pour les scénarios d’utilisation courants. Si un module accepte plusieurs entrées, le total de toutes les tailles d’entrée est de 10 Go. Vous pouvez échantillonner des jeux de données plus importants par le biais de requêtes Hive ou Azure SQL Database ou encore via un prétraitement Learning by Counts avant d'importer les données.  
...
Vous pouvez charger un fichier de données à partir de votre disque dur pour vous en servir comme données d’apprentissage dans la version classique de Studio. Lorsque vous importez un fichier de données, vous créez un module de jeu de données prêt à être utilisé dans des expériences au sein de votre espace de travail.
...
Le module **Importer les données** de la version classique d’Azure Machine Learning Studio prend en charge les sources de données suivantes :
