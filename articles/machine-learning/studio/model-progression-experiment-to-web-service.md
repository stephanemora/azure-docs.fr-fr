---
title: Comment un modèle Machine Learning Studio devient un service web - Azure | Microsoft Docs
...
* Entraîner un modèle à l’aide d’algorithmes de machine learning
* Noter le modèle
...
### <a name="the-training-experiment"></a>Expérience d’entraînement
L’***expérience d’entraînement*** est la phase initiale de développement de votre service web dans Machine Learning Studio. L’expérience d’entraînement vous permet de développer, tester, itérer et entraîner un modèle Machine Learning. Vous pouvez même entraîner plusieurs modèles simultanément en recherchant la meilleure solution possible ; mais une fois que vous aurez terminé votre expérience, vous devrez sélectionner un seul modèle entraîné et supprimer le reste de l’expérience. Pour obtenir un exemple de développement d’une expérience d’analyse prédictive, consultez [Guide pas à pas : développer une solution d’analyse prédictive pour l’évaluation des risques de crédit dans Azure Machine Learning](walkthrough-develop-predictive-solution.md).

### <a name="the-predictive-experiment"></a>Expérience prédictive
Après avoir entraîné un modèle dans votre expérience d’entraînement, cliquez sur **Configurer le service web** puis sélectionnez **Service web prédictif** dans Machine Learning Studio pour commencer le processus de conversion de votre expérience d’entraînement en ***expérience prédictive***. L’objectif de l’expérience prédictive consiste à utiliser votre modèle entraîné afin d’évaluer les nouvelles données, ce dans le but de devenir opérationnel sous la forme d’un service web Azure.
...
Dans ce processus de conversion, l’expérience d’entraînement n’est pas supprimée. Une fois le processus terminé, deux onglets s’affichent dans Studio : le premier pour l’expérience d’entraînement, le second pour l’expérience prédictive. Ainsi, avant de déployer votre service web, vous pouvez apporter des modifications à l’expérience d’entraînement et reconstruire l’expérience prédictive. Vous pouvez également enregistrer une copie de l’expérience d’entraînement pour démarrer une autre ligne de l’expérimentation.

> [!NOTE]
> Lorsque vous cliquez sur **Service web prédictif**, vous démarrez un processus automatique permettant de convertir votre expérience d’entraînement en expérience prédictive. Ce processus fonctionne bien dans la plupart des cas. Si vous avez défini une expérience d’entraînement complexe (par exemple, si vous avez plusieurs chemins pour des entraînements que vous souhaitez relier entre eux), vous préférerez peut-être effectuer cette conversion manuellement. Pour plus d’informations, consultez [Guide pratique pour préparer votre modèle pour le déploiement dans Azure Machine Learning Studio](convert-training-experiment-to-scoring-experiment.md).
> 
> 
...
Si votre expérience n’entraîne pas un modèle d’analytique prédictive, vous n’avez pas besoin de créer une expérience d’entraînement et une expérience de scoring ; vous ne disposez que d’une seule expérience, que vous pouvez déployer en tant que service web. Machine Learning Studio détecte si votre expérience contient ou non un modèle prédictif en analysant les modules que vous avez utilisés.
...
**Vous souhaitez réentraîner le modèle avec de nouvelles données**
...
1. **Réentraîner le modèle pendant l’exécution du service web** : si vous souhaitez réentraîner votre modèle pendant l’exécution du service web prédictif, vous pouvez procéder en apportant quelques modifications à l’expérience d’entraînement pour en faire une ***expérience de réentraînement***, puis le déployer en tant que ***service web de réentraînement***. Pour connaître la procédure à suivre, consultez la page [Réentraîner des modèles Machine Learning programmatiquement](retrain-models-programmatically.md).
2. **Revenir à l’expérience d’entraînement d’origine et utiliser d’autres données d’entraînement pour développer votre modèle** : votre expérience prédictive est liée au service web, mais l’expérience d’entraînement n’est pas directement liée de cette façon. Si vous modifiez l’expérience d’entraînement d’origine, cliquez sur **Configurer le service web**. Vous obtiendrez alors une *nouvelle* expérience prédictive qui, une fois déployée, créera un *nouveau* service web. L’opération ne se résume pas à mettre simplement à jour le service web d’origine.
   
   Si vous devez modifier l’expérience d’entraînement, ouvrez-la et cliquez sur **Enregistrer sous** pour effectuer une copie. Cette précaution garantira l’intégrité de l’expérience d’apprentissage d’origine, de l’expérience prédictive et du service web. Vous pouvez désormais créer un service web avec vos modifications. Après avoir déployé le nouveau service web, vous pouvez alors décider d’arrêter le service web précédent ou de le maintenir en exécution simultanée.

**Vous souhaitez entraîner un modèle différent**

Si vous souhaitez apporter des modifications à votre expérience prédictive d’origine (par exemple, sélection d’un autre algorithme d’apprentissage automatique, test d’une méthode d’entraînement différente, etc.), vous devez suivre la deuxième procédure décrite ci-dessus pour réentraîner votre modèle : ouvrez l’expérience d’entraînement, cliquez sur **Enregistrer sous** pour effectuer une copie, puis commencez à développer votre modèle, à créer l’expérience prédictive et à déployer le service web. Cela créera un nouveau service web non lié à l’original. Vous pouvez alors décider du ou des services qui continueront à fonctionner.
...
* réentraînement du modèle : [Réentraîner des modèles Machine Learning programmatiquement](retrain-models-programmatically.md)
