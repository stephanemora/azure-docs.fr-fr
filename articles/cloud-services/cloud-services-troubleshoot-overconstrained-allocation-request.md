---
title: Résolution des exceptions OverconstrainedAllocationRequest lors du déploiement d’un service cloud (classique) sur Azure | Microsoft Docs
description: Cet article explique comment résoudre une exception OverconstrainedAllocationRequest lors du déploiement d’un service cloud (classique) sur Azure.
...
ms.locfileid: "113090058"
---
# <a name="troubleshoot-overconstrainedallocationrequest-when-deploying-cloud-services-classic-to-azure"></a>Résolution des exceptions OverconstrainedAllocationRequest lors du déploiement d’un service cloud (classique) sur Azure

Dans cet article, vous allez résoudre les échecs d’allocation trop contrainte qui empêchent le déploiement d’Azure Cloud Services (Classic).
...
![Image affichant le panneau Journal des opérations (classique).](./media/cloud-services-troubleshoot-overconstrained-allocation-failed/cloud-services-troubleshoot-allocation-logs.png)

|Type d’exception  |Message d’erreur  |
|---------|---------|
|OverconstrainedAllocationRequest |La taille de machine virtuelle (ou la combinaison de tailles de machines virtuelles) requise par ce déploiement ne peut pas être provisionnée en raison des contraintes de la demande de déploiement. Si possible, essayez de relâcher des contraintes comme les liaisons de réseau virtuel, d’effectuer le déploiement dans un service hébergé sans aucun autre déploiement et dans un autre groupe d’affinités ou sans groupe d’affinités, ou bien dans une autre région.|

## <a name="cause"></a>Cause

La cause racine varie selon que le service cloud est **épinglé** ou **non épinglé**.

- [**Non épinglé :** Échecs d’un premier déploiement d’un nouveau service cloud (classique)](#not-pinned-to-a-cluster)
- [**Épinglé :** Échecs d’un service cloud existant (classique)](#pinned-to-a-cluster)
...
>
> Au fil du temps, il arrive que les ressources du cluster soient entièrement exploitées. Si un service cloud (classique) effectue une demande d’allocation de ressources supplémentaires lorsque les ressources disponibles dans le cluster épinglé sont insuffisantes, la demande aboutit à un [échec d’allocation](cloud-services-allocation-failures.md).
