...
Le runtime IoT Edge est une collection de programmes dont l’activation d’un appareil dans un appareil IoT Edge. Collectivement, les composants du runtime IoT Edge permettent aux appareils IoT Edge recevoir le code à exécuter à la périphérie et communiquent les résultats. 
...
Les responsabilités du runtime IoT Edge se répartissent en deux catégories : communication et gestion des modules. Ces deux rôles sont effectuées par deux composants qui font partie du runtime IoT Edge. Le *hub IoT Edge* est responsable de la communication, tandis que le *agent IoT Edge* déploie et surveille les modules. 
...
>[!NOTE]
> Hub IoT Edge prend en charge les clients qui se connectent à l’aide de MQTT ou AMQP. Il ne prend pas en charge les clients qui utilisent HTTP. 
...
* **settings.createOptions** – chaîne qui est transmise directement au démon du conteneur Moby lors du démarrage du conteneur d’un module. Ajout d’options dans cette propriété permet des configurations avancées telles que de transfert de port ou le montage de volumes dans le conteneur d’un module.  
* **status** : état dans lequel l’agent IoT Edge place le module. En règle générale, cette valeur est définie sur *en cours d’exécution* comme la plupart des gens que l’agent de IoT Edge pour démarrer immédiatement tous les modules sur l’appareil. Toutefois, vous pouvez spécifier l’arrêt comme état initial d’un module, et demander ultérieurement à l’agent IoT Edge de démarrer le module. L’agent IoT Edge signale l’état de chaque module au cloud dans les propriétés déclarées. Une différence entre la propriété souhaitée et la propriété rapportée est un indicateur du dysfonctionnement de l’appareil. Les états pris en charge sont :
   * Downloading
   * Exécution
   * Unhealthy
   * Échec
...
* **restartPolicy** : indique la façon dont l’agent IoT Edge redémarre un module. Les valeurs possibles incluent :
   ...
   * `on-failure` -Si le module tombe en panne, l’agent IoT Edge le redémarre. Si le module se ferme correctement, l’agent IoT Edge ne le redémarre pas.
   * `on-unhealthy` -Si le module se bloque ou est considéré comme défectueux, l’agent IoT Edge le redémarre.
   * `always` -Si le module se bloque, est considéré comme défectueux ou s’arrête en aucune façon, l’agent IoT Edge le redémarre. 
...
L’agent IoT Edge envoie la réponse d’exécution à IoT Hub. Voici une liste de réponses possibles :
  ...
  * 417 - l’appareil n’a pas une configuration de déploiement définie.
...
Pour plus d’informations, consultez [apprendre à déployer des modules et établir des itinéraires dans IoT Edge](module-composition.md).   
...
Pour plus d’informations sur l’infrastructure de sécurité Azure IoT Edge, en savoir plus sur la [Gestionnaire de sécurité IoT Edge](iot-edge-security-manager.md).
