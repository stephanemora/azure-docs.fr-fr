---
title: Développer un module pour les appareils Linux - Azure IoT Edge | Microsoft Docs
...
7. Ouvrez la section Explorateur de Visual Studio Code en sélectionnant l’icône dans la barre d’activités sur la gauche, ou en sélectionnant **Affichage** > **Explorateur**. 

8. En bas de la section Explorateur, développez le menu **Appareils Azure IoT Hub** réduit. Vous devez voir les appareils et les appareils IoT Edge associés au hub IoT que vous avez sélectionné via la palette de commandes. 
...
Pour ce tutoriel, nous utilisons le modèle de module C, car il possède les prérequis minimaux à installer. 
...
Visual Studio Code peut développer des modules C pour les appareils Linux AMD64 et Linux ARM32v7. Vous devez sélectionner l’architecture que vous ciblez avec chaque solution, car cela affecte la façon dont le conteneur est généré et exécuté. Linux AMD64 est la valeur par défaut. 
...
1. Ouvrez le fichier **main.c**, qui se trouve dans le dossier **modules/SampleModules/** . 

2. Le SDK C IoT Hub utilise la fonction [SetInputMessageCallback](https://docs.microsoft.com/azure/iot-hub/iot-c-sdk-ref/iothub-module-client-ll-h/iothubmoduleclient-ll-setinputmessagecallback) pour initialiser les files d’attente d’entrée du module. Recherchez cette fonction dans le fichier main.c.

3. Comme vous pouvez le constater dans le constructeur de la fonction SetInputMessageCallback, une file d’attente d’entrée appelée **input1** est initialisée dans le code. 
...
4. Les files d’attente de sortie de module sont initialisées de manière similaire. Recherchez la fonction [SendEventToOutputAsync](https://docs.microsoft.com/azure/iot-hub/iot-c-sdk-ref/iothub-module-client-ll-h/iothubmoduleclient-ll-sendeventtooutputasync) dans le fichier main.c. 

5. Comme vous pouvez le constater dans le constructeur de la fonction SendEventToOutputAsync, une file d’attente de sortie appelée **output1** est initialisée dans le code. 
...
7. Recherchez la propriété **modules** des propriétés souhaitées $edgeAgent. 
   ...
   Deux modules doivent y être listés. Le premier est **tempSensor**, inclus par défaut dans tous les modèles pour fournir des données de température simulée que vous pouvez utiliser pour tester vos modules. Le second est le module **SampleModule** que vous avez créé dans le cadre de cette solution.

7. En bas du fichier, recherchez les propriétés souhaitées pour le module **$edgeHub**. 

   Une des fonctions du module de hub IoT Edge consiste à router les messages entre tous les modules dans un déploiement. Examinez les valeurs dans la propriété **routes**. La première route, **SampleModuleToIoTHub**, utilise un caractère générique ( **\*** ) pour indiquer tous les messages provenant des files d’attente de sortie dans le module SampleModule. Ces messages passent dans *$upstream*, nom réservé indiquant le hub IoT. La deuxième route, sensorToSampleModule, prend les messages en provenance du module tempSensor et les route vers la file d’attente d’entrée *input1* que vous avez vu initialisée dans le code SampleModule. 
...
* Votre machine exécute-t-elle le même type de conteneurs que celui que vous générez ? Ce tutoriel concerne les appareils Linux IoT Edge ; Visual Studio Code doit donc indiquer **amd64** ou **arm32v7** dans la barre latérale, et Docker Desktop doit exécuter des conteneurs Linux. Les modules C dans Visual Studio Code ne prennent pas en charge les conteneurs Windows. 
...
5. Utilisez le bouton Actualiser pour mettre à jour la vue de l’appareil jusqu’à ce que vous voyiez les modules tempSensor et SampleModule en train de s’exécuter sur votre appareil. 
...
* Affichez tous les modules déployés sur votre appareil et vérifiez leur état :
   ...
   Vous devez voir quatre modules : les deux modules du runtime IoT Edge, tempSensor et SampleModule. Les quatre doivent être listés comme étant en cours d’exécution.
