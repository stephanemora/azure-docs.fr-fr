---
title: Déboguer les modules Python pour Azure IoT Edge | Microsoft Docs
description: Utiliser Visual Studio Code pour développer, générer et déboguer un module Python pour Azure IoT Edge
services: iot-edge
keywords: ''
author: shizn
manager: philmea
...
ms.locfileid: "52441249"
---
# <a name="use-visual-studio-code-to-develop-and-debug-python-modules-for-azure-iot-edge"></a>Utiliser Visual Studio Code pour développer et déboguer des modules Python pour Azure IoT Edge

Vous pouvez transformer votre logique métier en modules pour Azure IoT Edge. Cet article vous montre comment utiliser Visual Studio Code (VS Code) comme outil principal pour développer et déboguer des modules Python.
...
* Installez **Cookiecutter** avec la commande suivante
...
Effectuez les étapes suivantes pour créer un module IoT Edge basé sur le Kit de développement logiciel (SDK) Azure IoT Python à l’aide de Visual Studio Code et de l’extension Azure IoT Edge. Vous commencez par créer une solution, puis vous y générez le premier module. Chaque solution peut contenir plusieurs modules. 
...
6. Sélectionnez **Module Python** comme modèle pour le premier module de la solution.
...
Le code du module Python par défaut qui est fourni avec la solution se trouve sous **modules** > [nom de votre module] > **main.py**. Le module et le fichier deployment.template.json sont définis de manière à vous permettre de générer la solution, de l’envoyer vers votre registre de conteneurs et de la déployer sur un appareil pour commencer les tests, sans avoir à utiliser de code. Le module est conçu pour récupérer les entrées d’une source (dans ce cas, le module tempSensor qui simule des données) et les acheminer vers IoT Hub. 

Lorsque vous êtes prêt à personnaliser le modèle Python avec votre propre code, utilisez les [Kits de développement logiciel (SDK) Azure IoT Hub](../iot-hub/iot-hub-devguide-sdks.md) pour créer des modules répondant aux besoins des solutions IoT, tels que la sécurité, la gestion des appareils et la fiabilité. 

## <a name="build-and-deploy-your-module-for-debugging"></a>Générer et déployer votre module à des fins de débogage

Chaque dossier du module contient plusieurs fichiers Docker pour différents types de conteneurs. Utilisez l’un des fichiers se terminant par l’extension **.debug** pour générer votre module à des fins de test. Les modules Python ne prennent en charge le débogage que dans les conteneurs Linux amd64. 

1. Dans VS Code, accédez au fichier `deployment.debug.template.json`. Ce fichier contient la version de débogage des images de votre module avec les options de création appropriées. 

3. Accédez à `main.py`, ajoutez les codes suivants après la section d’importation
...
4. Ajoutez la ligne de code unique suivante dans le rappel que vous voulez déboguer

    ```python
    ptvsd.break_into_debugger()
    ```

   Par exemple, si vous voulez déboguer la méthode `receive_message_callback`. Vous pouvez insérer la ligne de code unique comme ci-dessous.
...
## <a name="start-debugging-python-module-in-vs-code"></a>Commencer le débogage du module Python dans VS Code
...
1. Accédez à la fenêtre de débogage de Visual Studio Code. Sélectionnez le fichier de configuration de débogage de votre module. Le nom de l’option de débogage doit ressembler à **Débogage distant de ModuleName (Python)**

2. Accédez à `main.c`. Ajoutez un point d’arrêt dans la méthode de rappel dans laquelle vous avez placé `ptvsd.break_into_debugger()`.
...
L’exemple précédent montre comment déboguer des modules Python IoT Edge sur des conteneurs. Il a ajouté des ports exposés dans votre conteneur de module createOptions. Une fois le débogage de vos modules Python terminé, nous vous recommandons de supprimer ces ports exposés pour les modules IoT Edge prêts à l’emploi.
