...
description: Dans ce guide de démarrage rapide, découvrez comment déployer à distance un code prédéfini sur un appareil IoT Edge.
author: kgremban
manager: philmea
ms.author: kgremban
ms.date: 10/14/2018
...
Azure IoT Edge déplace la puissance du cloud vers vos appareils Internet des Objets. Dans ce guide de démarrage rapide, découvrez comment utiliser l’interface cloud pour déployer à distance un code prédéfini sur un appareil IoT Edge.
...
4. Déployer à distance un module sur un appareil IoT Edge.

![Architecture du démarrage rapide](./media/quickstart-linux/install-edge-full.png)

Ce guide de démarrage rapide change votre machine virtuelle ou ordinateur Linux en appareil IoT Edge. Vous pouvez ensuite déployer un module du portail Azure sur votre appareil. Le module que vous déployez dans ce démarrage rapide est un capteur simulé qui génère des données de pression, d’humidité et de température. Les autres tutoriels Azure IoT Edge s’appuient sur le travail que vous effectuez ici en déployant des modules qui analysent les données simulées des informations métier.
...
* Un appareil ou une machine virtuelle Linux faisant office d’appareil IoT Edge. Si vous souhaitez créer une machine virtuelle dans Azure, utilisez la commande suivante pour démarrer rapidement :
...
Le niveau gratuit d'IoT Hub fonctionne pour ce démarrage rapide. Si vous avez déjà utilisé IoT Hub et que vous avez créé gratuitement un hub, vous pouvez l’utiliser. Chaque abonnement peut avoir un seul IoT hub gratuit. 
...
Inscrivez un appareil IoT Edge au hub IoT que vous venez de créer.
...
Pendant la configuration du runtime, vous fournissez une chaîne de connexion d’appareil. Utilisez la chaîne que vous avez récupérée dans Azure CLI. Cette chaîne associe votre appareil physique à l’identité d’appareil IoT Edge dans Azure. 
