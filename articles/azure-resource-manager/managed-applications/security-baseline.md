---
title: Ligne de base de sécurité Azure pour les Applications managées Azure
description: La ligne de base de sécurité Applications managées Azure fournit des instructions et des ressources pour la mise en œuvre des recommandations de sécurité spécifiées dans le benchmark de sécurité Azure.
author: msmbaldwin
...
Cette ligne de base de sécurité applique les conseils du [benchmark de sécurité Azure version 2.0](../../security/benchmarks/overview.md) à Applications managées Azure. Le benchmark de sécurité Azure fournit des recommandations sur la façon dont vous pouvez sécuriser vos solutions cloud sur Azure. Le contenu est regroupé par les **contrôles de sécurité** définis par le benchmark de sécurité Azure et les conseils associés applicables à Applications managées Azure. Les **contrôles** non applicables à Applications managées Azure ont été exclus.

Pour voir comment Applications managées Azure est entièrement mappé au benchmark de sécurité Azure, consultez le [fichier de mappage complet de la ligne de base de sécurité Applications managées Azure](https://github.com/MicrosoftDocs/SecurityBenchmarks/tree/master/Azure%20Offer%20Security%20Baselines).
...
**Conseils** : Utilisez les étiquettes de service Réseau virtuel Azure pour définir des contrôles d’accès réseau sur les groupes de sécurité réseau ou le Pare-feu Azure configurés pour vos ressources Applications managées Azure. Vous pouvez utiliser des balises de service à la place des adresses IP spécifiques lors de la création de règles de sécurité. En spécifiant le nom de l’étiquette de service (par exemple : AzureResourceManager) dans le champ source ou de destination d’une règle, vous pouvez autoriser ou refuser le trafic pour le service correspondant. Microsoft gère les préfixes d’adresse englobés par la balise de service et met à jour automatiquement la balise de service quand les adresses changent.

- [Utiliser les étiquettes de service Azure Resource Manager](../../virtual-network/service-tags-overview.md#available-service-tags)
...
**Conseils** : Applications managées Azure utilise Azure Active Directory (Azure AD) en tant que service de gestion des identités et des accès par défaut. Normalisez Azure AD pour régir la gestion des identités et des accès de votre organisation.

Azure fournit les rôles intégrés Azure suivants pour autoriser l’accès aux applications managées à l’aide d’Azure AD et d’OAuth :

- Rôle Contributeur d'application managée : Permet de créer des ressources d’application managées.

- Rôle opérateur d’application managée : Permet de lire les ressources d’application managée et d’effectuer des actions sur ces ressources.

- Lecteur Applications managées : Vous permet de lire les ressources dans une application managée et de demander un accès JIT.

Pour plus d’informations, consultez les références suivantes :

- [Rôle contributeur d'application managée](../../role-based-access-control/built-in-roles.md#managed-application-contributor-role)

- [Rôle opérateur d’application managée](../../role-based-access-control/built-in-roles.md#managed-application-contributor-role)

- [Lecteur applications managées](../../role-based-access-control/built-in-roles.md#managed-applications-reader)
...
**Conseils** : Utilisez des identités managées par Azure pour accorder des autorisations à vos Applications managées. Il est recommandé de recourir à la fonctionnalité d’identité managée Azure plutôt que de créer un compte humain plus puissant pour accéder aux ressources ou les exécuter. En effet, cela permet de limiter les informations d’identification à gérer. Le service Applications managées Azure peut également se voir affecter une identité managée pour s’authentifier en mode natif auprès d’autres services/ressources Azure prenant en charge l’authentification Azure AD. Cela peut être utile pour faciliter l’accès d’Applications managées Azure à Azure Key Vault lors de la récupération des secrets. Les identités managées sont gérées par la plateforme Azure, ce qui vous évite d’avoir à provisionner et à renouveler des secrets.

Applications managées Azure prend en charge l’octroi de deux types d’identités à votre application :
...
**Conseils** : Applications managées Azure utilise Azure Active Directory pour fournir la gestion des identités et des accès aux ressources Azure, aux applications cloud et aux applications locales. Cela inclut les identités d’entreprise, comme les employés, ainsi que les identités externes, comme les partenaires et les fournisseurs. Cela permet à l’authentification unique de gérer et sécuriser l’accès aux données et ressources de votre organisation localement et dans le cloud. Connectez l’ensemble de vos utilisateurs, applications et appareils à Azure AD pour un accès transparent et sécurisé, ainsi qu’une visibilité et un contrôle accrus.
...
**Conseils** : Applications managées Azure utilise Azure Active Directory qui prend en charge des contrôles d’authentification forts avec l’authentification multifacteur et des méthodes sans mot de passe fortes.
- Authentification multifacteur : activez l’authentification multifacteur Azure AD et suivez les recommandations relatives à la gestion des identités et des accès Azure Security Center pour certaines meilleures pratiques de configuration de l’authentification multifacteur. L’authentification multifacteur peut être appliquée à tous les utilisateurs, certains utilisateurs ou au niveau de chaque utilisateur en fonction des conditions de connexion et des facteurs de risque.
...
Pour les administrateurs et les utilisateurs privilégiés, vérifiez que le niveau le plus élevé de la méthode d’authentification forte est utilisé.
...
**Conseils** : Applications managées Azure est intégré à Azure Active Directory, qui fournit les sources de données suivantes :
...
**Conseils** : Applications managées Azure prend en charge l’accès conditionnel Azure AD pour un contrôle d’accès plus granulaire en fonction des conditions définies par l’utilisateur (par exemple, les utilisateurs de certaines plages d’adresses IP devront utiliser l’authentification multifacteur pour se connecter). Une stratégie de gestion granulaire des sessions d’authentification peut également être utilisée dans différents cas d’usage.
...
**Conseils** : Applications managées Azure utilise Azure RBAC pour isoler l'accès aux systèmes vitaux pour l'entreprise en limitant les comptes qui bénéficient d'un accès privilégié aux abonnements et aux groupes d'administration auxquels ils appartiennent.
...
**Conseils** : Applications managées Azure utilise Azure Active Directory pour gérer ses ressources. Pour empêcher le verrouillage accidentel de votre organisation Azure AD, configurez un compte d’accès d’urgence pour conserver un accès lorsque les comptes administratifs normaux ne peuvent pas être utilisés. Les comptes d’accès d’urgence sont généralement hautement privilégiés et ne doivent pas être attribués à des personnes spécifiques. Les comptes d’accès d’urgence sont limités à des cas d’urgence ou à des scénarios « de secours » où il est impossible d’utiliser des comptes d’administration normaux.
...
**Conseils** : Applications managées Azure s’intègre à Azure Active Directory pour gérer ses ressources. Utilisez les fonctionnalités de gestion des droits d’utilisation d’Azure AD pour automatiser les workflows de demandes d’accès, notamment les attributions, les révisions et l’expiration des accès. L’approbation en deux ou plusieurs étapes est également prise en charge.
...
**Conseils** : Implémentez un processus d’approbation organisationnel pour les scénarios de support dans lesquels Microsoft est susceptible d’avoir besoin d’accéder à vos données Application managée Azure. Customer Lockbox n’est à l’heure actuelle pas disponible pour les scénarios de support d’Application managée.
...
**Conseils** : Les définitions d’Application managée qui définissent votre application et ses ressources peuvent être stockées dans vos propres comptes de stockage qui peuvent être configurés avec des clés de chiffrement managées par le client.
...
Les étiquettes fournies lors de la création de l’application managée sont également appliquées au Groupe de ressources managées. L’éditeur de votre application peut fournir son propre étiquetage supplémentaire des ressources managées après le déploiement.

- [Étiquettes par élément de l’interface utilisateur d’Application managée par la ressource](microsoft-common-tagsbyresource.md)
...
**Conseils** : Azure AD fournit les journaux d’utilisateur suivants qui peuvent être consultés dans les rapports Azure AD ou intégrés à Azure Monitor, Azure Sentinel ou d’autres outils de surveillance/SIEM pour des cas d’usage plus sophistiqués de surveillance et d’analytique : Connexions : le rapport de connexions fournit des informations sur l’utilisation des applications managées et les activités de connexion des utilisateurs.
Journaux d’audit : traçabilité proposée via des journaux d’activité pour toutes les modifications effectuées par diverses fonctionnalités au sein d’Azure AD. Par exemple, les journaux d’audit peuvent inclure les modifications apportées à des ressources dans Azure AD comme l’ajout ou la suppression d’utilisateurs, d’applications, de groupes, de rôles, de stratégies, etc.
Connexions risquées : une connexion risquée est une tentative de connexion susceptible de provenir d’un utilisateur autre que le propriétaire légitime d’un compte d’utilisateur.
Utilisateurs avec indicateur de risque : un utilisateur à risque correspond à un indicateur de compte d’utilisateur susceptible d’être compromis.
...
Utilisez les fonctionnalités d’automatisation des workflows dans Azure Security Center et Azure Sentinel pour déclencher automatiquement des actions ou exécuter un playbook pour répondre aux alertes de sécurité entrantes. Le playbook prend des mesures, telles que l’envoi de notifications, la désactivation de comptes et l’isolement des réseaux problématiques. 
...
Configurez Azure Policy pour auditer et appliquer les configurations de vos ressources liées à vos déploiements d’Applications managées.

Vous pouvez utiliser Azure Blueprints pour automatiser le déploiement et la configuration de services et d’environnements d’application, notamment des modèles Azure Resource Manager, des affectations Azure RBAC et des affectations Azure Policy, dans une même définition de blueprint.

- [Zone d’atterrissage à l’échelle de l’entreprise du Cloud Adoption Framework](/azure/cloud-adoption-framework/ready/enterprise-scale/architecture#landing-zone-expanded-definition)
...
**Conseils** : Utilisez Azure Security Center pour surveiller vos ressources liées à Applications managées Azure et utilisez les effets Azure Policy [refuser] et [déployer si inexistant] pour maintenir des configurations sécurisées.
