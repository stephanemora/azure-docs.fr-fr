...
Les informations de cette section peuvent être utiles lorsque vous travaillez avec des [paramètres](./parameters.md).
...
Les informations suivantes peuvent être utiles lorsque vous travaillez avec des [variables](./variables.md) :
...
Lorsque vous décidez des [dépendances](./resource-dependency.md) à définir, respectez les recommandations suivantes :

* Utilisez la fonction `reference` et transmettez le nom de la ressource pour définir une dépendance implicite entre des ressources qui doivent partager une propriété. N’ajoutez pas d’élément `dependsOn` explicite lorsque vous avez déjà défini une dépendance implicite. Cette approche permet de réduire le risque d’avoir des dépendances inutiles. Pour obtenir un exemple de définition de dépendance implicite, consultez les [fonctions reference et list](./resource-dependency.md#reference-and-list-functions).
...
Les informations suivantes peuvent être utiles lorsque vous travaillez avec des [ressources](./syntax.md#resources) :
...
* Pour plus d’informations sur la structure du fichier de modèle, consultez [Comprendre la structure et la syntaxe des modèles ARM](./syntax.md).
* Pour obtenir des recommandations sur la création de modèles qui fonctionnent dans tous les environnements cloud Azure, consultez [Développer des modèles ARM pour la cohérence cloud](./template-cloud-consistency.md).