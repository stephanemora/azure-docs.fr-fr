...
Pour faciliter l’intégration de l’intégrité, Microsoft collabore avec certaines des principales sociétés de supervision de l’intégrité de service pour vous fournir une solution copier/coller simple vous permettant d’intégrer des contrôles d’intégrité à vos déploiements. Si vous n’utilisez pas encore un analyseur d’intégrité, ces solutions sont excellentes pour démarrer :
...
La nouvelle étape `healthCheck` dans Azure Deployment Manager vous permet de déclarer des codes HTTP qui indiquent un service sain. Pour des résultats REST complexes, vous pouvez spécifier des expressions régulières qui, lorsqu’elles sont mises en correspondance, indiquent une réponse saine.

Flux de la configuration des vérifications de l’intégrité avec Azure Deployment Manager :

1. Créez vos analyseurs d’intégrité via un fournisseur d’analyse d’intégrité de votre choix.
1. Créez au moins une étape `healthCheck` dans le cadre de votre lancement d’Azure Deployment Manager. Renseignez les étapes `healthCheck` avec les informations suivantes :
    ...
    1. Les [codes de statut HTTP](https://www.wikipedia.org/wiki/List_of_HTTP_status_codes) ou les expressions régulières qui définissent une réponse « sain ». Vous pouvez fournir des expressions régulières qui doivent TOUTES correspondre afin que la réponse soit considérée comme « saine » ou vous pouvez fournir des expressions dans lesquelles N’IMPORTE LAQUELLE doit correspondre pour que la réponse soit considérée comme « saine ». Les deux méthodes sont prises en charge.

    Le code JSON suivant est un exemple qui permet d’intégrer Azure Monitor à Azure Deployment Manager. L’exemple utilise `RolloutIdentity` et établit un contrôle d’intégrité où un lancement se poursuit en l’absence d’alertes. La seule API prise en charge par Azure Monitor est : [Alertes - Tout obtenir](/rest/api/monitor/alertsmanagement/alerts/getall).
...
1. Appelez les étapes `healthCheck` au moment opportun dans votre lancement d’Azure Deployment Manager. Dans l’exemple suivant, une étape `healthCheck` est appelée dans `postDeploymentSteps` sur `stepGroup2`.
...
Pour parcourir un exemple, consultez [Tutoriel : Utiliser le contrôle d’intégrité dans Azure Deployment Manager](./deployment-manager-tutorial-health-check.md).
...
1. Wait
    ...
    1. Pendant la phase d’attente, l’intégrité du service n’est pas surveillée. Cela permet aux ressources d’avoir le temps de s’installer avant le début du processus de vérification de l’intégrité.

1. Elastic

    1. Comme il est impossible de savoir dans tous les cas le temps nécessaire pour que les ressources deviennent stables, la phase Elastic offre une période flexible entre le moment où les ressources sont potentiellement instables et le moment où elles doivent maintenir un état stable et sain.
    ...
    1. Lorsque l’analyseur d’intégrité revient avec des signaux indiquant que le service est sain, la phase Elastic prend fin et la phase HealthyState débute.
