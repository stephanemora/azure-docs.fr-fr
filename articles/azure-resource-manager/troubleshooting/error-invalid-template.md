...
Lorsque vous recevez ce type d’erreur, examinez attentivement la syntaxe d’expression. Vous pouvez utiliser un éditeur JSON comme [Visual Studio](../templates/create-visual-studio-deployment-project.md) ou [Visual Studio Code](../templates/quickstart-create-templates-use-visual-studio-code.md), qui vous signale des erreurs de syntaxe.
...
Vérifiez les valeurs autorisées dans le modèle et fournissez-en une pendant le déploiement. Pour plus d’informations sur les valeurs de paramètres autorisées, consultez [Section Parameters des modèles Azure Resource Manager](../templates/syntax.md#parameters).
