...
```
**CustomResource** est une classe générique simple qui accepte tout type de données d’entrée. Elle est basée sur **ITableEntity**, qui est utilisé pour stocker des données. La classe **CustomResource** implémente toutes les propriétés de l’interface **ITableEntity** : **timestamp**, **eTag**, **partitionKey** et **rowKey**.
