...
+ Un exemple d’index et des documents sont fournis dans cet article ainsi que dans la [solution Visual Studio](https://github.com/Azure-Samples/azure-search-dotnet-samples/quickstart) pour ce démarrage rapide.
...
1. Dans Hotel.cs, la classe définit la structure globale de l’index, notamment des références à la classe d’adresses.
    ...
    Les attributs du champ déterminent la façon dont il est utilisé dans une application. Par exemple, l’attribut `IsSearchable` est affecté à tous les champs qui doivent être inclus dans une recherche en texte intégral. Dans le kit SDK .NET, la valeur par défaut consiste à désactiver des comportements de champ qui ne sont pas activés explicitement.
...
Pour supprimer un index dont vous n’avez plus besoin, appelez la méthode `Indexes.Delete` sur votre `SearchServiceClient`.

```csharp
serviceClient.Indexes.Delete("hotels");
```

Si vous avez également terminé avec le service de recherche, vous pouvez supprimer les ressources du portail Azure.
