---
title: Ajouter des profils de score à un index de recherche - Recherche Azure
description: Améliorez le classement des recherches avec Recherche Azure en ajoutant des profils de score.
ms.date: 05/02/2019
services: search
ms.service: search
ms.topic: conceptual
...
ms.locfileid: "65021625"
---
# <a name="add-scoring-profiles-to-an-azure-search-index"></a>Ajouter des profils de score à un index Recherche Azure
...
```  
 ...
 Cette requête effectue une recherche du terme « inn », puis transmet l'emplacement actuel. Notez que cette requête inclut d’autres paramètres, tels que `scoringParameter`. Les paramètres de requête sont décrits dans [Recherche dans des documents &#40;API REST du service Recherche Azure&#41;](https://docs.microsoft.com/rest/api/searchservice/Search-Documents).  
...
|`freshness`|La fonction de calcul de score freshness permet de modifier les scores de classement d'éléments en fonction des valeurs des champs `DateTimeOffset`. Par exemple, un élément dont la date est plus récente peut être classé plus haut que des éléments plus anciens.<br /><br /> Il est également possible de classer les éléments comme les événements de calendrier avec des dates futures telles que plus près des éléments jusqu'à présent peuvent être classés supérieures à celles des éléments supplémentaires à l’avenir.<br /><br /> Dans la version de service actuelle, une extrémité de la plage doit être définie sur l'heure réelle. L’autre extrémité est une heure dans le passé selon l’attribut `boostingDuration`. Pour privilégier une plage d’heures à venir, utilisez un attribut `boostingDuration` négatif.<br /><br /> La vitesse à laquelle la valorisation passe d'une plage maximale à une plage minimale est déterminée par l'interpolation appliquée au profil de calcul de score (voir la figure ci-dessous). Pour inverser le facteur de valorisation appliqué, choisissez un facteur inférieur à 1.|  
...
|`distance` &#124; `referencePointParameter`|Paramètre à transmettre dans des requêtes, à utiliser comme emplacement de référence. `scoringParameter` est un paramètre de requête. Les paramètres de requête sont décrits dans [Recherche dans des documents &#40;API REST du service Recherche Azure&#41;](https://docs.microsoft.com/rest/api/searchservice/Search-Documents).|  
...
|`tag` &#124; `tagsParameter`|Paramètre à transmettre dans des requêtes pour spécifier des balises pour une requête spécifique. `scoringParameter` est un paramètre de requête. Les paramètres de requête sont décrits dans [Recherche dans des documents &#40;API REST du service Recherche Azure&#41;](https://docs.microsoft.com/rest/api/searchservice/Search-Documents).|  
...
|`defaultScoringProfile`|Lors de l'exécution d'une requête de recherche, le calcul de score par défaut est utilisé ([tf-idf](http://www.tfidf.com/) uniquement) si aucun profil de calcul de score n'est spécifié.<br /><br /> Un nom de profil de calcul de score par défaut peut être défini ici de façon à ce qu'Azure Search utilise ce profil quand aucun profil spécifique n'est fourni dans la requête de recherche.|  
...
|`Logarithmic`|Par rapport à une interpolation de type Linear dont la valorisation décroît de façon constante, une interpolation de type Logarithmic décroît initialement plus rapidement, puis, lorsqu’elle approche de la plage de fin, beaucoup plus lentement. Cette option d'interpolation n'est pas autorisée dans les fonctions de calcul de score de balises.|  
 ...
 ![Lignes constantes, linéaires, quadratiques et log10 d'un graphique](media/scoring-profiles/azuresearch_scorefunctioninterpolationgrapht.png "AzureSearch_ScoreFunctionInterpolationGrapht")  
