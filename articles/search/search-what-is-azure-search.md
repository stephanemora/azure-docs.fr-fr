...
La recherche est fondamentale pour toute application exposant du contenu aux utilisateurs, avec des scénarios courants comme la recherche dans un catalogue ou dans des documents, la vente en ligne ou l’exploration de connaissances pour la science des données.
...
+ Indexation complète, avec [analyse de texte](search-analyzers.md) et [enrichissement de l’IA facultatif](cognitive-search-concept-intro.md) pour l’extraction et la transformation du contenu avancé
...
Dans l’ensemble de la plateforme Azure, la recherche cognitive peut s’intégrer à d’autres services Azure. Elle le fait sous la forme d’*indexeurs* qui automatisent l’ingestion et la récupération des données à partir de sources de données Azure, et sous la forme d’*ensembles de compétences* qui incorporent l’IA consommable de Cognitive Services (comme l’analyse des images et le traitement en langage naturel) ou l’IA personnalisée que vous créez dans Azure Machine Learning ou wrappez dans Azure Functions.
