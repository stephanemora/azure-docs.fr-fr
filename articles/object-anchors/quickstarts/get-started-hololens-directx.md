...
Assurez-vous que l’appareil HoloLens est sous tension et connecté au PC via un câble USB. Assurez-vous que l'**appareil** est la cible de déploiement choisie (voir ci-dessus).
...
Avant de lancer l’application, vous devez avoir téléchargé un modèle objet, **chair.ou** par exemple, dans le dossier **3D Objects** de votre HoloLens. Si vous ne l’avez pas fait, suivez les instructions de la section [« Charger votre modèle »](#upload-your-model).
