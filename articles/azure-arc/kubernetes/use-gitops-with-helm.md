...
Vérifiez que vous disposez d’un cluster connecté Kubernetes avec Azure Arc. Si vous avez besoin d’un cluster connecté, consultez [Démarrage rapide Connecter un cluster Kubernetes avec Azure Arc](./connect-cluster.md).
...
| `spec.chart.path` | Répertoire contenant le chart, donné par rapport à la racine de référentiel. |
...
À l’aide de l’extension Azure CLI pour `k8sconfiguration`, liez votre cluster connecté à un exemple de référentiel Git. Nous donnerons à cette configuration le nom `azure-arc-sample` et déploierons l’opérateur Flux dans l’espace de noms `arc-k8s-demo`.
...
À l’aide d’Azure CLI, vérifiez la création de `sourceControlConfiguration`.
