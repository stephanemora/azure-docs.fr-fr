...
* Si vous avez déjà installé l’agent, exécutez la commande `/opt/azcmagent/bin/hybridrp_proxy add https://{proxy-url}:{proxy-port}`, qui configure le proxy et redémarre l’agent.
