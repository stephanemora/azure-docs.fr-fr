...
Cet article a pour but de vous aider à planifier et à réussir la migration de votre machine virtuelle ou serveur local géré par des serveurs avec Azure Arc vers Azure. En suivant ces étapes, vous pouvez effectuer une transition de gestion à partir de serveurs avec Arc en fonction des extensions de machines virtuelles prises en charge installées et des services Azure basés sur l’identité de la ressource du serveur avec Arc.
...
* Désinstaller toutes les extensions de machine virtuelle du serveur avec Arc.
* Identifier les services Azure configurés pour vous authentifier avec votre identité managée par le serveur avec Arc et vous préparer à mettre à jour ces services afin d’utiliser l’identité de machine virtuelle Azure après la migration.
* Examiner les droits d’accès Azure RBAC (contrôle d’accès en fonction du rôle Azure) accordés à la ressource du serveur avec Arc pour gérer les utilisateurs ayant accès à la ressource après sa migration vers une machine virtuelle Azure. 
* Supprimer d’Azure l’identité de la ressource du serveur avec Arc et supprimer l’agent du serveur avec Arc.
...
Pour inventorier les extensions de machine virtuelle installées sur votre serveur avec Arc, vous pouvez les lister à l’aide d’Azure CLI ou avec Azure PowerShell.
...
Supprimez l’ID de ressource du serveur avec Arc dans Azure à l’aide de l’une des méthodes suivantes :
