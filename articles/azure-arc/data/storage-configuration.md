...
description: Explique les options de configuration du stockage des services de données compatibles avec Azure Arc
services: azure-arc
...
## <a name="nfs-based-storage-classes"></a>Classes de stockage NFS
...
La propriété `supplementalGroups` prend un tableau de valeurs et peut être définie dans le cadre du déploiement du contrôleur de données Azure arc et sera utilisée par toutes les instances de base de données configurées par le contrôleur de données Azure arc.
...
Quand vous créez une instance en utilisant les commandes `az sql mi-arc create` ou `az postgres arc-server create`, vous pouvez utiliser quatre paramètres pour définir les classes de stockage :

|Nom du paramètre, nom abrégé|Utilisé pour|
|---|---|
|`--storage-class-data`, `-d`|Utilisé pour spécifier la classe de stockage pour tous les fichiers de données, y compris les fichiers journaux de transactions|
|`--storage-class-logs`, `-g`|Utilisé pour spécifier la classe de stockage pour tous les fichiers journaux|
|`--storage-class-data-logs`|Utilisé pour spécifier la classe de stockage pour les fichiers journaux de transactions de la base de données.|
|`--storage-class-backups`|Utilisé pour spécifier la classe de stockage pour tous les fichiers de sauvegarde.|
