---
title: Résoudre les problèmes liés au pare-feu d’applications web pour Azure Application Gateway
...
Tout d’abord, veillez à lire les documents de [présentation du WAF](waf-overview.md) et de [configuration du WAF](application-gateway-waf-configuration.md). De même, vérifiez que vous avez activé la [supervision du WAF](application-gateway-diagnostics.md). Ces articles expliquent comment fonctionne le WAF et ses ensembles de règles et comment accéder aux journaux du WAF.
...
![Listes d’exclusions du WAF](media/waf-tshoot/waf-config.png)
...
![Règles WAF](media/waf-tshoot/waf-rules.png)
...
![Exclusion du WAF](media/waf-tshoot/waf-exclusion-02.png)

L’examen des journaux du pare-feu peut vous renseigner sur les éléments que vous devez ajouter à la liste d’exclusion. Pour activer la journalisation, consultez [Intégrité du serveur principal, journaux de diagnostic et métriques pour la passerelle Application Gateway](application-gateway-diagnostics.md).
...
Consultez [Guide pratique pour configurer le pare-feu d’applications web dans Application Gateway](tutorial-restrict-web-traffic-powershell.md).
