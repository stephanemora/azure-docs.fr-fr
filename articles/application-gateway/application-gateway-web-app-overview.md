---
title: Présentation des serveurs principaux multilocataires, tels que Azure App service, avec Azure Application Gateway
...
### <a name="ssl-termination-and-end-to-end-ssl-with-multi-tenant-services"></a>Arrêt SSL et protocole SSL de bout en bout avec les services multi-locataires

L’arrêt SSL et le chiffrement SSL de bout en bout est pris en charge avec les services multi-locataires. Pour un arrêt SSL au niveau de la passerelle d’application, l’ajout du certificat SSL à l’écouteur de passerelle d’application est toujours requis. Toutefois, en cas de protocole SSL de bout en bout, les services Azure approuvés, tels que les applications web Azure App service ne nécessitent pas de répertorier les serveurs principaux de la passerelle d’application dans une liste verte. Par conséquent, il n’est pas nécessaire d’ajouter de certificat d’authentification. 

![Protocole SSL de bout en bout](./media/application-gateway-web-app-overview/end-to-end-ssl.png)
