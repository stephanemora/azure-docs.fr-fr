...
|---------|---------|---------|
| Noms d’hôte avec caractères génériques dans les écouteurs (préversion) | Vous pouvez maintenant définir des noms d’hôte avec caractères génériques dans un écouteur multisite et jusqu’à 5 noms d’hôte par écouteur. Pour plus d’informations, consultez [Noms d’hôtes comportant des caractères génériques dans l’écouteur (préversion)](multiple-site-overview.md#wildcard-host-names-in-listener-preview). | Juillet 2020 |
