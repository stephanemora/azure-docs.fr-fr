...
8. Vérifiez que, sous **Sous-réseau**, **myBackendSubnet** est sélectionné pour le sous-réseau, puis sélectionnez **Suivant : Gestion**.
9. Sélectionnez **Désactiver** pour désactiver les diagnostics de démarrage.
...
3. Installez IIS sur les autres machines virtuelles à l’aide des étapes que vous venez de terminer. Utilisez *myVM2* et *myVM3* pour les valeurs de VMName dans Set-AzVMExtension.
...
1. Sous l’onglet **Informations de base**, entrez ces valeurs pour les paramètres de passerelle d’application suivants :
   ...
   - **Nom de passerelle applicative** : saisissez *myAppGateway* comme nom de passerelle applicative.
...
3. Sous **Type de cible**, sélectionnez **Machine virtuelle** dans la liste déroulante.
...
3. Une règle de routage requiert un écouteur. Sous l’onglet **Écouteur** dans la fenêtre **Ajouter une règle d’acheminement**, entrez les valeurs suivantes pour l’écouteur :
...
5. Pour **Paramètre HTTP**, sélectionnez **Ajouter nouveau** pour créer un nouveau paramètre HTTP. Le paramètre HTTP détermine le comportement de la règle de routage. 
...
9. Pour **Nom de la cible**, saisissez *Images*.
...
> [Activer le protocole TLS de bout en bout sur Azure Application Gateway](./ssl-overview.md)