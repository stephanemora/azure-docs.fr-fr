...
Les coûts fixes variables sont calculés en fonction du type d’Application Gateway.
...
Les coûts fixes variables sont calculés en fonction du type d’Application Gateway approvisionné.
...
Supposons que vous avez approvisionné une Application Gateway Standard de type moyen avec une instance et qu’elle traite 500 Go par mois. Vos coûts d’Application Gateway selon la tarification précitée sont calculés comme suit :
...
> [!NOTE]
> Pour prendre en charge les scénarios de haute disponibilité, il est nécessaire de configurer au minimum 2 instances pour les références (SKU) V1.
...
Supposons que vous avez approvisionné une Application Gateway Standard de type moyen avec 5 instance et qu’elle traite 500 Go par mois. Vos coûts d’Application Gateway selon la tarification précitée sont calculés comme suit :
...
Les instances Application Gateway V2 et WAF V2 prennent en charge la mise à l’échelle automatique et garantissent une haute disponibilité par défaut.
...
L’unité Compute est la mesure de la capacité consommée du processeur. Les facteurs affectant la consommation d’unité Compute sont les connexions TLS/s, les calculs de réécriture d’URL et le traitement de règle WAF. Le nombre de requêtes qu’une unité Compute peut gérer dépend de différents critères parmi lesquels, la taille de clé du certificat TLS, l’algorithme d’échange de clés, les réécritures d’en-têtes et la taille de la requête entrante WAF, le cas échéant.
...
##### <a name="instance-count"></a>Nombre d'instances 
 Le pré-approvisionnement des ressources pour les références (SKU) Application Gateway V2 est défini en termes de nombres d’instances. Chaque instance garantit un minimum de 10 unités de capacité en termes de capacité de traitement. La même instance peut potentiellement prendre en charge plus de 10 unités de capacité pour différents modèles de trafic en fonction des paramètres d’unité de capacité.

L’échelle et les limites définies manuellement pour la mise à l’échelle automatique (minimale ou maximale) sont définies en termes de nombre d’instances. L’échelle définie manuellement pour le nombre d’instances et le nombre minimal d’instances dans la configuration de la mise à l’échelle automatique réserve 10 unités de capacité/instance. Ces unités de capacité réservées sont facturées tant que l’Application Gateway est active, quelle que soit la consommation réelle de ressources. Si la consommation réelle dépasse le seuil de 10 unités de capacité/instance, les unités de capacité supplémentaires sont facturées sous la composante des coûts variables.

Les références (SKU) V2 sont facturées en fonction de la consommation, et sont constituées de deux composantes :

* Coûts fixes

    Coût basé sur le temps pendant lequel Application Gateway V2/WAF V2 est approvisionné et disponible pour le traitement des demandes. Cela garantit une haute disponibilité, même si aucune instance n’est réservée en spécifiant 0 dans les unités d’échelle minimales dans le cadre de la mise à l’échelle automatique. 
...
* Coûts d’unité de capacité 
    ...
    Coût en fonction du nombre d’unités de capacité réservées ou créées avec la mise à l’échelle automatique, comme requis pour le traitement des demandes entrantes.  Les coûts basés sur la consommation sont calculés sur une base horaire.
...
Supposons que vous avez approvisionné une instance Application Gateway Standard_V2 avec une mise à l’échelle manuelle définie sur 8 unités d’échelle pendant le mois entier. Pendant ce temps, elle reçoit un transfert de données moyen de 88,8 Mbits/s.
...
### <a name="example-1-b--manual-scaling-with-traffic-going-beyond-set-capacity"></a>Exemple 1 (b) – Mise à l’échelle manuelle avec trafic allant au-delà de la capacité définie

Supposons que vous avez approvisionné une instance Application Gateway Standard_V2 avec une mise à l’échelle manuelle définie sur 3 unités d’échelle pendant le mois entier. Pendant ce temps, elle reçoit un transfert de données moyen de 88,8 Mbits/s.
...
Si une capacité de traitement équivalant à 5 unités de capacité supplémentaires était disponible pour une utilisation dans les 3 instances réservées.
...
Supposons que vous avez approvisionné une instance WAF_V2 avec la mise à l’échelle automatique activée, et défini la configuration de l’unité d’échelle minimale sur 0 unité pendant le mois entier. La charge de la demande sur l’instance WAF est minimale, mais systématiquement présente à chaque heure pendant le mois entier. La charge est inférieure à la capacité d’une unité de capacité.
Vos coûts d’Application Gateway selon la tarification précitée sont calculés comme suit :
...
### <a name="example-3-b--waf_v2-instance-with-autoscaling-with-0-min-scale-config"></a>Exemple 3 (b) – Instance WAF_V2 avec mise à l’échelle automatique et configuration de l’échelle minimale 0 

Supposons que vous avez approvisionné une instance WAF_V2 avec la mise à l’échelle automatique activée, et défini la configuration de l’unité d’échelle minimale sur 0 unité pendant le mois entier. Toutefois, aucun trafic n’est dirigé vers l’instance WAF pendant le mois entier.
Vos coûts d’Application Gateway selon la tarification précitée sont calculés comme suit :
...
### <a name="example-3-c--waf_v2-instance-with-manual-scale-set-to-1-instance"></a>Exemple 3 (C) – Instance WAF_V2 avec une mise à l’échelle manuelle définie sur 1 instance

Supposons que vous avez approvisionné une instance WAF_V2 et l’avez définie sur une mise à l’échelle manuelle avec la valeur minimale acceptable de 1 unité d’instance pendant le mois entier. Toutefois, aucun trafic n’est dirigé vers l’instance WAF pendant le mois entier.
Vos coûts d’Application Gateway selon la tarification précitée sont calculés comme suit :
...
Coûts variables = 0,0144 USD * 1 (nombre d’instances) * 10 (unités de capacité) * 730 (heures) = 105,12 USD (1 unité d’instance est réservée)
...
Supposons que vous avez approvisionné une instance WAF_V2 avec la mise à l’échelle automatique activée, et défini la configuration du nombre d’instances minimal sur 0 pendant le mois entier. Pendant ce temps, l’instance reçoit 25 nouvelles connexions TLS/s, avec un transfert de données moyen de 8,88 Mbits/s.
Vos coûts d’Application Gateway selon la tarification précitée sont calculés comme suit :
...
Supposons que vous avez approvisionné une instance standard_V2 avec la mise à l’échelle automatique activée, et défini la configuration de l’unité d’échelle minimale sur 0 unité. Cette instance de passerelle d’application est active pendant 2 heures.
...
Vous pouvez afficher la quantité de consommation pour différents paramètres (unité de calcul, débit et connexions persistantes), ainsi que les unités Compute utilisées dans le cadre des métriques d’Application Gateway dans la section **Surveillance**.
...
D’autres métriques telles que le débit, les connexions actuelles et les unités Compute sont également disponibles pour comprendre les goulots d’étranglement et estimer le nombre d’unités de capacité requises. Des informations détaillées sont disponibles dans [Métriques d’Application Gateway](https://docs.microsoft.com/azure/application-gateway/application-gateway-metrics#application-gateway-metrics)

#### <a name="example-1-a---estimating-capacity-units-being-utilized"></a>Exemple 1 (a) – Estimation des unités de capacité utilisées

**Métriques observées :**

Unités Compute = 22,25

Débit = 16 Mbits/s (2 000 000 d’octets)

Connexions en cours = 178,25 K

Unités de capacité calculées = Max (22,25, 16/2,22, 178.25 * 1000/2500) = 71,3

Unités de capacité observées dans les métriques = 72,3
