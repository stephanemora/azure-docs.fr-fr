...
Ce didacticiel vous montre comment utiliser le portail Azure pour créer un [passerelle d’application](application-gateway-introduction.md) avec un [pare-feu d’applications web](application-gateway-web-application-firewall-overview.md) (WAF). Le WAF utilise des règles [OWASP](https://www.owasp.org/index.php/Category:OWASP_ModSecurity_Core_Rule_Set_Project) pour protéger votre application. Ces règles incluent la protection contre les attaques telles que l’injection de code SQL, les attaques de script entre sites et les détournements de session. Après avoir créé la passerelle d’application, vous tester pour vous assurer qu’elle fonctionne correctement. Avec Azure Application Gateway, vous diriger le trafic web de votre application à des ressources spécifiques en affectant écouteurs aux ports, création de règles et ajout de ressources à un pool principal. Par souci de simplicité, cet article utilise une configuration simple avec une adresse IP frontale publique, un écouteur de base pour héberger un site unique sur cette instance application gateway, deux machines virtuelles utilisées pour le pool principal et une règle de routage de demande de base.
...
4. Choisissez le **configuration IP de Frontend**. Sous **Configuration d’adresse IP frontale**, vérifiez que **Type d’adresse IP** a la valeur **Public**. Sous **Adresse IP publique**, vérifiez que l’option **Créer** est sélectionnée. <br>Vous pouvez configurer le Frontend IP pour être Public ou privé conformément à votre cas d’utilisation. Dans cet exemple, nous allons choisir une adresse IP Frontend publique. 
5. Entrez *myAGPublicIPAddress* comme nom d’adresse IP publique. 
6. Acceptez les valeurs par défaut des autres paramètres, puis sélectionnez **OK**.<br>Nous allons choisir des valeurs par défaut dans cet article pour plus de simplicité, mais vous pouvez configurer des valeurs personnalisées pour les autres paramètres selon votre cas d’utilisation 
...
Le pool principal est utilisé pour acheminer les demandes vers les serveurs principaux qui prendra en charge la demande. Les pools back-end peuvent être composés de cartes d’interface réseau, de groupes de machines virtuelles identiques, d’adresses IP publiques, d’adresses IP internes, de noms de domaine complets et de serveurs back-end multi-locataires comme Azure App Service. Vous devez ajouter des cibles de votre serveur principal à un pool principal.

Dans cet exemple, nous allons utiliser des machines virtuelles que le serveur principal cible. Nous pouvons utiliser des machines virtuelles existantes ou créer de nouveaux. Dans cet exemple, nous allons créer deux machines virtuelles qu’Azure utilise en tant que serveurs principaux pour la passerelle d’application. Pour ce faire, nous allons :

1. Créer un sous-réseau, *myBackendSubnet*, dans lequel les nouvelles machines virtuelles seront créés. 
2. Créer des machines 2 virtuelles, *myVM* et *myVM2*, pour être utilisé en tant que serveurs principaux.
3. Installez IIS sur les machines virtuelles pour vérifier que la passerelle d’application a été créée avec succès.
4. Ajoutez les serveurs principaux pour le pool principal.
...
### <a name="install-iis-for-testing"></a>Installer IIS pour le test

Dans cet exemple, nous installons IIS sur les machines virtuelles uniquement à des fins de vérification Qu'azure a créé la passerelle d’application avec succès. 
...
1. Rechercher l’adresse IP publique pour la passerelle d’application sur son **vue d’ensemble** page.![ Enregistrer l’adresse IP publique d’application gateway](./media/application-gateway-create-gateway-portal/application-gateway-record-ag-address.png)vous pouvez également sélectionner **toutes les ressources**, entrez *myAGPublicIPAddress* dans la recherche de zone, puis sélectionnez-le dans la recherche résultats. Azure affiche l’adresse IP publique dans la page **Vue d’ensemble**.
2. Copiez l’adresse IP publique, puis collez-la dans la barre d’adresses de votre navigateur.
3. Vérification de la réponse. Une réponse valide vérifie que la passerelle d’application a été créée avec succès et qu’il est en mesure de se connecter avec le serveur principal.![Tester la passerelle d’application](./media/application-gateway-create-gateway-portal/application-gateway-iistest.png)
