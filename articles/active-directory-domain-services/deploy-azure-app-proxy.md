---
title: 'Azure Active Directory Domain Services : Déployer le proxy d’application Azure AD | Microsoft Docs'
...
Si vous n’êtes pas familiarisé avec le proxy d’application Azure AD, consultez l’article suivant : [Offrir un accès à distance sécurisé aux applications locales](../active-directory/manage-apps/application-proxy.md).
...
3. Une **licence Azure AD Premium** est nécessaire pour utiliser le proxy d’application Azure AD.
4. **services de domaine Azure AD** , qui doivent être activés pour le répertoire Azure AD. Si ce n’est déjà fait, suivez l’ensemble des tâches décrites dans le [Guide de mise en route](tutorial-create-instance.md).
...
3. Cliquez sur **Proxy d’application**.
...
3. Au cours de l’installation, vous êtes invité à inscrire le connecteur auprès du service de proxy d’application de votre annuaire Azure AD.
   * Fournissez vos **informations d’identification d’administrateur d’application Azure AD**. Votre client d’administrateur d’application peut être différent de vos informations d’identification Microsoft Azure.
...
5. Vous devez voir le connecteur nouvellement installé sur la page Proxy d’application dans votre annuaire Azure AD.
    ...
    ![Dans le Portail Azure, le connecteur installé s’affiche comme étant disponible](./media/app-proxy/app-proxy-connector-page.png)
...
* **Publier des applications à l’aide du proxy d’application Azure AD :** publiez des applications exécutées sur vos machines virtuelles à l’aide du proxy d’application Azure AD. Pour plus d'informations, consultez [Publier des applications avec le proxy d'application Azure AD](../active-directory/manage-apps/application-proxy-publish-azure-portal.md)
...
Utilisez plutôt la KCD basée sur la ressource, comme décrit dans cet [article](deploy-kcd.md).
