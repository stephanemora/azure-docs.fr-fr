...
**Stockage Azure** : pour satisfaire aux exigences de chiffrement des données au repos, l’ensemble du service [Stockage](https://azure.microsoft.com/services/storage/) utilise la fonctionnalité de chiffrement [Storage Service Encryption](../../storage/common/storage-service-encryption.md). Celle fonctionnalité permet de protéger et de sauvegarder les données dans le respect des engagements de l’organisation en matière de sécurité et des exigences de conformité.

**Azure Disk Encryption** : [Disk Encryption](../azure-security-disk-encryption-overview.md) utilise la fonctionnalité BitLocker de Windows pour fournir le chiffrement des volumes des disques de données et du système d’exploitation. La solution s’intègre à Key Vault pour favoriser le contrôle et la gestion des clés de chiffrement de disque.
...
-   [L’audit SQL Database](../../sql-database/sql-database-auditing.md) suit les événements de base de données et les consigne dans un journal d’audit conservé dans un compte de stockage Azure.
