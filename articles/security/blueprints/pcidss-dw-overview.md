---
title: Blueprint de sécurité et de conformité Azure - Entrepôt de données pour PCI DSS
description: Blueprint de sécurité et de conformité Azure - Entrepôt de données pour PCI DSS
services: security
author: meladie
...
ms.locfileid: "57243514"
---
# <a name="azure-security-and-compliance-blueprint-data-warehouse-for-pci-dss"></a>Blueprint de sécurité et de conformité Azure : Entrepôt de données pour la norme PCI DSS

## <a name="overview"></a>Présentation

Ce blueprint de sécurité et de conformité Azure fournit des conseils pour déployer une architecture d’entrepôt de données dans Azure qui contribue au respect des exigences des normes PCI DSS 3.2 (Payment Card Industry Data Security Standards). Il présente une architecture de référence courante et montre comment gérer les données de carte de crédit (notamment le numéro de carte, la date d’expiration et les données de vérification) dans un environnement multiniveau, conforme et sécurisé. Ce blueprint indique aux clients comment respecter des exigences de sécurité et de conformité spécifiques, et leur sert de base pour créer et configurer leurs propres solutions d’entrepôt de données dans Azure.
...
![Diagramme de l’architecture de référence d’entrepôt de données pour PCI DSS](images/pcidss-dw-architecture.png "Diagramme de l’architecture de référence d’entrepôt de données pour PCI DSS")
...
**SQL Data Warehouse**: [SQL Data Warehouse](https://docs.microsoft.com/azure/sql-data-warehouse/sql-data-warehouse-overview-what-is) est un Enterprise Data Warehouse (EDW) qui tire parti de traitement massivement parallèle (MPP) pour exécuter rapidement des requêtes complexes parmi des pétaoctets de données, permettant aux utilisateurs d’identifier efficacement les données financières. Les utilisateurs peuvent importer le Big Data dans le service SQL Data Warehouse à l’aide de requêtes T-SQL PolyBase simples, puis exploiter la puissance du traitement massivement parallèle pour exécuter des analyses hautes performances.

**SQL Server Reporting Services (SSRS)**: [SQL Server Reporting Services](https://docs.microsoft.com/sql/reporting-services/report-data/sql-azure-connection-type-ssrs) fournit la création rapide de rapports avec des tables, des graphiques, des cartes, des jauges, des matrices et plus pour Azure SQL Data Warehouse.
...
**Azure Disk Encryption** : [Azure Disk Encryption](https://docs.microsoft.com/azure/security/azure-security-disk-encryption) exploite la fonctionnalité BitLocker de Windows pour fournir le chiffrement des volumes des disques de données et du système d’exploitation. La solution s’intègre à Azure Key Vault pour faciliter le contrôle et la gestion des clés de chiffrement de disque.
...
Le diagramme de flux de données de cette architecture de référence figure ci-dessous et est également disponible en [téléchargement](https://aka.ms/pcidss-dw-tm). Ce modèle peut aider les clients à comprendre les points de risque potentiel de l’infrastructure du système lors de l’apport de modifications.

![Diagramme d’architecture de référence d’entrepôt de données pour PCI DSS](images/pcidss-dw-threat-model.png "Modèle de risque d’application web PaaS pour PCI DSS")
...
Le [Blueprint de sécurité et de conformité Azure - Matrice d’implémentation d’entrepôt de données pour PCI DSS](https://aka.ms/pcidss-dw-cim) fournit des informations sur les exigences de PCI DSS 3.2 que prend en compte l’architecture d’entrepôt de données, y compris une description détaillée de la façon dont l’implémentation répond aux exigences de chaque contrôle traité.
