...
|---|---|
| **Fournisseur<br />(Microsoft&nbsp;Azure)** | Non applicable. |
| **Client<br />(Plan&nbsp;PCI&#8209;DSS)** | Contoso Webstore utilise le [contrôle d’accès en fonction du rôle (RBAC) Azure](/azure/role-based-access-control/role-assignments-portal) pour isoler les rôles d’utilisateur. RBAC permet une gestion précise de l’accès pour Azure. Il existe des configurations spécifiques pour l’accès à l’abonnement et l’accès à Azure Key Vault.|
