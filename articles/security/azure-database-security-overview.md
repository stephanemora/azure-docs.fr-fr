...
-   [Azure Key Vault avec SQL Server dans une machine virtuelle Azure](http://blogs.technet.com/b/kv/archive/2015/01/12/using-the-key-vault-for-sql-server-encryption.aspx), s’il vous faut un module de sécurité matériel ou si vous devez centraliser la gestion de votre hiérarchie de clés de chiffrement.
...
Pour plus d’informations, consultez [Chiffrement transparent des données (TDE)](https://docs.microsoft.com/sql/relational-databases/security/encryption/transparent-data-encryption-tde).
...
![Fonctionnalité Always Encrypted illustrée sous la forme d’une serrure et d’une clé](./media/azure-databse-security-overview/azure-database-fig1.png)
...
![Masquage d’un numéro envoyé entre une base de données SQL et des applications métiers](./media/azure-databse-security-overview/azure-database-fig3.png)
...
![Sécurité au niveau des lignes permettant à un utilisateur d’accéder aux lignes d’une table via une application cliente](./media/azure-databse-security-overview/azure-database-fig4.png)
...
L’[audit Azure SQL Database](https://docs.microsoft.com/azure/sql-database/sql-database-auditing-get-started) augmente votre capacité à analyser les événements et modifications qui se produisent au sein de la base de données. Par exemple, les mises à jour et les requêtes sur les données.
