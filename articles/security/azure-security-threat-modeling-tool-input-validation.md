...
| **Attributs**              | N/A  |
| **Informations de référence**              | N/A  |
| Étape | ASP.NET Web pages (Razor) effectuer un encodage HTML automatique. Toutes les chaînes imprimées par des pépites, ou nuggets, de code (blocs @) sont automatiquement encodées au format HTML. Toutefois, lorsque la méthode `HtmlHelper.Raw` est appelée, elle renvoie un balisage qui n’est pas encodé au format HTML. Si la méthode d’assistance `Html.Raw()` est utilisée, elle ignore la protection par encodage automatique fournie par Razor.|
...
| **Technologies applicables** | MVC5, MVC6 |
| **Attributs**              | N/A  |
| **Informations de référence**              | [Validation de modèle dans l’API Web ASP.NET](https://www.asp.net/web-api/overview/formats-and-model-binding/model-validation-in-aspnet-web-api) |
