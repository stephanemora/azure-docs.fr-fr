...
Dans cet article, nous abordons un ensemble de bonnes pratiques de sécurité [Azure SQL Database](../../azure-sql/database/sql-database-paas-overview.md) et [SQL Data Warehouse](../../synapse-analytics/sql-data-warehouse/sql-data-warehouse-overview-what-is.md) pour protéger vos applications mobiles et web PaaS (Platform-as-a-Service). Ces bonnes pratiques sont issues de notre expérience d’Azure, mais également de celle des clients, comme vous.
...
- Prend en charge les connexions à partir de SQL Server Management Studio qui utilisent l’authentification universelle Active Directory, notamment [Multi-Factor Authentication (MFA)](/azure/active-directory/authentication/multi-factor-authentication). MFA comprend une authentification forte avec une gamme d’options de vérification simples (appel téléphonique, SMS, cartes à puce avec code PIN ou notification d’application mobile). Pour plus d’informations, consultez [Authentification universelle avec SQL Database et SQL Data Warehouse](../../azure-sql/database/authentication-mfa-ssms-overview.md).

Pour en savoir plus sur l'authentification Azure AD, consultez :

- [Utiliser l’authentification Azure Active Directory pour l’authentification auprès de SQL Database, de Managed Instance ou de SQL Data Warehouse](../../azure-sql/database/authentication-aad-overview.md)
...
- [Contrôle de l’accès à Azure SQL Database et SQL Data Warehouse](../../azure-sql/database/logins-create-manage.md)
- [Règles de pare-feu Azure SQL Database et SQL Data Warehouse](../../azure-sql/database/firewall-configure.md)
