---
title: Connectez les données de Cisco pour Azure en version préliminaire Sentinel | Microsoft Docs
description: Découvrez comment connecter des données de Cisco pour Azure Sentinel.
services: sentinel
...
Cisco ASA ne prend en charge le format CEF, donc les journaux sont envoyées comme Syslog et l’agent Azure Sentinel sait comment les analyser comme si elles étaient des journaux CEF. Configurer Cisco ASA pour transférer les messages Syslog à votre espace de travail Azure par le biais de l’agent Syslog :
...
Pour utiliser le schéma pertinent dans Analytique de journal pour les événements de Cisco, recherchez `CommonSecurityLog`.
