...
### <a name="architecture-and-background"></a>Architecture et contexte
   ...
   ![Architecture de Logstash](./media/connect-logstash/logstash-architecture.png)

Le moteur Logstash est constitué de trois composants :

- Plug-ins d'entrée - collecte personnalisée de données provenant de sources diverses
...
- [Découvrez-en plus sur l'API REST Log Analytics](https://docs.microsoft.com/rest/api/loganalytics/create-request).
- [Découvrez-en plus sur les journaux personnalisés](https://docs.microsoft.com/azure/azure-monitor/platform/data-sources-custom-logs).

## <a name="deploying-the-azure-sentinel-output-plugin-in-logstash"></a>Déployer le plug-in de sortie Azure Sentinel dans Logstash
...
1. **Configuration**
    ...
    À l'aide des informations du document Logstash [Structure d'un fichier de configuration](https://www.elastic.co/guide/en/logstash/current/configuration-file-structure.html), ajoutez le plug-in de sortie Azure Sentinel à la configuration avec les clés et valeurs suivantes (voir la syntaxe appropriée du fichier de configuration sous le tableau) :

    | **Nom du champ** | **Type de données** | **Description** |
    |----------------|---------------|-----------------|
    | `workspace_id` | string | Entrez le GUID de l'ID de l'espace de travail |
    | `workspace_key` | string | Entrez le GUID de la clé primaire de l'espace de travail |
    ...
    | `amount_resizing` | boolean | True/False. Activez ou désactivez le mécanisme de mise à l'échelle automatique, qui ajuste la taille de la mémoire tampon des messages en fonction du volume de données de journal reçues. |
    ...
    **Exemple de configuration**
    ...
    > [!NOTE]
    > Visitez le [GitHub](https://github.com/Azure/Azure-Sentinel/tree/master/DataConnectors/microsoft-logstash-output-azure-loganalytics) du plug-in de sortie pour en savoir plus sur son fonctionnement interne, sa configuration et ses paramètres de performances.

1. **Redémarrer Logstash**

1. **Afficher les journaux entrants dans Azure Sentinel**
    ...
    1. Dans le menu de navigation d'Azure Sentinel, cliquez sur **Journaux**. Sous l'en-tête **Tables**, développez la catégorie **Journaux personnalisés**. Recherchez et cliquez sur le nom de la table que vous avez spécifiée (avec le suffixe `_CL`) dans la configuration.
        ...
        :::image type="content" source="./media/connect-logstash/logstash-custom-logs-menu.png" alt-text="Journaux personnalisés Logstash":::

    1. Pour afficher les enregistrements de la table, interrogez-la en utilisant son nom comme schéma.

        :::image type="content" source="./media/connect-logstash/logstash-custom-logs-query.png" alt-text="Requête dans les journaux personnalisés Logstash":::
