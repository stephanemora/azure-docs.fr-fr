...
1. Dans la barre de menus supérieure, sélectionnez **+ Créer**, puis **Règle de requête planifiée**. L’**Assistant de création de règle personnalisée** s’ouvre.

    ![Créer une requête planifiée](media/tutorial-detect-threats-custom/create-scheduled-query.png)

1. Sous l’onglet **Général** , entrez un nom descriptif et une description. Définissez le niveau **Gravité de l’alerte** approprié. Quand vous créez la règle, vous pouvez aussi l’activer. La règle sera alors exécutée dès la fin du processus de création. Si vous créez la règle sans l’activer, la règle est ajoutée à l’onglet **Règles actives** , d’où vous pourrez l’activer quand vous en aurez besoin.

    ![Démarrer la création d’une règle d’analytique personnalisée](media/tutorial-detect-threats-custom/general-tab.png)

1. Sous l’onglet **Paramètres**, vous pouvez soit écrire une requête directement, soit créer la requête dans Log Analytics, puis la coller dans le champ **Requête de recherche** . À mesure que vous configurez et modifiez votre requête, Azure Sentinel simule les résultats de la requête dans la fenêtre **Aperçu des résultats** , sur la droite. Cela vous renseigne sur la quantité de données susceptibles d’être générées sur l’intervalle de temps spécifié pour l’alerte que vous êtes en train de créer. Cette quantité dépend des paramètres choisis pour **Run query every** (Exécuter la requête tous/toutes les) et **Lookup data from the last** (Antériorité des données à rechercher). Si vous constatez qu’en moyenne votre requête déclencherait des alertes trop fréquemment, vous pouvez augmenter le nombre de résultats à une valeur supérieure à votre référence moyenne.
   ...
   > La requête peut comporter entre 1 et 10 000 caractères, et ne doit pas contenir « search \* » ou « union \* ».
    ...
    1. Sous **Query scheduling** (Planification de la requête), définissez les paramètres suivants :

        1.  Définissez le paramètre **Run query every** (Exécuter la requête tous/toutes les) pour indiquer à quelle **Fréquence** exécuter la requête (au maximum toutes les cinq minutes et au minimum une fois par jour).
...
1.  Pour voir les résultats de vos règles d’alerte, accédez à la page **Incidents** dans laquelle vous pouvez ensuite trier, [examiner les incidents](tutorial-investigate-cases.md) et corriger les menaces.
