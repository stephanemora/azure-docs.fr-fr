---
title: Comment utiliser le contrôle de version et le marquage avec une application d’Apprenant de conversation - Microsoft Cognitive Services | Microsoft Docs
titleSuffix: Azure
description: Découvrez comment utiliser le contrôle de version et le marquage avec une application d’Apprenant de conversation.
...
Ce tutoriel montre comment marquer des versions de votre application d’Apprenant de conversation apprenant et définir la version « active ».  
...
- Collez l’ID d’application en tant que la valeur de CONVERSATION_LEARNER_APP_ID
...
4. Définissez « version 1 » sur « active ».  
    ...
    - Les versions avec balise des applications ne sont pas affectées par les modifications (modification d’actions, d’entités, ajout de boîtes de dialogue d’apprentissage).  
    - Les modifications apportées à une application (en modifiant les actions, entités, en ajoutant des boîtes de dialogue train) sont toujours effectuées sur la balise « maître ».  En d’autres termes, « maître » est la seule balise modifiable ; les autres balises sont des captures instantanées fixes.
...
Notez que la version a été créée dans les paramètres :
...
2. Notez que le bot répond « bonjour (version 1) ».
...
1. Cliquez sur les boîtes de dialogue journal (si vous ne voyez pas de boîtes de dialogue, actualisez l’application).
2. Cliquez sur « bonjour (version 2) »

Notez que nous pouvons apporter des corrections en choisissant parmi les actions actuellement disponibles. Ces modifications seront effectuées sur le maître.
