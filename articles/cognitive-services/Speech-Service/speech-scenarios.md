...
Vous devrez d’abord obtenir une [clé d’abonnement Azure](https://azure.microsoft.com/try/cognitive-services/), puis vous devrez consulter la [documentation]([Batch transcription API](batch-transcription.md)).
...
Le développeur peut doter son application d’une sortie vocale. Le Service Speech peut assurer la synthétisation vocale de plusieurs [langages](supported-languages.md) ainsi que fournir les [points de terminaison](rest-apis.md) permettant d’accéder à cette fonctionnalité et de l’ajouter à votre application.
