---
title: 'Démarrage rapide : Reconnaissance vocale, Swift - Service Speech'
...
Des guides de démarrage rapide sont également disponibles pour la [synthèse vocale](quickstart-text-to-speech-swift-ios.md).
...
1. Ajoutez le chemin relatif `helloworld/MicrosoftCognitiveServicesSpeech-Bridging-Header.h` à l’en-tête de pontage dans les paramètres de projet Swift pour la cible helloworld dans le champ **Objective-C Bridging Header**.
   ...
   ![Propriétés d’en-tête](media/sdk/qs-swift-ios-bridging-header.png)
...
1. Remplacez la chaîne `YourServiceRegion` par la [région](regions.md) associée à votre abonnement. Par exemple, utilisez `westus` pour l’abonnement d’essai gratuit.
