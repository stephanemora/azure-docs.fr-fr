...
Chaque entraînement génère automatiquement 100 échantillons audio pour vous aider à tester le modèle. Une fois votre modèle vocal créé, vous pouvez le tester avant de procéder à son déploiement.

1.  Accédez à **Synthèse vocale > Custom Voice > [nom du projet] > Modèle**.

2.  Cliquez sur le nom du modèle que vous souhaitez tester.

3.  Dans la page de détails du modèle, vous trouverez les échantillons audio sous l’onglet **Test**. 
