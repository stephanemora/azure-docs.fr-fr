...
4. Sur la page [Publier](PublishApp.md), cochez la case **Inclure tous les scores intentionnels prédits**. Lorsque la case à cocher est activée, toutes les intentions sont retournées. Lorsque la case à cocher est désactivée, seule l’intention principale est retournée. 

5. [Publier](PublishApp.md) l’application. Publier l’application vous permet de la tester à l’aide du point de terminaison HTTPS. 
...
> * Le [test du point de terminaison](PublishApp.md#test-your-published-endpoint-in-a-browser) vous permet d’afficher le texte JSON exact de la réponse LUIS. 

## <a name="get-the-entity-score-with-the-endpoint-test"></a>Obtenir le score d’entité avec le test de point de terminaison
Pour afficher le score de l’entité, [publiez le modèle](PublishApp.md) et interrogez le point de terminaison. 
