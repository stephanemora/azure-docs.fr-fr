---
title: Didacticiel permettant de découvrir comment appeler une application Language Understanding Intelligent Service (LUIS) à l’aide de Java | Microsoft Docs
description: Dans ce didacticiel, vous allez apprendre à appeler une application LUIS à l’aide de Java.
...
ms.locfileid: "36263718"
---
# <a name="tutorial-call-a-luis-endpoint-using-java"></a>Didacticiel : Appeler un point de terminaison LUIS à l’aide de Java
...
4. Retournez sur le site web [LUIS](luis-reference-regions.md) et connectez-vous avec votre compte Azure. 
    ...
    [![](media/luis-get-started-java-get-intent/app-list.png "Capture d’écran de la liste des applications")](media/luis-get-started-java-get-intent/app-list.png)
...
2. Collez l’URL dans une fenêtre de navigateur, puis appuyez sur Entrée. Le navigateur affiche un résultat JSON qui indique que l’application LUIS a détecté l’intention `HomeAutomation.TurnOn` et l’entité `HomeAutomation.Room` pourvue de la valeur `bedroom`.
    ...
    ![Résultat JSON indiquant la détection de l’intention TurnOn](./media/luis-get-started-java-get-intent/turn-on-bedroom.png)
3. Modifiez la valeur du paramètre `q=` dans l’URL et remplacez-la par `turn off the living room light`, puis appuyez sur Entrée. Le résultat indique maintenant que l’application LUIS a détecté l’intention `HomeAutomation.TurnOff` et l’entité `HomeAutomation.Room` pourvue de la valeur `living room`. 
    ...
    ![Résultat JSON indiquant la détection de l’intention TurnOff](./media/luis-get-started-java-get-intent/turn-off-living-room.png)


## <a name="consume-a-luis-result-using-the-endpoint-api-with-java"></a>Utiliser un résultat LUIS à l’aide de l’API de point de terminaison avec Java 
...
1. Copiez le code suivant pour créer une classe dans votre environnement de développement intégré :
...
2. Remplacez la valeur de la variable `SubscriptionKey` par votre clé d’abonnement LUIS.

3. Dans votre environnement de développement intégré, ajoutez des références aux bibliothèques `httpclient` et `httpcore`.
