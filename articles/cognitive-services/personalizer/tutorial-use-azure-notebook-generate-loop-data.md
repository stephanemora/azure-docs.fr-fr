...
Le notebook sélectionne un utilisateur aléatoire, l’heure de la journée et le type de météo à partir du jeu de données. Voici un résumé des informations sur les utilisateurs :
...
Au début, seules 20 à 30 % des suggestions de Personalizer sont correctes (indiqué par le score de récompense de 1). Après plusieurs requêtes, le système s’améliore.

Après les 10 000 premières requêtes, exécutez une évaluation hors connexion. Cela permet à Personalizer d’examiner les données et de suggérer une meilleure stratégie d’apprentissage. Appliquez la nouvelle stratégie d’apprentissage et réexécutez le notebook avec 2 000 requêtes. La boucle sera plus performante.
...
* Contexte : un choix aléatoire d’utilisateur, de météo et d’heure de la journée, simulant un utilisateur sur un site web ou un appareil mobile
* Caractéristiques : _toutes_ les données sur le café, à partir desquelles Personalizer fait une suggestion

Le système reçoit le classement des choix de café, puis compare cette prédiction avec le choix connu de l’utilisateur pour la même heure de la journée et la même météo. Si le choix connu est le même que le choix prédit, la **récompense** de 1 est renvoyée à Personalizer. Sinon, la récompense est 0. 
