...
Cette valeur `ImageInsightsToken` sera envoyé à Recherche visuelle Bing dans une requête.
...
Les résultats de Recherche visuelle sont des objets `ImageTag`.  Chaque balise contient une liste d’objets `ImageAction`.  Chaque `ImageAction` contient un champ `Data` qui est une liste de valeurs qui varient selon le type d’action. Vous pouvez itérer à travers les objets `ImageTag` dans `visualSearchResults.Tags`, par exemple, et obtenir la balise `ImageAction` intégrée. L’exemple ci-dessous imprime les détails des actions `PagesIncluding`.
