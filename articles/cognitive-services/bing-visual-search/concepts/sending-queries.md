...
- Avec un jeton Insights que vous obtenez à partir d’une image dans un appel précédent à l’un des points de terminaison de [l’API Recherche d’images Bing](https://docs.microsoft.com/rest/api/cognitiveservices/bing-images-api-v7-reference).
...
Chaque demande doit inclure l’en-tête `Content-Type`. L’en-tête doit être défini sur : `multipart/form-data; boundary=\<boundary string\>`, où \<chaîne de limite\> est une chaîne opaque unique qui identifie la limite des données de formulaire. Par exemple : `boundary=boundary_1234-abcd`.
