...
Vous pouvez contrôler si ces jetons sont en lecture seule ou s’ils permettent la modification en spécifiant **allowEdit=true/false**.
...
- Lorsque vous appelez l’API qui obtient des aperçus pour la vidéo spécifiée, vous obtenez une sortie JSON détaillée en tant que contenu de réponse. [Obtenez des détails sur la sortie JSON renvoyée dans cette rubrique](video-indexer-output-json.md).
