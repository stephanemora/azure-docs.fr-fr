...
description: Effectuez le test des listes de texte personnalisées dans la console d’API Content Moderator.
...
Avant de pouvoir évaluer l’API dans la console en ligne, vous avez besoin de votre clé d’abonnement. Cette clé se trouve sous l’onglet **Paramètres**, dans la zone **Ocp-Apim-Subscription-Key**. Pour plus d’informations, consultez [Vue d’ensemble](overview.md).
