...
## <a name="step-1-create-a-console-application"></a>Étape 1 : créer une application console

Dans le cadre de cette étape, vous allez créer une application console et préparer la clé d’apprentissage et les images nécessaires pour l’exemple :

1. Démarrez Visual Studio 2015 Community Edition. 
2. Créez une application console.
3. Ajoutez des références aux deux packages NuGet :
...
Pour créer un nouveau projet Service Vision personnalisée, ajoutez le code suivant à la fin de votre méthode **Main()**.
...
Pour ajouter des balises à votre projet, insérez le code suivant après l’appel de **CreateProject()**  :
...
Pour les projets de détection d’objet, nous devons identifier la région de l’objet à l’aide de coordonnées normalisées et d’une balise. Pour ajouter les images et les régions balisées, insérez le code suivant à la fin de la méthode **Main()**  :
...
## <a name="step-5-train-the-project"></a>Étape 5 : former le projet

Maintenant que vous avez ajouté des balises et des images au projet, vous pouvez former celui-ci : 

1. Insérez le code suivant à la fin de la méthode **Main()**. Cela crée la première itération dans le projet.
...
1. Obtenez le point de terminaison associé à l’itération par défaut en insérant le code suivant à la fin de la méthode **Main()**. 
2. Envoyez une image de test au projet en utilisant ce point de terminaison.
