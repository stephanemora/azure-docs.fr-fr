...
3. Utiliser la requête `setHeader()` fonction permettant de définir le `Content-Type` en-tête à pour `application/json`et ajoutez votre clé d’abonnement pour le `Ocp-Apim-Subscription-Key` en-tête.
