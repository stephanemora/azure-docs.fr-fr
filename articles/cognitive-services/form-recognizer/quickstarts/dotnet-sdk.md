...
* Entraîner un modèle Form Recognizer personnalisé
* Analyser les formulaires avec un modèle personnalisé
* Obtenir une liste de modèles personnalisés
...
* Un blob Stockage Azure qui contient un jeu de données d’apprentissage. Consultez [Créer un jeu de données d’apprentissage pour un modèle personnalisé](../build-training-data-set.md) pour obtenir des conseils et des options pour constituer vos données d’apprentissage. 
...
De plus, si vous avez entraîné un modèle personnalisé que vous souhaitez supprimer de votre compte, utilisez la méthode suivante :
