---
title: Utiliser des métadonnées dans votre base de connaissances, avec l’API GenerateAnswer | Microsoft Docs
description: Utiliser des métadonnées avec l’API GenerateAnswer
...
Avec QnA Maker, vous pouvez ajouter des métadonnées, sous la forme de paires clé-valeur, à vos séries de questions et réponses. Ces données peuvent être utilisées de différentes manières, par exemple, pour filtrer les résultats des requêtes utilisateur, valoriser certains résultats et enregistrer des informations supplémentaires utilisables dans les conversations de suivi. Pour plus d’informations, consultez [Base de connaissances](../Concepts/knowledge-base.md).
