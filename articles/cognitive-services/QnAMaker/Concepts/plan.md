...
description: Pour planifier votre application QnA Maker, vous devez comprendre comment QnA Maker fonctionne et interagit avec les autres services Azure. En outre, vous devez connaître certains concepts relatifs aux bases de connaissances.
ms.topic: conceptual
ms.date: 07/2/2020
...
ms.locfileid: "85875382"
---
# <a name="plan-your-qna-maker-app"></a>Planifier votre application QnA Maker

Pour planifier votre application QnA Maker, vous devez comprendre comment QnA Maker fonctionne et interagit avec les autres services Azure. En outre, vous devez connaître certains concepts relatifs aux bases de connaissances.

## <a name="azure-resources"></a>Ressources Azure

Chaque [ressource Azure](azure-resources.md#resource-purposes) créée avec QnA Maker a une utilité précise. Étant donné que chaque ressource a sa propre utilité, ses propres limites et son propre [niveau tarifaire](azure-resources.md#pricing-tier-considerations), il est important de comprendre le rôle que jouent ces ressources dans le cadre de votre processus de planification.
...
Lorsque vous apprenez à créer et à prédire des requêtes, vous pouvez utiliser le niveau gratuit `F0` de chaque ressource. Lorsque vous passez à un scénario de production, vous devez réévaluer votre choix de ressources.
...
Lorsque vous prévoyez de créer une application réelle, vous devez planifier vos ressources en fonction de la taille de votre base de connaissances et de la quantité de demandes de prédiction des requêtes auxquelles vous vous attendez.
...
Si certaines de ces ressources sont déjà utilisées, vous pouvez décider de partager ces ressources. Même si certaines ressources [peuvent être partagées](azure-resources.md#share-services-with-qna-maker), il s’agit d’un scénario avancé.
...
### <a name="understanding-impact-of-resource-selection"></a>Comprendre l’impact du choix des ressources
...
Une base de connaissances est directement liée à sa ressource QnA Maker. Elle contient des paires de questions-réponses qui sont utilisées pour répondre aux demandes de prédiction de requête.
...
Structurez vos ressources QnA Maker en fonction de la langue ou utilisez [Translator](../../translator/translator-info-overview.md) pour traduire une requête dans la langue de la base de connaissances avant de l’envoyer au point de terminaison de prédiction de requête.

### <a name="ingesting-data-sources"></a>Ingestion de sources de données

Les [sources de données](knowledge-base.md) ingérées qui sont utilisées pour créer une base de connaissances peuvent être :

* Une URL publique
* Une URL SharePoint privée
* Fichier

Le processus d’ingestion convertit les [types de contenu pris en charge](content-types.md) en Markdown. Toutes les modifications apportées à la *réponse* sont effectuées avec Markdown. Après avoir créé votre base de connaissances, vous pouvez modifier les [paires de questions-réponses](question-answer-set.md) dans le portail QnA Maker à l’aide de la [création de texte enrichi](../how-to/edit-knowledge-base.md#rich-text-editing-for-answer).
...
Les images liées doivent être disponibles à partir d’une URL publique afin d’être affichées dans le volet de test du portail QnA Maker. Il en va de même pour toutes les applications clientes, car QnA Maker ne fournit pas d’authentification pour le contenu, y compris pour les images.
...
Une personnalité de bot est recommandée si votre bot se connecte à votre base de connaissances. Si vous vous connectez à plusieurs services, dont l’un est votre base de connaissances, vous pouvez choisir de continuer à utiliser chit-chat dans votre base de connaissances. Toutefois, vous devrez examiner la façon dont le service bot interagit pour savoir si la conception architecturale est adaptée à votre utilisation.
...
L’intégration aux [applications clientes](integration-with-other-applications.md) consiste à envoyer une requête au point de terminaison du runtime de prédiction. Une requête est envoyée à votre base de connaissances, en même temps qu’une requête SDK ou REST envoyée au point de terminaison de l’application web QnA Maker.

Afin d’authentifier correctement une requête du client, l’application cliente doit envoyer les bonnes informations d’identification et le bon ID de la base de connaissances. Si vous utilisez Azure Bot Service, configurez le paramètre dans le cadre de la configuration du bot dans le portail Azure.
...
Si votre application cliente prend en charge le flux de conversation en fournissant d’autres solutions pour gérer les invites de suivi ou Chit-chat, vous devez vous y prendre en avance pour concevoir ces solutions. En outre, vous devez vérifier que la requête utilisée dans l’application cliente est gérée correctement, c’est-à-dire qu’elle est gérée par un autre service ou qu’elle est envoyée à votre base de connaissances.

### <a name="dispatching-between-qna-maker-and-language-understanding-luis"></a>Distribution entre QnA Maker et LUIS

Une application cliente peut fournir plusieurs fonctionnalités, dont une seule bénéficie des réponses de la base de connaissances. Les autres fonctionnalités doivent comprendre le texte conversationnel et en extraire la signification.

Souvent, l’architecture d’une application cliente consiste à utiliser QnA Maker et [LUIS](../../LUIS/what-is-luis.md) ensemble. LUIS permet la classification et l’extraction de texte pour toutes les requêtes, y compris celles envoyées aux autres services, tandis que QnA Maker fournit les réponses de votre base de connaissances.

Dans une [architecture partagée](../choose-natural-language-processing-service.md), la distribution entre ces deux services s’effectue à l’aide de l’outil [Dispatch](https://github.com/Microsoft/botbuilder-tools/tree/master/packages/Dispatch) fourni par Bot Framework.

### <a name="active-learning-from-a-client-application"></a>Apprentissage actif à partir d’une application cliente

QnA Maker utilise l’_apprentissage actif_pour améliorer la qualité de votre base de connaissances en suggérant des questions alternatives pour votre réponse. L’application cliente est responsable en partie de cet [apprentissage actif](active-learning-suggestions.md). L’application cliente, par le biais d’invites de conversation, peut déterminer que la réponse retournée par la base de connaissances n’était pas celle que l’utilisateur attendait, et déterminera alors la réponse la mieux adaptée. L’application cliente doit [renvoyer ces informations à la base de connaissances](active-learning-suggestions.md#how-you-give-explicit-feedback-with-the-train-api) pour améliorer la qualité de la prédiction.

### <a name="providing-a-default-answer"></a>Fournir une réponse par défaut

Si votre base de connaissances ne trouve pas de réponse, elle retourne une _réponse par défaut_. Cette réponse est configurable sur le portail QnA Maker, dans la page **Paramètres** ou [API](https://docs.microsoft.com/rest/api/cognitiveservices/qnamaker/knowledgebase/update#request-body).

Cette réponse par défaut est différente de la réponse par défaut du bot Azure. La réponse par défaut du bot Azure est configurée dans le portail Azure pour votre bot, dans le cadre des paramètres de configuration, et elle est renvoyée lorsque le seuil du score n’est pas atteint.

## <a name="prediction"></a>Prédiction

La prédiction correspond à la réponse de votre base de connaissances et elle contient plus d’informations que la simple réponse. Pour obtenir une réponse de prédiction de requête, utilisez l’[API GenerateAnswer](query-knowledge-base.md).
...
* Premier classement - Recherche cognitive : Pour qu’une réponse soit renvoyée par la Recherche cognitive, le nombre de _réponses autorisées_ doit être suffisamment élevé pour que les meilleures réponses soient retournées par la Recherche cognitive afin d’être sélectionnées par le ranker QnA Maker
* Deuxième classement - QnA Maker : Utilisez la caractérisation et le machine learning pour déterminer la meilleure réponse.

### <a name="service-updates"></a>Mises à jour de service

Les mises à jour de service sont gérées automatiquement par l’application des [dernières mises à jour du runtime](../how-to/set-up-qnamaker-service-azure.md#get-the-latest-runtime-updates).
...
* Des métadonnées, filtrables lors de l’interrogation. Cela vous permet d’étiqueter vos paires de questions-réponses avec des informations supplémentaires sur la source, le contenu, le format et l’utilité de vos données.
* Des invites de suivi, pour déterminer un chemin au sein de votre base de connaissances permettant à l’utilisateur d’obtenir la réponse qu’il attend.
* Des questions alternatives, qui sont importantes pour trouver une réponse à partir de plusieurs formulations d’une même question. Les [suggestions d’apprentissage actif](active-learning-suggestions.md) deviennent des questions alternatives.
