---
title: Mettre à jour une base de connaissances - Démarrage rapide Java - Azure Cognitive Services | Microsoft Docs
...
Si vous n’avez pas encore de base de connaissances, vous pouvez créer un exemple à utiliser pour ce démarrage rapide : [Créer une base de connaissances](create-new-kb-java.md).
...
1. Vous devez disposer de la bibliothèque [apache httpclient](http://hc.apache.org/downloads.cgi). Par exemple, téléchargez le client HTTP binaire 4.x.x.tar.gz pour rechercher tous les fichiers jar dont vous avez besoin pour l’importation dans votre projet.
1. La bibliothèque [GSON](https://github.com/google/gson) est également nécessaire. Vous pouvez [créer](https://stackoverflow.com/questions/5258159/how-to-make-an-executable-jar-file) et importer manuellement le fichier .jar ou ajouter une dépendance à votre outil de gestion de projet favori, tel que Maven.
...
Une fois votre base de connaissances mise à jour, vous pouvez l’afficher sur votre portail QnA Maker, à la page [Mes bases de connaissances](https://www.qnamaker.ai/Home/MyServices). Vous remarquerez que votre base de connaissances a été modifiée selon votre requête personnalisée.

Pour modifier d’autres éléments de votre base de connaissances, reportez-vous au [schéma JSON](https://westus.dev.cognitive.microsoft.com/docs/services/5a93fcf85b4ccd136866eb37/operations/5ac266295b4ccd1554da7600) QnA Maker et modifiez l’objet `req` dans la méthode `getRequest()`.
