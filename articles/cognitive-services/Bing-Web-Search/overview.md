---
title: Qu’est-ce que la Recherche web Bing ? | Microsoft Docs
description: L’API Recherche Web Bing est un service RESTful permettant à votre application d’utiliser les fonctionnalités de recherche Web Bing, incluant les pages web, images, vidéos, actualités et entités, ainsi que les requêtes de recherche associées, corrections orthographiques, conversions d’unités, traductions et calculs.
...
Outre les réponses instantanées, le service Recherche Web Bing offre des fonctionnalités et capacités supplémentaires vous permettant de personnaliser les résultats de la recherche pour vos utilisateurs.
...
L’API Recherche Web Bing peut être facilement appelée à partir de n’importe quel langage de programmation permettant d’exécuter des requêtes HTTP et d’analyser des réponses JSON. Ce service est accessible à l’aide de [l’API REST](quickstarts/python.md) ou des [Kits de développement logiciel (SDK) Recherche Web Bing](web-sdk-python-quickstart.md).  
...
* Utilisez notre [guide de démarrage rapide Python](quickstarts/python.md) pour effectuer votre premier appel de l’API Recherche Web Bing.  
* [Créez une application web à page unique](tutorial-bing-web-search-single-page-app.md).
* Consultez la documentation de [référence sur l’API Recherche Web v7](https://docs.microsoft.com/rest/api/cognitiveservices/bing-web-api-v7-reference).  
