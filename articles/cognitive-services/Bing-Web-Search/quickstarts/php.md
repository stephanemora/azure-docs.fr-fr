---
title: 'Démarrage rapide : Utiliser PHP pour appeler l’API Recherche Web Bing'
...
Quelques variables doivent être définies avant de continuer. Vérifiez que le `$endpoint` est correct et remplacez la valeur `$accesskey` par une clé d’abonnement valide à partir de votre compte Azure. N’hésitez pas à personnaliser la requête de recherche en remplaçant la valeur de `$term`.
...
Ce code déclare une fonction appelée `BingWebSearch` utilisée pour construire des requêtes destinées à l’API Recherche Web Bing. Il tient compte de trois arguments : `$url`, `$key` et `$query`.
...
Ce code valide la clé d’abonnement, effectue une requête et imprime la réponse.
