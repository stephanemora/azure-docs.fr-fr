...
Dans la classe `Program`, créez une fonction nommée `BreakSentenceRequest()`. Cette fonction prend quatre arguments : `subscriptionKey`, `endpoint`, `route` et `inputText`.
...
Vous remarquerez que dans `Main`, vous déclarez `subscriptionKey`, `endpoint`, `route` et le texte à évaluer `breakSentenceText`.
