---
title: Client d’auto-test pour prévalider une application Azure SaaS | Microsoft Docs
description: Guide pratique pour créer un client d’auto-test en vue de prévalider une application Azure SaaS.
...
---
# <a name="create-a-self-test-client-to-pre-validate-an-azure-saas-application"></a>Créer un client d’auto-test pour prévalider une application Azure SaaS
...
|          DNSName          |         Nom DNS de la machine virtuelle que vous allez tester.          |
|          Utilisateur          |          Nom d’utilisateur pour l’établissement d’une connexion et la connexion à la machine virtuelle.         |
|           Mot de passe         |         Mot de passe pour l’établissement d’une connexion et la connexion à la machine virtuelle.          |
|          SE          |         Système d’exploitation de la machine virtuelle : « Linux » ou « Windows ».          |
...
La capture d’écran suivante montre un exemple de résultats des tests JSON affichés dans une visionneuse JSON en ligne.
...
3. Sous **Créer**, fournissez les informations nécessaires pour les champs suivants :
   ...
   - **URL de connexion** : tapez « https://isvapp.azurewebsites.net/selftest »
...
Après avoir testé avec succès votre application Azure SaaS, vous pouvez [publier l’offre](./cpp-publish-offer.md).