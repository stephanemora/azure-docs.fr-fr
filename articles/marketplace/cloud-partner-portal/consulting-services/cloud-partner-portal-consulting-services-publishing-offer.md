---
title: Offre de service Azure et Dynamics 365 consulting | Place de marché Azure
description: Guide pour la définition et publication d’une offre de service de conseil Azure ou Dynamics 365 dans le portail Cloud Partner.
services: Azure, Marketplace, Cloud Partner Portal,
author: qianw211
ms.service: marketplace
ms.topic: conceptual
ms.date: 11/30/2018
ms.author: pabutler
ms.openlocfilehash: a033ad80b59dc29b6d08624dee400d496eb28005
ms.sourcegitcommit: c53a800d6c2e5baad800c1247dce94bdbf2ad324
ms.translationtype: MT
ms.contentlocale: fr-FR
ms.lasthandoff: 04/30/2019
ms.locfileid: "64942844"
---
# <a name="azure-and-dynamics-365-consulting-service-offer"></a>Offre de service de conseil Azure et Dynamics 365

Cet article explique comment publier une offre de service de conseil sur la <a href="https://azuremarketplace.microsoft.com">Place de marché Microsoft Azure</a> ou sur <a href="https://appsource.microsoft.com">AppSource</a>. Les solutions basées sur Microsoft <a href="https://dynamics.microsoft.com">Dynamics 365</a>, <a href="https://powerbi.microsoft.com">Power BI</a> et <a href="https://powerapps.microsoft.com">PowerApps</a> peuvent être répertoriées sur AppSource. D'autres offres basées sur d'autres <a href="https://azure.microsoft.com/services">services Microsoft Azure</a> peuvent être répertoriées sur la Place de marché Azure.

## <a name="publishing-benefits"></a>Avantages liés à la publication

La publication sur l'une des places de marché Microsoft présente les avantages suivants :

- Promouvez votre entreprise en tirant parti de la marque Microsoft.
- Touchez potentiellement plus de 100 millions d'utilisateurs Office 365 et Dynamics 365 sur AppSource, ainsi que plus de 200 000 organisations via la Place de marché Microsoft Azure.
- Obtenez des prospects de grande qualité à partir de ces places de marché.
- Bénéficiez d’une promotion de vos services assurée par les équipes de terrain et de télévente de Microsoft.

## <a name="publish-a-consulting-service-offer"></a>Publier une offre de service de conseil

Cette section décrit les éléments de la publication d’une offre de service de conseil. La publication comprend les parties principales suivantes :

- [Conditions préalables à la publication d'un service de conseil](./cpp-consulting-service-prerequisites.md) répertorie les conditions requises pour publier une offre de service de conseil pour :
 
    - Dynamics 365 for Customer Engagement 
    - Dynamics 365 for Finance & Operations 
    - Dynamics 365 Business Central 
    - Power BI 
    - PowerApps
- [Créer une offre](./cpp-consulting-service-create-offer.md) répertorie les étapes à suivre pour créer une entrée d'offre de service de conseil à l'aide du portail Cloud Partner :
    - [Définissez les paramètres de l'offre](./cpp-consulting-service-define-offer-settings.md).
    - [Entrez les détails de la vitrine et indiquez si vous souhaitez effectuer la publication sur la Place de marché Microsoft Azure ou sur AppSource.](./cpp-consulting-service-storefront-details.md)
- [Publier votre offre](./cpp-consulting-service-publish-offer.md) explique comment envoyer l'offre afin de la publier.

## <a name="next-steps"></a>Étapes suivantes

Avant d’entreprendre ces étapes pour publier votre offre de service de conseil, vous devez respecter les [prérequis](./cpp-consulting-service-prerequisites.md) liés à cette publication.