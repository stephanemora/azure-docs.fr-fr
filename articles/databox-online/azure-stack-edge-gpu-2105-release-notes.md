---
title: Notes de publication d’Azure Stack Edge 2105
description: Décrit les problèmes critiques ouverts de la version 2105 d’Azure Stack Edge et leur résolution.
services: databox
author: alkohli
ms.service: databox
ms.subservice: edge
...
ms.locfileid: "110614912"
---
# <a name="azure-stack-edge-2105-release-notes"></a>Notes de publication d’Azure Stack Edge 2105

[!INCLUDE [applies-to-GPU-and-pro-r-and-mini-r-skus](../../includes/azure-stack-edge-applies-to-gpu-pro-r-mini-r-sku.md)]

Les notes de publication suivantes identifient les problèmes critiques et les résolutions de la version 2105 pour les appareils Azure Stack Edge. Ces notes de publication s’appliquent aux appareils Azure Stack Edge Pro avec GPU, Azure Stack Edge Pro R et Azure Stack Edge Mini R. Les fonctionnalités et les problèmes qui correspondent à un modèle spécifique sont indiqués dans la mesure du possible.
...
Cet article concerne la version **2105 d’Azure Stack Edge** et correspond au numéro de version logicielle **2.2.1606.3320**. Vous pouvez appliquer ce logiciel à votre appareil si vous exécutez au moins un logiciel Azure Stack Edge 2010 (2.1.1377.2170).

## <a name="whats-new"></a>Nouveautés

Les nouvelles fonctionnalités suivantes sont disponibles dans la version 2105 d’Azure Stack Edge. 

- **Prise en charge de la configuration du réseau local virtuel (VLAN)**  : dans cette version, la configuration du VLAN peut être modifiée en se connectant à l’interface PowerShell de l’appareil. Pour plus d’informations, consultez [Créer des réseaux VLAN sur un commutateur virtuel](azure-stack-edge-gpu-create-virtual-switch-powershell.md).
- **Prise en charge du transfert IP** : à compter de cette version, le transfert IP est pris en charge pour les interfaces réseau attachées aux machines virtuelles. 
    - Le transfert IP permet aux machines virtuelles de recevoir le trafic réseau depuis une adresse IP non attribuée à l’une des configurations IP affectées à une interface réseau sur la machine virtuelle. 
    - Le transfert IP permet également aux machines virtuelles d’envoyer le trafic réseau avec une adresse IP source différente de celle affectée aux configurations IP pour l’interface réseau de la machine virtuelle.
        
    Pour plus d’informations, consultez [Activer et désactiver le transfert IP](../virtual-network/virtual-network-network-interface.md#enable-or-disable-ip-forwarding). 
- **Améliorations apportées à Kubernetes** : dans cette version, plusieurs améliorations liées à Kubernetes ont été apportées.
    - Les mises à jour de version Kubernetes suivantes sont disponibles : 

        - Version du serveur Kubernetes : v1.20.2
        - Version d’IoT Edge : 0.1.0-beta14
        - Version Kubernetes avec Azure Arc : 1.1
    - Kubernetes avec Azure Arc prend désormais en charge différents clouds, la journalisation est améliorée et l’expérience de l’applet de commande via l’interface PowerShell a changé. 
    - Des correctifs de diagnostic et de télémétrie ont été effectués.
    - La collecte proactive des journaux est améliorée pour les journaux de calcul. 

- **Prise en charge des applets de commande Az** : à partir de cette version, les applets de commande Az sont disponibles (en préversion) lors de la connexion à l’instance Azure Resource Manager locale de l’appareil ou lors du déploiement de charges de travail de machine virtuelle. Pour plus d’informations, consultez [Applets de commande Az](/powershell/azure/new-azureps-module-az?view=azps-5.9.0&preserve-view=true).
- **Activer la session PowerShell à distance sur HTTP** : le démarrage de cette version vous permet d’activer une session PowerShell à distance dans un appareil, via *http*, par le biais de l’interface utilisateur locale. Pour plus d’informations, découvrez comment [Activer PowerShell à distance sur http](azure-stack-edge-gpu-manage-access-power-connectivity-mode.md#enable-device-access-via-remote-powershell-over-http) pour votre appareil.


## <a name="issues-fixed-in-2105-release"></a>Problèmes résolus dans la version 2105
...
| --- | --- | --- |
|**1.**|Machine virtuelle |Une défaillance lors du renouvellement du bail DHCP ne doit pas entraîner la suppression de l’enregistrement de l’interface réseau.|
|**2.**|Machine virtuelle | Améliorations de la supervision apportées à la résolution des problèmes de verrouillage lors du provisionnement des machines virtuelles.| 

## <a name="known-issues-in-2105-release"></a>Problèmes connus dans la version 2105

Le tableau suivant récapitule les problèmes connus de la version 2105.
...
| Non. | Fonctionnalité | Problème | Solution de contournement/commentaires |
| --- | --- | --- | --- |
| **1.** |Azure Stack Edge Pro + SQL Azure | La création d’une base de données SQL nécessite un accès administrateur.   |Effectuez les étapes suivantes à la place des étapes 1 à 2 dans [Create-the-sql-database](../iot-edge/tutorial-store-data-sql-server.md#create-the-sql-database). <ul><li>Dans l’interface utilisateur locale de votre appareil, activez l’interface de calcul. Sélectionnez **Compute > N° de port > Activer pour le calcul > Appliquer.**</li><li>Téléchargez `sqlcmd` sur votre ordinateur client à partir de l’[utilitaire de commande SQL](/sql/tools/sqlcmd-utility). </li><li>Connectez-vous à l’adresse IP de votre interface de calcul (port activé), en ajoutant un « 1401 » à la fin de l’adresse.</li><li>La commande finale ressemble à ceci : sqlcmd -S {Interface IP},1401 -U SA -P "Strong!Passw0rd".</li>Ensuite, les étapes 3-4 de la documentation actuelle devraient être identiques. </li></ul> |
