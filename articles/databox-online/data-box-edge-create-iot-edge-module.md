---
title: Module IoT Edge en C# pour Azure Data Box Edge | Microsoft Docs
description: Découvrez comment développer un module IoT Edge C# pouvant être déployé sur votre appareil Data Box Edge.
services: databox
author: alkohli
ms.service: databox
ms.subservice: edge
...
Cet article vous guide tout au long de la création d’un module IoT Edge pour un déploiement avec votre appareil Data Box Edge. Azure Data Box Edge est une solution de stockage qui vous permet de traiter les données et de les envoyer vers Azure via le réseau.

Vous pouvez utiliser les modules Azure IoT Edge avec votre appareil Data Box Edge pour transformer les données déplacées vers Azure. Le module de cet article implémente une logique conçue pour copier un fichier à partir d’un partage local vers un partage cloud sur votre appareil Data Box Edge.

Dans cet article, vous apprendrez comment :

> [!div class="checklist"]
> * Créer un registre de conteneurs pour stocker et gérer vos modules (images Docker).
> * Créer un module IoT Edge à déployer sur votre appareil Data Box Edge.
...
Votre appareil Data Box Edge peut déployer et exécuter des modules IoT Edge. Les modules Edge sont essentiellement des conteneurs Docker qui effectuent une tâche spécifique, comme la réception d’un message à partir d’un appareil, la transformation d’un message ou l’envoi d’un message à un IoT Hub. À l’aide de cet article, vous allez créer un module qui copie des fichiers à partir d’un partage local vers un partage cloud sur votre appareil Data Box Edge.

1. Les fichiers sont écrits sur le partage local de votre appareil Data Box Edge.
2. Le générateur d’événements de fichier crée un événement de fichier pour chaque fichier écrit sur le partage local. Les événements de fichier sont ensuite envoyés au hub IoT Edge (dans le runtime IoT Edge).

   > [!IMPORTANT]
   > Les événements de fichier sont générés uniquement pour les fichiers nouvellement créés. La modification des fichiers existants ne génère pas d’événement de fichier.
...
![Fonctionnement du module Azure IoT Edge sur Data Box Edge](./media/data-box-edge-create-iot-edge-module/how-module-works.png)
...
- Un appareil Data Box Edge en cours d’exécution.

    - L’appareil doit également être associé à une ressource IoT Hub. Pour plus d’informations, accédez à la section vous expliquant comment [Créer une ressource IoT Hub](data-box-edge-deploy-configure-compute.md#create-an-iot-hub-resource) pour votre appareil Data Box Edge.
    - Le rôle de calcul Edge est configuré pour l’appareil. Pour plus d’informations, accédez à la section [Configurer le rôle de calcul](data-box-edge-deploy-configure-compute.md#set-up-compute-role) relative à votre appareil Data Box Edge.
...
5. Une fois votre registre de conteneurs créé, accédez à celui-ci, puis sélectionnez **Clés d’accès**.
    ...
    ![Obtenir les clés d’accès](./media/data-box-edge-create-iot-edge-module/get-access-keys-1.png)
...
3. Dans la palette de commandes, entrez et exécutez la commande **Azure IoT Edge : nouvelle solution IoT Edge**. Dans la palette de commandes, spécifiez les informations suivantes pour créer votre solution :
    ...
    2. Spécifiez un nom pour votre solution ou acceptez le nom par défaut **EdgeSolution**.
        ...
        ![Créer une solution 1](./media/data-box-edge-create-iot-edge-module/create-new-solution-1.png)
    ...
    4. Remplacez le nom de module par défaut par le nom que vous souhaitez lui attribuer, dans ce cas, **FileCopyModule**.
        ...
        ![Créer une solution 2](./media/data-box-edge-create-iot-edge-module/create-new-solution-2.png)
    ...
    5. Spécifiez le registre de conteneurs que vous avez créé dans la section précédente comme référentiel d’images pour votre premier module. Remplacez **localhost:5000** par la valeur de serveur de connexion que vous avez copiée.
        ...
        ![Créer une solution 3](./media/data-box-edge-create-iot-edge-module/create-new-solution-3.png)

4. Accédez à **Fichier > Ouvrir le dossier**.

    ![Créer une solution 4](./media/data-box-edge-create-iot-edge-module/create-new-solution-4.png)
...
5. Parcourez les dossiers et sélectionnez le dossier **EdgeSolution** créé précédemment. La fenêtre VS Code charge l’espace de travail de votre solution IoT Edge, avec ses cinq composants de niveau supérieur. Cet article ne vous fera pas modifier le dossier **.vscode**, ni les fichiers **.gitignore**, **.env** et **deployment.template.json**.
    ...
    ![Créer une solution 5](./media/data-box-edge-create-iot-edge-module/create-new-solution-5.png)
...
3. Ajouter les variables **InputFolderPath** et **OutputFolderPath** à la classe Program.
    ...
    > [!IMPORTANT]
    > Notez les valeurs `InputFolderPath` et `OutputFolderPath`. Vous devrez fournir ces chemins d’accès lorsque vous déploierez ce module.
...
6. Insérez le code pour **FileCopy**.
...
1. Connectez-vous à Docker en entrant la commande suivante dans le terminal intégré Visual Studio Code.

    `docker login <ACR login server> -u <ACR username>`

    Utilisez le serveur de connexion et le nom d’utilisateur copiés plus tôt à partir du registre de conteneurs. 

    ![Générer et envoyer (push) la solution IoT Edge](./media/data-box-edge-create-iot-edge-module/build-iot-edge-solution-1.png)
...
3. Une fois les informations d’identification fournies, vous pouvez transmettre l’image de votre module au registre Azure Container Registry. Dans l’Explorateur VS Code, cliquez avec le bouton droit sur le fichier **module.json** et sélectionnez **Build and Push IoT Edge solution** (Générer et envoyer (push) la solution IoT Edge).
    ...
    ![Générer et envoyer (push) la solution IoT Edge](./media/data-box-edge-create-iot-edge-module/build-iot-edge-solution-2.png)
    ...
    *Program.cs(77,44): warning CS1998: Cette méthode async n’a pas d’opérateur « await » et elle s’exécutera de façon synchrone. Utilisez l’opérateur « await » pour attendre les appels d’API non bloquants ou « await Task.Run(…) » pour effectuer un travail utilisant le processeur sur un thread d’arrière-plan.*
...
Pour déployer et exécuter ce module sur Data Box Edge, consultez les étapes décrites dans la section [Ajouter un module personnalisé](data-box-edge-deploy-configure-compute.md#add-a-custom-module).
