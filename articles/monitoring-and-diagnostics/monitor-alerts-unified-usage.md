...
Cet article explique comment configurer des alertes à l’aide de la nouvelle interface d’alertes dans le portail Azure. La définition d’une règle d’alerte se fait en trois parties :
...
Les alertes Azure fournissent également une vue unifiée pour toutes vos règles d’alerte et permettent de les gérer à un emplacement unique, y compris l’affichage de toutes les alertes non résolues. En savoir plus sur les fonctionnalités des [alertes Azure – Vue d’ensemble](monitoring-overview-unified-alerts.md).

Alerts utilise le terme **alertes de journal** pour décrire les alertes où le signal est une requête personnalisée basée sur [Log Analytics](../log-analytics/log-analytics-tutorial-viewdata.md) ou [Application Insights](../application-insights/app-insights-analytics.md). La [nouvelle fonctionnalité d’alerte métrique](monitoring-near-real-time-metric-alerts.md) permet d’émettre des alertes concernant des [métriques multidimensionnelles](monitoring-metric-charts.md) pour des ressources Azure spécifiques. Les alertes pour ces ressources peuvent utiliser des filtres supplémentaires pour les dimensions de création d’**alertes de métrique multidimensionnelle**.


> [!NOTE]
> Bien que les alertes Azure offrent une nouvelle expérience améliorée pour la création d’alertes dans Azure, les [alertes (classique)](monitoring-overview-alerts.md) existantes restent utilisables.
>

Vous trouverez ci-dessous un guide détaillé pour l’utilisation des alertes Azure.
...
1.  Définissez la condition d’alerte en utilisant le lien **Sélectionner une ressource**, puis en spécifiant la cible via la sélection d’une ressource. Filtrez en choisissant *Abonnement, *Type de ressource, puis en sélectionnant la *ressource* nécessaire.
 ...
 L’interface utilisateur vous permet de créer différents types d’alertes à partir d’un seul emplacement. Selon le type d’alerte souhaité, choisissez l’une des étapes suivantes :
    ...
    > Les alertes de journal d’activité sont également prises en charge, mais cette fonctionnalité est en version préliminaire pour l'instant. [Plus d’informations](monitoring-activity-log-alerts-new-experience.md)

1. *Alertes de métrique* : assurez-vous que le **type de ressource** est sélectionné avec le type de signal **Métrique**. Ensuite, une fois la **ressource** appropriée choisie, cliquez sur le bouton *Terminé* pour revenir à la création de l’alerte. Utilisez ensuite le bouton **Ajouter des critères** pour choisir le signal spécifique dans la liste des options de signal, le service de surveillance et le type répertorié, qui sont disponibles pour la ressource sélectionnée précédemment.
...
1. Le **tableau de bord des alertes** s’affiche. Il contient toutes les alertes Azure unifiées et rassemblées dans un ![tableau de bord d’alertes](./media/monitoring-alerts-unified-usage/alerts-preview-overview.png) unique.
