---
title: Exemples PowerShell et Graph pour les licences de groupe – Azure AD | Microsoft Docs
description: Exemples et scénarios PowerShell + Graph pour les licences basées sur les groupes Azure Active Directory
...
Les fonctionnalités complètes pour les licences basées sur les groupes sont disponibles via le [portail Azure](https://portal.azure.com), et il existe actuellement des tâches utiles qui peuvent être effectuées à l’aide des [applets de commande PowerShell MSOnline](/powershell/module/msonline) et Microsoft Graph existantes. Ce document fournit des exemples de ce qui est possible.

> [!NOTE]
> Avant de commencer à exécuter des cmdlets, connectez-vous à votre organisation en exécutant la cmdlet `Connect-MsolService`.
...
## <a name="assign-licenses-to-a-group"></a>Attribuer des licences à un groupe

Utilisez l’exemple suivant pour attribuer des licences à un groupe à l’aide de Microsoft Graph :
...
L’applet de commande [Get-MsolGroup](/powershell/module/msonline/get-msolgroup) peut être utilisé pour récupérer l’objet de groupe et vérifier la propriété *Licences* : elle répertorie toutes les licences de produit actuellement affectées au groupe.
...
Utilisez l’exemple suivant pour obtenir les mêmes données de Microsoft Graph.
...
Utilisez ce qui suit pour obtenir les mêmes données de Microsoft Graph :
...
## <a name="get-all-users-with-license-errors-in-the-entire-organization"></a>Obtenir tous les utilisateurs avec des erreurs de licence dans l’organisation entière
...
> [!NOTE]
> Ce script énumère tous les utilisateurs dans l’organisation, ce qui peut ne pas être optimal pour les organisations de grande taille.
...
Graph n’offre aucun moyen simple de montrer les résultats, mais il est possible de les voir à partir cette API :
...
L'objectif de ce script est de supprimer les licences directes inutiles des utilisateurs qui héritent déjà de la même licence auprès d'un groupe ; par exemple, dans le cadre d'une [transition vers la gestion des licences basée sur les groupes](licensing-groups-migrate-users.md).
> [!NOTE]
...
```
> [!NOTE]
> Avant d'exécuter le script ci-dessus, mettez à jour les valeurs des variables `$skuId` et `$groupId` qui sont visées par une suppression de licences directes comme indiqué dans votre environnement de test. 
