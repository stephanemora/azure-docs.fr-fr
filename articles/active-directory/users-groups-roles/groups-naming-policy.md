...
> [!IMPORTANT]
> À l’aide de la stratégie de nommage d’Azure AD pour les groupes Office 365 nécessite que vous possédez, mais pas nécessairement affectez une licence d’Azure Active Directory Premium P1 ou Azure AD Basic EDU pour chaque utilisateur unique qui est membre d’un ou plusieurs groupes Office 365.

La stratégie de nommage s’applique à la création ou à la modification des groupes créés dans toutes les charges de travail (par exemple, Outlook, Microsoft Teams, SharePoint, Exchange ou Planner). Elle s’applique à la fois au nom de groupe et à l’alias de groupe. Si vous définissez votre stratégie d’affectation de noms dans Azure AD et que vous disposez d’un groupe Exchange stratégie de nommage, Azure AD stratégie de nommage est appliqué dans votre organisation.

## <a name="naming-policy-features"></a>Caractéristiques de la stratégie de nommage

Vous pouvez appliquer une stratégie d’affectation de noms pour les groupes de deux manières différentes :
...
## <a name="configure-the-group-naming-policy-for-a-tenant-using-azure-portal-preview"></a>Configurer le groupe de stratégie de nommage pour un client à l’aide du portail Azure (version préliminaire)
...
1. Enregistrer les paramètres pour la nouvelle stratégie pour accéder en vigueur, comme dans l’exemple suivant.
...
## <a name="export-or-import-the-list-of-custom-blocked-words-using-azure-ad-powershell"></a>Exporter ou importer la liste de mots bloqués personnalisés à l’aide d’Azure AD PowerShell
