...
1. Une fois le contenu du fichier validé, corrigez les informations qu’il contient, puis envoyez le nouveau le fichier s’il existe des erreurs. L’envoi d’un fichier valide démarre automatiquement le travail de chargement des données.
1. Une fois votre fichier validé, sélectionnez **Envoyer** pour démarrer le programme de traitement par lots Azure qui restaure les utilisateurs. Si des erreurs se produisent, vous pouvez télécharger et consulter le fichier de résultats sur la page Résultats de l’opération en bloc. Le fichier contient la raison de chaque erreur.
