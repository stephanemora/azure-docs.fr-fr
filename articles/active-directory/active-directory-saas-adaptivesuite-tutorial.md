...
3. Dans la section **Domaine et URL Adaptive Suite**, procédez comme suit :
    ...
    Cochez la case **Afficher les paramètres d’URL avancés**. Dans la zone de texte **Identificateur (ID d’entité)**, entrez la même URL que vous avez entrée dans **URL de réponse**.
    
    >[!NOTE]
    > L’ID d’entité pour chaque domaine Adaptative Insights est identique à son URL de réponse.
    >
