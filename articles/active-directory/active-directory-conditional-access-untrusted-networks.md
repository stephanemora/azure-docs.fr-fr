...
- [Démarrage rapide : Exiger l’authentification multifacteur (MFA) pour certaines applications disposant d’un accès conditionnel Azure Active Directory](active-directory-conditional-access-app-based-mfa.md) pour acquérir de l’expérience dans la configuration de stratégies d’accès conditionnel. 
...
Le défi de ce scénario consiste à traduire *lorsque la tentative d’accès provient d’un emplacement non approuvé* en une condition d’accès conditionnel. Dans une stratégie d’accès conditionnelle, vous pouvez configurer la [condition d’emplacements](active-directory-conditional-access-locations.md) de façon à gérer les scénarios liés aux emplacements réseau. Cette condition permet de sélectionner des [emplacements nommés](active-directory-conditional-access-locations.md#named-locations), qui représentent des regroupements logiques de plages d’adresses IP, de pays et de régions.  
