...
Gardez les points suivants à l’esprit lorsque vous allez le didacticiel :
...
* Ajouter au moins un utilisateur à l’application et vérifiez que le compte de test a accès à l’application en local.
...
1. Dans le **définir des Single Sign-On with SAML** page, modifiez le **base SAML Configuration** données et suivez les étapes de [configuration de SAML base entrée](configure-single-sign-on-non-gallery-applications.md#saml-based-single-sign-on) configuration basée sur SAML authentification pour l’application.
   ...
   * Assurez-vous que le **URL de réponse** racine correspond ou est un chemin d’accès sous le **URL externe** pour l’application en local que vous avez ajouté pour l’accès à distance via le Proxy d’Application dans Azure AD.
...
1. Connectez-vous au compte de test que vous avez attribué à l’application.
