...
- Un locataire Azure Active Directory avec un plan qui comprend le proxy d’application. En savoir plus sur [les plans et la tarification d’Azure Active Directory](https://azure.microsoft.com/pricing/details/active-directory/).
- Un [domaine vérifié personnalisé](../fundamentals/add-custom-domain.md) dans le locataire Azure AD.
- Un Active Directory local synchronisé avec Azure AD Connect avec des utilisateurs capables de [se connecter à Azure](../hybrid/plan-connect-user-signin.md).
...
- Une URL externe, visible par les utilisateurs finaux et déterminée dans Azure Active Directory. Cette URL peut utiliser un domaine personnalisé. Pour plus d’informations, consultez [Utilisation des domaines personnalisés dans le proxy d’application Azure AD](application-proxy-configure-custom-domain.md).
...
> - Dans le DNS d’entreprise, créer un hôte (A) pointant vers le serveur web frontal (ou un équilibreur de charge) SharePoint, et pas un Alias (CName)

Nous utilisons les valeurs suivantes dans cet article :
...
Dans cette étape, vous allez créer une application dans votre locataire Azure Active Directory qui utilisera le proxy d’application. Vous allez définir l’URL externe et spécifier l’URL interne, les deux utilisées ultérieurement dans SharePoint.
...
1. Créez l’application comme décrit avec les paramètres suivants. Pour obtenir des instructions pas à pas, consultez [Publication d’applications à l’aide du proxy d’application Azure AD](application-proxy-add-on-premises-application.md#add-an-on-premises-app-to-azure-ad).
   ...
   * **Traduire l’URL dans le corps de l’application** : Non
...
1. Créez une application web et utilisez uniquement la zone par défaut. Il s’agit de l’option recommandée pour une expérience optimale avec SharePoint (par exemple, les liens des alertes par e-mail générées par SharePoint pointent toujours vers la zone par défaut).
1. Étendez une application web existante pour configurer Kerberos sur une zone autre que celle par défaut.

> [!IMPORTANT]
> Quelle que soit la zone qui sera utilisée, le compte du pool d’applications de l’application web SharePoint doit être un compte de domaine pour que Kerberos fonctionne correctement.
...
### <a name="ensure-that-the-sharepoint-web-application-is-running-under-a-domain-account"></a>Vérifier que l’application web SharePoint s’exécute sous un compte de domaine

Procédez comme suit pour identifier le compte qui exécute le pool d’applications de l’application web SharePoint et vérifiez qu’il s’agit d’un compte de domaine :
...
1. Confirmez que **Sélectionner un compte pour ce composant** retourne un compte de domaine et souvenez-vous qu’il sera nécessaire à l’étape suivante.

### <a name="ensure-that-an-https-certificate-is-configured-for-the-iis-site-of-the-extranet-zone"></a>Vérifier qu’un certificat HTTPS est configuré pour le site IIS de la zone Extranet

Étant donné que l’URL interne utilise le protocole HTTPS (`https://SharePoint/`), un certificat doit être défini sur le site IIS.
...
## <a name="step-3-configure-kerberos-constrained-delegation-kcd"></a>Étape 3 : Configurer une délégation Kerberos contrainte (KCD)

Les utilisateurs s’authentifient initialement dans Azure Active Directory, puis dans SharePoint à l’aide de Kerberos via le connecteur de proxy Azure AD. Pour permettre au connecteur d’obtenir un jeton Kerberos pour le compte de l’utilisateur Azure Active Directory, il est nécessaire de configurer la délégation Kerberos contrainte avec la transition de protocole. Pour en savoir plus sur la KCD, consultez [Présentation de la délégation Kerberos contrainte](https://docs.microsoft.com/previous-versions/windows/it-pro/windows-server-2012-R2-and-2012/jj553400(v=ws.11)).

### <a name="set-the-service-principal-name-for-the-sharepoint-service-account"></a>Définir le nom de principal de service pour le compte de service SharePoint

Dans cet article, l’URL interne est `https://sharepoint`, donc le SPN est `HTTP/sharepoint`. Vous devez remplacer ces valeurs par celles correspondant à votre environnement.
...
La commande Setspn recherche le SPN avant de l’ajouter. S’il existe déjà, une erreur **Valeur SPN en double** apparaît. Dans ce cas, envisagez de supprimer le SPN existant s’il n’est pas défini sous le compte de pool d’applications correct.  
Vous pouvez vérifier que le SPN a bien été ajouté en exécutant la commande Setspn avec l’option -L. Pour en savoir plus sur cette commande, consultez [Setspn](https://docs.microsoft.com/previous-versions/windows/it-pro/windows-server-2012-R2-and-2012/cc731241(v=ws.11)).
...
1. Cliquez sur le bouton **Ajouter**, cliquez sur **Utilisateurs ou ordinateurs**, puis recherchez le compte de pool d’applications SharePoint, par exemple `Contoso\spapppool`.
