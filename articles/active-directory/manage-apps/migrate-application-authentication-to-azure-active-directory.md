...
[Azure Active Directory (Azure AD)](/azure/active-directory/fundamentals/active-directory-whatis) propose une plateforme d’identités universelle qui offre à vos employés, partenaires et clients une identité unique pour accéder aux applications qu’ils souhaitent et collaborer à partir de n’importe quelle plateforme et de n’importe quel appareil.
...
Azure AD comprend une [suite complète de fonctionnalités de gestion des identités](/azure/active-directory/fundamentals/active-directory-whatis#which-features-work-in-azure-ad). La standardisation de l’authentification et de l’autorisation de votre application avec Azure AD permet d’offrir les avantages de ces fonctionnalités.

Consultez les ressources supplémentaires sur la migration : [https://aka.ms/migrateapps](https://aka.ms/migrateapps)
...
- Améliorer l’accès des utilisateurs sécurisés aux applications et aux données d’entreprise associées à l’aide de [stratégies d’accès conditionnel](/azure/active-directory/active-directory-conditional-access-azure-portal), [Multi-Factor Authentication](/azure/active-directory/authentication/concept-mfa-howitworks) et des technologies [Identity Protection](/azure/active-directory/active-directory-identityprotection) en temps réel basées sur les risques.

- Protéger l’accès des utilisateurs privilégiés à votre environnement avec un accès administrateur [juste-à-temps](/azure/managed-applications/request-just-in-time-access).
...
- Offrant un accès à distance sécurisé aux applications locales utilisant le [proxy d’application Azure AD](/azure/active-directory/manage-apps/application-proxy).

- Découplant les applications de l’approche locale en matière d’informations d’identification dans votre locataire en [configurant Azure AD comme fournisseur d’identité universel approuvé](/azure/active-directory/hybrid/plan-connect-user-signin#choosing-the-user-sign-in-method-for-your-organization).
...
- Améliorer l’expérience d’[authentification unique (SSO)](/azure/active-directory/manage-apps/what-is-single-sign-on) de l’utilisateur final grâce à un accès transparent et sécurisé à toute application, depuis n’importe quel appareil et n’importe quel emplacement.

- Tirer parti des fonctionnalités IAM en libre-service, telles que les [réinitialisations des mots de passe en libre-service](/azure/active-directory/authentication/concept-sspr-howitworks) et la [gestion des groupes en libre-service](/azure/active-directory/users-groups-roles/groups-self-service-management).
...
- Réduire la charge administrative en gérant une seule identité pour chaque utilisateur dans les environnements cloud et locaux :
  ...
  - [Automatiser le provisionnement](/azure/active-directory/active-directory-saas-app-provisioning) des comptes utilisateur (dans la [galerie Azure AD](https://azuremarketplace.microsoft.com/marketplace/apps/category/azure-active-directory-apps)) basé sur les identités Azure AD
  - Accéder à toutes vos applications à partir du panneau MyApps dans le [portail Azure ](https://portal.azure.com/)

- Permettre aux développeurs de sécuriser l’accès à leurs applications et d’améliorer l’expérience de l’utilisateur final à l’aide de la [Plateforme d’identités Microsoft](/azure/active-directory/develop/about-microsoft-identity-platform) avec la bibliothèque d’authentification Microsoft (MSAL).

- Donner à vos partenaires l’accès aux ressources cloud à l’aide d’[Azure AD B2B Collaboration](/azure/active-directory/active-directory-b2b-what-is-azure-ad-b2b). Cela supprime la surcharge liée à la configuration de la fédération point à point avec vos partenaires.

### <a name="address-compliance-and-governance"></a>Assurer la gouvernance et la conformité

Garantissez la conformité aux exigences réglementaires en appliquant des stratégies d’accès d’entreprise et en surveillant l’accès des utilisateurs aux applications et aux données associées à l’aide d’outils d’audit et d’API intégrés. Avec Azure AD, vous pouvez surveiller les connexions aux applications grâce aux rapports tirant profit des [outils SIEM (Security Incident and Event Monitoring)](/azure/active-directory/reports-monitoring/plan-monitoring-and-reporting). Vous pouvez accéder aux rapports à partir du portail ou des API et effectuer un audit programmatiquement pour identifier les utilisateurs qui ont accès à vos applications et supprimer l’accès aux utilisateurs inactifs via des révisions d’accès.
...
- **Utiliser Azure AD Connect Health** Si vous disposez d’une licence Azure AD Premium, nous vous recommandons de déployer [Azure AD Connect Health](/azure/active-directory/hybrid/how-to-connect-health-adfs) pour analyser l’utilisation des applications dans votre environnement local. Vous pouvez utiliser le [rapport d’application ADFS](/azure/active-directory/manage-apps/migrate-adfs-application-activity) (préversion) pour découvrir les applications ADFS pouvant migrer et évaluer si l’application à migrer est prête. Une fois la migration terminée, déployez [Cloud Discovery](/cloud-app-security/set-up-cloud-discovery) qui vous permet de surveiller en continu l’informatique fantôme dans votre organisation une fois que vous êtes dans le cloud.

- **Analyse des journaux ADFS**. Si vous n’avez pas de licences Azure AD Premium, nous vous recommandons d’utiliser les outils de migration d’application ADFS vers Azure AD basés sur [PowerShell](https://github.com/AzureAD/Deployment-Plans/tree/master/ADFS%20to%20AzureAD%20App%20Migration). Reportez-vous au [Guide de solution](https://aka.ms/migrateapps/adfssolutionguide) :

[Migration d’applications des services de fédération Active Directory (AD FS) vers Azure AD.](https://aka.ms/migrateapps/adfssolutionguide)
...
- **API** – pour les applications connectées à l’infrastructure cloud, vous pouvez utiliser les API et les outils sur ces systèmes pour commencer à effectuer un inventaire des applications hébergées. Dans l’environnement Azure :
  ...
  - Utilisez la cmdlet [Get-AzureWebsite](/powershell/module/servicemanagement/azure/get-azurewebsite?view=azuresmps-4.0.0&redirectedfrom=MSDN&preserve-view=true) pour obtenir des informations sur les sites web Azure.

  - Utilisez la cmdlet [Get-AzureRMWebApp](/powershell/module/azurerm.websites/get-azurermwebapp?view=azurermps-6.13.0&viewFallbackFrom=azurermps-6.2.0&preserve-view=true) pour obtenir des informations sur vos applications web Azure.
...
Outre les choix proposés dans la [galerie d’applications Azure AD](https://azuremarketplace.microsoft.com/marketplace/apps/category/azure-active-directory-apps), il peut s’agir d’applications qui existent déjà dans votre organisation ou d’applications tierces d’un fournisseur qui ne fait pas partie de la galerie Azure AD ([applications hors galerie](/azure/active-directory/manage-apps/add-non-gallery-app)).
...
Azure AD peut apporter de formidables avantages à ces applications héritées, car vous pouvez activer des fonctionnalités de sécurité et de gouvernance modernes Azure AD telles que l’[authentification multifacteur](/azure/active-directory/authentication/concept-mfa-howitworks), l’[accès conditionnel](/azure/active-directory/conditional-access/overview), [Identity Protection](/azure/active-directory/identity-protection/), l’[accès à l’application déléguée](/azure/active-directory/manage-apps/access-panel-manage-self-service-access) et les [révisions d’accès](https://docs.microsoft.com/azure/active-directory/governance/manage-user-access-with-access-reviews#create-and-perform-an-access-review) sur ces applications sans toucher à l’application elle-même.

Commencez par **étendre ces applications dans le cloud** avec le [proxy d’application](/azure/active-directory/manage-apps/application-proxy-configure-single-sign-on-password-vaulting) Azure AD à l’aide d’une simple authentification (comme le coffre des mots de passe) pour faire migrer rapidement vos utilisateurs ou via nos [intégrations de partenaires](https://azure.microsoft.com/services/active-directory/sso/secure-hybrid-access/) avec les contrôleurs de livraison d’application que vous avez peut-être déjà déployés.

### <a name="new-line-of-business-lob-apps"></a>Nouvelles applications métier (LOB)

En général, vous développez des applications métier pour l’utilisation interne dans votre organisation. Si vous avez de nouvelles applications dans le pipeline, nous vous recommandons d’utiliser la [Plateforme d’identités Microsoft](/azure/active-directory/develop/about-microsoft-identity-platform) pour implémenter OpenID Connect.
...
Bien entendu, **ne déconseillez pas les applications qui ont un impact important et qui sont vitales pour l’entreprise**. Dans ce cas, collaborez avec les propriétaires commerciaux pour déterminer la stratégie appropriée.
...
- Liste des applications qui inclut :
  ...
  - Quels sont les systèmes auxquels ces applications se connectent, quels sont les emplacements et les appareils à partir desquels les utilisateurs y accèdent.

  - Ces applications vont-elles être migrées, déconseillées ou connectées à [Azure AD Connect](/azure/active-directory/hybrid/whatis-azure-ad-connect).

> [!NOTE]
> Vous pouvez télécharger la [feuille de calcul de découverte des applications](/azure/active-directory/hybrid/whatis-azure-ad-connect) (Application Discovery Worksheet) pour enregistrer les applications que vous souhaitez migrer vers Azure AD Authentification et celles que vous souhaitez conserver mais gérer à l’aide d’[Azure AD Connect](https://download.microsoft.com/download/2/8/3/283F995C-5169-43A0-B81D-B0ED539FB3DD/Application%20Discovery%20worksheet.xlsx).
...
- **Durée de vie planifiée** – quelle est la durée de vie de cette application ? Moins de 6 mois ? Plus de 2 ans ?
...
- **Si vous envisagez de conserver l’application en local** – voulez-vous conserver l’application dans votre centre de données à long terme ?
...
Azure AD présente également des [limites de service et des restrictions](/azure/active-directory/users-groups-roles/directory-service-limits-restrictions) que vous devez connaître.
...
Avant de lancer le processus de migration, prenez le temps de prendre en compte la posture de sécurité que vous souhaitez développer pour votre système d’identité d’entreprise. Pour cela, collectez ces informations : **Identités et données, personnes ayant accès à vos données, appareils et emplacements**.

### <a name="identities-and-data"></a>Identités et données

La plupart des organisations ont des exigences spécifiques en matière d’identité et de protection des données qui varient en fonction des secteurs d’activité et des postes dans les organisations. Reportez-vous aux [configurations d’accès aux identités et aux appareils](/microsoft-365/enterprise/microsoft-365-policies-configurations) pour connaître nos recommandations, notamment un ensemble prescrit de [stratégies d’accès conditionnel](/azure/active-directory/active-directory-conditional-access-azure-portal) et les fonctionnalités associées.

Vous pouvez utiliser ces informations pour protéger l’accès à tous les services intégrés à Azure AD. Ces recommandations sont alignées sur le niveau de sécurité Microsoft, ainsi que sur le [score d’identité dans Azure AD](/azure/active-directory/fundamentals/identity-secure-score). Le degré de sécurisation vous aide à accomplir les tâches suivantes :
...
Cela vous aidera également à mettre en œuvre les [cinq étapes de sécurisation de votre infrastructure d’identité](/azure/security/azure-ad-secure-steps). Utilisez les conseils comme point de départ pour votre organisation et ajustez les stratégies pour répondre aux besoins spécifiques de votre organisation.
...
- **Externes :** Prestataires, fournisseurs, serveurs de distribution ou autres partenaires commerciaux qui interagissent avec votre organisation dans le cadre d’une activité régulière avec [Azure AD B2B Collaboration.](/azure/active-directory/b2b/what-is-b2b)

Vous pouvez définir des groupes pour ces utilisateurs et remplir ces groupes de différentes façons. Vous pouvez choisir l’ajout manuel des membres à un groupe par un administrateur ou activer l’appartenance au groupe en libre-service. Des règles peuvent être établies pour ajouter automatiquement des membres à des groupes en fonction des critères spécifiés à l’aide de [groupes dynamiques](/azure/active-directory/users-groups-roles/groups-dynamic-membership).

Les utilisateurs externes peuvent également faire référence à des clients qui nécessitent une attention particulière. [Azure AD B2C](/azure/active-directory-b2c/active-directory-b2c-overview), un produit distinct prend en charge l’authentification du client. Toutefois, ce contenu sort du cadre de cet article.
...
- **Conseils généraux sur la migration** – utilisez le livre blanc, les outils, les modèles d’e-mails et le questionnaire sur les applications dans la [boîte à outils de migration des applications d’Azure AD](https://aka.ms/migrateapps) pour découvrir, classifier et migrer vos applications.

- **Applications SaaS** – consultez notre liste de [centaines de tutoriels sur les applications SaaS](/azure/active-directory/active-directory-saas-tutorial-list) et le [plan de déploiement complet SSO Azure AD](https://aka.ms/ssodeploymentplan) pour parcourir le processus de bout en bout.

- **Applications en cours d’exécution en local** – découvrez le [proxy d’application Azure AD](/azure/active-directory/manage-apps/application-proxy) et utilisez le [plan de déploiement complet du proxy d’application Azure AD](https://aka.ms/AppProxyDPDownload) pour avancer rapidement.

- **Applications que vous développez** – lisez notre guide d’[intégration](/azure/active-directory/develop/active-directory-integrating-applications) et d’[inscription](/azure/active-directory/develop/active-directory-v2-app-registration) pas à pas.
...
Vous pouvez tester chaque application en vous connectant avec un utilisateur de test et en vous assurant que toutes les fonctionnalités sont les mêmes qu’avant la migration. Si vous décidez pendant le test que les utilisateurs devront mettre à jour leurs paramètres [MFA](/active-directory/authentication/howto-mfa-userstates) ou [SSPR](/azure/active-directory/authentication/quickstart-sspr), ou si vous ajoutez cette fonctionnalité pendant la migration, veillez à l’ajouter à votre plan de communication auprès des utilisateurs finaux. Consultez les modèles de communication pour [l’authentification multifacteur](https://aka.ms/mfatemplates) et [SSPR](https://aka.ms/ssprtemplates).

Une fois que vous avez migré les applications, accédez au [portail Azure](https://aad.portal.azure.com/) pour tester si la migration a réussi. Procédez comme suit :
...
- Sélectionnez **Gérer&gt; Accès conditionnel**. Passez en revue votre liste de stratégies et assurez-vous que vous ne bloquez pas l’accès à l’application avec une [stratégie d’accès conditionnel](/azure/active-directory/active-directory-conditional-access-azure-portal).
...
| **Authentification unique SAML** | Utilisez le bouton [Tester les paramètres SAML](/azure/active-directory/develop/howto-v1-debug-saml-sso-issues) sous **Authentification unique**. |
| **Authentification unique par mot de passe** | Téléchargez et installez l’[extension de connexion sécurisée à MyApps](/azure/active-directory/user-help/active-directory-saas-access-panel-introduction#my-apps-secure-sign-in-extension). Cette extension vous permet de lancer les applications cloud de votre organisation qui vous obligent à utiliser un processus d’authentification unique. |
| **[Application Proxy (Proxy d’application)](/azure/active-directory/manage-apps/application-proxy)** | Vérifiez que votre connecteur est en cours d’exécution et affecté à votre application. Pour obtenir de l’aide, consultez le [Guide de résolution des problèmes de proxy d’application](/azure/active-directory/manage-apps/application-proxy-troubleshoot). |

### <a name="troubleshoot"></a>Dépanner

Si vous rencontrez des problèmes, consultez notre [Guide de résolution des problèmes liés aux applications](https://aka.ms/troubleshoot-apps) pour obtenir de l’aide. Consultez également [Problèmes de connexion à une application personnalisée](/azure/active-directory/manage-apps/application-sign-in-problem-custom-dev).
...
- Vous pouvez également envisager de **fournir des liens vers l’authentification héritée**, en cas de problèmes avec l’authentification cloud.
...
- Assurez-vous que votre application a un **bouton Commentaires** ou des instructions pour contacter le **support technique** en cas de problèmes.
...
**Dirigez votre utilisateur** vers l’expérience du portail [MyApps](/azure/active-directory/user-help/my-apps-portal-end-user-access#my-apps-secure-sign-in-extension). À cet endroit, ils peuvent accéder à toutes les applications basées sur le cloud. Ces applications sont rendues disponibles à l’aide d’[Azure AD Connect](/azure/active-directory/hybrid/whatis-azure-ad-connect), et aux applications qui utilisent le [proxy d’application](/azure/active-directory/manage-apps/application-proxy), à condition qu’ils disposent d’autorisations pour accéder à ces applications.

Vous pouvez guider vos utilisateurs sur la façon de découvrir leurs applications :

- Utiliser la fonctionnalité d’[authentification unique existante](/azure/active-directory/active-directory-saas-custom-apps#existing-single-sign-on) pour **lier vos utilisateurs à n’importe quelle application**

- Activer l’[accès aux applications en libre-service](/azure/active-directory/application-access-self-service-how-to) à une application et **permettre aux utilisateurs d’ajouter des applications que vous organisez**

- [Masquer les applications des utilisateurs finaux](/azure/active-directory/manage-apps/hide-application-from-user-portal) (applications Microsoft par défaut ou autres applications) pour **rendre les applications dont ils ont besoin plus détectables**

### <a name="make-apps-accessible"></a>Rendre les applications accessibles

**Permettez aux utilisateurs d’accéder aux applications à partir de leurs appareils mobiles**. Les utilisateurs peuvent accéder au portail MyApps avec le navigateur géré par Intune sur leurs appareils [iOS](/azure/active-directory/manage-apps/hide-application-from-user-portal) 7.0 ou version ultérieure ou [Android](/azure/active-directory/manage-apps/hide-application-from-user-portal).

Les utilisateurs peuvent télécharger un **navigateur géré par Intune** :

- **Pour les appareils Android**, à partir de [Google Play Store](https://play.google.com/store/apps/details?id=com.microsoft.intune.mam.managedbrowser)
...
- **Convertir automatiquement les URL internes** que vous avez configurées dans le [proxy d’application](/azure/active-directory/manage-apps/application-proxy) en URL externes appropriées. Vos utilisateurs peuvent désormais travailler avec des liens qui leur sont familiers, quel que soit leur emplacement.
...
- **Sécuriser l’accès des utilisateurs aux applications.** Activez les [stratégies d’accès conditionnel](/azure/active-directory/active-directory-conditional-access-azure-portal) ou [Identity Protection](/azure/active-directory/active-directory-identityprotection) pour sécuriser l’accès des utilisateurs aux applications en fonction de l’état de l’appareil, de l’emplacement, etc.

- **Provisionnement automatique.** Configurez le [provisionnement automatique des utilisateurs](/azure/active-directory/manage-apps/user-provisioning) avec diverses applications SaaS tierces auxquelles les utilisateurs doivent accéder. En plus de créer des identités utilisateur, le provisionnement automatique comprend la maintenance et la suppression d’identités utilisateur en cas de modification de l’état ou des rôles.

- **Déléguer la gestion** des **accès utilisateur**. Le cas échéant, activez l’accès aux applications en libre-service à vos applications et *attribuez un approbateur d’entreprise pour approuver l’accès à ces applications*. Utilisez la [gestion de groupes en libre-service](/azure/active-directory/users-groups-roles/groups-self-service-management) pour les groupes affectés à des regroupements d’applications.
...
- **Auditez votre application** à l’aide des options **Applications d’entreprise, Audit** ou accédez aux mêmes informations à partir de l’[API de création de rapports Azure AD](/azure/active-directory/active-directory-reporting-api-getting-started-azure-portal) pour les intégrer à vos outils favoris.
...
- **Obtenez des insights sur la connexion** à l’aide des options **Applications d’entreprise, Connexions**. Accédez aux mêmes informations à partir de l’[API de création de rapports Azure AD.](/azure/active-directory/active-directory-reporting-api-getting-started-azure-portal)

- **Visualisez l’utilisation de votre application** à partir du [pack de contenu Azure AD Power BI](/azure/active-directory/active-directory-reporting-power-bi-content-pack-how-to)
...
De nombreux [plans de déploiement](https://aka.ms/deploymentplans) sont disponibles et nous en ajoutons sans cesse de nouveaux !
