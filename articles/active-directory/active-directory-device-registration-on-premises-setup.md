...
* Microsoft Azure Active Directory Connect (Azure AD Connect) [(Télécharger Azure AD Connect)](http://www.microsoft.com/download/details.aspx?id=47594)
* Domaine vérifié
...
| Tâche | Informations de référence |
| --- | --- |
| Terminez la 2ème partie de l’activation de l’écriture différée des appareils dans Azure AD Connect. Une fois cette tâche terminée, revenez à ce guide. |[Activation de l’écriture différée des appareils dans Azure AD Connect](hybrid/how-to-connect-device-writeback.md) |
...
- [Intégrer des répertoires locaux à Azure Active Directory](hybrid/whatis-hybrid-identity.md) (informations conceptuelles).

- [Installation personnalisée d’Azure AD Connect](hybrid/how-to-connect-install-custom.md) (instructions d’installation).
...
1. Copiez le répertoire et les sous-répertoires **[média]\support\adprep** sur l’un de vos contrôleurs de domaine Active Directory (**[média]** correspondant au chemin du média d’installation Windows Server 2012 R2).
1. À partir d’une invite de commandes, accédez au répertoire **adprep**, puis exécutez **adprep.exe /forestprep**. Suivez les instructions à l’écran pour terminer la mise à niveau du schéma.
...
1. Quand vous êtes invité à renseigner le **ServiceAccountName**, entrez le nom du compte de service que vous avez sélectionné pour AD FS. S’il s’agit d’un compte gMSA, entrez le nom au format **domaine\nomcompte$**. Pour un compte de domaine, utilisez le format **domaine\nomcompte**.
...
1. Dans le volet **Actions**, sélectionnez **Modifier l’authentification principale globale**.
1. Cochez **Activer l’authentification des appareils**, puis sélectionnez **OK**.
1. Par défaut, AD FS supprime régulièrement les appareils non utilisés d’Active Directory. Désactivez cette tâche quand vous utilisez le service Azure Active Directory Device Registration pour permettre la gestion des appareils dans Azure.
...
1. Entrez votre nom d’utilisateur au format UPN (par exemple, **dan@contoso.com**).
1. Sélectionnez **Joindre**.
1. Quand vous y êtes invité, connectez-vous avec vos informations d’identification. L’appareil est à présent joint.
...
Vous trouverez des instructions sur l’utilisation du package sur la page [Packages Windows Installer pour les ordinateurs autres que Windows 10](devices/hybrid-azuread-join-control.md#control-windows-down-level-devices).
...
1. Recherchez l’application à laquelle s’applique la nouvelle règle d’accès. Cliquez avec le bouton droit sur l’application, puis sélectionnez **Modifier les règles de revendication**.
1. Sélectionnez l’onglet **Règles d’autorisation d’émission**, puis sélectionnez **Ajouter une règle**.
1. Dans la liste déroulante **Modèle de règle de revendication**, sélectionnez **Autoriser ou refuser l’accès des utilisateurs en fonction d’une revendication entrante**. Sélectionnez ensuite **Suivant**.
1. Dans le champ **Nom de la règle de revendication**, tapez **Autoriser l’accès aux appareils inscrits**.
1. Dans la liste déroulante **Type de revendication entrante**, sélectionnez **Est un utilisateur inscrit**.
1. Dans le champ **Valeur de revendication entrante**, tapez **true**.
1. Sélectionnez la case d'option **Autoriser l'accès aux utilisateurs avec cette revendication entrante** .
1. Sélectionnez **Terminer**, puis **Appliquer**.
1. Supprimez les règles moins strictes que celle que vous avez créées. Par exemple, supprimez la règle par défaut **Autoriser l’accès à tous les utilisateurs**.
