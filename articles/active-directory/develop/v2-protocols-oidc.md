...
> [!IMPORTANT]
> Pour demander un jeton d’ID du point de terminaison /authorization, l’inscription d’application dans le [portail d’inscription](https://portal.azure.com) doit avoir l’octroi implicite d’id_tokens activée dans l’onglet authentification (qui définit la `oauth2AllowIdTokenImplicitFlow`indicateur dans le [manifeste d’application](reference-app-manifest.md) à `true`). S’il n’est pas activé, une erreur `unsupported_response` est retournée : « La valeur fournie pour le paramètre d'entrée 'response_type' n'est pas autorisée pour ce client. La valeur attendue est ’code’ »
...
| domain_hint |Facultatif | Le domaine de l’utilisateur dans un annuaire fédéré.  Ce paramètre ignore le processus de découverte par courrier électronique que l’utilisateur doit se soumettre sur la version 2.0-page de connexion, pour une peu plus l’expérience utilisateur. Pour les clients qui sont fédérés via un répertoire local comme AD FS, cela génère souvent un transparente connectez-vous en raison de l’ouverture de session existante. |
