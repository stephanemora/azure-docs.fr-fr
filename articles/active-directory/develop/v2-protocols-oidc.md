---
title: Plateforme d’identité Microsoft et le protocole OpenID Connect | Azure
...
OpenID Connect est un protocole d’authentification basé sur OAuth 2.0 que vous pouvez utiliser pour connecter de façon sécurisée un utilisateur à une application Web. Lorsque vous utilisez la mise en œuvre de Microsoft identity plateforme du point de terminaison d’OpenID Connect, vous pouvez ajouter de connexion et accès à l’API à vos applications web. Cet article est indépendant du langage. Il explique comment envoyer et recevoir des messages HTTP sans utiliser l’une des bibliothèques open source de Microsoft.
...
OpenID Connect décrit un document de métadonnées qui contient la plupart des informations requises pour une application se connecter. Cela inclut des informations telles que les URL à utiliser et l’emplacement des clés de signature publiques du service. Point de terminaison de plateforme d’identité Microsoft, il s’agit du document de métadonnées OpenID Connect que vous devez utiliser :
...
En règle générale, vous utilisez ce document de métadonnées pour configurer une bibliothèque OpenID Connect ou un Kit de développement logiciel (SDK). La bibliothèque utilise les métadonnées pour faire son travail. Toutefois, si vous n’utilisez pas une bibliothèque OpenID Connect prédéfinie, vous pouvez suivre les étapes décrites dans le reste de cet article pour connexion dans une application web à l’aide de l’extrémité de plateforme Microsoft identity.
...
> [!IMPORTANT]
> Pour demander un jeton d’ID du point de terminaison /authorization, l’inscription d’application dans le [portail d’inscription](https://portal.azure.com) doit avoir l’octroi implicite d’id_tokens activée dans l’onglet authentification (qui définit la `oauth2AllowIdTokenImplicitFlow`indicateur dans le [manifeste d’application](reference-app-manifest.md) à `true`). S’il n’est pas activé, un `unsupported_response` erreur est retournée : « La valeur fournie pour le paramètre d’entrée « response_type » n’est pas autorisée pour ce client. La valeur attendue est ’code’ »
...
| `redirect_uri` | Recommandé | L’URI de redirection de votre application, vers lequel votre application peut envoyer et recevoir des réponses d’authentification. Il doit correspondre exactement à l’un des URI de redirection enregistrés dans le portail, auquel s’ajoute le codage dans une URL. Si pas présent, le point de terminaison sera redirect_uri inscrites de prélèvement un au hasard à envoyer à l’utilisateur nouveau. |
...
| `domain_hint` | Facultatif | Le domaine de l’utilisateur dans un annuaire fédéré.  Ce paramètre ignore le processus de découverte par courrier électronique que l’utilisateur doit se soumettre sur la page de connexion, pour une peu plus l’expérience utilisateur. Pour les clients qui sont fédérés via un répertoire local comme AD FS, cela aboutit généralement à une connexion transparente sign-in en raison de l’ouverture de session existante. |

À ce stade, l’utilisateur est invité à saisir ses informations d’identification et à exécuter l’authentification. Le point de terminaison Microsoft identity plateforme vérifie que l’utilisateur a accepté les autorisations indiquées dans le `scope` paramètre de requête. Si l’utilisateur n’a pas donné son consentement à un de ces autorisations, le point de terminaison Microsoft identity plateforme invite l’utilisateur à donner son consentement pour les autorisations requises. Vous pouvez en savoir plus sur [autorisations, consentements et applications mutualisées](v2-permissions-and-consent.md).
...
| `invalid_resource` | La ressource cible n’est pas valide car il n’existe pas, Azure AD ne peut pas trouver ou il n’est pas configuré correctement. |Cela indique que la ressource, si elle existe, n’a pas été configurée dans le client. L’application peut proposer à l’utilisateur des instructions pour installer l’application et l’ajouter à Azure AD. |

## <a name="validate-the-id-token"></a>Validation du jeton d’ID

Réception simplement du jeton id_token n’est pas suffisante pour authentifier l’utilisateur ; Vous devez valider la signature du jeton de l’id_token et vérifier les revendications du jeton selon les besoins de votre application. Le point de terminaison Microsoft identity plateforme [jetons Web JSON (Jwt)](https://self-issued.info/docs/draft-ietf-oauth-json-web-token.html) et chiffrement à clé publique pour signer les jetons et vérifier qu’ils sont valides.

Vous pouvez décider de valider le `id_token` dans le client le code, mais une pratique courante consiste à envoyer le `id_token` à un serveur principal et les faites la validation. Une fois que vous avez validé la signature du jeton id_token, il existe certaines revendications, que vous devrez vérifier. Pour plus d’informations, consultez la page de [référence `id_token`](id-tokens.md), notamment les sections [Validation des jetons](id-tokens.md#validating-an-id_token) et [Informations importantes sur la substitution des clés de signature](active-directory-signing-key-rollover.md). Nous vous recommandons d’utiliser une bibliothèque pour analyser et valider les jetons. Il en existe au moins une pour la plupart des langages et plateformes.
...
Si vous souhaitez déconnecter l’utilisateur de votre application, vous ne pouvez pas vous contenter de supprimer les cookies de votre application ou d’arrêter la session de l’utilisateur. Vous devez également rediriger l’utilisateur vers le point de terminaison de l’identité de plate-forme Microsoft pour se déconnecter. Si vous ne le faites pas, l’utilisateur se réauthentifie à votre application sans entrer leurs informations d’identification à nouveau, car il disposera d’une session unique valide connectez-vous avec le point de terminaison Microsoft identity platform.
...
Lorsque vous redirigez l’utilisateur à la `end_session_endpoint`, le point de terminaison Microsoft identity plateforme efface la session utilisateur à partir du navigateur. Toutefois, l’utilisateur peut rester connecté à d’autres applications qui utilisent des comptes Microsoft pour s’authentifier. Pour permettre à ces applications déconnecter l’utilisateur simultanément, le Microsoft endpoint de plateforme d’identité envoie une requête HTTP GET à inscrit `LogoutUrl` de toutes les applications auxquelles l’utilisateur est actuellement connecté. Les applications doivent répondre à cette requête en effaçant toute session qui identifie l’utilisateur et en renvoyant une réponse `200`. Si vous souhaitez prendre en charge la déconnexion unique dans votre application, vous devez implémenter ce paramètre `LogoutUrl` dans le code de votre application. Vous pouvez définir le paramètre `LogoutUrl` à partir du portail d’inscription des applications.
...
En incluant des étendues d’autorisation dans la demande et à l’aide de `response_type=id_token code`, le point de terminaison Microsoft identity plateforme garantit que l’utilisateur a accepté les autorisations indiquées dans le `scope` paramètre de requête. Il renvoie un code d’autorisation à votre application afin de l’échanger contre un jeton d’accès.
