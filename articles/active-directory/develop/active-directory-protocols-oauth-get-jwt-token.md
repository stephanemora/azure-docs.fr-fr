...
| client_secret | requis pour les applications Web | Le secret d’application que vous avez créé dans le portail d’inscription des applications pour votre application. Ne l’utilisez pas dans une application native, car les secrets clients ne peuvent pas être stockés de manière fiable sur les appareils. Il est requis pour les applications web et les API web qui peuvent stocker de manière sécurisée les secrets clients côté serveur.  Les secrets clients doivent être encodés en URL avant d’être envoyés.                                                                                 |
