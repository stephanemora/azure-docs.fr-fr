...
description: Cet article fournit une vue d’ensemble des cinq scénarios d’authentification les plus courants pour Azure Active Directory (AAD).
...
Les jetons de sécurité (jetons d’accès et d’ID) émis par Azure AD contiennent des revendications, ou assertions d’informations, sur le sujet qui a été authentifié. Ces revendications peuvent être utilisées par l’application pour différentes tâches. Par exemple, des applications peuvent utiliser des revendications pour valider le jeton, identifier le client d’annuaire du sujet, afficher des informations sur l’utilisateur, déterminer l’autorisation du sujet, etc. Les revendications présentes dans un jeton de sécurité dépendent du type de jeton, du type d’informations d’identification utilisées pour authentifier l’utilisateur et de la configuration de l’application. Une brève description de chaque type de revendication émise par Azure AD est fournie dans le tableau ci-dessous. Pour plus d’informations, voir [Types de jeton et de revendication pris en charge](v1-id-and-access-tokens.md).
...
Consultez les exemples de code pour les scénarios du type navigateur web vers application web. Et consultez régulièrement cette page à laquelle nous ajoutons fréquemment de nouveaux exemples. [Application web](sample-v1-code.md#web-applications)
...
![Diagramme Application à page unique (SPA)](./media/authentication-scenarios/single_page_app.png)
...
Consultez les exemples de code pour les scénarios du type application à page unique (SPA). Veillez à consultez régulièrement cette page à laquelle nous ajoutons fréquemment de nouveaux exemples. [Application à page unique (SPA)](sample-v1-code.md#single-page-applications).
...
![Diagramme Application native vers API web](./media/authentication-scenarios/native_app_to_web_api.png)
...
1. L’application web doit obtenir un jeton d’accès pour pouvoir s’authentifier auprès de l’API web et extraire la ressource souhaitée. Elle envoie une demande au point de terminaison de jeton d’Azure AD, avec les informations d’identification, l’ID d’application et l’URI ID d’application de l’API web.
...
Consultez les exemples de code pour les scénarios du type application démon ou serveur vers API web. Et consultez régulièrement cette page à laquelle nous ajoutons fréquemment de nouveaux exemples. [Application serveur ou démon vers API web](sample-v1-code.md#daemon-applications-accessing-web-apis-with-the-applications-identity)
