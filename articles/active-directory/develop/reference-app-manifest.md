...
| `availableToOtherTenants` | Boolean | valeur True si l’application est partagée avec d’autres locataires ; Sinon, false. <br><br> _Remarque : Il est remplacé par `signInAudience` dans le [inscriptions d’application (version préliminaire)](https://developer.microsoft.com/en-us/graph/blogs/new-app-registration/) rencontrer._ | |
...
| `displayName` | String | Nom d’affichage de l’application. <br><br> _Remarque : Il est remplacé par `name` dans le [inscriptions d’application (version préliminaire)](https://developer.microsoft.com/en-us/graph/blogs/new-app-registration/) rencontrer._ | `"MyRegisteredApp"` |
...
| `homepage` | String | URL vers la page d'accueil de l'application. <br><br> _Remarque : Il est remplacé par `signInUrl` dans le [inscriptions d’application (version préliminaire)](https://developer.microsoft.com/en-us/graph/blogs/new-app-registration/) rencontrer._ | `"https://MyRegisteredApp"` |
| `objectId` | String | Identificateur unique de l’application dans le répertoire. <br><br> _Remarque : Il est remplacé par `id` dans le [inscriptions d’application (version préliminaire)](https://developer.microsoft.com/en-us/graph/blogs/new-app-registration/) rencontrer._ | `"f7f9acfc-ae0c-4d6c-b489-0a81dc1652dd"` |
...
| `publicClient` | Boolean | Spécifie si cette application est un client public (par exemple, une application installée s’exécutant sur un appareil mobile). <br><br> _Remarque : Il est remplacé par `allowPublicClient` dans le [inscriptions d’application (version préliminaire)](https://developer.microsoft.com/en-us/graph/blogs/new-app-registration/) rencontrer._ | |
| `publisherDomain` | String | Le domaine de l’Éditeur vérifié pour l’application. En lecture seule. | https://www.contoso.com |
| `replyUrls` | Tableau de chaînes | Cette propriété à valeurs multiples contient la liste des valeurs de redirect_uri inscrites que Azure AD acceptera comme destinations lors du renvoi des jetons. <br><br> _Remarque : Il est remplacé par `replyUrlsWithType` dans le [inscriptions d’application (version préliminaire)](https://developer.microsoft.com/en-us/graph/blogs/new-app-registration/) rencontrer._ | |
...
1. Si votre flux de travail vous oblige à enregistrer les manifestes dans votre référentiel de code source pour une utilisation ultérieure, nous vous suggérons de relocalisation les manifestes enregistrés dans votre référentiel avec celle que vous voyez dans le **inscriptions d’application (version préliminaire)** rencontrer.
