...
Le manifeste d’application contient une définition de tous les attributs d’un objet d’application dans la plateforme d’identité Microsoft. Il sert également de mécanisme de mise à jour de l’objet d’application. Pour plus d’informations sur l’entité de l’Application et son schéma, consultez le [documentation de l’entité Application d’API Graph](https://msdn.microsoft.com/Library/Azure/Ad/Graph/api/entity-and-complex-type-reference#application-entity).

Vous pouvez configurer les attributs d’une application par le biais du portail Azure ou par programmation à l’aide [API REST](https://docs.microsoft.com/previous-versions/azure/ad/graph/api/entity-and-complex-type-reference#application-entity) ou [PowerShell](https://docs.microsoft.com/powershell/module/azuread/?view=azureadps-2.0#applications). Toutefois, il existe des scénarios dans lesquels vous devez modifier le manifeste de l’application pour configurer un attribut de l’application. Ces scénarios sont les suivants :

* Si vous avez inscrit l’application en tant que comptes Microsoft personnels et d’architecture mutualisées d’Azure AD, vous ne pouvez pas modifier les comptes Microsoft pris en charge dans l’interface utilisateur. À la place, vous devez utiliser l’éditeur de manifeste d’application pour changer le type de compte pris en charge.
...
1. Connectez-vous le [Azure portal](https://portal.azure.com).
1. Sélectionnez le **Azure Active Directory** de service, puis sélectionnez **inscriptions**.
...
| `availableToOtherTenants` | Boolean | valeur True si l’application est partagée avec d’autres locataires ; Sinon, false. <br><br> _Remarque : Il est disponible uniquement dans l’expérience d’application des enregistrements (hérité). Remplacé par `signInAudience` dans le [inscriptions](https://go.microsoft.com/fwlink/?linkid=2083908) rencontrer._ | |
| `appId` | Chaîne | Spécifie l’identificateur unique de l’application qui est affectée à une application par Azure AD. | `"601790de-b632-4f57-9523-ee7cb6ceba95"` |
...
| `displayName` | Chaîne | Nom d’affichage de l’application. <br><br> _Remarque : Il est disponible uniquement dans l’expérience d’application des enregistrements (hérité). Remplacé par `name` dans le [inscriptions](https://go.microsoft.com/fwlink/?linkid=2083908) rencontrer._ | `"MyRegisteredApp"` |
| `errorUrl` | Chaîne | Non pris en charge. | |
| `groupMembershipClaims` | Chaîne | Configure la revendication `groups` émise dans un utilisateur ou un jeton d’accès OAuth 2.0 attendu par l’application. Pour définir cet attribut, utilisez l’une des valeurs de chaîne valides suivantes :<br/><br/>- `"None"`<br/>- `"SecurityGroup"` (pour groupes de sécurité et rôles Azure AD)<br/>- `"All"` (ceci permet d’obtenir tous les groupes de sécurité, les groupes de distribution et les rôles d’annuaire Azure AD dont l’utilisateur connecté est membre) | `"SecurityGroup"` |
| `homepage` | Chaîne | URL vers la page d'accueil de l'application. <br><br> _Remarque : Il est disponible uniquement dans l’expérience d’application des enregistrements (hérité). Remplacé par `signInUrl` dans le [inscriptions](https://go.microsoft.com/fwlink/?linkid=2083908) rencontrer._ | `"https://MyRegisteredApp"` |
| `objectId` | Chaîne | Identificateur unique de l’application dans le répertoire. <br><br> _Remarque : Il est disponible uniquement dans l’expérience d’application des enregistrements (hérité). Remplacé par `id` dans le [inscriptions](https://go.microsoft.com/fwlink/?linkid=2083908) rencontrer._ | `"f7f9acfc-ae0c-4d6c-b489-0a81dc1652dd"` |
...
| `id` | Chaîne | Identificateur unique de l’application dans le répertoire. Cet ID n’est l’identificateur utilisé pour identifier l’application d’une transaction de protocole. Il est utilisé pour référencer l’objet dans les requêtes de répertoire. | `"f7f9acfc-ae0c-4d6c-b489-0a81dc1652dd"` |
| `identifierUris` | String Array | Les URI définis par l’utilisateur qui identifient de manière unique une application web au sein de son locataire Azure AD ou au sein d’un domaine personnalisé vérifié si l’application est multi-locataires. | <code>[<br>&nbsp;&nbsp;"https://MyRegisteredApp"<br>]</code> |
...
| `parentalControlSettings` | Chaîne | `countriesBlockedForMinors` spécifie les pays dans lesquels l’application est bloquée pour les utilisateurs mineurs.<br>`legalAgeGroupRule` spécifie la règle de groupe de tranche d’âge légal qui s’applique aux utilisateurs de l’application. Peut être défini sur `Allow`, `RequireConsentForPrivacyServices`, `RequireConsentForMinors`, `RequireConsentForKids` ou `BlockMinors`.  | <code>{<br>&nbsp;&nbsp;&nbsp;"countriesBlockedForMinors":[],<br>&nbsp;&nbsp;&nbsp;"legalAgeGroupRule":"Allow"<br>}</code> |
...
| `publicClient` | Boolean | Spécifie si cette application est un client public (par exemple, une application installée s’exécutant sur un appareil mobile). <br><br> _Remarque : Il est disponible uniquement dans l’expérience d’application des enregistrements (hérité). Remplacé par `allowPublicClient` dans le [inscriptions](https://go.microsoft.com/fwlink/?linkid=2083908) rencontrer._ | |
| `publisherDomain` | Chaîne | Le domaine de l’Éditeur vérifié pour l’application. En lecture seule. | https://www.contoso.com |
| `replyUrls` | Tableau de chaînes | Cette propriété à valeurs multiples contient la liste des valeurs de redirect_uri inscrites que Azure AD acceptera comme destinations lors du renvoi des jetons. <br><br> _Remarque : Il est disponible uniquement dans l’expérience d’application des enregistrements (hérité). Remplacé par `replyUrlsWithType` dans le [inscriptions](https://go.microsoft.com/fwlink/?linkid=2083908) rencontrer._ | |
...
| `signInAudience` | Chaîne | Spécifie les comptes Microsoft pris en charge pour l’application actuelle. Les valeurs prises en charge sont les suivantes :<ul><li>**AzureADMyOrg** : utilisateurs avec un compte professionnel ou scolaire Microsoft dans le locataire Azure AD de mon organisation (par exemple, un seul locataire)</li><li>**AzureADMultipleOrgs** : utilisateurs avec un compte professionnel ou scolaire Microsoft dans le locataire Azure AD de n’importe quelle organisation (par exemple, multi-locataire)</li> <li>**AzureADandPersonalMicrosoftAccount** : utilisateurs avec un compte Microsoft personnel, ou un compte professionnel ou scolaire dans le locataire Azure AD de n’importe quelle organisation</li></ul> | `AzureADandPersonalMicrosoftAccount` |
| `tags` | String Array | Chaînes personnalisées pouvant être utilisées pour classer et identifier l’application. | <code>[<br>&nbsp;&nbsp;"ProductionApp"<br>]</code> |

## <a name="common-issues"></a>Problèmes courants

### <a name="manifest-limits"></a>Limites du manifeste

Un manifeste d’application comporte plusieurs attributs qui sont désignés comme des collections ; par exemple, approles keycredentials, knownClientApplications, identifierUris, rediretUris, requiredResourceAccess et oauth2Permissions. Dans le manifeste d’application complète pour n’importe quelle application, le nombre total d’entrées dans toutes les collections combinées a été limité à 1200. Si vous disposez déjà de redirection 100 URI spécifié dans le manifeste d’application, vous êtes la seule gauche avec les entrées restantes 1100 à utiliser dans toutes les autres collections combinées qui composent le manifeste.

> [!NOTE]
> Dans le cas où vous essayez d’ajouter des entrées de plus de 1 200 dans le manifeste d’application, vous pouvez rencontrer une erreur **« Échec de la mise à jour d’application xxxxxx. Détails de l’erreur : La taille du manifeste a dépassé sa limite. Réduisez le nombre de valeurs et renouvelez votre demande. »**

### <a name="unsupported-attributes"></a>Attributs non pris en charge

Le manifeste d’application représente le schéma du modèle d’application sous-jacent dans Azure AD. À mesure que le schéma sous-jacent évolue, l’éditeur de manifeste sera mis à jour pour refléter le nouveau schéma de temps à autre. Par conséquent, vous pouvez remarquer des nouveaux attributs s’affiche dans le manifeste d’application. Dans de rares occasions, vous pouvez remarquer une modification syntaxique ou sémantique dans les attributs existants, ou vous pouvez trouver un attribut qui existaient précédemment ne sont pas pris en charge plus. Par exemple, vous verrez les nouveaux attributs dans le [inscriptions](https://go.microsoft.com/fwlink/?linkid=2083908) qui sont connus avec un nom différent dans l’expérience d’application des enregistrements (hérité).


| Inscriptions (hérité)| Inscriptions des applications           |
|---------------------------|-----------------------------|
...
Pour obtenir une description de ces attributs, consultez le [manifeste référence](#manifest-reference) section.

Lorsque vous essayez de charger un manifeste téléchargé précédemment, vous pouvez voir les erreurs suivantes. Cela est probablement parce que l’éditeur de manifeste prend désormais en charge une version plus récente du schéma, qui ne correspond pas à celui que vous tentez de télécharger.

- «**N’a pas pu mettre à jour xxxxxx application. Détail de l’erreur : Identificateur d’objet non valide 'undefined'. [].** "
- «**N’a pas pu mettre à jour xxxxxx application. Détail de l’erreur : Un ou plusieurs valeurs de propriété spécifiées ne sont pas valides. [].** "
- «**N’a pas pu mettre à jour xxxxxx application. Détail de l’erreur : Pas autorisé à définir availableToOtherTenants dans cette version d’api pour la mise à jour. [].** "
- «**N’a pas pu mettre à jour xxxxxx application. Détail de l’erreur : Mises à jour de la propriété de 'replyUrls' n’est pas autorisée pour cette application. Utilisez plutôt 'replyUrlsWithType' propriété. [].** "
- «**N’a pas pu mettre à jour xxxxxx application. Détail de l’erreur : Une valeur sans nom de type a été trouvée et aucun type attendu n’est disponible. Lorsque le modèle est spécifié, chaque valeur dans la charge utile doit avoir un type qui peut être spécifié dans la charge utile, explicitement par l’appelant ou implicitement déduit de la valeur parent. []**"

Lorsque vous voyez une de ces erreurs, nous recommandons ce qui suit :

1. Modifier les attributs individuellement dans l’éditeur de manifeste au lieu de charger un manifeste téléchargé précédemment. Utilisez le [référence du manifeste](#manifest-reference) table pour comprendre la syntaxe et sémantique des anciens et les nouveaux attributs afin que vous pouvez modifier correctement les attributs qui vous intéresse. 
1. Si votre flux de travail vous oblige à enregistrer les manifestes dans votre référentiel de code source pour une utilisation ultérieure, nous vous suggérons de relocalisation les manifestes enregistrés dans votre référentiel avec celle que vous voyez dans le **inscriptions** rencontrer.

## <a name="next-steps"></a>Étapes suivantes

* Pour plus d’informations sur la relation entre l’application et les objets principal du service d’une application, consultez [Application et principal du service dans Azure AD](app-objects-and-service-principals.md).
* Consultez le [glossaire du développeur Microsoft identity plateforme](developer-glossary.md) pour les définitions de certains des principaux Microsoft identity plateforme concepts de développement.
