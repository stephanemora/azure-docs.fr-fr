...
| `errorUrl` | String | Non pris en charge. | |
...
| `homepage` | String | L’URL de la page d’accueil de l’application. <br><br> _Remarque : Il est disponible uniquement dans l’expérience d’application des enregistrements (hérité). Remplacé par `signInUrl` dans le [inscriptions](https://go.microsoft.com/fwlink/?linkid=2083908) rencontrer._ | `"https://MyRegisteredApp"` |
| `objectId` | String | Identificateur unique de l’application dans le répertoire. <br><br> _Remarque : Il est disponible uniquement dans l’expérience d’application des enregistrements (hérité). Remplacé par `id` dans le [inscriptions](https://go.microsoft.com/fwlink/?linkid=2083908) rencontrer._ | `"f7f9acfc-ae0c-4d6c-b489-0a81dc1652dd"` |
...
| `oauth2RequiredPostResponse` | Booléen | Spécifie si, dans le cadre des requêtes de jeton OAuth 2.0, Azure AD autorise les requêtes POST, par opposition aux requêtes GET. La valeur par défaut est false, ce qui indique que seules les requêtes GET sont autorisées. | `false` |
...
| `publicClient` | Booléen | Spécifie si cette application est un client public (par exemple, une application installée s’exécutant sur un appareil mobile). <br><br> _Remarque : Il est disponible uniquement dans l’expérience d’application des enregistrements (hérité). Remplacé par `allowPublicClient` dans le [inscriptions](https://go.microsoft.com/fwlink/?linkid=2083908) rencontrer._ | |
| `publisherDomain` | String | Le domaine de l’Éditeur vérifié pour l’application. En lecture seule. | https://www.contoso.com |
