...
[Requêtes courantes de l’API Graph Azure AD](https://msdn.microsoft.com/Library/Azure/Ad/Graph/howto/azure-ad-graph-api-supported-queries-filters-and-paging-options#CommonQueries) liste les requêtes courantes qui peuvent être utilisées avec Graph Azure AD, notamment les requêtes permettant d’accéder aux ressources de niveau supérieur dans votre annuaire, et d’y effectuer des opérations.
...
Pour plus d’informations sur les entités et types Azure AD exposés par Graph, ainsi que sur les opérations exécutables avec Graph, consultez [Informations de référence sur l’API REST  Graph Azure AD](https://msdn.microsoft.com/Library/Azure/Ad/Graph/api/api-catalog).
