...
Pour accéder à cette fonctionnalité et pour obtenir plus d’informations, consultez l’article [Azure Active Directory - Where is your data located?](http://aka.ms/AADDataMap) (Azure Active Directory - Où se trouvent vos données ?).
...
Consultez le nouveau plan de déploiement disponible pour le panneau d’accès MyApps (http://aka.ms/deploymentplans).
...
Les applications suivantes se trouvent sur la liste des [applications clientes approuvées](https://docs.microsoft.com/azure/active-directory/conditional-access/technical-reference.md#approved-client-app-requirement) :
...
À compter du 10 octobre 2018, Azure AD n’acceptera plus les codes d’authentification déjà utilisés pour les applications. Cette modification de sécurité a pour but d’assurer la conformité d’Azure AD avec la spécification OAuth et s’appliquera aux points de terminaison v1 et v2.
...
>[!Note]
>Dans le but de contribuer à réduire le nombre d’applications endommagées, une exception a été attribuée aux applications qui s’appuient sur ce modèle et qui comportent plus de 10 connexions par jour.
