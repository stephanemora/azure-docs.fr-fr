...
Chaque appel au système d’authentification est soumis à une interruption si un composant de la chaîne de l’appel à Azure AD échoue. Cela signifie que si une partie de votre infrastructure présente un problème, le travail peut être interrompu, car les utilisateurs ne peuvent pas accéder aux applications dont ils ont besoin. Il est donc important pour votre résilience de réduire le nombre d’appels d’authentification et le nombre de dépendances dans ces appels. Les développeurs d’applications peuvent exercer un certain contrôle sur la fréquence à laquelle les jetons sont demandés. Par exemple, collaborez avec vos développeurs pour vous assurer qu’ils utilisent des identités managées Azure AD pour leurs applications, dans la mesure du possible. 
