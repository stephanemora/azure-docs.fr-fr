...
5. Certains rôles requièrent l’authentification multifacteur (MFA) avant de pouvoir activer le rôle. Vous ne devez vous authentifier qu’une seule fois par session.
    ...
    ![Vérification multifacteur avant activation du rôle : capture d’écran](./media/pim-how-to-activate-role/PIM_activation_MFA.png)
...
7. Sélectionnez **OK**.  Si le rôle ne nécessite pas d’approbation, il est activé et apparaît dans la liste des rôles actifs (juste sous la liste des affectations de rôles éligibles). Si [l’activation du rôle nécessite une approbation](./azure-ad-pim-approval-workflow.md), une notification toast apparaît brièvement dans le coin supérieur droit de votre navigateur pour vous informer que la demande est en attente d’approbation.

    ![Notification de demande en attente - capture d’écran](./media/pim-how-to-activate-role/PIM_Request_Pending_Toast2.png)
...
5. Sélectionnez **Annuler** en haut du panneau **En attente d’approbation**.
   ...
   ![Capture d’écran d’annulation de demande en attente](./media/pim-how-to-activate-role/PIM_Request_Pending_Banner_Cancel.png)
