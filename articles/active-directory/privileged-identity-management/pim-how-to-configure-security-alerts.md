...
| --- | --- |
| **Severity** | Faible |
| **Je reçois cette alerte ?** | L’affectation de rôles privilégiés à des utilisateurs qui n’en ont pas besoin augmente la probabilité d’une attaque. Il est également plus facile pour les attaquants de passer inaperçus dans des comptes qui ne sont pas activement utilisés. |
| **Comment la corriger ?** | Passez en revue les utilisateurs de la liste et retirez-les des rôles privilégiés dont ils n’ont pas besoin. |
...
| **Action d’atténuation de dans le portail** | Supprime le compte de leur rôle privilégié. |
...
| --- | --- |
| **Severity** | Faible |
| **Je reçois cette alerte ?** | Sans authentification multifacteur, les utilisateurs compromis peuvent activer des rôles privilégiés. |
| **Comment la corriger ?** | Passez en revue la liste des rôles et [exigez l’authentification multifacteur](pim-how-to-change-default-settings.md) pour chaque rôle. |
| **Prévention** | [Demander l’authentification multifacteur](pim-how-to-change-default-settings.md) pour chaque rôle.  |
| **Action d’atténuation de dans le portail** | Rend l’authentification multifacteur obligatoire pour l’activation du rôle privilégié. |
...
| --- | --- |
| **Severity** | Faible |
| **Je reçois cette alerte ?** | Le locataire actuel ne dispose pas d’Azure AD Premium P2. |
| **Comment la corriger ?** | Passez en revue les informations sur les [éditions d’Azure AD](../fundamentals/active-directory-whatis.md). Effectuez une mise à niveau vers Azure AD Premium P2. |
...
| --- | --- |
| **Severity** | Moyenne |
| **Je reçois cette alerte ?** | Comptes dans un rôle privilégié qui n’ont pas changé leur mot de passe au cours des 90 derniers jours. Ces comptes peuvent être des comptes partagés ou de service, qui ne sont pas gérés et qui sont vulnérables aux attaques. |
| **Comment la corriger ?** | Passez en revue les comptes de la liste. S’ils n’ont plus besoin d’un accès, supprimez-les de leurs rôles privilégiés. |
...
| --- | --- |
| **Severity** | Élevé |
| **Je reçois cette alerte ?** | Les affectations de rôle privilégié effectuées en dehors de PIM ne sont pas analysées correctement et peuvent indiquer une attaque active. |
| **Comment la corriger ?** | Passez en revue les utilisateurs de la liste et retirez-les des rôles privilégiés qui leur ont été attribués en dehors de PIM. |
...
| --- | --- |
| **Severity** | Faible |
| **Je reçois cette alerte ?** | Administrateur général est le rôle privilégié le plus élevé. Si un administrateur général est victime d’une attaque, la personne malveillante a accès à toutes ses autorisations, ce qui met en danger l’intégralité de votre système. |
| **Comment la corriger ?** | Passez en revue les utilisateurs de la liste et supprimez ceux qui n’ont absolument pas besoin du rôle Administrateur général. </br>Affectez à ces utilisateurs des rôles privilégiés inférieurs. |
...
| --- | --- |
| **Severity** | Faible |
| **Je reçois cette alerte ?** | Plusieurs activations au même rôle privilégié par le même utilisateur sont le signe d’une attaque. |
| **Comment la corriger ?** | Passez en revue les utilisateurs de la liste et vérifiez que la [durée d’activation](pim-how-to-change-default-settings.md) de leur rôle privilégié est suffisante pour qu’ils puissent effectuer leurs tâches. |
...
| **Action d’atténuation de dans le portail** | S.O. |
...
| **Délai de renouvellement d'activation** | Ce paramètre spécifie la période, en jours, heures, minutes et secondes, que vous souhaitez utiliser pour effectuer le suivi des renouvellements suspects. |
