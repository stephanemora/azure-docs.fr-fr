...
L’objectif de ce didacticiel est de montrer les étapes que vous devez effectuer pour importer des profils de travail à partir de Workday dans Active Directory et Azure Active Directory, avec l’écriture différée facultative de l’adresse de messagerie et le nom d’utilisateur à Workday.
...
* **Écriture différée des adresses de messagerie et le nom d’utilisateur Workday** -l’utilisateur Azure AD service d’approvisionnement peut réécrire les adresses de messagerie et le nom d’utilisateur à partir d’Azure AD à Workday.
...
* **Employés ré-embauchés** : lorsqu'un employé est ré-embauché dans Workday, son ancien compte peut être automatiquement réactivé ou réapprovisionné (selon votre préférence) dans Active Directory, Azure Active Directory et, éventuellement dans Office 365 et d'[autres applications SaaS prises en charge par Azure AD](../manage-apps/user-provisioning.md).
...
* **E-mail et l’écriture différée du nom d’utilisateur flux – d’on-premises Active Directory à Workday :** Une fois la création du compte terminée dans Active Directory, il est synchronisé avec Azure AD via Azure AD Connect et attribut de messagerie et le nom d’utilisateur permettre être différée dans Workday.
...
7. Si le connecteur de l’écriture différée Workday est configuré, il réécrit dans attribut de messagerie et le nom d’utilisateur Workday, selon l’attribut correspondant est utilisé.
...
* **Approvisionnement d'utilisateurs de Workday vers Azure AD** : AAD Connect est l'outil à utiliser pour synchroniser les utilisateurs Active Directory avec Azure Active Directory, mais cette application peut aussi faciliter l'approvisionnement d'utilisateurs « cloud only » de Workday vers un locataire Azure Active Directory unique.
...
Utilisez l'organigramme décisionnel ci-dessous pour identifier les applications d'approvisionnement Workday pertinentes pour votre scénario.
    ![Organigramme décisionnel](./media/workday-inbound-tutorial/wday_app_flowchart.png "Organigramme décisionnel")
...
1. Lancez le programme d’installation de l’Agent d’approvisionnement, acceptez les termes du contrat, puis cliquez sur le **installer** bouton.
...
* L'attribut *userPrincipalName* d'Active Directory est généré à l'aide de la fonction de déduplication [SelectUniqueValue](../manage-apps/functions-for-customizing-application-data.md#selectuniquevalue) qui recherche l'existence d'une valeur générée dans le domaine AD cible et ne la définit que si elle est unique.  

* Vous pouvez consulter la [documentation sur l’écriture d’expressions](../manage-apps/functions-for-customizing-application-data.md), Cette section comprend des exemples de suppression de caractères spéciaux.
...
### <a name="part-2-configure-workday-and-azure-ad-attribute-mappings"></a>Partie 2 : Configurer les mappages d'attributs Workday et Azure AD
...
## <a name="configuring-azure-ad-attribute-writeback-to-workday"></a>Configuration de l’écriture différée des attributs Azure AD à Workday

Suivez ces instructions pour configurer la réécriture d’adresses de messagerie d’utilisateur et le nom d’utilisateur à partir d’Azure Active Directory à Workday.
...
### <a name="part-1-adding-the-writeback-connector-app-and-creating-the-connection-to-workday"></a>Partie 1 : Ajouter l'application de connecteur de réécriture et établir la connexion avec Workday
...
### <a name="part-2-configure-writeback-attribute-mappings"></a>Partie 2 : Configurer les mappages d'attributs de réécriture

Dans cette section, vous allez configurer le transfert des attributs de réécriture d'Azure AD vers Workday. À l’heure actuelle, le connecteur prend uniquement en charge l’écriture différée de l’adresse de messagerie et le nom d’utilisateur à Workday.
...
4. En général, vous mappez l’Azure AD *userPrincipalName* attribut Workday *UserID* d’attribut et mapper l’Azure AD *mail* d’attribut pour la journée de travail  *EmailAddress* attribut. Pour enregistrer vos mappages, cliquez sur **Enregistrer** en haut de la section Mappage d’attributs.
...
* **Questions relatives à l'agent d'approvisionnement**
  ...
  * [Puis-je installer l'agent d'approvisionnement sur le serveur qui exécute AAD Connect ?](#can-i-install-the-provisioning-agent-on-the-same-server-running-aad-connect)
...
* Update_Workday_Account (v31.2) pour la fonctionnalité de Username Writeback
...
#### <a name="can-i-install-the-provisioning-agent-on-the-same-server-running-aad-connect"></a>Puis-je installer l'agent d'approvisionnement sur le serveur qui exécute AAD Connect ?

Oui, vous pouvez installer l'agent d'approvisionnement sur le serveur qui exécute AAD Connect.
...
![Mettre une action à jour](./media/workday-inbound-tutorial/wd_target_update_only.png)
...
* Dans le panneau qui s'ouvre, localisez l'attribut « Mobile » et cliquez sur la ligne afin de pouvoir modifier l'**Expression de l'API** ![RGPD mobile](./media/workday-inbound-tutorial/mobile_gdpr.png).
...
* Dans le prolongement de l'exemple ci-dessus, supposons que vous souhaitiez convertir les noms de ville issus de Workday en valeurs abrégées, puis les utiliser pour générer des noms d'affichage tels que *Smith, John (CHI)* ou *Doe, Jane (NYC)*. Pour ce faire, vous pouvez utiliser une expression Switch avec l'attribut Workday *Municipality* comme variable déterminante.
    ...
    (
     ...
     ```
    Voir aussi :
  * [Syntaxe de la fonction Switch](../manage-apps/functions-for-customizing-application-data.md#switch)
  * [Syntaxe de la fonction Join](../manage-apps/functions-for-customizing-application-data.md#join)
  * [Syntaxe de la fonction Append](../manage-apps/functions-for-customizing-application-data.md#append)
...
* [Syntaxe de la fonction Mid](../manage-apps/functions-for-customizing-application-data.md#mid)
* [Syntaxe de la fonction Replace](../manage-apps/functions-for-customizing-application-data.md#replace)
* [Syntaxe de la fonction SelectUniqueValue](../manage-apps/functions-for-customizing-application-data.md#selectuniquevalue)
...
Utilisez la fonction [NormalizeDiacritics](../manage-apps/functions-for-customizing-application-data.md#normalizediacritics) pour supprimer les caractères spéciaux du prénom et du nom de l'utilisateur lors de la création de l'adresse e-mail ou de la valeur CN de celui-ci.
...
* [Configuration des journaux d'audit du portail Azure pour résoudre les problèmes de service](#setting-up-azure-portal-audit-logs-for-service-troubleshooting)
* [Familiarisation avec les journaux pour les opérations de création de compte d'utilisateur AD](#understanding-logs-for-ad-user-account-create-operations)
* [Familiarisation avec les journaux pour les opérations de mise à jour de l'attribut manager](#understanding-logs-for-manager-update-operations)
...
|2.| Le service Windows « Agent d'approvisionnement Microsoft Azure AD Connect » est bloqué sur l'état *Démarrage* et ne bascule pas vers l'état *En cours d'exécution*. | Lors de l'installation, l'Assistant de l'agent crée un compte local (**Service NT\\AADConnectProvisioningAgent**) sur le serveur. Il s'agit du compte de **Connexion** utilisé pour démarrer le service. Vous rencontrerez cette erreur si une stratégie de sécurité de votre serveur Windows empêche les comptes locaux d'exécuter les services. | Ouvrez la *console Services*. Cliquez avec le bouton droit sur le service Windows « Agent d'approvisionnement Microsoft Azure AD Connect » et, dans l'onglet Connexion, spécifiez le compte d'un administrateur de domaine pour exécuter le service. Redémarrez le service. |
