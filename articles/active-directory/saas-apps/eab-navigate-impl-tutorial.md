...
Le fait de cliquer sur la vignette EAB Navigate IMPL dans le volet d’accès doit vous connecter automatiquement à l’application EAB Navigate IMPL pour laquelle vous avez configuré l’authentification unique (SSO). Pour plus d’informations sur le panneau d’accès, consultez [Présentation du panneau d’accès](../user-help/my-apps-portal-end-user-access.md).
