...
Quand vous cliquez sur la vignette Greenlight Integration Platform dans le volet d’accès, vous devez être automatiquement connecté à l’application Greenlight Integration Platform pour laquelle vous avez configuré l’authentification unique. Pour plus d’informations sur le panneau d’accès, consultez [Présentation du panneau d’accès](../user-help/my-apps-portal-end-user-access.md).
...
- [Comment protéger Greenlight Integration Platform avec une visibilité et des contrôles avancés](/cloud-app-security/proxy-intro-aad)