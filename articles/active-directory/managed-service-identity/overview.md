---
title: Managed Service Identity pour les ressources Azure
description: Vue d’ensemble de la fonctionnalité d’Managed Service Identity pour les ressources Azure.
...
Votre code peut utiliser une identité du service administré pour faire une demande de jetons d’accès pour les services qui prennent en charge l’authentification Azure AD. Azure prend en charge la restauration des informations d’identification utilisées par l’instance de service.
...
### <a name="how-a-system-assigned-identity-works-with-an-azure-vm"></a>Fonctionnement d’une identité attribuée au système avec une machine virtuelle Azure
...
3. Azure Resource Manager configure l’identité sur la machine virtuelle :
    ...
    1. Provisionne l’extension de machine virtuelle et ajoute l’ID client et le certificat du Principal de service. (Cette étape sera bientôt abandonnée).
...
5. Votre code en cours d’exécution sur la machine virtuelle peut demander un jeton à partir de deux points de terminaison qui sont uniquement accessibles à partir de la machine virtuelle :
    ...
    - Point de terminaison d’extension de machine virtuelle (abandon planifié) : `http://localhost:50342/oauth2/token` 
        - Le paramètre de ressource spécifie le service vers lequel le jeton est envoyé. Pour vous authentifier à Azure Resource Manager, utilisez `resource=https://management.azure.com/`.
...
### <a name="how-a-user-assigned-identity-works-with-an-azure-vm"></a>Fonctionnement d’une identité attribuée au système avec une machine virtuelle Azure

1. Azure Resource Manager reçoit une requête pour créer une identité affectée par l’utilisateur.
2. Azure Resource Manager crée un principal de service dans Azure AD pour représenter l’identité affectée par l’utilisateur. Le principal de service est créé dans le locataire Azure AD approuvé par cet abonnement.
3. Azure Resource Manager reçoit une requête pour configurer l’identité affectée par l’utilisateur sur une machine virtuelle :
    1. Met à jour le point de terminaison d’identité Azure Instance Metadata Service avec l’ID client et le certificat du Principal de service de l’identité affectée par l’utilisateur.
    1. Provisionne l’extension de machine virtuelle et ajoute l’ID client et le certificat du Principal de service de l’identité affectée par l’utilisateur. (Cette étape sera bientôt abandonnée).
4. Maintenant que l’identité affectée par l’utilisateur a été créée, utilisez les informations du Principal de service pour lui accorder l’accès aux ressources Azure. Pour appeler Azure Resource Manager, utilisez RBAC dans Azure AD pour attribuer le rôle approprié au principal de service de l’Identité assignée de l’utilisateur. Pour appeler Key Vault, accordez à votre code un accès au secret spécifique ou à la clé dans Key Vault.
...
## <a name="how-can-i-use-managed-service-identities"></a>Comment me servir des identités de service administré ?

Pour découvrir comment utiliser les identités de service administré pour accéder aux différentes ressources Azure, essayez ces didacticiels.

Découvrez comment utiliser une identité du service administré avec une machine virtuelle Windows :
...
Découvrez comment utiliser une identité du service administré avec une machine virtuelle Linux :
...
Découvrez comment utiliser une identité du service administré avec d’autres services Azure :
...
## Quels sont les services Azure qui prennent en charge la fonctionnalité ?<a name="which-azure-services-support-managed-service-identity"></a>

Les identités de service gérées peuvent servir à l’authentification auprès des services prenant en charge l’authentification Azure AD. Pour obtenir la liste des services Azure qui prennent en charge la fonctionnalité d’Identité de service administré, consultez [Services qui prennent en charge l’identité du service administré](services-support-msi.md).

## <a name="next-steps"></a>Étapes suivantes

Bien démarrer avec la fonctionnalité d’identité du service administré grâce aux guides de démarrage rapide suivants :

* [Utiliser une identité MSI (Managed Service Identity) de machine virtuelle Windows pour accéder au gestionnaire des ressources Azure Resource Manager](tutorial-windows-vm-access-arm.md)
* [Utiliser une identité MSI (Managed Service Identity) de machine virtuelle Linux pour accéder au gestionnaire des ressources Azure Resource Manager](tutorial-linux-vm-access-arm.md)
