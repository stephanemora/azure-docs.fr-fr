...
> [!NOTE]
> Le service du connecteur évalue la configuration **defaultProxy** pour une utilisation dans `%SystemRoot%\Microsoft.NET\Framework64\v4.0.30319\Config\machine.config` si **defaultProxy** n’est pas configuré (ce qui est le cas par défaut) dans ApplicationProxyConnectorService.exe.config. Il en va de même pour le service Connector Updater (ApplicationProxyConnectorUpdaterService.exe.config). 
