...
-   des groupes identiques de machines virtuelles (VMSS) pour l’exécution des validateurs PoA ;
...
![architecture de déploiement](./media/ethereum-poa-deployment-guide/deployment-architecture.png)
...
La décentralisation de la gouvernance est au centre du concept de preuve d’autorité. La Governance DApp est un ensemble prédéployé de contrats intelligents associés à une application web servant à administrer les autorités sur le réseau.
...
Ce processus requiert un abonnement Azure qui peut prendre en charge le déploiement groupe de machines virtuelles identiques et disques managés. Si nécessaire, [créez un compte Azure gratuit](https://azure.microsoft.com/free/) pour commencer.
...
Voici un exemple de déploiement : ![panneau de base](./media/ethereum-poa-deployment-guide/basic-blade.png)
...
Voici un exemple de déploiement : ![régions de déploiement](./media/ethereum-poa-deployment-guide/deployment-regions.png)
...
Voici une description détaillée de chaque paramètre :
  ...
  Performances de stockage des nœuds de validateur|Type de disque managé prenant en charge chacun des nœuds de validateur déployés.|Standard ou Premium|standard
...
Voici un exemple de déploiement : ![taille et performances du réseau](./media/ethereum-poa-deployment-guide/network-size-and-performance.png)
...
Voici un exemple de déploiement : ![paramètres ethereum](./media/ethereum-poa-deployment-guide/ethereum-settings.png)

#### <a name="azure-monitor"></a>Azure Monitor

Le panneau Azure Monitor vous permet de configurer une ressource Azure Monitor pour votre réseau. Azure Monitor collecte et fournit des métriques et journaux utiles à partir de votre réseau, en offrant la possibilité de vérifier rapidement l’intégrité du réseau ou les problèmes de débogage.

  Nom du paramètre|Description|Valeurs autorisées|Valeurs par défaut
  ---|---|---|---
Surveillance|Option permettant d’activer Azure Monitor|Activer ou désactiver|Activer
Se connecter à une instance Log Analytics existante|Créer une instance Log Analytics dans le cadre d’Azure Monitor ou rejoindre une instance existante|Créer ou rejoindre existant|Création
Emplacement Azure Monitor (Se connecter à une instance Azure Monitor existante = Créer)|Région dans laquelle la nouvelle instance Azure Monitor sera déployée|Toutes les régions Azure Monitor|N/D
ID de l’espace de travail Log Analytics existant (Se connecter à une instance Azure Monitor existante = Rejoindre existant)|ID d’espace de travail de l’instance Log Analytics existante||N/D
Clé primaire de l’instance Log Analytics existante (Se connecter à une instance Azure Monitor existante = Rejoindre existant)|Clé primaire utilisée pour se connecter à l’instance Azure Monitor existante||N/D


Voici un exemple de déploiement : ![azure monitor](./media/ethereum-poa-deployment-guide/azure-monitor.png)
...
![e-mail de déploiement](./media/ethereum-poa-deployment-guide/deployment-email.png)
...
Cette première étape s’effectue en toute transparence si vous utilisez un déploiement basé sur une adresse IP publique. Si vous effectuez le déploiement derrière un VPN, consultez la section [Connexion des passerelles de réseau virtuel](#connecting-vnet-gateways) pour exécuter la connexion réseau dans le cadre de [l’étape 2](#step-2-new-admin-deployment).

#### <a name="step-1-add-the-new-admin"></a>Étape 1 : ajout du nouvel administrateur

1.  Récupérez l’adresse Ethereum publique du nouvel administrateur.

2.  Accédez à la Governance DApp et créez un administrateur avec cette adresse Ethereum et un alias approprié.

3.  Informez les autres membres existants de la nouvelle requête afin qu’ils peuvent voter pour l’ajout de ce nouvel administrateur.

4.  Une fois que le vote atteint 51 %, le membre est ajouté en tant qu’administrateur.

![ajout d’un administrateur](./media/ethereum-poa-deployment-guide/adding-admin.png)

#### <a name="step-2-new-admin-deployment"></a>Étape 2 : déploiement du nouvel administrateur
...
#### <a name="step-3-delegate-validators"></a>Étape 3 : délégation de validateurs

Le nouvel administrateur devra autoriser les validateurs à participer en son nom. Cela ne peut pas être fait automatiquement dans la mesure où seul le membre contrôle leurs clés.\*

_\*Les nœuds du premier membre sur le réseau peuvent être ajoutés automatiquement, car nous pouvons précompiler leurs nœuds de validateur dans le bloc Genesis sur le réseau._

Le nouvel administrateur doit effectuer les étapes suivantes :

1.  Ouvrir la Governance DApp déployée dans le cadre de *son* déploiement

2.  Accéder à l’onglet Validateurs

3.  Sélectionner chacun des validateurs qui ont été déployés et cliquer sur **Ajouter**

![ajout de validateurs](./media/ethereum-poa-deployment-guide/adding-validators.png)
...
1.  L’ID de ressource de la passerelle de réseau virtuel (voir la sortie du déploiement)
...
![catégories monitor](./media/ethereum-poa-deployment-guide/monitor-categories.png)

Cliquez sur **Vue d’ensemble des nœuds** pour accéder à un portail contenant des statistiques sur l’infrastructure par nœud.

![stats des nœuds](./media/ethereum-poa-deployment-guide/node-stats.png)

En sélectionnant **Statistiques réseau**, vous accédez aux statistiques du réseau Ethereum.

![stats réseau](./media/ethereum-poa-deployment-guide/network-stats.png)

#### <a name="sample-log-analytics-queries"></a>Exemples de requêtes Log Analytics
...
Pour des raisons de sécurité, l’accès au port SSH est refusé par défaut par une règle de sécurité de groupe de réseau. Pour accéder aux instances des groupes de machines virtuelles identiques dans le réseau PoA, vous devez modifier cette règle en sélectionnant le paramètre \"Autoriser\"

1.  Commencez dans la section Vue d’ensemble du groupe de ressources déployé à partir du portail Azure.

    ![vue d’ensemble ssh](./media/ethereum-poa-deployment-guide/ssh-overview.png)

2.  Sélectionnez le groupe de sécurité réseau
...
![recherche d’azure traffic manager](./media/ethereum-poa-deployment-guide/traffic-manager-search.png)

Donnez un nom unique au profil et sélectionnez le groupe de ressources qui a été créé pendant le déploiement de PoA.

![créer un profil traffic manager](./media/ethereum-poa-deployment-guide/traffic-manager-create.png)
...
![Rechercher le DNS Traffic Manager](./media/ethereum-poa-deployment-guide/traffic-manager-dns.png)

Sélectionnez l’onglet Points de terminaison et cliquez sur le bouton Ajouter. Modifiez ensuite le type de ressource Cible en Adresse IP publique. Sélectionnez l’adresse IP publique de l\'équilibreur de charge de la première région.

![acheminement de traffic manager](./media/ethereum-poa-deployment-guide/traffic-manager-routing.png)
...
### <a name="debug-a-smart-contract-truffle-develop"></a>Déboguer un contrat intelligent (Truffle Develop)
...
### <a name="i-notice-a-bunch-of-transactions-on-the-network-that-i-didnt-send-where-are-these-coming-from"></a>Je remarque une série de transactions sur le réseau que je n\'ai pas envoyées. D’où viennent-elles ?

Il est dangereux de déverrouiller [l’API personnelle](https://web3js.readthedocs.io/en/1.0/web3-eth-personal.html). Les comptes de bots écoutent les comptes Ethereum déverrouillés et tentent d’en extraire des fonds. Le bot suppose que ces comptes contiennent real-ether et essaie d’être le premier à permettre l’équilibre. N’activez pas l’API personnelle sur le réseau. Il est préférable de signer au préalable les transactions manuellement à l’aide un portefeuille comme MetaMask ou par programmation, comme indiqué dans la section Interaction par programmation avec un contrat intelligent.

### <a name="how-to-ssh-onto-a-vm"></a>Comment exécuter SSH sur une machine virtuelle ?

Nous verrouillons le port SSH pour des raisons de sécurité. Suivez [ce guide pour activer le port SSH](#_Accessing_Nodes_via).
...
Ils servent également de mécanisme de vérifiabilité pour les organismes de réglementation du réseau. Pour cela, suivez simplement l’étape 2 de la section Développer le consortium.
