...
**Objectif :** détermine l’ensemble de méthodes HTTP supplémentaires pouvant être mis en cache sur le réseau.
...
- Par défaut, seules les requêtes dont le corps est inférieur à 14 Ko sont mises en cache. Utilisez la fonction de la taille de corps de requête mis en cache pour définir la taille de corps de requête maximale.
...
**Objectif :** définit le seuil pour déterminer si une réponse POST peut être mis en cache.
...
- Cette fonctionnalité n’est applicable que lorsque les réponses POST peuvent prétendre à la mise en cache. Utilisez la fonctionnalité de méthodes HTTP pouvant pour activer la mise en cache de demande POST.
...
Valeur|Résultat
--|--
Remplacer|Garantit que les actions suivantes se produisent :<br/> -Remplace le `Cache-Control` en-tête généré par le serveur d’origine. <br/>-Ajoute la `Cache-Control` en-tête généré par la fonctionnalité External Max-Age à la réponse.
Transmettre directement|Garantit que le `Cache-Control` en-tête généré par la fonctionnalité External Max-Age n’est jamais ajouté à la réponse. <br/> Si le serveur d’origine génère un `Cache-Control` en-tête, il est transmis à l’utilisateur final. <br/> Si le serveur d’origine ne génère pas un `Cache-Control` en-tête, cette option risque de ne pas contenir l’en-tête de réponse un `Cache-Control` en-tête.
Ajouter si manquant|Si un `Cache-Control` en-tête n’a pas été reçu du serveur d’origine, puis cette option ajoute le `Cache-Control` en-tête généré par la fonctionnalité External Max-Age. Cette option est utile pour s’assurer que toutes les ressources reçoivent un `Cache-Control` en-tête.
Supprimer| Cette option permet à un `Cache-Control` en-tête n’est pas inclus dans la réponse de l’en-tête. Si un `Cache-Control` en-tête a déjà été attribué, puis il est supprimé de la réponse de l’en-tête.
...
**Objectif :** détermine si la clé de cache inclut ou exclut les paramètres de chaîne de requête associés à une demande.
...
--|--
 ...
 Exclure tous  |Indique que tous les paramètres de chaîne de requête sont exclus de la clé de cache. Cette configuration duplique la valeur par défaut « standard-cache » mise en cache de comportement dans la page mise en cache de la chaîne de requête.  
...
Pour dupliquer la chaîne de requête de « no-cache » mise en cache de comportement dans la page mise en cache de la chaîne de requête, créez une règle qui contient une condition de correspondance caractère générique de requête URL et une fonctionnalité ignorer le Cache. Définissez la condition de correspondance de caractère générique de requête d’URL sur un astérisque (*).
...
- **Exemple de demande :** http://wpc.0001.&lt; domaine&gt;/800001/Origin/folder/asset.htm?sessionid=1234 & language = EN & userid = 01
...
**Objectif :** réécrit la clé de cache associée à une demande.
...
Option|Description
--|--
Chemin d’accès d’origine| Permet de définir le chemin d’accès relatif aux types de requêtes dont la clé de cache est réécrite. Un chemin d’accès relatif peut être définie en sélectionnant un chemin d’accès de base d’origine et en définissant un modèle d’expression régulière.
Nouveau chemin d’accès|Permet de définir le chemin d’accès relatif pour la nouvelle clé de cache. Un chemin d’accès relatif peut être défini en sélectionnant un chemin d’accès d’origine de base et en définissant un modèle d’expression régulière. Ce chemin d’accès relatif peut être construite dynamiquement à l’aide de [variables HTTP](cdn-http-variables.md).
...
**Objectif :** permet une remarque à ajouter au sein d’une règle.
...
**Objectif :** définit les formats de fichier pour les fichiers qui sont compressés sur le serveur.
...
**Objectif :** détermine le format et le contenu qui sera affecté au champ de journal personnalisé dans un fichier journal brut.
...
Par défaut, le champ du fichier journal personnalisé est appelé « x-ec_custom-1 ». Le nom de ce champ peut être personnalisé à partir de la page Paramètres de journalisation bruts.
...
Valeur|Résultat
-|-
activé|Requêtes pour les en-têtes de réponse de cache de débogage renvoient une réponse qui inclut l’en-tête X-EC-Debug.
...
**Objectif :** détermine l’intervalle d’âge maximal par défaut pour le point de présence à la revalidation du cache de serveur origine. En d’autres termes, il s’agit de la durée s’écoulant avant qu’un point de présence ne vérifie si une ressource mise en cache correspond à la ressource stockée sur le serveur d’origine.

Informations essentielles :

- Cette action a lieu uniquement pour les réponses à partir d’un serveur d’origine qui n’a pas attribué une indication d’âge maximal dans le `Cache-Control` ou `Expires` en-tête.
- Cette action n’aura pas lieu pour les ressources qui ne sont pas considérées comme susceptibles d’être mises en cache.
- Cette action n’affecte pas les revalidations du cache entre le navigateur et le point de présence. Ces types de revalidations sont déterminés par le `Cache-Control` ou `Expires` en-têtes envoyés au navigateur, qui peut être personnalisé avec la fonctionnalité External Max-Age.
...
- Définissez l’unité de temps sur « Désactivé » pour affecter un intervalle d’âge maximal interne par défaut de 7 jours pour les requêtes qui n’ont pas reçu d’une indication d’âge maximal dans leurs `Cache-Control` ou `Expires` en-tête.

**Valeur par défaut :** des 7 derniers jours
...
**Objectif :** contrôle la génération des `Expires` en-têtes par un point de présence quand la fonctionnalité External Max-Age est active.
...
Valeur|Résultat
--|--
Remplacer|Garantit que les actions suivantes auront lieu :<br/>-Remplace le `Expires` en-tête généré par le serveur d’origine.<br/>-Ajoute la `Expires` en-tête généré par la fonctionnalité External Max-Age à la réponse.
Transmettre directement|Garantit que le `Expires` en-tête généré par la fonctionnalité External Max-Age n’est jamais ajouté à la réponse. <br/> Si le serveur d’origine génère un `Expires` en-tête, il sera transmis directement à l’utilisateur final. <br/>Si le serveur d’origine ne génère pas un `Expires` en-tête, cette option risque de ne pas contenir l’en-tête de réponse un `Expires` en-tête.
Ajouter si manquant| Si un `Expires` en-tête n’a pas été reçu du serveur d’origine, puis cette option ajoute le `Expires` en-tête généré par la fonctionnalité External Max-Age. Cette option est utile pour s’assurer que toutes les ressources recevront un `Expires` en-tête.
Supprimer| Garantit qu’un `Expires` en-tête n’est pas inclus dans la réponse de l’en-tête. Si un `Expires` en-tête a déjà été attribué, puis il est supprimé de la réponse de l’en-tête.
...
**Objectif :** détermine l’intervalle d’âge maximal pour le navigateur et la revalidation du cache de point de présence. En d’autres termes, il s’agit de la durée s’écoulant avant qu’un navigateur puisse rechercher la nouvelle version d’une ressource à partir d’un point de présence.

L’activation de cette fonctionnalité génère `Cache-Control: max-age` et `Expires` en-têtes à partir de points de présence et les envoyer au client HTTP. Par défaut, ces en-têtes remplacent ceux créés par le serveur d’origine. Toutefois, le traitement d’en-tête Cache-Control et les fonctionnalités de traitement d’en-tête expire peuvent servir à modifier ce comportement.

Informations essentielles :

- Cette action n’affecte pas les revalidations du cache entre le point de présence et le serveur d’origine. Ces types de revalidations sont déterminés par le `Cache-Control` et `Expires`  en-têtes reçus du serveur d’origine et peut être personnalisées avec la valeur par défaut l’âge maximal interne et les fonctionnalités de forcer l’âge maximal interne.
...
- Définissez cette fonctionnalité sur une valeur négative provoque les points de présence envoyer un `Cache-Control: no-cache` et un `Expires` heure définie dans le passé avec chaque réponse au navigateur. Même si un client HTTP ne met pas en cache la réponse, ce paramètre n’affecte pas la capacité des points de présence à mettre en cache la réponse provenant du serveur d’origine.
- La définition de l’unité de temps sur « Désactivé » a pour effet de désactiver cette fonctionnalité. Le `Cache-Control` et `Expires`  en-têtes mis en cache avec la réponse du serveur d’origine sont transmis directement au navigateur.
...
**Objectif :** détermine si les requêtes peuvent être redirigées vers le nom d’hôte défini dans l’en-tête Location retourné par un serveur d’origine client.
...
**Objectif :** détermine l’intervalle max-age POP pour la revalidation du cache de serveur origine. En d’autres termes, il s’agit de la durée s’écoulant avant qu’un point de présence ne vérifie si une ressource mise en cache correspond à la ressource stockée sur le serveur d’origine.

Informations essentielles :

- Cette fonctionnalité remplace l’intervalle d’âge maximal défini dans `Cache-Control` ou `Expires` les en-têtes générés à partir d’un serveur d’origine.
- Cette fonctionnalité n’affecte pas les revalidations du cache entre le navigateur et le point de présence. Ces types de revalidations sont déterminés par le `Cache-Control` ou `Expires` en-têtes envoyés au navigateur.
...
- La définition de l’unité de temps sur « Désactivé » a pour effet de désactiver cette fonctionnalité. Un intervalle d’âge maximal interne n’est pas attribué aux ressources demandées. Si l’en-tête d’origine ne contient pas d’instructions de mise en cache, la ressource sera mis en cache en fonction du paramètre actif de la fonctionnalité par défaut l’âge maximal interne.
...
**Objectif :** détermine les types de formats de fichier H.264 qui peuvent être utilisées pour diffuser du contenu.

Informations essentielles :

- Définir un ensemble séparés par des extensions de nom de fichier H.264 autorisées dans l’option Extensions de fichier. L’option fichier remplacera le comportement par défaut. Maintenez la prise en charge de MP4 et F4V en incluant ces extensions de noms de fichier lors de la définition de cette option. 
...
**Objectif :** détermine si un client HTTP non-cache les requêtes sont transmises au serveur d’origine.

Une demande non-cache se produit lorsque le client HTTP envoie un `Cache-Control: no-cache` et/ou `Pragma: no-cache` en-tête dans la requête HTTP.
...
L’état du cache qui est signalé pour une demande qui peut être transférée à un serveur d’origine en raison de cette fonctionnalité est `TCP_Client_Refresh_Miss`. Le rapport des États du Cache, qui est disponible dans le module de création de rapports de base, fournit des informations statistiques par état du cache. Ce rapport vous permet de suivre le nombre et le pourcentage de requêtes transférées vers un serveur d’origine en raison de cette fonctionnalité.
...
**Objectif :** détermine la réponse qui sera retournée aux clients quand une requête génère un code d’état 416 plage demandée non satisfaisante.

Par défaut, ce code d’état est retourné lorsque la demande de plage d’octets spécifiée ne peut pas être satisfaite par un point de présence et un champ d’en-tête de demande If-Range n’a pas été spécifié.

Valeur|Résultat
-|-
activé|Empêche les points de présence de répondre à une requête de plage d’octets non valide avec un code d’état 416 plage demandée non satisfaisante. Au lieu de cela, les serveurs seront fournissent la ressource demandée et renvoyer une réponse 200 OK au client.
...
**Objectif :** contrôles combien de temps après l’expiration normale d’une ressource mise en cache peut être pris en charge à partir d’un point de présence quand le point de présence ne parvient pas à la revalider avec le serveur d’origine.

Normalement, lors de l’expiration du délai de max-age d’une ressource, le point de présence envoie une requête de revalidation au serveur d’origine. Le serveur d’origine ensuite répondre par un 304 non modifié pour permettre le point de présence une nouvelle du bail sur la ressource mise en cache, soit avec 200 OK pour fournir le point de présence avec une version mise à jour de la ressource mise en cache.
...
Notez que cet intervalle de temps démarre lorsque l’âge maximal de la ressource expire, pas lorsque l’échec d’une revalidation. Par conséquent, la durée maximale pendant laquelle une ressource peut être traitée sans réussir la revalidation est la quantité de temps spécifié par la combinaison de max-age max- Stale. Par exemple, si une ressource a été mis en cache à 9 h 00 avec un âge maximal de 30 minutes et une obsolescence maximale de 15 minutes, puis une tentative de revalidation a échoué à 9:44 entraînerait un utilisateur final reçoit la ressource mise en cache obsolète, pendant une tentative de revalidation a échoué à 9:46 entraînerait l’en utilisateur reçoit un 504 expiration de la passerelle d.

Toute valeur configurée pour cette fonctionnalité est remplacée par `Cache-Control: must-revalidate` ou `Cache-Control: proxy-revalidate` en-têtes reçus du serveur d’origine. Si l’un de ces en-têtes issus du serveur d’origine est reçu lors de la mise en cache initiale d’une ressource, le point de présence ne traite pas de ressource mise en cache obsolète. Dans ce cas, si le point de présence ne parvient pas à la revalider avec l’origine lors de l’intervalle d’âge maximal de la ressource a expiré, le point de présence retourne une erreur 504 expiration de la passerelle.
...
**Objectif :** définit le nombre maximal de requêtes pour une connexion Keep-Alive avant sa fermeture.
...
- Assurez-vous que la valeur spécifiée dans l’option de nom est une correspondance exacte pour l’en-tête de demande souhaitée.
- La casse n’est pas prise en compte pour l’identification d’un en-tête. Par exemple, une des variantes suivantes de la `Cache-Control` nom de l’en-tête peut être utilisé pour l’identifier :
    - cache-control
    - CACHE-CONTROL
    - cachE-Control
...
- Assurez-vous que la valeur spécifiée dans l’option de nom est une correspondance exacte pour l’en-tête de réponse voulu. 
- La casse n’est pas prise en compte pour l’identification d’un en-tête. Par exemple, une des variantes suivantes de la `Cache-Control` nom de l’en-tête peut être utilisé pour l’identifier :
    - cache-control
    - CACHE-CONTROL
    - cachE-Control
...
**Objectif :** détermine si contenu mis en cache est éligible pour une revalidation anticipée avant l’expiration de sa durée de vie.
...
**Comportement par défaut :** tous les en-têtes de requête propres à CDN seront être transférés vers le serveur d’origine.
...
**Objectif :** détermine comment la demande d’un client HTTP pour une ressource de cache de 0 octet est gérée par les points de présence.
...
**Objectif :** définit l’ensemble des codes d’état qui peuvent entraîner de contenu mis en cache.

Par défaut, la mise en cache est uniquement activée pour les 200 réponses OK.

Définissez un ensemble de codes d’état souhaités séparés par un espace.

Informations essentielles :

- Activer la fonctionnalité ignorer origine No-Cache. Si cette fonctionnalité n’est pas activée, puis les réponses non - 200 OK ne peuvent pas être mis en cache.
- L’ensemble des codes d’état valides pour cette fonctionnalité sont : 203, 300, 301, 302, 305, 307, 400, 401, 402, 403, 404, 405, 406, 407, 408, 409, 410, 411, 412, 413, 414, 415, 416, 417, 500, 501, 502, 503, 504, and 505.
- Cette fonctionnalité ne peut pas être utilisée pour désactiver la mise en cache des réponses qui génèrent un code d’état OK 200.

**Comportement par défaut :** mise en cache est activée seulement pour les réponses qui génèrent un code d’état OK 200.
...
L’option de nom d’en-tête définit le nom de l’en-tête de requête personnalisée où adresse IP du client est stockée.

Cette fonctionnalité permet à un serveur d’origine du client de trouver les adresses IP du client via un en-tête de requête personnalisé. Si la requête est traitée à partir du cache, le serveur d’origine n’est pas informé de l’adresse IP du client. Par conséquent, il est recommandé que cette fonctionnalité est utilisée avec les ressources qui ne sont pas mis en cache.
...
**Objectif :** détermine si un contenu mis en cache expiré est remis quand une erreur se produit pendant la revalidation du cache ou extraction du contenu demandé à partir du serveur d’origine client.
...
- Le comportement de cette fonctionnalité varie en fonction de l’unité de temps sélectionnée.
    - **Unité de temps :** spécifier une durée et sélectionnez une unité de temps (par exemple, secondes, Minutes, heures, etc.) pour permettre la distribution de contenu obsolète. Ce type d’installation permet au CDN d’étendre la durée pendant laquelle il peut fournir de contenu avant de demander une validation en fonction de la formule suivante : **Durée de vie** + **obsolètes lors de la Revalidate** 
    - **OFF :** sélectionnez « désactivé » pour demander la revalidation avant qu’une requête pour le contenu peut être rendu obsolète.
...
**Objectif :** détermine si l’authentification basée sur le jeton est appliquée à une demande.
...
La clé de chiffrement qui est utilisée pour chiffrer et déchiffrer des valeurs de jeton est déterminée par la clé primaire et les options de la clé de sauvegarde sur la page d’authentification du jeton. N’oubliez pas que les clés de chiffrement sont spécifiques de la plateforme.
...
401|Non autorisé|Associant ce code d’état de l’en-tête de réponse WWW-Authenticate vous permet de demander à un utilisateur pour l’authentification.
403|Interdit|Ce message est le message 403 Interdit standard état un utilisateur non autorisé qui s’affiche que lorsque vous tentez d’accéder au contenu protégé.
...
Cette fonctionnalité prend en charge la redirection d’URL vers une URL défini par l’utilisateur lorsqu’il est configuré pour retourner un code d’état 3xx. Cette URL définie par l’utilisateur peut être spécifiée en effectuant les opérations suivantes :

1. Sélectionnez un code de réponse 3xx pour la fonction Token Auth Denial Code.
2. Sélectionnez « Location » à partir de l’option de nom de l’en-tête facultatif.
3. Définissez l’option de valeur d’en-tête facultatif à l’URL souhaitée.

Si une URL n’est pas définie pour un code d’état 3xx, puis la page de réponse standard pour un code d’état 3xx s’affichera à l’utilisateur.

La redirection d’URL est uniquement applicable pour les codes de réponse 3xx.

L’option de valeur d’en-tête facultative prend en charge les caractères alphanumériques, des guillemets et des espaces.

#### <a name="authentication"></a>Authentication

Cette fonctionnalité prend en charge la capacité d’inclure l’en-tête WWW-Authenticate lors de la réponse à une demande non autorisée pour le contenu protégé par l’authentification basée sur le jeton. Si l’en-tête WWW-Authenticate a été défini sur « basic » dans votre configuration, l’utilisateur non autorisé sera invité à entrer des informations d’identification de compte.
...
3. Définissez l’option de valeur d’en-tête facultative à « basic ».
...
**Objectif :** détermine si les comparaisons d’URL effectuées par l’authentification basée sur le jeton respectent la casse.
...
**Objectif :** détermine si le paramètre de chaîne de requête de l’authentification basée sur le jeton doit être renommé.

Informations essentielles :

- L’option valeur définit le nom de paramètre de chaîne de requête par le biais duquel un jeton peut être spécifié.
- L’option de valeur ne peut pas être définie sur « ec_token ».
- Assurez-vous que le nom défini dans l’option valeur contient des caractères d’URL valides uniquement.
...
**Objectif :** redirige les demandes via l’en-tête Location.
...
Source et modèle| Ces paramètres définissent un modèle d’URI de requête qui identifie le type de requêtes pouvant être redirigées. Seules les requêtes dont l’URL satisfait aux deux critères suivants seront redirigées : <br/> <br/> **Source (ou point d’accès au contenu) :** sélectionner un chemin d’accès relatif qui identifie un serveur d’origine. Il s’agit de la section _/XXXX/_ et de votre nom de point de terminaison. <br/><br/> **Source (modèle) :** un modèle qui identifie les requêtes par le chemin d’accès relatif doit être défini. Ce modèle d’expression régulière doit définir un chemin d’accès qui commence directement après l’accès au contenu sélectionné précédemment pointe (voir ci-dessus). <br/> - Vérifiez que les critères d’URI de requête (c’est-à-dire, Source et Modèle) définis précédemment ne sont pas en conflit avec les conditions de correspondance définies pour cette fonctionnalité. <br/> - Spécifiez un modèle. Si vous utilisez une valeur vide comme modèle, toutes les chaînes sont mises en correspondance.
...
Il est vivement recommandé d’utiliser une URL absolue. L’utilisation d’une URL relative peut rediriger les URL CDN vers un chemin d’accès non valide.
...
- Toutes les demandes correspondants seront redirigés vers l’URL CNAME edge définie dans l’option de Destination. 
...
- La variable de modèle de requête (%{Scheme})) est utilisée dans l’option de Destination, ce qui garantit que le modèle de requête reste inchangé après la redirection.
...
**Objectif :** réécrit l’URL de demande.
...
Option|Description
-|-
 Source et modèle | Ces paramètres définissent un modèle d’URI de requête qui identifie le type de requêtes pouvant être réécrites. Seules les requêtes dont l’URL satisfait aux deux critères suivants seront réécrites : <br/><br/>  - **Source (ou point d’accès au contenu) :** sélectionner un chemin d’accès relatif qui identifie un serveur d’origine. Il s’agit de la section _/XXXX/_ et de votre nom de point de terminaison. <br/><br/> - **Source (modèle) :** un modèle qui identifie les requêtes par le chemin d’accès relatif doit être défini. Ce modèle d’expression régulière doit définir un chemin d’accès qui commence directement après l’accès au contenu sélectionné précédemment pointe (voir ci-dessus). <br/> Vérifiez que les critères d’URI de requête (c’est-à-dire, Source et Modèle) définis précédemment ne sont pas en conflit avec les conditions de correspondance définies pour cette fonctionnalité. Spécifiez un modèle. Si vous utilisez une valeur vide comme modèle, toutes les chaînes sont mises en correspondance. 
 Destination  |Définissez l’URL relative vers laquelle les requêtes ci-dessus seront réécrites en : <br/>    1. Sélection d’un point d’accès au contenu qui identifie un serveur d’origine. <br/>    2. Définissant un chemin d’accès relatif à l’aide des éléments suivants : <br/>        -Un modèle d’expression régulière <br/>        - [Variables HTTP](cdn-http-variables.md) <br/> <br/> Remplacez les valeurs capturées dans le modèle source dans le modèle de destination à l’aide de $_n_ où _n_ identifie une valeur par l’ordre dans lequel elle a été capturée. Par exemple, $1 représente la première valeur capturée dans le modèle source, tandis que $2 représente la deuxième valeur. 
