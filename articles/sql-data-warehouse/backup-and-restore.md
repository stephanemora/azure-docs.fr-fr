...
Vous pouvez conserver les entrepôts de données restauré et actuel ou les supprimer tous les deux. Si vous voulez remplacer l’entrepôt de données actuel par l’entrepôt de données restauré, vous pouvez le renommer à l’aide d’[ALTER DATABASE (pool SQL)](/sql/t-sql/statements/alter-database-azure-sql-data-warehouse) avec l’option MODIFY NAME.

Pour restaurer un entrepôt de données, consultez [Restaurer un pool SQL avec le portail Azure](sql-data-warehouse-restore-database-portal.md), [Restaurer un pool SQL de avec PowerShell](sql-data-warehouse-restore-database-powershell.md) ou [Restaurer un pool SQL de avec des API REST](sql-data-warehouse-restore-database-rest-api.md).
...
Vous pouvez [restaurer votre pool SQL](https://docs.microsoft.com/azure/sql-data-warehouse/sql-data-warehouse-restore-from-geo-backup#restore-from-an-azure-geographical-region-through-powershell) dans n’importe quelle région prenant en charge le pool SQL au niveau de performances que vous avez choisi.
