...
<a name="boundary"></a> **Délimitation** : Une ligne ou la séparation des entités politiques adjacentes, tels que pays/régions, les circonscriptions et les propriétés des polygones. Une délimitation est une ligne qui peut suivre ou non des caractéristiques physiques telles que des rivières, des montagnes ou des murs.
...
<a name="country-code"></a> **Code pays** : Un identificateur unique pour un pays/région selon la norme ISO. ISO2 est un code à deux caractères pour un pays (par exemple, US), et ISO3 est un code à trois caractères (par exemple, USA).

<a name="country-subdivision"></a> **Sous-division de pays** : Une sous-division de premier niveau d’une pays/région, communément appelée un département ou une province.

<a name="country-secondary-subdivision"></a> **Sous-division secondaire de pays** : Une sous-division de second niveau d’une pays/région, communément appelée un comté.

<a name="country-tertiary-subdivision"></a> **Sous-division tertiaire de pays** : Une troisième niveau subdivision d’une pays/région, généralement à une zone nommée comme une ward.
...
<a name="postal-code"></a> **Code postal** : Une série de lettres ou chiffres ou les deux, dans un format spécifique, utilisé par le service postal d’un pays/région pour diviser des zones géographiques en zones afin de simplifier la remise du courrier.
...
<a name="thematic-maps"></a> **Cartes thématiques** : Carte simple qui a pour but d’illustrer un thème d’une zone géographique. Un scénario courant pour ce type de carte consiste aux régions d’administration telles que des pays/régions en fonction d’une mesure de données de couleur.
...
<a name="time-zone"></a> **Fuseau horaire** : Région du globe qui observe une heure uniforme à des fins commerciales, sociales et juridiques. Fuseaux horaires ont tendance à suivre les limites de leurs subdivisions et pays/régions.
...
<a name="Zoom level"></a> **Niveau de zoom** : Spécifie le niveau de détail et la partie visible de la carte. Avec un zoom arrière un niveau 0, la carte du monde complète sera souvent dans la vue, mais affiche les informations limitées telles que les bordures et les noms de pays/région, ainsi que noms de l’océan. Avec un zoom plus proche du niveau 17, la carte affiche une zone de quelques rues d’une ville avec des informations routières détaillées. Pour plus d’informations, consultez la [documentation Niveaux de zoom et grille mosaïque](zoom-levels-and-tile-grid.md).
