...
5. Dans le fichier **MainActivity.kt**, vous devez importer le Kit de développement logiciel (SDK) Google Maps. Transférez toutes les méthodes de cycle de vie de l’activité contenant la vue cartographique aux éléments correspondants dans la classe map. Récupérez une instance de `MapView` à partir du fragment de carte à l’aide de la méthode `getMapAsync(OnMapReadyCallback)`. `MapView` initialise automatiquement le système maps et l’affichage. Modifiez le fichier **MainActivity.kt** comme suit :
...
Avec Google Maps, une couche de mosaïques peut être superposée au-dessus de la carte. Utilise la classe `TileOverlayOptions`. Ajoutez la couche de mosaïques à la carte à l’aide de la méthode `addTileLayer`. Pour rendre les mosaïques semi-transparentes, l’option `transparency` est définie sur 0,2 (20 % de transparence).
