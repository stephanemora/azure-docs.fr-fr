---
title: "Outils de dessin d'Azure Maps : types d'interaction et raccourcis clavier | Microsoft Docs"
description: Tracer et modifier des formes à l'aide d'une souris, d'un écran tactile ou d'un clavier dans le kit de développement logiciel (SDK) web Azure Maps
author: rbrundritt
ms.author: richbrun
ms.date: 12/05/2019
ms.topic: conceptual
...
Cet article décrit les différentes méthodes disponibles pour tracer et modifier des formes sur une carte à l'aide d'une souris, d'un écran tactile ou de raccourcis clavier.
...
Vous trouverez ci-dessous les différentes méthodes disponibles pour tracer des formes sur la carte. Avant de pouvoir tracer une forme, l'option `drawingMode` du gestionnaire de dessins doit être définie sur un paramètre de dessin pris en charge. Cette action peut être effectuée par programmation ou en appuyant sur l'un des boutons de dessin de la barre d'outils. Le mode Dessin reste activé, même après la création d'une forme, ce qui facilite l'ajout d'autres formes du même type. Le mode Dessin peut être désactivé par programmation ou en cliquant sur le bouton des modes de dessin actuels de la barre d'outils. 
...
**Commencer à dessiner**
 ...
 - Si la souris se trouve sur la carte, appuyez sur la touche `F`. Un point sera alors ajouté en utilisant les coordonnées correspondant à l'emplacement du pointeur de la souris. Vous bénéficierez ainsi d'une précision accrue pour ajouter le point sur la carte car aucune pression ne sera exercée sur le bouton gauche de la souris, ce qui risquerait de déplacer légèrement celle-ci.
...
Lorsque le gestionnaire de dessins est en mode `draw-line`, les actions suivantes peuvent être effectuées pour tracer des points sur la carte en fonction du mode d'interaction défini.

**Commencer à dessiner**
 - Mode Clic
   * Cliquez sur le bouton gauche de la souris ou appuyez sur la carte pour y ajouter chacun des points d'une ligne. Les coordonnées correspondant à chaque clic/interaction tactile sont ajoutées à la ligne. 
   * Si la souris se trouve sur la carte, appuyez sur la touche `F`. Un point sera alors ajouté en utilisant les coordonnées correspondant à l'emplacement du pointeur de la souris. Vous bénéficierez ainsi d'une précision accrue pour ajouter le point sur la carte car aucune pression ne sera exercée sur le bouton gauche de la souris, ce qui risquerait de déplacer légèrement celle-ci.
...
Lorsque le gestionnaire de dessins est en mode `draw-polygon`, les actions suivantes peuvent être effectuées pour tracer des points sur la carte en fonction du mode d'interaction défini.

**Commencer à dessiner**
 - Mode Clic
   * Cliquez sur le bouton gauche de la souris ou appuyez sur la carte pour y ajouter chacun des points d'un polygone. Les coordonnées correspondant à chaque clic/interaction tactile sont ajoutées au polygone. 
   * Si la souris se trouve sur la carte, appuyez sur la touche `F`. Un point sera alors ajouté en utilisant les coordonnées correspondant à l'emplacement du pointeur de la souris. Vous bénéficierez ainsi d'une précision accrue pour ajouter le point sur la carte car aucune pression ne sera exercée sur le bouton gauche de la souris, ce qui risquerait de déplacer légèrement celle-ci.
   * Continuez à cliquer jusqu'à ce que tous les points souhaités aient été ajoutés au polygone.
 - Mode Main levée
   * Appuyez sur le bouton gauche de la souris ou sur la carte et faites glisser la souris ou le point tactile. Les coordonnées sont ajoutées au polygone à mesure que la souris ou le point tactile se déplace sur la carte. Dès que l'événement associé à la souris ou à l'interaction tactile est déclenché, le dessin est effectué. Notez que la fréquence à laquelle les coordonnées sont ajoutées est définie par l'option `freehandInterval` du gestionnaire de dessins.
 - Mode hybride
...
Lorsque le gestionnaire de dessins est en mode `draw-rectangle`, les actions suivantes peuvent être effectuées pour tracer des points sur la carte en fonction du mode d'interaction défini. La forme générée suivra la [spécification GeoJSON étendue relative aux rectangles](extend-geojson.md#rectangle).
...
Lorsque le gestionnaire de dessins est en mode `draw-circle`, les actions suivantes peuvent être effectuées pour tracer des points sur la carte en fonction du mode d'interaction défini. La forme générée suivra la [spécification GeoJSON étendue relative aux cercles](extend-geojson.md#circle).
...
Les outils de dessin prennent en charge les raccourcis clavier afin de faciliter la création et la modification de formes sur la carte. Ces raccourcis clavier fonctionnent lorsque le focus est placé sur la carte.
