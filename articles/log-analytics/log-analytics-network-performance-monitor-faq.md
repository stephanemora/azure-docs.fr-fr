...
[Network Performance Monitor](/azure/networking/network-monitoring-overview) est une solution [d’analyse réseau hybride](log-analytics-network-performance-monitor-performance-monitor.md) basé sur le cloud qui vous permet d’analyser les performances du réseau entre différents points de votre infrastructure réseau. Vous pouvez également analyser la connectivité réseau avec le [service et les points de terminaison d’application](log-analytics-network-performance-monitor-service-endpoint.md) ainsi que les [performances d’Azure ExpressRoute](log-analytics-network-performance-monitor-expressroute.md). 
...
* Exécutez le script Powershell [EnableRules.ps1](https://aka.ms/npmpowershellscript) sur le nœud. Pour plus d'informations, voir [Instructions](log-analytics-network-performance-monitor.md#configure-log-analytics-agents-for-monitoring).
...
Oui. Pour plus d'informations, voir l'article [Surveiller Azure, AWS et les réseaux locaux à l’aide de NPM](https://blogs.technet.microsoft.com/msoms/2016/08/30/monitor-on-premises-cloud-iaas-and-hybrid-networks-using-oms-network-performance-monitor/).
...
NPM peut surveiller la connectivité entre les réseaux dans n’importe quelle partie du monde, à partir d’un espace de travail hébergé dans une des [régions prises en charge](log-analytics-network-performance-monitor.md#supported-regions)
...
NPM peut surveiller la connectivité des services dans n’importe quelle partie du monde, à partir d’un espace de travail hébergé dans une des [régions prises en charge](log-analytics-network-performance-monitor.md#supported-regions)
...
NPM peut surveiller vos circuits ExpressRoute situés dans n’importe quelle région Azure. Pour intégrer NPM, vous devez disposer d'un espace de travail Log Analytics hébergé dans l'une des [régions prises en charge](/azure/expressroute/how-to-npm#regions)
...
* Pour vérifier qu’un pare-feu réseau intermédiaire ou groupe de sécurité réseau Azure ne bloque pas la communication sur le port requis, utilisez l’utilitaire PsPing tiers comme suit :
  * L'utilitaire PsPing peut être téléchargé [ici](https://technet.microsoft.com/sysinternals/psping.aspx). 
...
Cela peut se produire si votre circuit et vos connexions d’appairage sont répartis entre plusieurs abonnements. NPM détecte uniquement les connexions d'appairage privées ExpressRoute dans lesquelles les réseaux virtuels connectés à ExpressRoute se trouvent dans le même abonnement que celui lié à l’espace de travail NPM. En outre, NPM détecte les connexions d’appairage Microsoft dans lesquelles le circuit ExpressRoute connecté se trouve dans le même abonnement que celui lié à l'espace de travail NPM. Voici un exemple :

 Vous disposez de 2 réseaux virtuels - le réseau virtuel A dans l'abonnement A et le réseau virtuel B dans l'abonnement B, respectivement connectés à ExpressRoute dans l'abonnement C. En outre, il existe un autre réseau virtuel, le réseau virtuel C dans l'abonnement C. Le réseau ER dispose aussi d'un appairage MS dans l'abonnement C. 

Ainsi,

* Si l’espace de travail NPM est lié à l’abonnement A, vous serez en mesure de surveiller la connectivité entre le réseau ER et le réseau virtuel A uniquement.
* Si l’espace de travail NPM est lié à l’abonnement B, vous serez en mesure de surveiller la connectivité entre le réseau ER et le réseau virtuel B uniquement.
* Si l’espace de travail NPM est lié à l’abonnement C, vous serez en mesure de surveiller la connectivité entre le réseau ER et le réseau virtuel C, ainsi que l'appairage MS.

La prise en charge entre abonnements sera bientôt disponible. Dès lors, vous serez en mesure de surveiller toutes vos connexions d'appairage privées ExpressRoute et Microsoft dans différents abonnements, à partir d'un même espace de travail.
...
- Pour plus d'informations sur Network Performance Monitor, voir [Solution Network Performance Monitor dans Azure](log-analytics-network-performance-monitor.md).
