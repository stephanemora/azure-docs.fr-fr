...
* **ServiceBus** (Resource Manager uniquement) : cette balise désigne les préfixes d’adresse du service Azure ServiceBus. Si vous spécifiez *ServiceBus* comme valeur, le trafic vers ServiceBus est autorisé ou refusé. Si vous souhaitez uniquement autoriser l’accès à ServiceBus dans une [région](https://azure.microsoft.com/regions) spécifique, vous pouvez spécifier la région au format suivant : ServiceBus.[nom_région]. 
...
* **ApiManagement** (Resource Manager uniquement) : cette balise désigne les préfixes d’adresse du service Azure Api Management. Si vous spécifiez *ApiManagement* comme valeur, le trafic vers ApiManagement est autorisé ou refusé. Si vous souhaitez uniquement autoriser l’accès à ApiManagement dans une [région](https://azure.microsoft.com/regions) spécifique, vous pouvez spécifier la région au format suivant : ApiManagement.[nom_région]. 
