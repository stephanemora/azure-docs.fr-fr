...
5. <a name="add-peering"></a> :
    ...
    - **Autoriser l’accès au réseau virtuel** : Sélectionnez **Activé** (par défaut) pour activer la communication entre les deux réseaux virtuels via le flux `VirtualNetwork` par défaut. L’activation de la communication entre les réseaux virtuels permet aux ressources connectées à ceux-ci de communiquer entre elles avec les mêmes bande passante et latence que si elles étaient connectées au même réseau virtuel. Toute communication entre les ressources des deux réseaux virtuels passe par le réseau privé Azure. L’étiquette de service **VirtualNetwork** pour les groupes de sécurité réseau englobe le réseau virtuel et le réseau virtuel appairé lorsque ce paramètre est **activé**. (Pour en savoir plus sur les étiquettes de service de groupes de sécurité réseau, consultez [Vue d’ensemble des groupes de sécurité réseau](./network-security-groups-overview.md#service-tags).) Si vous ne souhaitez pas que le trafic soit dirigé vers le réseau virtuel appairé par défaut, sélectionnez **Désactivé**. Vous pouvez sélectionner **Désactivé** si vous avez appairé un réseau virtuel avec un autre réseau virtuel, mais souhaitez occasionnellement désactiver le flux de trafic par défaut entre les deux réseaux virtuels. Il se peut que vous trouviez plus pratique d’activer ou de désactiver les peerings que de les supprimer et recréer. Lorsque ce paramètre est désactivé, le trafic n’est pas transmis entre les réseaux virtuels appairés par défaut ; toutefois, le trafic peut continuer à circuler s’il est explicitement autorisé par le biais d’une règle de [groupe de sécurité réseau](./network-security-groups-overview.md) incluant les adresses IP ou les groupes de sécurité d’application appropriés.
      > [!WARNING]
      > La désactivation du paramètre **Autoriser l’accès au réseau virtuel** modifie uniquement la définition de l’étiquette de service **VirtualNetwork**. Cela *n’empêche pas* totalement le flux de trafic sur la connexion entre pairs, comme l’explique la description de ce paramètre.    
