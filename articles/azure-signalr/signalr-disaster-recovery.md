...
L’instance principale et le serveur d’applications sont connectés via des connexions serveur normales, mais l’instance secondaire et le serveur d’applications sont connectés via un type spécial de connexions, appelées connexions faibles.
La principale différence d’une connexion faible est qu’elle n’accepte pas le routage des connexions client, car l’instance secondaire se trouve généralement dans une autre région. Le routage d’un client vers une autre région n’est généralement pas un choix optimal (augmente la latence).
...
Figure 1 Avant le basculement ![Avant le basculement](media/signalr-disaster-recovery/before-failover.png)

Figure 2 Après le basculement ![Après le basculement](media/signalr-disaster-recovery/after-failover.png)

Fig.3 Peu après la récupération de l’instance principale ![](media/signalr-disaster-recovery/after-recover.png)
...
Veuillez noter que, quel que soit le modèle que vous choisissez d’utiliser, vous devez connecter chaque instance du service SignalR à un serveur d’applications en tant qu’instance principale.
