...
Cet article explique comment migrer un environnement hybride existant vers un WAN virtuel.

## <a name="scenario"></a>Scénario

Contoso est une organisation financière internationale avec des bureaux en Europe et en Asie. Cette organisation envisage de déplacer ses applications existantes depuis son contrôleur de domaine local vers Azure et a développé une conception de base qui s’appuie sur l’architecture VDC, y compris des réseaux virtuels de type hub managés par les clients régionaux pour la connectivité hybride. Dans le cadre du passage aux technologies basées sur le cloud, l’équipe réseau a été chargée de s’assurer que la connectivité est optimisée pour les activités à venir de l’organisation.
...
- Le modèle hub-and-spoke est utilisé dans plusieurs régions. Des circuits ExpressRoute Premium sont utilisés pour la connectivité à un WAN privé commun.
...
- Supprimer la dépendance aux contrôleurs de domaine (DC) locaux existants afin d’arrêter le VPN tout en conservant les chemins de connectivité suivants :
...
- Les hubs fournissent également une connexion VPN pour les utilisateurs itinérants sur plusieurs types de clients à l’aide de la connectivité OpenVPN au réseau de maillage global.Ainsi, ils peuvent accéder non seulement aux applications migrées vers Azure, mais également à toutes les ressources locales. 
- Connectivité Internet pour les ressources au sein d’un réseau virtuel fournie par Azure Virtual WAN. Connectivité Internet pour les sites distants fournie par Azure Virtual WAN. Local Internet Breakout pris en charge via l’intégration des partenaires pour un accès optimisé aux services SaaS tels qu’Office 365.
...
- Services partagés (toute fonction commune requise par plusieurs spokes) : Contoso utilise par exemple des contrôleurs de domaine Windows Server IaaS sur des machines virtuelles IaaS.
...
La première étape consiste à déployer un hub Virtual WAN dans chaque région. Déployez le hub Virtual WAN avec la passerelle VPN et la passerelle ExpressRoute, comme décrit dans les articles suivants : 
...
### <a name="connect-remote-sites-expressroute-and-vpn-to-virtual-wan"></a>Connexion des sites distants (ExpressRoute et VPN) au WAN virtuel

À présent, nous allons connecter le hub Virtual WAN aux circuits ExpressRoute de l’entreprise et configurer des VPN de site à site via Internet vers toutes les branches distantes.
...
![Connexion des sites distants au WAN virtuel](./media/migrate-from-hub-spoke-topology/figure6.png)
**Figure 6 : Migration du VDC hub-and-spoke vers Virtual WAN - Étape 3**

À ce stade, l’équipement réseau local commencera à recevoir des itinéraires reflétant l’espace d’adressage IP affecté au réseau virtuel hub managé par le WAN. À ce stade, les branches distantes connectées via un VPN voient deux chemins d’accès à toutes les applications existantes sur les réseaux virtuels spoke. Ces appareils doivent être configurés pour continuer à utiliser le tunnel vers le hub VDC pour garantir un routage symétrique pendant la phase de transition.

### <a name="test-hybrid-connectivity-via-virtual-wan"></a>Test de la connectivité hybride via un WAN virtuel
...
![Test de la connectivité hybride via un WAN virtuel](./media/migrate-from-hub-spoke-topology/figure7.png)
...
**b**. Connectez les réseaux virtuels spoke au hub Virtual WAN via des connexions de réseau virtuel.
...
**d**. Les passerelles VPN et ExpressRoute existantes dans le hub VDC sont maintenant désactivées pour autoriser le passage à l’étape 5.

**e**. Connectez l’ancien hub VDC (réseau virtuel hub) au hub Virtual WAN via une nouvelle connexion de réseau virtuel.
...
Étant donné que le hub Virtual WAN est une entité managée et qu’il n’autorise pas le déploiement de ressources personnalisées telles que des machines virtuelles, le bloc de services partagés existe désormais en tant que réseau virtuel spoke et héberge des fonctions telles que l’entrée Internet via la passerelle Azure Application ou l’appliance virtuelle de réseau. Le trafic entre l’environnement de services partagés et les machines virtuelles back-end transite désormais par le hub managé par Virtual WAN.
...
**Figure 10 : Migration du VDC hub-and-spoke vers Virtual WAN - Étape 7**
 ...
 Pour tirer parti de toutes les fonctionnalités d’Azure Virtual WAN, Contoso a décidé de désactiver sa connexion VPN locale héritée. Toutes les branches qui continuent d’accéder aux réseaux du siège social ou du contrôleur de domaine sont en mesure de transiter par le réseau global de Microsoft à l’aide du routage de transit intégré à Azure Virtual WAN. Les clients qui souhaitent tirer parti de l’infrastructure principale de Microsoft pour compléter leurs WAN privés existants peuvent aussi utiliser ExpressRoute Global Reach.
...
Le chemin 1 décrit le flux de trafic à partir de la branche S2S VPN en Asie vers le réseau virtuel Azure au sein de la région Asie Sud-Est.
...
Le trafic est routé comme suit :
- Le siège européen est connecté via le circuit ExpressRoute standard au hub Virtual WAN de la région Europe Ouest.
- La connectivité globale de hub à hub Virtual WAN permet un transit transparent du trafic vers le réseau virtuel connecté dans une région distante.

![Flux 2](./media/migrate-from-hub-spoke-topology/flow2.png)

### <a name="path-3"></a>Chemin 3
Le chemin 3 décrit le flux du trafic depuis le contrôleur de domaine local en Asie connecté au WAN privé de la branche européenne connectée de site à site.
...
- La connectivité globale de hub à hub Virtual WAN permet un transit transparent du trafic des branches connectées au hub distant en Europe.

![Flux 3](./media/migrate-from-hub-spoke-topology/flow3.png)


### <a name="path-4"></a>Chemin 4
Le chemin 4 décrit le flux du trafic à partir du réseau virtuel Azure de la région Asie Sud-Est vers le réseau virtuel Azure de la région Europe Ouest.
...
Le chemin 5 décrit le flux du trafic à partir de l’utilisateur itinérant de VPN (P2S) vers le réseau virtuel Azure de la région Europe Ouest.

Le trafic est routé comme suit :
- Les utilisateurs d’ordinateurs portables et de téléphones utilisent le client OpenVPN pour une connectivité transparente vers la passerelle VPN P2S de la région Europe Ouest.
...
L’étape ci-dessus permet au hub Virtual WAN existant de fonctionner en tant que hub virtuel sécurisé et permet de déployer les ressources de pare-feu Azure requises.
...
Le chemin 6 décrit le flux du trafic à partir d’un réseau virtuel vers le transit sécurisé de réseau virtuel au sein de la même région.
...
Le chemin 7 décrit le flux du trafic à partir du réseau virtuel vers Internet ou un service de sécurité tiers.
