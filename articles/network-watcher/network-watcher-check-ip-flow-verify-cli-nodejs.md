...
L’adresse IP d’une carte réseau sur l’ordinateur virtuel est nécessaire. Récupérez les cartes réseau pour une machine virtuelle avec la commande qui suit. Si vous connaissez déjà l’adresse IP que vous souhaitez tester sur la machine virtuelle, vous pouvez ignorer cette étape.
...
Exécutez l’applet de commande `network watcher ip-flow-verify` pour tester le trafic. Dans cet exemple, la première adresse IP sur la première carte réseau est utilisée :
