...
## <a name="ingest-data-sinceutc"></a>Ingérer des données depuis (UTC)

L’ingestion des données depuis (UTC) est l’heure à laquelle vous souhaitez que Metrics Advisor commence à ingérer des données à partir de votre source de données. Votre source de données doit contenir des données à l’heure de début d’ingestion spécifiée.
...
Metrics Advisor fournit des outils pour ajuster la sensibilité des algorithmes utilisés. Découvrez [comment configurer des métriques et affiner la configuration de détection ](how-tos/configure-metrics.md) pour plus d’informations.
...
Après l’application d’une configuration de détection aux mesures, des incidents sont générés chaque fois qu’une série au sein de celle-ci présente une anomalie. Dans les jeux de données volumineux, cela peut s’avérer écrasant. Metrics Advisor regroupe alors une série d’anomalies au sein d’une mesure en un incident. Le service évaluera également la gravité et fournira des outils pour [procéder au diagnostic d’un incident](how-tos/diagnose-an-incident.md).

### <a name="diagnostic-tree"></a>Arborescence de diagnostic
