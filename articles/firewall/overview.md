...
|Insuffisance de ports SNAT|Actuellement, le pare-feu Azure prend en charge 1024 ports par adresse IP publique par instance de groupe de machines virtuelles identiques back-end. Par défaut, il existe deux instances VMSS.|Il s’agit de limites de l’équilibreur de charge standard, et nous cherchons constamment des opportunités de les repousser. En attendant, il est recommandé de configurer les déploiements de pare-feu Azure avec un minimum de cinq adresses IP publiques pour les déploiements sujets à une insuffisance de ports SNAT. Ceci multiplie par cinq le nombre de ports SNAT disponibles. Effectuez l’allocation à partir d’un préfixe d’adresse IP pour simplifier les autorisations en aval.|
