...
description: Découvrez les fonctionnalités de sauvegarde et de restauration dans Azure SQL Edge (préversion).
keywords: ''
services: sql-edge
ms.service: sql-edge
ms.topic: conceptual
...
ms.locfileid: "84667860"
---
# <a name="back-up-and-restore-databases-in-azure-sql-edge-preview"></a>Sauvegarder et restaurer des bases de données dans Azure SQL Edge (préversion) 

Azure SQL Edge repose sur les dernières versions du moteur de base de données SQL Server sur Linux. Il fournit des fonctionnalités de sauvegarde et de restauration de base de données similaires à celles disponibles dans SQL Server sur Linux et SQL Server s’exécutant dans des conteneurs. Le composant de sauvegarde et restauration apporte une sécurité essentielle pour la protection des données stockées dans vos bases de données Azure SQL Edge. 
...
Azure SQL Edge prend en charge les sauvegardes dans les objets blob de pages et les objets blob de blocs. Pour plus d’informations, consultez [Sauvegarder dans un objet blob de blocs ou un objet blob de pages](https://docs.microsoft.com/sql/relational-databases/backup-restore/sql-server-backup-to-url?view=sql-server-ver15#blockbloborpageblob). Dans l’exemple suivant, la base de données *IronOreSilicaPrediction* est sauvegardée dans un objet blob de blocs. 
