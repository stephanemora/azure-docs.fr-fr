...
ms.locfileid: "87022905"
---
# <a name="manage-assets"></a>Gérer les ressources

Dans Azure Media Services, un [actif multimédia](/rest/api/media/assets) est l’endroit où vous 
...
2. Obtenez une URL SAS avec des autorisations de lecture-écriture qui sera utilisée pour charger des fichiers numériques dans le conteneur d’actifs multimédias.
    ...
    Vous pouvez utiliser l’API Media Services pour [lister les URL de conteneurs d’actifs multimédias](/rest/api/media/assets/listcontainersas).

    **AssetContainerSas. listContainerSas** utilise un paramètre [ListContainerSasInput](/rest/api/media/assets/listcontainersas#listcontainersasinput) pour lequel vous définissez `expiryTime`. L’heure doit être définie sur < 24 heures.
...
4. Utilisez des API Media Services v3 pour créer une transformation et un travail afin de traiter votre actif multimédia « d’entrée ». Pour plus d’informations, consultez [Transformations et travaux](./transforms-jobs-concept.md).
...
Pour obtenir un exemple REST, consultez l’exemple [Créer un actif multimédia avec REST](/rest/api/media/assets/createorupdate#examples).
...
Dans Media Services, un objet [Sortie en direct](/rest/api/media/liveoutputs) fonctionne comme un magnétoscope numérique qui capture et enregistre votre stream en direct dans un actif multimédia dans votre compte Media Services. Le contenu enregistré est conservé dans le conteneur défini par la ressource d’[actif multimédia](/rest/api/media/assets).
