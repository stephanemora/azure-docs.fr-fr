...
ms.locfileid: "69990614"
---
# <a name="dynamic-packaging"></a>l’empaquetage dynamique

Vous pouvez utiliser Microsoft Azure Media Services pour encoder de nombreux formats de fichiers sources multimédias et les remettre via différents protocoles de streaming, avec ou sans protection du contenu, aux principaux appareils (par exemple, les appareils iOS et Android). Ces clients comprennent différents protocoles. Par exemple, iOS demande que les flux soient remis au format HTTP Live Streaming (HLS) et que les appareils Android prennent en charge TLS et MPEG DASH. Pour préparer vos fichiers sources en vue d’un streaming à débit adaptatif, vous devez les [encoder](encoding-concept.md) en un ensemble de fichiers MP4 ( format de fichier multimédia de base ISO 14496-12) (également appelé débit adaptatif). À partir de cet ensemble de fichiers MP4, vous pouvez diffuser des vidéos via les protocoles HLS, MPEG-DASH ou Smooth Streaming à l’aide de l’**empaquetage dynamique**.
...
Dans Media Services, l’empaquetage dynamique est utilisé, que le streaming concerne des vidéos en direct ou à la demande. 
