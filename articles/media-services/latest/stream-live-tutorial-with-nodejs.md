...
Cette section montre comment créer un type *Pass-through* de base d’événement en direct (`LiveEventEncodingType` défini avec `PassthroughBasic`). Pour plus d’informations sur les types disponibles, consultez [Types d’événements en direct](live-event-outputs-concept.md#live-event-types). Outre Pass-through (de base ou standard), vous pouvez utiliser un événement d’encodage en direct pour un encodage cloud à débit adaptatif 720p ou 1080p.
