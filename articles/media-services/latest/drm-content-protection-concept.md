...
1. Créer et configurer une [stratégie de clé de contenu](drm-content-key-policy-concept.md).
   ...
   * Configurez les licences associées à [PlayReady](drm-playready-license-template-concept.md), [Widevine](drm-widevine-license-template-concept.md) et/ou [FairPlay](drm-fairplay-license-overview.md). Les modèles vous permettent de configurer les droits et autorisations pour chacun des DRM.
...
2. Créez un [localisateur de streaming](stream-streaming-locators-concept.md) configuré pour diffuser en continu une ressource chiffrée.
  
   Le localisateur de streaming doit être associé à une [stratégie de diffusion en continu](stream-streaming-policy-concept.md). Dans cet exemple, nous définissons `StreamingLocator.StreamingPolicyName` sur la stratégie « Predefined_MultiDrmCencStreaming ».
