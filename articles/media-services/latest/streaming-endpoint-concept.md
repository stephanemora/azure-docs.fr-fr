...
Quand vous créez un compte Media Services, un point de terminaison de streaming **par défaut** est créé pour vous dans l’état Arrêté. Vous pouvez créer d’autres points de terminaison de streaming sous votre compte (voir [Quotas et limites](limits-quotas-constraints-reference.md)).
...
Veillez à consulter également la rubrique [Empaquetage dynamique](encode-dynamic-packaging-concept.md). 
