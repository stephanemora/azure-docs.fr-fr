...
1. Une fois que vous avez accédé au compte Azure Media Services dans le portail, sélectionnez **Stream en direct** dans la liste Media Services.<br/>
![Sélectionner le lien Stream en direct](media/live-events-wirecast-quickstart/select-live-streaming.png)<br/>
1. Cliquez sur **Ajouter un événement en direct** pour créer un événement de streaming en direct.<br/>
![Icône Ajouter un événement en direct](media/live-events-wirecast-quickstart/add-live-event.png)<br/>
1. Entrez un nom pour votre nouvel événement, par exemple *TestLiveEvent*, dans le champ **Nom** de l’événement en direct.<br/>
![Champ de texte Nom de l’événement en direct](media/live-events-wirecast-quickstart/live-event-name.png)<br/>
1. Entrez une description facultative de l’événement dans le champ **Description**.
1. Sélectionnez la case d’option **Transfert direct - pas d’encodage cloud**.<br/>
![Case d’option Encodage cloud](media/live-events-wirecast-quickstart/cloud-encoding.png)
1. Sélectionnez la case d’option **RTMP**.
1. Vérifiez que la case d’option **Non** est sélectionnée pour Démarrer l’événement en direct, afin d’éviter la facturation de l’événement en direct avant qu’il ne soit prêt.  (La facturation commence au démarrage de l’événement en direct.) ![Case d’option Démarrer l’événement en direct](media/live-events-wirecast-quickstart/start-live-event-no.png)<br/>
...
1. Pointez sur la première couche dans la zone des couches Wirecast.  Cliquez sur l’icône **Ajouter** qui s’affiche, puis sélectionnez l’entrée vidéo que vous voulez diffuser.  La boîte de dialogue Couche principale 1 s’ouvre.<br/>
![Icône Ajouter dans Wirecast](media/live-events-wirecast-quickstart/add-icon.png)
1. Sélectionnez **Capture vidéo** dans le menu, puis sélectionnez l’appareil photo que vous voulez utiliser. Si vous sélectionnez un appareil photo, la vue de l’appareil photo s’affiche dans la zone Aperçu.
![Écran de sélection de la capture vidéo dans Wirecast](media/live-events-wirecast-quickstart/video-shot-selection.png)
...
![Écran de sélection de la capture audio dans Wirecast](media/live-events-wirecast-quickstart/audio-shot-select.png)
1. Dans le menu principal, sélectionnez **Paramètres de sortie**.  La boîte de dialogue Sortie s’affiche.
1. Sélectionnez **Azure Media Services** dans la liste déroulante de sortie.  Le paramètre de sortie pour Azure Media Services remplit automatiquement *la plupart* des paramètres de sortie.<br/>
...
1. De retour dans la page Azure Media Services du portail, cliquez sur **Démarrer** pour démarrer l’événement de stream en direct. (La facturation commence maintenant.)<br/>
...
4. Basculez vers l’application Wirecast, puis collez l’**URL d’entrée** dans le champ **Adresse** dans les paramètres de sortie.<br/>
...
![Champ Nom de sortie](media/live-events-wirecast-quickstart/output-name.png)
...
![Champ Nom du localisateur](media/live-events-wirecast-quickstart/live-event-locator.png)
...
1. Vérifiez que votre point de terminaison de streaming est en cours d’exécution en sélectionnant **Points de terminaison de streaming** dans la liste Media Services. Vous êtes dirigé vers la page des points de terminaison de streaming.<br/>
![Élément de menu Point de terminaison de streaming](media/live-events-wirecast-quickstart/streaming-endpoints.png)
...
1. Cliquez sur **Start**.  Cette action démarre le point de terminaison de streaming.<br/>
![Élément de menu Point de terminaison de streaming](media/live-events-wirecast-quickstart/start.png)
