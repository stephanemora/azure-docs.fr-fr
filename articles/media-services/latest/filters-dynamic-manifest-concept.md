...
Azure Media Services prend en charge les protocoles HLS, MPEG DASH et Smooth Streaming. Dans le cadre du l'[empaquetage dynamique](encode-dynamic-packaging-concept.md), les manifestes du client de streaming (liste de lecture principale HLS, DASH MPD (Media Presentation Description) et Smooth Streaming) sont générés de façon dynamique en fonction du sélecteur de format de l'URL. Pour plus d’informations, consultez les protocoles de remise présentés dans [Workflow à la demande commun](encode-dynamic-packaging-concept.md#to-prepare-your-source-files-for-delivery).
