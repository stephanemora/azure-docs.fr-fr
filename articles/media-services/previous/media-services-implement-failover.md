...
2. Créez le dossier **SupportFiles** au même niveau que le fichier de projet **HandleRedundancyForOnDemandStreaming.csproj**. Sous le dossier **SupportFiles**, créez les dossiers **OutputFiles** et **MP4Files**. Copiez un fichier .mp4 dans le dossier **MP4Files**. (Dans cet exemple, le fichier **ignite.mp4** est utilisé.) 
3. Utilisez **NuGet** pour ajouter des références aux DLL associées à Media Services. Dans le **menu principal de Visual Studio**, sélectionnez **OUTILS** > **Gestionnaire de package NuGet** > **Console du Gestionnaire de package**. Dans la fenêtre de la console, tapez **Install-Package windowsazure.mediaservices**, puis appuyez sur Entrée.
4. Ajoutez les autres références requises pour ce projet : System.Runtime.Serialization et System.Web.
...
2. Remplacez la définition de la méthode Main par défaut par l’un des éléments suivants : Les définitions de méthode appelées à partir de Main sont définies ci-dessous.
    ...
    ```
3. Les définitions de méthode suivantes sont appelées à partir de Main. Pour plus d’informations sur chaque méthode, consultez les commentaires.
