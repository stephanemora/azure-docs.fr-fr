---
title: Configurer une stratégie d’autorisation de clé de contenu avec REST - Azure | Microsoft Docs
...
* Pour utiliser l’empaquetage et le chiffrement dynamiques, assurez-vous que le point de terminaison de streaming à partir duquel vous souhaitez diffuser votre contenu se trouve à l’état En cours d’exécution.
...
> 
> Lorsque vous accédez aux entités dans Media Services, vous devez définir les valeurs et les champs d’en-tête spécifiques dans vos requêtes HTTP. Pour plus d’informations, consultez [Installation pour le développement REST API de Media Services](media-services-rest-how-to-use.md).
> 
> 
> 
...
#### <a name="create-contentkeyauthorizationpolicies"></a><a id="ContentKeyAuthorizationPolicies"></a>Création de ContentKeyAuthorizationPolicies
Demande :
...
#### <a name="create-contentkeyauthorizationpolicyoptions"></a><a id="ContentKeyAuthorizationPolicyOptions"></a>Création de ContentKeyAuthorizationPolicyOptions
Demande :
...
#### <a name="link-contentkeyauthorizationpolicies-with-options"></a><a id="LinkContentKeyAuthorizationPoliciesWithOptions"></a>Lien de ContentKeyAuthorizationPolicies avec les options
Demande :
...
#### <a name="add-an-authorization-policy-to-the-content-key"></a><a id="AddAuthorizationPolicyToKey"></a>Ajouter une stratégie d’autorisation à la clé de contenu
Demande :
...
#### <a name="token-restriction-schema"></a><a id="schema"></a>Schéma de restriction par jeton
```xml
...
Créez une stratégie de restriction par jeton, comme illustré dans la section [Créer des stratégies ContentKeyAuthorizationPolicies](#ContentKeyAuthorizationPolicies).
...
Liez ContentKeyAuthorizationPolicies avec les options, comme illustré dans la section [Créer des stratégies ContentKeyAuthorizationPolicies](#ContentKeyAuthorizationPolicies).

#### <a name="add-an-authorization-policy-to-the-content-key"></a>Ajouter une stratégie d’autorisation à la clé de contenu
Ajoutez AuthorizationPolicy à ContentKey, comme indiqué dans la section [Ajouter une stratégie d’autorisation à la clé de contenu](#AddAuthorizationPolicyToKey).
...
Quand vous protégez votre contenu avec PlayReady, vous devez spécifier dans votre stratégie d’autorisation une chaîne XML qui définit le [modèle de licence PlayReady](media-services-playready-license-template-overview.md). 
...
#### <a name="create-contentkeyauthorizationpolicies"></a><a id="ContentKeyAuthorizationPolicies2"></a>Création de ContentKeyAuthorizationPolicies
Demande :
...
### <a name="token-restriction"></a>Restriction à jeton
Pour configurer l’option de restriction par jeton, vous devez utiliser un document XML pour décrire les exigences du jeton d’autorisation. Le XML de configuration de la restriction par jeton doit être conforme au schéma XML présenté dans la section [Schéma de restriction par jeton](#schema).

#### <a name="create-contentkeyauthorizationpolicies"></a>Création de ContentKeyAuthorizationPolicies
Créez ContentKeyAuthorizationPolicies, comme illustré dans la section [Créer des stratégies ContentKeyAuthorizationPolicies](#ContentKeyAuthorizationPolicies2).
...
## <a name="types-used-when-you-define-contentkeyauthorizationpolicy"></a><a id="types"></a>Types utilisés durant la définition de ContentKeyAuthorizationPolicy
...
> [!NOTE]
> La restriction sur les stratégies d’autorisation de clé de contenu n’est pas encore disponible dans le service.
