...
description: Découvrez comment activer et consulter les journaux pour le plan de contrôle Kubernetes dans Azure Kubernetes service (AKS)
...
ms.locfileid: "101735129"
---
# <a name="enable-and-review-kubernetes-control-plane-logs-in-azure-kubernetes-service-aks"></a>Activer et consulter les journaux de plan de contrôle dans Azure Kubernetes Service (AKS)

Avec Azure Kubernetes Service (AKS), les composants du plan de contrôle tels que *kube-apiserver* et *kube-controller-manager* sont fournis en tant que service géré. Vous créez et gérez les nœuds qui exécutent le *kubelet* et le runtime de conteneurs, et vous déployez vos applications par le biais du serveur d’API de l’environnement Kubernetes managés. Pour faciliter la résolution des problèmes de votre application et de vos services, il se peut que vous deviez consulter les journaux générés par ces composants du plan de contrôle. Cet article explique comment utiliser les journaux Azure Monitor pour activer et interroger les journaux des composants du plan de contrôle Kubernetes.
...
Les journaux Azure Monitor sont activés et gérés sur le portail Azure. Pour activer la collecte des journaux relatifs aux composants du plan de contrôle Kubernetes dans votre cluster AKS, ouvrez le portail Azure dans un navigateur web et procédez comme suit :
...
2. Sur le côté gauche, choisissez **Paramètres de diagnostic**.

3. Sélectionnez votre cluster AKS, par exemple *myAKSCluster*, puis choisissez **Ajouter un paramètre de diagnostic**.
  :::image type="content" source="media\view-control-plane-logs\select-add-diagnostic-setting.PNG" alt-text="Capture d’écran du portail Azure dans une fenêtre de navigateur montrant les paramètres de diagnostic, et indiquant que l’option « Ajouter un paramètre de diagnostic » doit être sélectionnée":::

4. Entrez un nom, par exemple *myAKSClusterLogs*, puis sélectionnez l’option pour **Envoyer vers l’espace de travail Log Analytics**.

5. Sélectionnez un espace de travail existant ou créez-en un. Si vous créez un espace de travail, indiquez un nom d’espace de travail, un groupe de ressources et un emplacement.

6. Dans la liste des journaux d’activité disponibles, sélectionnez les journaux d’activité que vous souhaitez activer. Pour cet exemple, activez les journaux *kube-audit* et *kube-audit-admin*. Les journaux *kube-apiserver*, *kube-controller-manager* et *kube-scheduler* compte parmi les journaux courants. Vous pourrez réaccéder à cet emplacement et modifier les journaux d’activité collectés une fois les espaces de travail Log Analytics activés.

7. Lorsque vous avez terminé, sélectionnez **Enregistrer** pour activer la collecte des journaux d’activité sélectionnés.
  :::image type="content" source="media\view-control-plane-logs\settings-selected.PNG" alt-text="Capture d’écran de l’écran « Ajouter un paramètre de diagnostic » dans le portail Azure. Une destination « Envoyer à l’espace de travail Log Analytics » et des journaux « kube-audit » et « kube-audit-admin » sont sélectionnés":::
