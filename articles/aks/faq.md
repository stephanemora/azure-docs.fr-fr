...
L’agent Log Analytics est pris en charge, car il s’agit d’une extension gérée par Microsoft. Sinon, non, AKS est un service géré et la manipulation des ressources IaaS n’est pas prise en charge. Pour installer des composants personnalisés, etc., utilisez les mécanismes et les API Kubernetes. Par exemple, utilisez des DaemonSets pour installer les composants nécessaires.
