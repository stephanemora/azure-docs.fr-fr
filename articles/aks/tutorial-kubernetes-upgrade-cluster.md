...
Dans l’exemple suivant, la version actuelle est *1.9.11*, et les versions disponibles s’affichent sous la colonne *Upgrades*.
...
Utilisez la commande [az aks upgrade][] pour mettre à niveau le cluster AKS. L’exemple suivant met à niveau le cluster vers Kubernetes version *1.10.9*.

> [!NOTE]
> Vous ne pouvez mettre à niveau qu’une version mineure à la fois. Par exemple, vous pouvez effectuer une mise à niveau de la version *1.9.11* vers la version *1.10.9*, mais pas de la version *1.9.6* directement vers la version *1.11.x*. Pour effectuer une mise à niveau de *1.9.11* vers *1.11.x*, commencez par mettre à niveau la version *1.9.11* vers la version *1.10.x*, puis mettez à niveau la version *1.10.x* vers la version *1.11.x*.

```azurecli
az aks upgrade --resource-group myResourceGroup --name myAKSCluster --kubernetes-version 1.10.9
```

L’exemple de sortie condensée suivant montre que la valeur *kubernetesVersion* indique à présent *1.10.9* :
...
L’exemple de sortie suivant indique que le cluster AKS exécute *KubernetesVersion 1.10.9* :
