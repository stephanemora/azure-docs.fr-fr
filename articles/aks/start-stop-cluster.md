...
Vous pouvez vérifier à quel moment votre cluster est arrêté à l’aide de la commande [az aks show][az-aks-show] et en confirmant que `powerState` s’affiche comme `Stopped` comme dans la sortie ci-dessous :
...
Vous pouvez vérifier à quel moment votre cluster est démarré à l’aide de la commande [az aks show][az-aks-show] et en confirmant que `powerState` s’affiche comme `Running` comme dans la sortie ci-dessous :
