...
...et cette directive `include` supplémentaire au fichier HolographicAppMain.cpp :
...
Pour simplifier le code, nous définissons le raccourci d’espace de noms suivant en haut du fichier HolographicAppMain.h, après la directive `include` :
...
```
Dans la fonction locale `SetNewSession`, nous nous inscrivons à un rappel qui est déclenché chaque fois que l’état de la connexion sur la session donnée change. Nous redirigeons cet appel vers notre propre fonction `OnConnectionStatusChanged`. Nous allons la déclarer et l’implémenter (ainsi que le reste du code de la machine d’état) dans le paragraphe suivant. Notez également que les informations d’identification sont codées en dur dans l’exemple et qu’elles doivent être renseignées sur place ([ID de compte, clé de compte](../../../how-tos/create-an-account.md#retrieve-the-account-information) et [domaine](../../../reference/regions.md)).
...
*Initialisation -> Création de session -> Chargement du modèle (avec la progression)*
...
Nous devons graduer (tick) le client une fois par cycle de simulation. La classe `HolographicApp1Main` fournit un bon raccordement pour les mises à jour image par image. Par conséquent, ajoutez le code suivant au corps de la fonction `HolographicApp1Main::Update` :
...
La dernière chose à faire est d’appeler le rendu du contenu distant. Nous devons effectuer cet appel à la position exacte dans le pipeline de rendu, une fois la cible de rendu effacée. Insérez l’extrait de code suivant dans le verrou `UseHolographicCameraResources` à l’intérieur de la fonction `HolographicAppMain::Render` :
