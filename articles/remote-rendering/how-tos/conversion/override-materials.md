...
ms.locfileid: "80679287"
---
# <a name="override-materials-during-model-conversion"></a>Remplacer des matériaux pendant la conversion d’un modèle

Pendant la conversion, les paramètres de matériau dans le modèle source sont utilisés pour définir les [matériaux PBR](../../overview/features/pbr-materials.md) utilisés par le renderer.
...
Pour illustrer cela de façon simple, imaginons qu’un modèle de boîte possède un seul matériau appelé « Default ». La couleur albedo doit être ajustée pour une utilisation dans ARR.
...
Le fichier `box_materials_override.json` est placé dans le conteneur d’entrée, et `ConversionSettings.json` est ajouté à côté de `box.fbx`. Ceci indique à la conversion où trouver le fichier de remplacement (consultez [Configuration de la conversion de modèle](configure-model-conversion.md)) :
...
Il s’avère utile pour les composants créés avec des algorithmes de photogrammétrie, par exemple.
