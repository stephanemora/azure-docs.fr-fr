...
description: En savoir plus sur l’activation de la redondance de zone dans Azure Container Registry en créant un registre de conteneurs ou une réplication dans une zone de disponibilité Azure. La redondance de zone est une fonctionnalité du niveau de service Premium.
ms.topic: article
ms.date: 12/11/2020
...
Cet article explique comment configurer un registre de conteneurs redondant interzone ou un réplica redondant interzone à l’aide du portail Azure ou d’un modèle Azure Resource Manager. 
...
* Actuellement pris en charge par l’intermédiaire des modèles Azure Resource Manager ou du Portail Azure. La prise en charge par Azure CLI sera activée dans une version ultérieure.
...
1. Sur la carte qui s’affiche, sélectionnez un hexagone vert dans une région qui prend en charge la redondance de zone pour Azure Container Registry, par exemple **USA Ouest 2**. Sélectionnez ensuite **Créer**.
1. Dans la fenêtre **Créer une réplication**, dans **Zones de disponibilité**, sélectionnez **Activé**, puis sélectionnez **Créer**.

## <a name="create-a-zone-redundant-registry---template"></a>Créer un registre redondant interzone (modèle)

### <a name="create-a-resource-group"></a>Créer un groupe de ressources

Si nécessaire, exécutez la commande [az group create](/cli/azure/group) pour créer un groupe de ressources pour le registre dans une région qui [prend en charge les zones de disponibilité](../availability-zones/az-region.md) pour Azure Container Registry, par exemple *eastus*.
...
Vous pouvez utiliser le modèle Resource Manager suivant pour créer un registre géorépliqué et redondant interzone. Par défaut, le modèle active la redondance de zone dans le registre et un réplica régional supplémentaire. 
...
Exécutez la commande [az deployment group create](/cli/azure/deployment?view=azure-cli-latest) suivante pour créer le Registre à l’aide du fichier de modèle précédent. Si indiqué, fournissez :
