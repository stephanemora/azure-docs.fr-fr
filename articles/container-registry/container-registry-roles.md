...
L’accès à Azure Resource Manager est nécessaire pour le portail Azure et la gestion du registre avec [Azure PowerShell](/powershell/azure/). Par exemple, pour obtenir la liste des registres à l’aide de l’applet de commande `Get-AzContainerRegistry`, vous devez disposer de cet ensemble d’autorisations.
...
* Apprenez-en davantage sur l’attribution de rôles Azure à une identité Azure à l’aide du [portail Azure](../role-based-access-control/role-assignments-portal.md), de l’[interface Azure CLI](../role-based-access-control/role-assignments-cli.md), d’[Azure PowerShell](../role-based-access-control/role-assignments-powershell.md) ou d’autres outils Azure.
