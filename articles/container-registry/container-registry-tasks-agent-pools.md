...
- **Mise à l’échelle en fonction des besoins** : augmentez le nombre d’instances dans un pool d’agents pour les tâches nécessitant beaucoup de ressources, ou mettez à l’échelle sur zéro. La facturation est basée sur l’allocation.
...
- Les pools d'agents de tâche sont disponibles en préversion dans les régions suivantes : USA Ouest 2, USA Centre Sud, USA Est 2 et USA Est.
- Pour chaque registre, le quota de processeurs virtuels (Core) total par défaut pour tous les pools d’agents est de 16. Ouvrez une [demande de support ][open-support-ticket] pour une allocation supplémentaire.
...
> [!NOTE]
> Si vos tâches requièrent des ressources supplémentaires provenant de l’Internet public, ajoutez les règles correspondantes. Par exemple, ces règles sont nécessaires si vous exécutez une tâche de génération de docker qui doit extraire les images de base de Docker Hub ou restaure un package NuGet.
...
### <a name="quick-task-run"></a>Exécution rapide des tâches

Mettez en file d’attente une exécution rapide sur le pool d’agents à l’aide de la commande [az acr build][az-acr-build] et transmettez le paramètre `--agent-pool` :
