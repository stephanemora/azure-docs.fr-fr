...
description: Découvrez comment importer une API de la spécification OpenAPI dans Gestion des API Azure, puis tester votre API dans le portail Azure.
...
1. Dans l’écran **Create from OpenAPI specification** (Créer à partir de la spécification OpenAPI), utilisez les valeurs du tableau suivant pour créer votre API.
   ...
   |**Passerelles**|**Managé**|La ou les passerelles API qui exposent l’API. Ce champ est disponible uniquement dans les services des niveaux **Développeur** et **Premium**.<br/>La passerelle **managée** indique la passerelle intégrée au service Gestion des API et hébergée par Microsoft dans Azure. Les autres passerelles sont des [passerelles auto-hébergées](self-hosted-gateway-overview.md) qui ne sont disponibles que dans les niveaux de service Développeur et Premium. Vous pouvez les déployer localement ou dans d’autres clouds.<br/>Si aucune passerelle n’est sélectionnée, l’API n’est pas disponible et vos demandes d’API n’aboutissent pas.|
