...
3. Utilisez ces valeurs pour configurer la configuration d’adresses IP frontales du trafic sortant :
    ...
    | Type IP | Sélectionnez **Adresse IP** ou **Préfixe d’adresse IP**.|
    | Adresse IP publique | Sélectionnez **Créer nouveau**. </br> Dans la page **Ajouter une adresse IP publique**, entrez **myPublicIPOutbound**. </br> Si vous utilisez le préfixe IP, dans **Ajouter un préfixe d’adresse IP publique**, entrez **myPublicIPPrefixOutbound**. Choisissez une **Taille de préfixe** pour le préfixe d’adresse IP publique </br> Sélectionnez **OK**.  |
