...
- En général, le transfert de fragments IP ou l’exécution d’une fragmentation IP de paquets UDP et TCP ne sont pas pris en charge sur les règles d’équilibrage de charge.  Les [règles d’équilibrage de charge des ports HA](load-balancer-ha-ports-overview.md) sont une exception à cette instruction générale et peuvent être utilisées pour transférer des fragments IP existants.
