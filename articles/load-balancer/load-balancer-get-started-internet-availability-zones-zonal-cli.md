---
title: Créer une instance publique de Load Balancer Standard avec un frontend zonal à l’aide d’Azure CLI | Microsoft Docs
description: Découvrez comment créer une instance publique de Load Balancer Standard avec un frontend zonal à l’aide d’Azure CLI
...
Cet article décrit les étapes de création d’une instance publique de [Load Balancer Standard](https://aka.ms/azureloadbalancerstandard) avec un frontend zonal. Le fait de disposer d’un frontend zonal signifie que tout flux entrant ou sortant est servi par une seule zone dans une région. Vous pouvez créer un équilibreur de charge avec un frontend zonal à l’aide d’une adresse IP publique Standard zonale dans sa configuration de frontend. Pour comprendre le fonctionnement des zones de disponibilité avec Standard Load Balancer, consultez [Standard Load Balancer et zones de disponibilité](load-balancer-standard-availability-zones.md). 
