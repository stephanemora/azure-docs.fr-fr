---
title: Résoudre les erreurs de l’interface CLI du serveur flexible Azure Database pour MySQL
description: Cette rubrique fournit des conseils sur la résolution des problèmes courants liés à l’interface CLI Azure lors de l’utilisation du serveur flexible MySQL.
author: mksuni
ms.author: sumuth
ms.service: mysql
ms.topic: how-to
...
ms.locfileid: "123440938"
---
# <a name="troubleshoot-azure-database-for-mysql-flexible-server-cli-errors"></a>Résoudre les erreurs de l’interface CLI du serveur flexible Azure Database pour MySQL
[!INCLUDE[applies-to-mysql-flexible-server](../includes/applies-to-mysql-flexible-server.md)]

Ce document vous aidera à résoudre les problèmes courants liés à l’interface CLI Azure lors de l’utilisation du serveur flexible MySQL.
...
1. Répertoriez les déploiements dans le groupe de ressources pour identifier le déploiement du serveur MySQL 
    ```azurecli
...
2. Obtenez le contenu de la demande du déploiement du serveur MySQL 
    ```azurecli
...
|LocationNotAvailableForResourceType| - Vérifiez la disponibilité du serveur flexible Azure Database pour MySQL dans les [régions Azure](https://azure.microsoft.com/global-infrastructure/services/?products=mysql). <br>- Vérifiez si les types de ressources Azure DB pour MySQL sont enregistrés dans votre abonnement. |
...
|InvalidParameterValue| Une valeur non valide a été spécifiée pour un paramètre. Consultez les [documents de référence de l’interface CLI](/cli/azure/mysql/flexible-server?view=azure-cli-latest&preserve-view=true) pour voir quelles sont les valeurs correctes prises en charge pour les arguments.|
|InvalidLocation| Un emplacement non valide a été spécifié. Vérifier la disponibilité du serveur flexible Azure Database pour MySQL dans les [régions Azure](https://azure.microsoft.com/global-infrastructure/services/?products=mysql) |
|InvalidServerName| Identification d’un nom de serveur non valide. Vérifier le nom du serveur. Exécutez la commande [az mysql flexible-server list](/cli/azure/mysql/flexible-server?view=azure-cli-latest&preserve-view=true#az_mysql_flexible_server_list) pour afficher la liste des serveurs flexibles disponibles. |
