...
### <a name="data-drift"></a>Dérive des données

Les stratégies de migration hors connexion peuvent entraîner une dérive des données. Une dérive des données se produit quand les données sources récemment modifiées deviennent non synchronisées avec les données migrées. Quand cela se produit, une exportation complète ou une exportation différentielle sera nécessaire. Vous pouvez limiter ce problème en arrêtant tout le trafic vers la base de données, puis en effectuant votre exportation. Si l’arrêt de l’ensemble du trafic de modification des données n’est pas possible, vous devrez prendre en compte la dérive.
...
### <a name="performing-the-migration"></a>Réalisation de la migration
...
### <a name="data-migration-checklist"></a>Check-list pour la migration des données
  ...
  - Comprenez bien la complexité de l’environnement et voyez si une approche en ligne est faisable.
...
> [Migration des données avec MySQL Workbench](./data-migration-with-mySQL-workbench.md)