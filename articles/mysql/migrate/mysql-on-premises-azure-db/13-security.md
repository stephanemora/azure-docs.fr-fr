...
Cette intégration étroite permet aux administrateurs et aux applications de tirer parti des fonctionnalités de sécurité renforcées d’[Azure Identity Protection](/azure/active-directory/identity-protection/overview-identity-protection) pour faire apparaître des problèmes liés aux identités.
...
Lors de l’utilisation d’une stratégie de clé gérée par le client, il est essentiel de comprendre les responsabilités liées à la gestion du cycle de vie des clés. Les clés des clients sont stockées dans un [coffre Azure Key Vault](/azure/key-vault/general/basic-concepts), puis leur accès est régi par des stratégies. Il est essentiel de suivre toutes les recommandations relatives à la gestion des clés, la perte de la clé de chiffrement équivalant à la perte de l’accès aux données.

En plus des clés gérées par le client, utilisez des clés au e niveau du service pour [ajouter le double chiffrement](/azure/mysql/concepts-infrastructure-double-encryption). L’implémentation de cette fonctionnalité peut produire des données au repos fortement chiffrées, mais elle pénalise les performances du chiffrement. Des tests doivent être effectués.
...
Lors du déplacement d’une application sur Azure avec la charge de travail MySQL, il est probable que plusieurs réseaux virtuels sont configurés dans un modèle hub-and-spoke, nécessitant la configuration de l’[appairage de réseaux virtuels](/azure/virtual-network/virtual-network-peering-overview).

## <a name="private-link"></a>Liaison privée

Pour limiter l’accès à Azure Database pour MySQL aux ressources Azure internes, activez [Private Link](/azure/mysql/concepts-data-access-security-private-link). Private Link vérifie qu’une adresse IP privée est affectée à l’instance MySQL, et non une adresse IP publique.
...
## <a name="security-checklist"></a>Liste de contrôle de sécurité
  ...
  - Activez la protection avancée contre les menaces.
