...
| Code d'erreur | Sous-code d’erreur | Message d’erreur | Description | Recommandation |
| ---------- | ------------- | ------------- | ----------- | -------------- |
| BadRequest | 40000 | Impossible de modifier PartitionCount pour un Event Hub. | Le niveau De base, Standard ou Premium d’Azure Event Hubs ne prend pas en charge la modification des partitions. | Créez un hub d’événements avec le nombre de partitions souhaité dans votre espace de noms de niveau De base, Standard ou Premium. La montée en charge des partitions est prise en charge pour les [clusters dédiés](event-hubs-dedicated-overview.md). |
