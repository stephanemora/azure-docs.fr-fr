...
## <a name="use-tez-instead-of-map-reduce"></a>Utilisez Tez à la place de MapReduce

[Apache Tez](http://hortonworks.com/hadoop/tez/) est un moteur d’exécution représentant une alternative au moteur MapReduce. Tez est activé par défaut pour les clusters HDInsight basés sur Linux.
...
![Partitionnement Hive][image-hdi-optimize-hive-partitioning_1]
...
* **Évitez le décalage de données** : choisissez votre clé de partitionnement avec soin, pour que toutes les partitions soient de taille égale. Par exemple, le partitionnement sur la colonne *État* risque de fausser la répartition des données. Comme l’État de la Californie a une population presque 30 fois plus nombreuse que celle du Vermont, la taille de la partition est potentiellement asymétrique et la performance peut varier considérablement.
...
Vous pouvez en savoir plus sur le format ORC dans le [manuel du langage Hive](https://cwiki.apache.org/confluence/display/Hive/LanguageManual+ORC).
