...
3. Copiez et collez le code suivant dans votre fichier Hive, puis enregistrez-le.
    ...
    ```
4. Cliquez avec le bouton droit sur l’éditeur de script, puis cliquez sur **HDInsight : Hive interactif** pour envoyer la requête. HDInsight Tools vous permet également d’envoyer un bloc de code au lieu du fichier de script entier à partir du menu contextuel. Peu après, le résultat de la requête s’affiche dans un nouvel onglet.
...
Une requête interactive est beaucoup plus rapide que [l’exécution d’un programme de traitement par lots Apache Hive](#submit-hive-batch-scripts).
...
3. Copiez et collez le code suivant dans votre fichier Hive, puis enregistrez-le.
    ...
    SELECT * FROM hivesampletable;
    ```
4. Cliquez avec le bouton droit sur l’éditeur de script, puis cliquez sur **HDInsight : Hive Batch** pour soumettre une tâche Hive. 
...
L’[envoi de requêtes Apache Hive interactives](#submit-interactive-hive-queries) prend bien moins de temps que l’envoi d’un programme de traitement par lots.
...
[Apache Hive](hdinsight-use-hive.md) est le composant le plus populaire utilisé dans HDInsight. Il existe de nombreuses façons d’exécuter des tâches Hive dans HDInsight. Dans ce didacticiel, vous allez utiliser l’affichage Ambari Hive à partir du portail. Pour d’autres méthodes d’envoi de tâches Hive, consultez [Utilisation d’Apache Hive avec HDInsight](hdinsight-use-hive.md).

1. Pour ouvrir Ambari, à partir de la vignette **Tableaux de bord de cluster**, sélectionnez **Vues Ambari**.  Vous pouvez également accéder à **https://&lt;Nom_cluster&gt;.azurehdinsight.net**, où &lt;Nom_cluster&gt; désigne le cluster que vous avez créé dans la section précédente.
...
3. Sélectionnez **Hive View 2.0** comme illustré dans la capture d’écran suivante :
