...
ms.locfileid: "58199492"
---
# <a name="get-started-with-an-apache-hbase-example-in-hdinsight"></a>Prise en main d’un exemple Apache HBase dans HDInsight

Découvrez comment créer un cluster [Apache HBase](https://hbase.apache.org/) dans HDInsight, créer des tables HBase et les interroger à l’aide d’[Apache Hive](https://hive.apache.org/).  Pour obtenir des informations générales sur HBase, voir [Vue d’ensemble de HDInsight HBase][hdinsight-hbase-overview].
...
1. Cliquez sur l’image suivante pour ouvrir le modèle dans le portail Azure. Le modèle se trouve dans [modèles de démarrage rapide Azure](https://azure.microsoft.com/resources/templates/).
...
4. Récupérez une seule ligne :
    ...
    Pour plus d’informations sur le schéma de la table HBase, consultez [Introduction à la conception de schémas Apache HBase][hbase-schema]. Pour obtenir plus de commandes HBase, consultez le [Guide de référence Apache HBase][hbase-quick-start].
...
Si vous le souhaitez, vous pouvez créer un fichier texte et le télécharger dans votre propre compte de stockage. Pour obtenir des instructions, consultez [Charger des données pour des tâches Apache Hadoop dans HDInsight][hdinsight-upload-data].
...
* [Vue d’ensemble de HDInsight HBase][hdinsight-hbase-overview] : Apache HBase est une base de données NoSQL open source Apache basée sur Apache Hadoop qui fournit un accès aléatoire et une forte cohérence pour de vastes quantités de données non structurées et semi-structurées.
