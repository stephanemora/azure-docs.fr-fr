...
1. Enregistrez vos modifications en cliquant sur le bouton **Save (Enregistrer)** vert en haut de l’écran de configuration.
    ...
    ![Apache Ambari - Enregistrer les configurations](./media/hdinsight-changing-configs-via-ambari/ambari-save-changes1.png)

## <a name="apache-hive-optimization"></a>Optimisation d’Apache Hive
...
1. Dans l’onglet **Configs (Configuration)** de Hive, tapez **execution engine** dans la zone de filtrage.
    ...
    ![Apache Ambari - Rechercher le moteur d’exécution](./media/hdinsight-changing-configs-via-ambari/ambari-search-execution.png)

1. La valeur par défaut de la propriété **Optimization (Optimisation)** est **Tez**.

    ![Optimisation - Moteur Apache Tez](./media/hdinsight-changing-configs-via-ambari/optimization-apache-tez.png)
...
1. Réglez ces deux paramètres sur **33 554 432** octets (32 Mo).
    ...
    ![Apache Ambari - Tailles de regroupement Tez](./media/hdinsight-changing-configs-via-ambari/apache-tez-grouping-size.png)
...
[Apache ORC](https://orc.apache.org/) et [Snappy](https://google.github.io/snappy/) offrent tous les deux des performances élevées. Toutefois, Hive peut avoir trop peu de réducteurs par défaut et ainsi générer des goulots d’étranglement.
...
1. Pour modifier ce paramètre, accédez à l’onglet **Configs (Configurations)** de Hive et recherchez le paramètre **Data per Reducer (Données par réducteur)** sur la page Settings (Paramètres).
    ...
    ![Apache Ambari - Données par réducteur](./media/hdinsight-changing-configs-via-ambari/ambari-data-per-reducer.png)
...
1. Sélectionnez **Edit (Modifier)** pour remplacer la valeur par 128 Mo (134 217 728 octets), puis appuyez sur **Entrée** pour enregistrer.
    ...
    ![Ambari - Données par réducteur après modification](./media/hdinsight-changing-configs-via-ambari/data-per-reducer-edited.png)
...
1. Pour limiter le nombre de travaux à exécuter en parallèle, modifiez la propriété `hive.exec.parallel.thread.number`. La valeur par défaut est 8.
    ...
    ![Apache Ambari - Affichage de l’exécution en parallèle](./media/hdinsight-changing-configs-via-ambari/apache-hive-exec-parallel.png)
...
1. Pour activer l’exécution vectorisée du volet réduit de la requête, réglez le paramètre `hive.vectorized.execution.reduce.enabled` sur true. La valeur par défaut est false.
    ...
    ![Apache - Exécution vectorisée dans Hive](./media/hdinsight-changing-configs-via-ambari/hive-vectorized-execution.png)
...
![Optimiseur basé sur les coûts HDInsight](./media/hdinsight-changing-configs-via-ambari/hdinsight-cbo-config.png)
...
* `hive.compute.query.using.stats`
    ...
    ![Apache Hive - Requête de calcul à l’aide de statistiques](./media/hdinsight-changing-configs-via-ambari/hive-compute-query-using-stats.png)
...
* `hive.stats.fetch.column.stats`
    ...
    ![Apache - Les statistiques de Hive déterminent les statistiques de colonne](./media/hdinsight-changing-configs-via-ambari/hive-stats-fetch-column-stats.png)
...
1. Pour ajouter un paramètre personnalisé :
    ...
    ![Apache Hive - Ajouter une propriété personnalisée](./media/hdinsight-changing-configs-via-ambari/hive-custom-property.png)
...
1. Pour choisir le codec de compression de la sortie, ajoutez la propriété personnalisée `mapred.output.compression.codec` dans le volet Custom hive-site (hive-site personnalisé), comme indiqué à l’étape 3 de la section précédente.
    ...
    ![Apache Hive - Ajouter une propriété personnalisée 2](./media/hdinsight-changing-configs-via-ambari/hive-custom-property2.png)
...
1. Sélectionnez le mode de partition dynamique *strict (strict)* . Dans ce mode, au moins une partition doit être statique. Cela bloque les requêtes sans filtre de partition dans la clause WHERE. Autrement dit, le mode *strict* bloque les requêtes qui analysent toutes les partitions. Accédez à l’onglet **Configs (Configurations)** de Hive, puis réglez `hive.exec.dynamic.partition.mode` sur **strict (strict)** . La valeur par défaut est **nonstrict (non strict)** .
...
![Apache - Exécution automatique de Hive en mode local](./media/hdinsight-changing-configs-via-ambari/hive-exec-mode-local-auto.png)
...
## <a name="apache-pig-optimization"></a>Optimisation d’Apache Pig

Vous pouvez modifier les propriétés [d’Apache Pig](https://pig.apache.org/) dans l’interface utilisateur web d’Ambari pour paramétrer les requêtes Pig. La modification directe des propriétés de Pig dans Ambari modifie les propriétés de Pig dans le fichier `/etc/pig/2.4.2.0-258.0/pig.properties`.
...
1. Sélectionnez **Enregistrer** en haut à droite de la fenêtre pour enregistrer la nouvelle valeur. Certaines propriétés peuvent nécessiter un redémarrage du service.
    ...
    ![Propriétés avancées d’Apache Pig](./media/hdinsight-changing-configs-via-ambari/advanced-pig-properties.png)
...
## <a name="apache-hbase-optimization-with-the-ambari-web-ui"></a>Optimisation d’Apache HBase avec l’interface utilisateur web d’Ambari
...
1. Remplacez la valeur par défaut par 5 000 Mo.
    ...
    ![Apache Ambari - Taille de segment de mémoire HBase](./media/hdinsight-changing-configs-via-ambari/ambari-hbase-heapsize.png)
...
1. Pour modifier ce paramètre, accédez à l’onglet **Settings (Paramètres)** dans l’onglet **Configs (Configurations)** de HBase, puis recherchez **% of RegionServer Allocated to Read Buffers (% du serveur de région alloué aux mémoires tampons de lecture)** .
    ...
    ![Apache HBase - Taille du cache de bloc de mémoire](./media/hdinsight-changing-configs-via-ambari/hbase-block-cache-size.png)
...
![Apache - Nombre de lignes extraites dans HBase](./media/hdinsight-changing-configs-via-ambari/hbase-num-rows-fetched.png)
...
![Apache - Taille maximale du fichier de région dans HBase](./media/hdinsight-changing-configs-via-ambari/hbase-hregion-max-filesize.png)
...
![Apache - Multiplicateur de bloc de région de HBase](./media/hdinsight-changing-configs-via-ambari/hbase-hregion-memstore-block-multiplier.png)
