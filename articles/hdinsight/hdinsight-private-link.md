...
La configuration de `resourceProviderConnection` sur Sortante vous permet également d’accéder à des ressources spécifiques au cluster, telles que des metastores Azure Data Lake Storage Gen2 ou externes, en utilisant des points de terminaison privés. Vous devez configurer les points de terminaison privés et les entrées DNS avant de créer le cluster HDInsight. Nous vous recommandons de créer et de fournir toutes les bases de données SQL externes dont vous avez besoin, telles que les metastores Apache Ranger, Ambari, Oozie et Hive, lors de la création du cluster.

Les points de terminaison privés pour Azure Key Vault ne sont pas pris en charge. Si vous utilisez Azure Key Vault pour le chiffrement CMK au repos, le point de terminaison Azure Key Vault doit être accessible à partir du sous-réseau HDInsight sans point de terminaison privé.
...
:::image type="content" source="media/hdinsight-private-link/before-cluster-creation.png" alt-text="Diagramme de l’architecture HDInsight utilisant une connexion de fournisseur de ressources sortante":::
...
:::image type="content" source="media/hdinsight-private-link/after-cluster-creation.png" alt-text="Diagramme de l’architecture HDInsight utilisant une connexion de fournisseur de ressources sortante":::
...
:::image type="content" source="media/hdinsight-private-link/access-private-clusters.png" alt-text="Diagramme de l’architecture HDInsight utilisant une connexion de fournisseur de ressources sortante":::

## <a name="arm-template-properties"></a>Propriétés du modèle Resource Manager
