---
title: Utiliser l’interface utilisateur Apache Tez avec HDInsight basé sur Windows - Azure
description: Apprenez à utiliser l’interface utilisateur Apache Tez pour déboguer les travaux Tez dans HDInsight sous Windows.
services: hdinsight
...
* Un cluster HDInsight Windows Pour plus d’informations sur la création d’un cluster, consultez [Prise en main de HDInsight sur Windows](hdinsight-hadoop-tutorial-get-started-windows.md).
  ...
  > [!IMPORTANT]  
  > L’interface utilisateur Apache Tez est disponible uniquement pour les clusters HDInsight sur Windows créés après le 8 février 2016.
...
Tez crée un graphe orienté acyclique (Directed Acyclic Graph - DAG) qui décrit l’ordre d’exécution des actions requises par le travail. Les actions individuelles sont appelées des vertex et exécutent une partie du travail global. L’exécution réelle du travail décrit par un vertex est appelée une tâche, et peut être répartie sur plusieurs nœuds du cluster.

### <a name="understanding-the-tez-ui"></a>Présentation de l’interface utilisateur Tez
L’interface utilisateur Tez est une page web qui fournit des informations sur les processus qui utilisent Tez. Elle peut fournir des informations utiles dans les scénarios suivants :
...
L’interface utilisateur Tez contient des données si un travail qui utilise le moteur Tez est en cours d’exécution ou s’il a déjà été exécuté. Les requêtes Hive simples peuvent généralement être résolues sans utiliser Tez. Les requêtes plus complexes qui effectuent des opérations de filtrage, de regroupement, de tri, de jointure, etc. doivent utiliser Tez.

Suivez les étapes ci-après pour exécuter une requête Hive utilisant Tez.
...
1. Dans le [portail Azure](https://portal.azure.com), sélectionnez votre cluster HDInsight. En haut du panneau HDInsight, sélectionnez l’icône **Bureau à distance**. Ce lien affiche le panneau du Bureau à distance
...
5. Dans Internet Explorer, accédez à http://headnodehost:8188/tezui/#/. Cela permet d’afficher l’interface utilisateur Tez
...
6. Sélectionnez le **Nom du Dag** pour l’entrée DAG la plus récente. Ce lien affiche les informations sur le DAG, et offre la possibilité de télécharger des fichiers JSON au format .zip contenant des informations sur le DAG.
...
9. Cliquez sur un vertex pour charger les **détails du vertex**. Cliquez sur le vertex **Map 1** pour afficher les détails de cet élément. Sélectionnez **Confirmer** pour confirmer la navigation.
...
11. Sélectionnez **Tâches**, puis sélectionnez l’élément nommé **00_000000**. Ce lien affiche les **détails de la tâche**. À partir de cet écran, vous pouvez consulter les **compteurs de tâche** et les **tentatives de tâche**.
