...
Le coffre de clés prend en charge jusqu’à 1 024 entrées de stratégie d’accès, chaque entrée accordant un ensemble distinct d’autorisations à un « principal » :   Par exemple, voici comment l’application console décrite dans le [guide de démarrage rapide de la bibliothèque de client Azure Key Vault pour .NET](quick-create-net.md) accède au coffre de clés.

Pour plus d’informations sur le contrôle d’accès Key Vault, consultez [Sécurité d’Azure Key Vault : Gestion de l’identité et de l’accès](overview-security.md#identity-and-access-management). Pour plus d’informations sur le contrôle d’accès aux [clés, secrets et certificats](about-keys-secrets-and-certificates.md), consultez : 

- [Contrôle d’accès aux clés](about-keys-secrets-and-certificates.md#key-access-control)
- [Contrôle d’accès aux secrets](about-keys-secrets-and-certificates.md#secret-access-control)
- [Contrôle d’accès aux certificats](about-keys-secrets-and-certificates.md#certificate-access-control)
...
La seconde consiste à créer un principal de service dans une fenêtre de terminal. Avec Azure CLI, utilisez la commande [az ad sp create-for-rbac](/cli/azure/ad/sp?view=azure-cli-latest#az-ad-sp-create-for-rbac).
...
### <a name="addition-prerequisites"></a>Prérequis supplémentaires
