...
description: Découvrez comment utiliser Azure PowerShell pour mettre en application la fonctionnalité de suppression réversible d’Azure Key Vault qui permet de récupérer des coffres de clés et des objets de coffre de clés.
services: key-vault
...
La suppression réversible est activée automatiquement par défaut pour tous les nouveaux coffres de clés. Le 31 décembre 2020, il ne sera plus possible de désactiver la suppression réversible sur un coffre de clés. 
...
- Pour obtenir une présentation de la fonctionnalité de suppression réversible, consultez [Présentation de la suppression réversible d’Azure Key Vault](soft-delete-overview.md).
- Pour une vue d’ensemble de l’utilisation d’Azure Key Vault, consultez [Présentation d’Azure Key Vault](overview.md).
