...
Vous avez aussi besoin d’une inscription d’application cliente. La plupart des applications SMART on FHIR sont des applications JavaScript monopages. Vous devez donc suivre les instructions relatives à la configuration d’une [application cliente publique dans Azure ad](register-public-azure-ad-client-app.md).

Une fois ces étapes terminées, vous devez disposer des éléments suivants :

- Un serveur FHIR avec l’audience définie sur `https://MYFHIRAPI.azurehealthcareapis.com` , où `MYFHIRAPI` est le nom de votre API Azure pour l’instance FHIR.
...
Pour tester l’API Azure pour FHIR et le proxy SMART on FHIR, la base de données doit contenir au moins un patient. Si vous n’avez pas encore interagi avec l’API et que vous n’avez pas de données dans la base de données, consultez [accéder au service FHIR à l’aide de poster](./../use-postman.md) pour charger un patient. Notez l’ID d’un patient spécifique.
