...
lorsque vous créez votre compte Azure API pour FHIR sur Portail Azure, vous remarquerez l’option de configuration **chiffrement des données** sous la **Paramètres de la base de données** sous l’onglet **Paramètres supplémentaires** . Par défaut, l’option de clé gérée par le service est sélectionnée.
