---
title: Déplacement de données dans l’API Azure pour FHIR vers Azure Synapse Analytics
...
* **Charger les données exportées dans Synapse à l’aide de T-SQL :** Utilisez `$export` l’opération pour déplacer des ressources FHIR dans un **stockage d’objets BLOB Azure Data Lake Gen 2 (ADL Gen 2)** au `NDJSON` format. chargez les données à partir du stockage dans des **pools de SQL sans serveur ou dédiés** dans Synapse à l’aide de T-SQL. Convertissez ces étapes en pipeline de déplacement de données robuste à l’aide des [pipelines Synapse](../../synapse-analytics/get-started-pipelines.md).
* **utilisez les outils de FHIR Analytics Pipelines référentiel OSS :** Le [pipeline FHIR Analytics](https://github.com/microsoft/FHIR-Analytics-Pipelines) référentiel contient des outils capables de créer un **pipeline d’Azure Data Factory (ADF)** pour déplacer des données FHIR dans un **dossier CDM (Common Data Model)** et du dossier CDM vers synapse.
...
pour déplacer vos données vers synapse, vous devez créer un service lié qui connecte votre compte stockage Azure à synapse. Vous trouverez plus d’instructions pas à pas [ici](../../synapse-analytics/data-integration/data-integration-sql-pool.md#create-linked-services).
...
maintenant que vous disposez d’un service lié entre votre stockage ADL Gen 2 et Synapse, vous êtes prêt à utiliser les pools de Synapse SQL pour charger et analyser vos données FHIR.

### <a name="decide-between-serverless-and-dedicated-sql-pool"></a>choisir entre un pool sans serveur et un pool de SQL dédié

Azure Synapse Analytics offre deux pools de SQL différents, un pool de SQL sans serveur et un pool de SQL dédié. le pool de SQL sans serveur offre la flexibilité de l’interrogation des données directement dans le stockage d’objets blob à l’aide du point de terminaison de SQL sans serveur sans l’approvisionnement des ressources. le pool de SQL dédié offre la puissance de traitement pour des performances élevées et l’accès concurrentiel, et est recommandé pour les capacités d’entreposage de données à l’échelle de l’entreprise. pour plus d’informations sur les deux pools de SQL, consultez la [page de documentation de Synapse](../../synapse-analytics/sql/overview-architecture.md) sur SQL architecture.

#### <a name="using-serverless-sql-pool"></a>utilisation d’un pool de SQL sans serveur
...
dans la requête ci-dessus, la `OPENROWSET` fonction accède aux fichiers dans stockage Azure et `OPENJSON` analyse le texte json et retourne les propriétés d’entrée json sous forme de lignes et de colonnes. chaque fois que cette requête est exécutée, le pool de SQL sans serveur lit le fichier à partir du stockage d’objets blob, analyse le JSON et extrait les champs.
...
#### <a name="using-dedicated-sql-pool"></a>utilisation du pool de SQL dédié

le pool de SQL dédié prend en charge les tables gérées et un cache hiérarchique pour les performances en mémoire. vous pouvez importer des Big Data avec des requêtes T-SQL simples, puis utiliser la puissance du moteur de requête distribuée pour exécuter des analyses hautes performances.

le moyen le plus simple et le plus rapide de charger des données à partir de votre stockage dans un pool de SQL dédié consiste à utiliser la **`COPY`** commande dans T-SQL, qui peut lire les fichiers CSV, Parquet et ORC. Comme dans l’exemple de requête ci-dessous, utilisez la `COPY` commande pour charger les `NDJSON` lignes dans une structure tabulaire. 
...
## <a name="use-fhir-analytics-pipelines-oss-tools"></a>utiliser FHIR Analytics Pipelines les outils OSS
...
5. Télécharger le contenu du dossier de configuration de la table dans le conteneur de configuration.
...
ajoutez l’identité gérée ADF en tant qu’utilisateur SQL dans SQL base de données. voici un exemple SQL script permettant de créer un utilisateur et un rôle assign :
...
dans cet article, vous avez appris deux façons de déplacer vos données FHIR vers la synapse : (1) à l’aide `$export` de pour déplacer des données dans le stockage d’objets blob ADL Gen 2, puis de charger les données dans des pools de synapse SQL, et (2) à l’aide du pipeline ADF pour déplacer des données FHIR vers le dossier CDM, puis à Synapse.
