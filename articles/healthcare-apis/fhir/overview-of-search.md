...
ms.locfileid: "108322554"
---
# <a name="overview-of-fhir-search"></a>Vue d’ensemble de la recherche FHIR

La spécification FHIR définit les notions de base de la recherche de ressources FHIR. Cet article vous guide tout au long des aspects clés de la recherche de ressources dans FHIR. Pour plus d’informations sur la recherche de ressources FHIR, reportez-vous à [recherche](https://www.hl7.org/fhir/search.html) dans la spécification HL7 FHIR.
...
Si la demande de recherche est réussie, vous recevrez une réponse de Bundle FHIR avec le type `searchset` . Si la recherche échoue, vous trouverez ces informations dans la `OperationOutcome` pour vous aider à comprendre la raison de l’échec de la recherche.

Dans les sections suivantes, nous allons aborder les différents aspects impliqués dans la recherche. Une fois que vous avez consulté ces détails, reportez-vous à notre **page d’exemples** qui contient des exemples de recherches que vous pouvez effectuer dans l’API Azure pour FHIR.

## <a name="search-parameters"></a>Paramètres de recherche

Lorsque vous effectuez une recherche, pensez à effectuer une recherche en fonction de divers attributs de la ressource.  Ces attributs sont appelés paramètres de recherche. Chaque ressource a un ensemble de paramètres de recherche définis. Le paramètre de recherche doit être défini et indexé dans la base de données pour que vous parvenez à effectuer une recherche sur celui-ci.

Chaque paramètre de recherche a un type de données défini. L’API Azure pour FHIR prend en charge tous les [types de données](https://www.hl7.org/fhir/search.html#ptypes) , à l’exception du type **Special**:


| **Type de paramètre de recherche**  | **Prise en charge - PaaS** | **Prise en charge - OSS (SQL)** | **Prise en charge - OSS (Cosmos DB)** |
| -------------------------  | -------------------- | ------------------------- | ------------------------------- |
|  nombre                    | Oui                  | Oui                       | Oui                             |
|  Date                      | Oui                  | Oui                       | Oui                             |
|  string                    | Oui                  | Oui                       | Oui                             |
...
| _profile                    | Oui                  | Oui                       | Oui                             | **Remarque**: Si vous avez créé votre base de données R4 avant le 20 février 2021, vous devez exécuter une tâche de réindexation pour activer **_profile**.                                                      |
| _text                       | Non                   | Non                        | Non                              |                                |
| _content                    | Non                   | Non                        | Non                              |                                |
| _has                        | Partiel                   | Partiel                        | Oui                              |                                |
| _query                      | Non                   | Non                        | Non                              |                                |
| _filter                     | Non                   | Non                        | Non                              |                                |
| _list                       | Non                   | Non                        | Non                              |                                |

### <a name="resource-specific-parameters"></a>Paramètres spécifiques aux ressources

Avec l’API Azure pour FHIR, nous prenons en charge presque tous les paramètres de recherche spécifiques aux ressources définis par la spécification FHIR. Les seuls paramètres de recherche que nous ne prenons pas en charge sont disponibles dans les liens ci-dessous :
...
> [!NOTE]
> L’API Azure pour FHIR ne crée pas ou n’indexe pas automatiquement les paramètres de recherche de prise en charge qui ne sont pas définis par la spécification FHIR. Toutefois, nous fournissons un support pour vous permettre de définir vos propres paramètres de recherche.
...
|  : ci-dessus (Uri) | No                   | Non                        | No                              | 
|  : in (jeton)  | No                   | Non                        | No                              | 
|  : ci-dessous (jeton) | No                 | Non                        | No                              | 
|  : ci-dessus (jeton) | No                 | Non                        | No                              | 
|  : not-in (jeton) | No                | Non                        | No                              | 
...
| _include                      | Oui                  | Oui                       | Oui                             | Les éléments inclus sont limités à 100. _include sur PaaS et OSS sur Cosmos DB n’inclut pas : la prise en charge des itérations [(#1313)](https://github.com/microsoft/fhir-server/issues/1313).                               |
| _revinclude                   | Oui                  | Oui                       | Oui                             |  Les éléments inclus sont limités à 100. _revinclude sur PaaS et OSS sur Cosmos DB n’inclut pas : la prise en charge des itérations [(#1313)](https://github.com/microsoft/fhir-server/issues/1313).  Problème [n° 1319](https://github.com/microsoft/fhir-server/issues/1319)                            |
| _summary                      | Oui             | Oui                   | Oui                        |                               |
| _total                        | Partiel              | Partiel                   | Partiel                         | _total = None et _total = exact                               |
| _sort                         | Partiel              | Partiel                   | Partiel                         | sort = _lastUpdated est pris en charge                               |
...
De même, vous pouvez effectuer une recherche chaînée par inversion. Cela vous permet d’accéder à des ressources où vous spécifiez des critères sur d’autres ressources qui y font référence. Pour obtenir plus d’exemples de chaînage et de chaînage inversé, reportez-vous à la page d' [exemples de recherche FHIR](search-samples.md) . 

**Remarque**: dans l’API Azure pour FHIR et la source open avec Cosmos dB, il existe une limitation où chaque sous-requête requise pour les recherches chaînées et enchaînées renverra uniquement 100 éléments. Si plus de 100 éléments sont trouvés, le message d’erreur suivant s’affiche :

« Les sous-requêtes dans une expression chaînée ne peuvent pas retourner plus de 100 résultats, veuillez utiliser un critère plus sélectif. » 

Pour obtenir une requête réussie, vous devez être plus précis dans ce que vous recherchez.
...
Maintenant que vous avez appris les bases de la recherche, consultez la page Rechercher des exemples pour plus d’informations sur la recherche à l’aide de différents paramètres de recherche, modificateurs et autres outils de recherche FHIR.
