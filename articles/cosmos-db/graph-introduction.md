...
* **Stockage et débit extensibles de façon élastique**
  ...
  Les graphiques dans le monde réel doivent pouvoir augmenter leur échelle au-delà de la capacité d’un serveur unique. Azure Cosmos DB prend en charge les bases de données de graphes horizontalement évolutives dont la taille peut être pratiquement illimitée en termes de débit et de provisionnement. Au fur et à mesure que l’échelle de la base de données des graphes augmente, les données sont automatiquement réparties à l’aide du [partitionnement graphique](https://docs.microsoft.com/azure/cosmos-db/graph-partitioning).

* **Réplication multirégion**

  Azure Cosmos DB peut répliquer automatiquement vos données de graphes dans n’importe quelle région Azure. La réplication simplifie le développement d’applications qui requièrent un accès global aux données. En plus de minimiser la latence de lecture, Azure Cosmos DB offre un mécanisme de basculement régional qui peut assurer la continuité de votre application dans les rares cas d’une interruption de service dans une région. 

* **Requêtes et traversées rapides avec la norme de requête graphique la plus largement adoptée**

  Stockez des sommets et des bords hétérogènes, et interrogez ces documents via une syntaxe Gremlin familière. Gremlin est un langage de requête fonctionnel impératif dont l’interface riche permet d’implémenter des algorithmes graphiques courants. 
...
* **Base de données de graphes complètement managée**
  ...
  En tant que service Microsoft Azure complètement managé, il est inutile de gérer les machines virtuelles, de mettre à jour les logiciels d’exécution, de gérer le partitionnement et la réplication et de vous préoccuper des mises à niveau complexes de la couche Données. Chaque graphique est automatiquement sauvegardé et protégé contre les défaillances régionales. Ces garanties permettent aux développeurs de se concentrer sur la création de valeur applicative plutôt que sur l’exploitation et la gestion de leurs bases de données. 
...
Les bases de données de graphes sont souvent incluses dans la catégorie de base de données NoSQL, ou non relationnelle, puisqu’il n’y a aucune dépendance sur un schéma ou un modèle de données contraint. Cette absence de schéma permet de modéliser et de stocker naturellement et efficacement les structures connectées. 
