...
Alors que le kit de développement logiciel (SDK) v2 utilisait `DocumentClientException` pour signaler des erreurs pendant des opérations, le kit de développement logiciel (SDK) v3 utilise `CosmosClientException`, qui expose les informations relatives aux réponses `StatusCode`, `Diagnostics` et autres. Toutes les informations complètes sont sérialisées lors de l’utilisation de la méthode `ToString()` :
...
Alors que le kit de développement logiciel (SDK) v2 avait des diagnostics uniquement directs disponibles via la propriété `ResponseDiagnosticsString`, le kit de développement logiciel (SDK) v3 utilise les `Diagnostics` disponibles dans toutes les réponses et exceptions, qui sont plus riches et non limitées au mode direct. Elles incluent non seulement le temps passé sur le kit de développement logiciel (SDK) pour l’opération, mais également les régions que l’opération a contactées :
...
Certains paramètres dans `ConnectionPolicy` ont été renommés ou remplacés :
...
Alors que le kit de développement logiciel (SDK) v2 exposait l’horodateur (timestamp) d’un document via la propriété `Timestamp`, parce que `Document` n’est plus disponible, les utilisateurs peuvent mapper la [propriété système](account-databases-containers-items.md#properties-of-an-item) `_ts` à une propriété dans leur modèle.

### <a name="openasync"></a>OpenAsync

Pour les cas d’utilisation où `OpenAsync()` était utilisé pour le préchauffage du client SDK v2, `CreateAndInitializeAsync` peut être utilisé pour [créer et préchauffer](https://devblogs.microsoft.com/cosmosdb/improve-net-sdk-initialization/) un client SDK v3.
