...
Ce guide de migration fait partie de la série sur la migration de bases de données de MongoDB vers l’API Azure CosmosDB pour MongoDB. Les principales étapes de migration sont la [prémigration](pre-migration-steps.md), la migration proprement dite et la [postmigration](post-migration-optimization.md), comme indiqué ci-dessous.

:::image type="content" source="./media/pre-migration-steps/overall-migration-steps.png" alt-text="Schéma des étapes de migration":::
...
- [Suivez les étapes de prémigration](pre-migration-steps.md) comme l’estimation du débit et le choix d’une clé de partition.
...
:::image type="content" source="./media/migrate-databricks/databricks-cluster-creation.png" alt-text="Illustration de la création d’un cluster dans Databricks.":::
...
:::image type="content" source="./media/migrate-databricks/databricks-cluster-dependencies.png" alt-text="Illustration de l’ajout de dépendances de cluster Databricks.":::
...
- **Nombre de Workers et de cœurs dans le cluster Spark** : plus le nombre de Workers est élevé, plus vous disposerez de nœuds de calcul pour l’exécution des tâches.
...
Après avoir migré les données, vous pouvez vous connecter à Azure Cosmos DB et gérer les données. Vous pouvez également effectuer d’autres étapes post-migration comme l’optimisation de la stratégie d’indexation, la mise à jour du niveau de cohérence par défaut ou la configuration de la distribution mondiale pour votre compte Azure Cosmos DB. Pour plus d’informations, consultez l’article [Optimisation de la post-migration](post-migration-optimization.md).
