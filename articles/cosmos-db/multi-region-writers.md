---
title: Multimaître à une échelle mondiale avec Azure Cosmos DB | Microsoft Docs
description: ''
services: cosmos-db
author: rimman
manager: kfile
...
ms.locfileid: "39266835"
---
# <a name="multi-master-at-global-scale-with-azure-cosmos-db"></a>Multimaître à une échelle mondiale avec Azure Cosmos DB 
...
3. Ensuite, sélectionnez **Inscrivez-vous pour une préversion aujourd’hui** sous le champ Multimaître (préversion).  
...
* **Amélioration de la reprise d’activité après sinistre, de la disponibilité d’écriture et du basculement** : Le multimaître peut servir à accroître la haute disponibilité d’une base de données critique. Par exemple, une base de données multimaître peut répliquer des données d’une région vers une région de basculement quand la région principale devient indisponible suite à une panne ou un sinistre régional. Cette région de basculement servira de région principale entièrement fonctionnelle pour la prise en charge de l’application. L’architecture multimaître augmente la « capacité de survie » en cas de catastrophe naturelle, de panne de courant ou de sabotage, car les régions restantes peuvent utiliser une architecture multimaître située dans une autre zone géographique, avec une disponibilité en écriture garantie supérieure à 99,999 %. 
