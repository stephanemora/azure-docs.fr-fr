...
Vous pouvez spécifier le nombre d’unités de requête par seconde (RU par seconde) à réserver pour un conteneur ou un ensemble de conteneurs. Azure Cosmos DB allouera des partitions physiques pour héberger vos conteneurs en fonction du débit provisionné et répartira/rééquilibrera les données sur les partitions au fur et à mesure de leur croissance.
...
![Provisionnement d’unités de requête pour des conteneurs individuels et des ensembles de conteneurs][6]
...
* **Modèles de requête**. La complexité d’une requête a un impact sur le nombre d’unités de requête consommées pour une opération. Le nombre de résultats de requêtes, le nombre de prédicats, la nature des prédicats, les projections, le nombre de fonctions définies par l’utilisateur et la taille des données sources ont tous une influence sur le coût des opérations de requête.
...
Si vous avez plusieurs clients qui opèrent en même temps au-delà du taux de requêtes, le comportement par défaut peut ne pas suffire, et le client générera une `DocumentClientException` avec le code d’état 429 dans l’application. Dans des cas semblables, vous pourriez gérer le comportement et la logique de nouvelles tentatives dans les routines de gestion d’erreurs de votre application, ou accroître le débit provisionné pour le conteneur (ou l’ensemble de conteneurs).
