...
* [Approvisionnez un compte d’API Cassandra Azure Cosmos DB](manage-data-dotnet.md#create-a-database-account).

* [Passez en revue les principes fondamentaux de la connexion à l’API Cassandra Azure Cosmos DB](connect-spark-configuration.md).
...
:::image type="content" source="./media/migrate-data-databricks/databricks-runtime.png" alt-text="Capture d’écran montrant la détection de la version du runtime Databricks.":::
...
:::image type="content" source="./media/migrate-data-databricks/databricks-search-packages.png" alt-text="Capture d’écran montrant la recherche de packages Maven dans Databricks.":::
...
> [!NOTE]
> Les valeurs `spark.cassandra.output.batch.size.rows` et `spark.cassandra.output.concurrent.writes` ainsi que le nombre de Workers dans votre cluster Spark sont des configurations importantes à régler afin d’éviter la [limitation du débit](/samples/azure-samples/azure-cosmos-cassandra-extensions-java-sample-v4/azure-cosmos-cassandra-extensions-java-sample-v4/). La limitation du débit se produit lorsque les demandes adressées à Azure Cosmos DB dépassent le débit approvisionné ou les [unités de requête](../request-units.md) (RU). Vous devrez peut-être ajuster ces paramètres en fonction du nombre d’Exécuteurs dans le cluster Spark, voire de la taille (et par conséquent du coût en RU) de chaque enregistrement écrit dans les tables cibles.
...
* [Mise à l’échelle élastique dans l'API Cassandra Azure Cosmos DB](scale-account-throughput.md)
