...
Azure Cosmos DB est le service de base de données multi-modèle de Microsoft distribué à l’échelle mondiale. Vous pouvez communiquer avec l’API Cassandra Azure Cosmos DB par le biais des [pilotes](https://cassandra.apache.org/doc/latest/cassandra/getting_started/drivers.html?highlight=driver) clients Cassandra open source et conformes au [protocole de transmission](https://github.com/apache/cassandra/blob/trunk/doc/native_protocol_v4.spec) CQL Binary Protocol v4. 
...
| CREATE INDEX | Oui (sans [spécifier de nom d’index](secondary-indexing.md) et sans prise en charge des index sur les clés de clustering ou la collection FROZEN complète) |
| CREATE FUNCTION | Non |
...
> [!NOTE] 
>  Consistency fonctionne différemment dans Azure Cosmos DB. Cliquez [ici](apache-cassandra-consistency-mapping.md) pour plus d’informations.  
...
L’API Cassandra Azure Cosmos DB offre un choix de cohérence pour les opérations de lecture.  Le mappage de cohérence est détaillé [ici](apache-cassandra-consistency-mapping.md#mapping-consistency-levels).
