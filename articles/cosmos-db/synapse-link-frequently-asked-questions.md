...
Dans la préversion publique, Synapse Link est pris en charge pour l’API Azure Cosmos DB SQL (Core) et l’API Azure Cosmos DB pour MongoDB. La prise en charge de l’API Cassandra est actuellement en préversion contrôlée. Pour demander l’accès à cette préversion limitée, contactez l’équipe [Azure Cosmos DB](mailto:cosmosdbsynapselink@microsoft.com).
...
|Synapse SQL serverless    | Lire, afficher |
