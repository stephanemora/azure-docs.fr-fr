...
* L’émulateur Azure Cosmos prend en charge un compte Azure Cosmos en mode [débit approvisionné](set-throughput.md). Actuellement, il ne prend pas en charge un compte Azure Cosmos en mode [serverless](serverless.md).
...
Comme Azure Cosmos DB, l’émulateur Azure Cosmos prend en charge uniquement la communication sécurisée via TLS.
...
## <a name="export-the-tlsssl-certificate"></a>Exporter le certificat TLS/SSL
...
:::image type="content" source="./media/local-emulator/database-local-emulator-ssl_certificate.png" alt-text="Certificat TLS/SSL de l’émulateur local Azure Cosmos DB":::
...
Lors de la connexion à l’émulateur à partir des SDK Python et Node.js, la vérification TLS est désactivée.

## <a name="command-line-tool-reference"></a><a id="command-line"></a>Référence sur l’outil en ligne de commande
...
## <a name="change-the-number-of-containers"></a><a id="set-partitioncount"></a>Changer le nombre de conteneurs
...
Utilisez maintenant le point de terminaison et la clé principale de la réponse sur votre client et importez le certificat TLS/SSL sur votre hôte. Pour importer le certificat TLS/SSL, procédez comme suit à partir d’une invite de commandes administrateur :
...
## <a name="running-on-mac-or-linux"></a>Exécution sur Mac ou Linux<a id="mac"></a>

Actuellement, l’émulateur Cosmos peut uniquement être exécuté sur Windows. Les utilisateurs sous Mac ou Linux peuvent exécuter l’émulateur sur une machine virtuelle Windows hébergée dans un hyperviseur tel que Parallels ou VirtualBox. Voici les étapes pour activer cela.
...
Enfin, nous devons résoudre le processus d’approbation de certificat entre l’application en cours d’exécution dans l’environnement Linux ou Mac et l’émulateur. Deux solutions s’offrent à nous :

1. Désactiver la validation SSL dans l’application :
...
> [!NOTE]
> La désactivation de la validation SSL est uniquement recommandée à des fins de développement et ne doit pas être effectuée lors de l’exécution dans un environnement de production.

2. Importer le certificat de l’autorité de certification de l’émulateur dans l’environnement Linux ou Mac :

### <a name="linux"></a>Linux

Si vous travaillez sur Linux, .NET passe le relais à OpenSSL pour effectuer la validation :

1. [Exportez le certificat au format PFX](./local-emulator-export-ssl-certificates.md#how-to-export-the-azure-cosmos-db-tlsssl-certificate) (PFX est disponible quand vous choisissez d’exporter la clé privée). 
...
Dans ce didacticiel, vous avez appris à utiliser l’émulateur local pour un développement local gratuit. Vous pouvez maintenant passer à l’étape suivante du tutoriel et découvrir comment exporter les certificats TLS/SSL de l’émulateur.
