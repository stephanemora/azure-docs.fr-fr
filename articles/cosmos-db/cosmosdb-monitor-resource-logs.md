...
* **MongoRequests** : sélectionnez cette option pour consigner les demandes initiées par l’utilisateur à partir du serveur front-end afin de traiter les demandes envoyées à l’API de Azure Cosmos DB pour MongoDB. Ce type de journal n’est pas disponible pour les autres comptes d’API. Les demandes de MongoDB apparaîtront dans MongoRequests, ainsi que dans DataPlaneRequests. Les propriétés importantes à noter sont : `Requestcharge`, `opCode`.
...
1. Quel est le débit maximal fourni par une partition ?
