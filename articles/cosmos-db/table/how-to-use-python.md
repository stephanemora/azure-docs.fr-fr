...
Pendant que vous étudiez les scénarios de cet exemple, vous pouvez vous référer à la [Référence du kit de développement logiciel (SDK) Azure Cosmos DB pour l’API Python](/python/api/overview/azure/cosmosdb).
...
* [Compte Stockage Azure](../../storage/common/storage-account-create.md) ou [Compte Azure Cosmos DB](https://azure.microsoft.com/try/cosmosdb/).
...
## <a name="query-for-an-entity-without-partition-and-row-keys"></a>Rechercher une entité sans clés de partition et de ligne

Vous pouvez également rechercher des entités dans une table sans utiliser les clés de partition et de ligne. Utilisez la méthode `table_service.query_entities` sans les paramètres « filter » et « select » comme illustré dans l’exemple suivant :
...
* [FAQ - Développer avec l’API Table](table-api-faq.yml)
* [Référence du kit de développement logiciel (SDK) Azure Cosmos DB pour l’API Python](/python/api/overview/azure/cosmosdb)
