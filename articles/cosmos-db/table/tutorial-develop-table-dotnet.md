...
Ce tutoriel décrit un exemple qui vous montre comment utiliser la [bibliothèque Microsoft Azure Cosmos DB Table pour .NET](https://www.nuget.org/packages/Microsoft.Azure.Cosmos.Table) dans des scénarios basés sur l’API Table d’Azure Cosmos DB et le Stockage Table Azure. Ces scénarios sont explorés à l’aide d’exemples en C# qui montrent comment créer des tables, ajouter ou mettre à jour des données, interroger des données et supprimer des tables.

Bien que cette procédure pas à pas aborde les spécificités de l’implémentation de Cosmos DB, vous pouvez créer une ressource Stockage Table et utiliser la même API et le même package NuGet pour accéder à la ressource. Seule la création de ressources est différente. Quel que soit le type de ressource que vous choisissez, vous devez utiliser la chaîne de connexion spécifique au service Azure que vous avez créé.
...
>[Migrer des données vers l’API Table Azure Cosmos DB](table-import.md)
