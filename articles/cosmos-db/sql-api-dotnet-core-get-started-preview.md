...
Vous pouvez créer une base de données à l’aide de la fonction [**CreateDatabaseIfNotExistsAsync**](https://aka.ms/CosmosDotnetAPIDocs) ou [**CreateDatabaseAsync**](https://aka.ms/CosmosDotnetAPIDocs) de la classe **CosmosDatabases**. Une base de données est le conteneur logique d’éléments partitionnés entre les conteneurs.
...
1.  Copiez et collez les classes **Family**, **Parent**, **Child**, **Pet** et **Address** dans **Family.cs**. Notez que votre espace de noms varie en fonction du nom de votre projet.
...
1.  Revenez à **Program.cs** et ajoutez la méthode **AddItemsToContainer** sous votre méthode **CreateContainer**.
    Le code procède à une vérification pour s’assurer qu’un élément doté du même ID n’existe pas déjà avant qu’il le crée. Nous allons insérer ensuite deux éléments, un pour la famille Andersen, l’autre pour la famille Wakefield.
