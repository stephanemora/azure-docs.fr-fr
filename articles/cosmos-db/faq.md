...
Le service Azure DocumentDB fait désormais partie du service Azure Cosmos DB et se présente sous la forme de l’API SQL. Les applications basées sur Azure DocumentDB s’exécutent exactement de la même façon avec l’API SQL Azure Cosmos DB. Par ailleurs, Azure Cosmos DB prend en charge l’API Gremlin, l’API Table, l’API MongoDB et l’API Cassandra (préversion).
...
Les modèles de données clé/valeur (table), en colonnes, document et graphe sont tous pris en charge de manière native en raison de la conception ARS (atomes, enregistrements et séquences) sur laquelle repose Azure Cosmos DB. Les atomes, enregistrements et séquences peuvent facilement être mappés et projetés vers différents modèles de données. Les API pour certains modèles sont déjà disponibles (API SQL, MongoDB, Table et Gremlin), tandis que d’autres API propres à des modèles de données supplémentaires seront disponibles plus tard.
...
Azure Cosmos DB est disponible dans le portail Azure. Tout d’abord, souscrivez un abonnement Azure. Une fois inscrit, vous pouvez ajouter un compte d’API SQL, d’API Gremlin, d’API Table, d’API MongoDB ou d’API Cassandra à votre abonnement Azure.
...
### <a name="how-do-i-create-fixed-collection-with-partition-key"></a>Comment créer une collection fixe avec une clé de partition
...
* Pour les comptes d’API SQL et MongoDB, les conteneurs sont mappés vers des collections. 
...
### <a name="i-have-setup-my-container-to-use-lazy-indexing-i-see-that-my-queries-do-not-return-expected-results"></a>J’ai configuré mon conteneur de façon à utiliser une indexation différée, et je constate que mes requêtes ne retournent pas les résultats attendus. 
Comme expliqué dans la section d’indexation, l’indexation différée peut entraîner ce comportement. Vous devez toujours utiliser l’indexation cohérente pour toutes les applications. 
...
Il s’agit d’une limitation propre à JavaScript. Ce dernier utilise des nombres de format à virgule flottante double précision comme spécifié dans la norme IEEE 754 ; les nombres qu’il peut représenter de la sorte sont compris entre -(253 - 1) et 253-1 (c’est-à-dire, 9007199254740991) uniquement.
...
La création d’autorisations à l’aide de ResourceTokens est autorisée au niveau du conteneur et de ses descendants (tels que les documents ou les pièces jointes). Cela implique qu’une tentative de création d’une autorisation au niveau de la base de données ou du compte n’est pas autorisée actuellement.
...
* Vous avez besoin d’un débit supérieur du conteneur ou d’un ensemble de conteneurs sur le backend. Par exemple, vous voyez que le débit utilisé est supérieur au débit approvisionné, et vous êtes limité. Pour plus d’informations, consultez [Définir le débit des conteneurs Azure Cosmos DB](set-throughput.md).
...
Si le taux de demandes dépasse la capacité du débit provisionné pour le conteneur sous-jacent ou un ensemble de conteneurs, vous recevez une erreur et le SDK essaie de renouveler l’appel en appliquant la stratégie de nouvelle tentative.
...
### <a name="azure-table-storage-has-been-inexpensive-for-me-because-i-pay-only-to-store-the-data-and-i-rarely-query-the-azure-cosmos-db-table-api-offering-seems-to-be-charging-me-even-though-i-have-not-performed-a-single-transaction-or-stored-anything-can-you-please-explain"></a>Le stockage Table Azure était peu onéreux pour moi, car je payais uniquement le stockage des données et n’effectuais que rarement des requêtes. L’offre API Table d’Azure Cosmos DB semble me facturer des frais alors que je n’ai pas effectué la moindre transaction ou stocké quoi que ce soit. Pouvez-vous m’en expliquer la raison ?
...
Oui, il est possible de créer plusieurs tables avec l’API Apache Cassandra. Chacune de ces tables est traitée en tant qu’unité de débit et de stockage. 
...
Azure Cosmos DB est un service de plateforme qui vous permet d’augmenter votre productivité sans vous soucier des opérations de gestion et de surveillance de l’infrastructure. Vous devez simplement vous soucier du débit relevé dans les indicateurs de performance accessibles sur le portail, pour vérifier si vous approchez de la limite et devez augmenter ou réduire ce débit en conséquence. Analyser les [contrats de niveau de service](monitor-accounts.md)
