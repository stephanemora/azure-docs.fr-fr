...
Le trafic multimédia transite par des composants appelés processeurs multimédias. Les processeurs multimédias sont placés dans les mêmes centres de données que les proxys SIP. En outre, il existe d’autres processeurs multimédias pour optimiser le flux multimédia. Par exemple, nous n’avons actuellement pas de composant proxy SIP en Australie (SIP transite via Singapour ou Hong Kong), mais nous avons un processeur multimédia local en Australie. La nécessité d’utiliser des processeurs multimédias localement est dictée par la latence que nous expérimentons en envoyant du trafic longue distance, par exemple de l’Australie à Singapour ou à Hong-Kong. Bien que la latence dans l’exemple de trafic transitant de l’Australie à Hong Kong ou à Singapour soit acceptable pour préserver une qualité d’appel correcte pour le trafic SIP, ce n’est pas le cas pour le trafic multimédia en temps réel.
...
- Asie (centre de données de Singapour et de Hong Kong)
