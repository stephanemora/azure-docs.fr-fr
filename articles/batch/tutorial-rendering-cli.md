...
Si vous n’êtes pas déjà connecté à votre compte Batch, utilisez la commande [az batch account login](/cli/azure/batch/account#az_batch_account_login) pour vous connecter. Ensuite, créez le pool en passant le fichier JSON à la commande `az batch pool create` :
...
Pour écrire des fichiers de sortie dans le conteneur, Batch doit utiliser un jeton de signature d’accès partagé (SAS). Créez le jeton avec la commande [az storage account generate-sas](/cli/azure/storage/account#az_storage_account_generate_sas). Cet exemple crée un jeton à écrire dans un conteneur d’objets blob du compte, et ce jeton expire le 15 novembre 2021 :
