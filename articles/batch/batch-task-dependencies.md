...
Par défaut, les tâches dépendantes sont planifiées pour que leur exécution ait lieu uniquement après la fin de l’exécution de la tâche parente. Vous pouvez éventuellement indiquer une [action de dépendance](#dependency-actions) pour remplacer le comportement par défaut et exécuter la tâche dépendante même en cas d’échec de la tâche parente.
...
Pour créer la dépendance, fournissez le premier et le dernier ID de tâche dans la plage à la méthode statique [TaskDependencies.OnIdRange](/dotnet/api/microsoft.azure.batch.taskdependencies.onidrange) quand vous renseignez la propriété [CloudTask.DependsOn](/dotnet/api/microsoft.azure.batch.cloudtask.dependson).
...
>
> Pour que la tâche dépendante soit exécutée, chaque tâche de la plage doit satisfaire la dépendance soit en se terminant avec succès, soit en échouant avec une erreur associée à une [action de dépendance](#dependency-actions) définie sur **Satisfy**.
...
Pour .NET, ces conditions sont définies comme propriétés de la classe [ExitConditions](/dotnet/api/microsoft.azure.batch.exitconditions).

Pour spécifier une action de dépendance, définissez la propriété [ExitOptions.DependencyAction](/dotnet/api/microsoft.azure.batch.exitoptions.dependencyaction) pour la condition de sortie sur l’une des valeurs suivantes :
