...
## <a name="step-1---create-runbook"></a>Étape 1 - Créer un runbook
...
1. Dans le contrôle Bibliothèque, cliquez dans la zone de recherche, puis tapez « write-output ». Les résultats de la recherche sont présentés dans l’image suivante : <br> ![Microsoft.PowerShell.Utility](media/automation-first-runbook-graphical/search-powershell-cmdlet-writeoutput.png)
...
1. Une fois le travail du runbook terminé, la page Test affiche sa sortie. En l’occurrence, vous voyez « Hello World ».<br> ![Hello World](media/automation-first-runbook-graphical/runbook-test-results.png)
...
1. Faites défiler la page vers la droite pour afficher la page MyFirstRunbook-Graphical.
...
1. Un volet Travail s’ouvre pour le travail du runbook que vous venez de créer. Vérifiez que l’option **État du travail** indique Terminé.
...
1. Dans la zone de recherche du contrôle Bibliothèque, tapez « Connect-AzAccount ».
...
1. Sur le canevas, sélectionnez **Connect-AzAccount**. Dans le volet Contrôle de configuration, dans le champ **Étiquette**, tapez « Login to Azure ».
...
1. Dans la page Valeur du paramètre, définissez les paramètres suivants, puis cliquez sur **OK**.
   ...
   * **Chemin du champ** : tapez « ApplicationId ». Vous spécifiez le nom de la propriété du chemin du champ parce que l’activité génère un objet doté de plusieurs propriétés.
1. Cliquez sur **CERTIFICATETHUMBPRINT**, puis, dans la page Valeur du paramètre, définissez les paramètres suivants et cliquez sur **OK**.
    ...
    * **Source de données** : sélectionnez **Sortie d’activité**.
    * Liste des sources de données : sélectionnez **Obtenir une connexion d’identification**.
    * **Chemin du champ** : tapez « CertificateThumbprint ».
...
1. Cliquez sur **TENANTID**, puis définissez les paramètres suivants sur la page Valeur du paramètre. Lorsque vous avez terminé, cliquez sur **OK** à deux reprises.
    ...
    * **Source de données** : sélectionnez **Sortie d’activité**. 
    * Liste des sources de données : sélectionnez **Obtenir une connexion d’identification**.
    * **Chemin du champ** : tapez « TenantId ». 
1. Dans la zone de recherche du contrôle Bibliothèque, tapez « Set-AzContext ».
1. Ajoutez **Set-AzContext** au canevas.
1. Sélectionnez **Set-AzContext** sur le canevas. Dans le volet Contrôle de configuration, dans le champ **Étiquette**, tapez « Specify Subscription Id ».
1. Cliquez sur **Paramètres** pour afficher la page Configuration des paramètres d’activité.
...
1. Sélectionnez **Name**. Pour le champ **Source de données**, sélectionnez **Expression PowerShell**. Pour la machine virtuelle que vous utilisez pour démarrer ce runbook, tapez le nom de la machine entouré de guillemets doubles. Cliquez sur **OK**.
1. Sélectionnez **ResourceGroupName**. Pour le champ **Source de données**, utilisez la valeur Expression PowerShell, puis tapez le nom du groupe de ressources entre guillemets. Cliquez sur **OK**.
1. Cliquez sur le volet de **Test** afin de tester le runbook.
1. Cliquez sur **Démarrer** pour démarrer le test. Une fois le test terminé, assurez-vous que la machine virtuelle a démarré. 
...
## <a name="step-9---create-a-conditional-link"></a>Étape 9 - Créer un lien conditionnel
...
1. Dans la zone de recherche du contrôle Bibliothèque, tapez « Get-Az ».
1. Ajoutez **Get-AzVM** au canevas.
1. Sélectionnez **Get-AzVM** puis **Jeu de paramètres** afin d’afficher les jeux pour **Get-AzVM**. 
...
1. Sélectionnez le lien puis, dans le panneau Configuration, changez **Appliquer la condition** en Oui. Notez que le lien se transforme en ligne pointillée, indiquant que l’activité cible s’exécute uniquement si la condition produit la valeur true.  
...
1. Pour **InputObject**, changez **Source de données** en **Expression PowerShell** puis tapez l’expression « $VMName could not start ».
