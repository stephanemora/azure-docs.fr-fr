...
Vous avez configuré **Contrôle du redémarrage** sur **Ne jamais redémarrer**, mais les ordinateurs sont toujours redémarrés après l’installation des mises à jour.

### <a name="cause"></a>Cause :

Le comportement de Windows Update peut être modifié par plusieurs clés de Registre qui peuvent modifier les comportements de redémarrage.
...
Lorsque c’est possible, utilisez les [groupes dynamiques](../automation-update-management-groups.md) pour vos déploiements de mise à jour.
...
1. Suivez les étapes de [Des machines n’apparaissent pas dans le portail sous Update Management](#nologs) pour vérifier que l’ordinateur est associé au bon espace de travail.
2. Éliminez les anciens artefacts sur la machine en [supprimant le groupe de Runbooks hybrides](../automation-hybrid-runbook-worker.md#remove-a-hybrid-worker-group), puis réessayez.
...
* Vous avez des machines qui s’affichent comme non évaluées
* Vous voyez un code d’erreur HRESULT dans le portail.

### <a name="cause"></a>Cause :

L’agent de mise à jour (agent Windows Update sur Windows ou le gestionnaire de package pour votre distribution Linux) n’est pas configuré correctement. Update Management s’appuie sur l’agent de mise à jour de l’ordinateur pour fournir les mises à jour nécessaires, l’état du correctif et les résultats des correctifs déployés. Sans ces informations, Update Management ne peut pas générer de rapport exact concernant les correctifs nécessaires ou installés.

### <a name="resolution"></a>Résolution :

Tentative d’exécution de mises à jour localement sur l’ordinateur. En cas d’échec, cela signifie généralement une erreur de configuration avec l’agent de mise à jour.

Les causes courantes d’échec sont les suivantes :

* La configuration réseau et les pare-feu.
* Pour Linux, consultez la documentation appropriée pour vous assurer que vous êtes en mesure d’atteindre le point de terminaison réseau de votre référentiel de packages.
* Pour Windows, vérifiez la configuration de votre agent, comme indiqué dans [Les mises à jour ne se téléchargent pas à partir du point de terminaison intranet (WSUS/SCCM)](/windows/deployment/update/windows-update-troubleshooting#updates-arent-downloading-from-the-intranet-endpoint-wsussccm).
  * Si le ou les ordinateurs sont configurés pour Windows Update, assurez-vous que vous êtes en mesure d’atteindre les points de terminaison répertoriés sous [](/windows/deployment/update/windows-update-troubleshooting#issues-related-to-httpproxy).
  * Si le ou les ordinateurs sont configurés pour WSUS, assurez-vous que vous êtes en mesure d’atteindre le serveur WSUS configuré par la [clé de Registre WUServer](/windows/deployment/update/waas-wu-settings).

Si vous voyez un HRESULT, double-cliquez sur l’exception affichée en rouge pour voir le message d’exception. Consultez le tableau suivant pour connaître les solutions possibles ou les actions appliquer :
...
## <a name="scenario-update-run-returns-status-failed"></a>Scénario : L’exécution de la mise à jour retourne l’état « échec »
...
* Si les mises à jour ne sont pas approuvées dans WSUS, elles ne seront pas installées. Vous pouvez rechercher les mises à jour non approuvées dans Log Analytics à l’aide de la requête suivante.
