...
description: Découvrez comment résoudre les erreurs d’intégration de solution.
services: automation
author: mgoedtel
ms.author: magoedte
ms.date: 05/22/2019
...
ms.locfileid: "81679231"
---
# <a name="troubleshoot-solution-onboarding"></a>Résolution des problèmes d’intégration de solution

Vous pouvez recevoir des erreurs lors de l’intégration de la solution Update Management ou de la solution de Change Tracking et d’inventaire. Cet article décrit les erreurs qui pourraient se produire et explique comment les résoudre.
...
Lors de la connexion via une solution proxy qui met fin au trafic HTTPS, puis chiffre à nouveau le trafic à l’aide d’un nouveau certificat, le service n’autorise pas la connexion.
...
Vérifiez que vous avez les [autorisations nécessaires requises pour intégrer des machines](../automation-role-based-access-control.md#onboarding-permissions), puis essayez à nouveau d'intégrer la solution. Si vous rencontrez l’erreur `The solution cannot be enabled on this VM because the permission to read the workspace is missing`, assurez-vous que vous disposez de l’autorisation `Microsoft.OperationalInsights/workspaces/read` pour pouvoir déterminer si la machine virtuelle est intégrée à un espace de travail.

### <a name="scenario-onboarding-fails-with-the-message-failed-to-configure-automation-account-for-diagnostic-logging"></a><a name="diagnostic-logging"></a>Scénario : L’intégration échoue avec le message suivant : Échec de la configuration du compte Automation pour la journalisation des diagnostics
...
Cette erreur peut survenir si le niveau tarifaire ne correspond pas au modèle de facturation de l’abonnement. Consultez [Surveiller l’utilisation et l’estimation des coûts dans Azure Monitor](https://aka.ms/PricingTierWarning).
...
Vous pouvez supprimer la requête pour cette solution, puis réintégrer la solution, ce qui recrée la requête. La requête se trouve dans votre espace de travail, sous **Recherches enregistrées**. Le nom de la requête est **MicrosoftDefaultComputerGroup**, et la catégorie de la requête est le nom de la solution associée. Si plusieurs solutions sont activées, la requête **MicrosoftDefaultComputerGroup** s’affiche plusieurs fois sous **Recherches enregistrées**.
...
* Pour Start/Stop VMs during off-hours, supprimez tous les verrous sur les composants de solution dans votre compte Automation sous **Paramètres** > **Verrous**. Consultez [Solution Start/stop VMs during off-hours](../automation-solution-vm-management.md#remove-the-solution).
