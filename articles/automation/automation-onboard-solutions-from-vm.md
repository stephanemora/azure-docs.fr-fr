...
3. Dans la page Machine virtuelle, sous **Opérations**, sélectionnez **Gestion des mises à jour**, **Inventaire** ou **Suivi des modifications**. La machine virtuelle peut exister dans n’importe quelle région, quelle que soit la localisation de votre compte Automation. Lors de l’intégration d’une solution à partir d’une machine virtuelle, vous avez besoin de l’autorisation `Microsoft.OperationalInsights/workspaces/read` pour déterminer si la machine virtuelle est intégrée à un espace de travail. Pour en savoir plus sur les autorisations supplémentaires nécessaires, consultez les [autorisations nécessaires pour intégrer des machines](automation-role-based-access-control.md#onboarding-permissions). Consultez la section [Intégrer les solutions Update Management, Change Tracking et Inventory](automation-onboard-solutions-from-automation-account.md) pour savoir comment intégrer plusieurs machines en même temps.

4. Sélectionnez l’espace de travail Log Analytics et le compte Automation, puis cliquez sur **Activer** pour activer la solution. L’activation de la solution prend jusqu’à 15 minutes.

![Intégrer la solution Update Management](media/automation-tutorial-update-management/manageupdates-update-enable.png)

5. Accédez aux autres solutions, puis sélectionnez **Activer**. Les listes déroulantes des espaces de travail Log Analytics et des comptes Automation sont désactivées, car ces solutions utilisent le même espace de travail et le même compte Automation que la solution précédemment activée.
...
2. Dans l’espace de travail, sous **Sources de données de l’espace de travail**, sélectionnez **Configurations des étendues**.
3. Si l’espace de travail sélectionné n’inclut pas encore la solution Update Management ou Change Tracking, les configurations d’étendue suivantes sont créées :
...
4. Cliquez sur le bouton de sélection ( **…** ) dans l’une des configurations, puis sur **Modifier**. 
5. Dans le volet **Modifier la configuration d’étendue**, sélectionnez **Sélectionner des groupes d’ordinateurs**. Le volet **Groupes d’ordinateurs** affiche les recherches enregistrées utilisées pour créer la configuration d’étendue.

## <a name="saved-searches"></a>Recherches enregistrées

Lorsqu’un ordinateur est ajouté à la solution Update Management, Change Tracking ou Inventory, il est ajouté à l’une des deux recherches enregistrées dans votre espace de travail. Les recherches enregistrées sont des requêtes qui contiennent les ordinateurs ciblés pour ces solutions.
...
1. Dans le Portail Azure, ouvrez votre compte Automation, puis sélectionnez **Espace de travail lié** dans la section **Ressources associées** sur la gauche.
