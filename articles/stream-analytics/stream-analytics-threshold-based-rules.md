...
- Toutes les colonnes ne seront incluses dans l’événement d’alerte de sortie. Dans ce cas, la clé numéro `includedDim` `2` est définie sur `TRUE` pour indiquer que le champ numéro 2 des données d’événement dans le flux de données sera inclus dans les événements de sortie correspondants. Les autres champs ne sont pas inclus dans la sortie d’alerte, mais la liste des champs peut être ajustée.
