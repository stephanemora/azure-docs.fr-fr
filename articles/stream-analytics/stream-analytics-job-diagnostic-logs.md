...
4. Faites défiler jusqu'à la section **Propriétés** du JSON, qui fournit des détails sur l’erreur à l'origine de l'échec de l'opération. Dans cet exemple, cet échec était dû à une erreur d'exécution liée à des valeurs de latitude hors limites. Incohérence dans les données qui sont traitées par un travail Stream Analytique provoque une erreur de données. Vous pouvez en savoir plus sur différents [les erreurs de données d’entrée et de sortie et pourquoi ils se produisent](https://docs.microsoft.com/azure/stream-analytics/data-errors).
