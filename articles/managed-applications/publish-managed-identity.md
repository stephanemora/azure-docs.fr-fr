...
Il existe deux manières courantes de créer une application managée avec **identity** : [CreateUIDefinition.json](./create-uidefinition-overview.md) et les [modèles Azure Resource Manager](../azure-resource-manager/resource-group-authoring-templates.md). Pour des scénarios de création uniques et simples, CreateUIDefinition doit être utilisé pour activer l’identité managée, car il propose une expérience plus riche. Toutefois, quand vous utilisez des systèmes élaborés ou complexes qui demandent des déploiements automatisés ou multiples d’applications managées, les modèles peuvent être utilisés.
