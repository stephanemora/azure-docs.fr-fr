---
title: Utiliser Azure Key Vault avec des applications managées | Microsoft Docs
...
Pour transmettre un secret à partir d’un coffre de clés à un modèle dans votre application managée, vous devez utiliser un [modèle lié](../azure-resource-manager/resource-group-linked-templates.md) et référencer le coffre de clés dans les paramètres de ce modèle lié. Fournissez l’ID de ressource du coffre de clés et le nom du secret.
...
* Pour obtenir des informations sur la transmission d’une valeur à partir d’un coffre de clés en tant que paramètre de modèle, consultez [Utiliser Azure Key Vault pour transmettre une valeur de paramètre sécurisée pendant le déploiement](../azure-resource-manager/resource-manager-keyvault-parameter.md).
