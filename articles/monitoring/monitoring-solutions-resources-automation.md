...
- La structure d’un [fichier de solution]( monitoring-solutions-solution-file.md).
...
La tâche comprend le nom du runbook et toute valeur de paramètre à lui envoyer.  La tâche doit [dépendre]( monitoring-solutions-solution-file.md#resources) du runbook qu’elle démarre, car le runbook doit être créé avant la tâche.  Si vous avez plusieurs runbooks qui doivent être lancés, vous pouvez définir leur ordre en rendant un travail dépendant des autres travaux qui doivent être exécutés en premier.

Le nom d’une ressource de tâche doit contenir un GUID, qui est généralement affecté par un paramètre.  Vous pouvez en savoir plus sur les paramètres GUID dans [Création d’un fichier de solution de gestion dans Azure]( monitoring-solutions-solution-file.md#parameters).  
...
L’exemple utilise des variables de [paramètres de solution standard]( monitoring-solutions-solution-file.md#parameters) qui seraient généralement utilisées dans une solution, par opposition aux valeurs de codage en dur dans les définitions de ressource.
...
* [Ajout d’une vue à votre solution]( monitoring-solutions-resources-views.md) pour visualiser les données collectées.
