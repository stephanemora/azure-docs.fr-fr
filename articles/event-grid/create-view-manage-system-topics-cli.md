...
- Pour créer d’abord une rubrique système sur une source Azure, puis créer un abonnement d’événement pour cette rubrique, consultez les rubriques de référence suivantes :
    ...
    - [az eventgrid system-topic event-subscription create](/cli/azure/ext/eventgrid/eventgrid/system-topic/event-subscription#ext-eventgrid-az-eventgrid-system-topic-event-subscription-create)
        ...
        ```
- Pour créer une rubrique système (implicitement) lors de la création d’un abonnement aux événements pour une source Azure, utilisez la méthode [az eventgrid event-subscription create](/cli/azure/ext/eventgrid/eventgrid/event-subscription#ext-eventgrid-az-eventgrid-event-subscription-create). Voici un exemple :
