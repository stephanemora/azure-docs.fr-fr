---
title: Azure VMware Solutions (AVS) - Utiliser Azure AD comme source d’identité sur le cloud privé AVS
description: Décrit comment ajouter Azure AD en tant que fournisseur d’identité sur votre cloud privé AVS pour authentifier les utilisateurs qui accèdent à AVS à partir d’Azure
author: sharaths-cs
ms.author: b-shsury
ms.date: 08/15/2019
ms.topic: article
...
ms.locfileid: "77015921"
---
# <a name="use-azure-ad-as-an-identity-provider-for-vcenter-on-avs-private-cloud"></a>Utiliser Azure AD comme fournisseur d’identité pour vCenter sur un cloud privé AVS

Vous pouvez configurer votre serveur vCenter de cloud privé AVS pour l’authentification auprès d’Azure Active Directory (Azure AD) afin que les administrateurs VMware accèdent à vCenter. Une fois la source d’identité d’authentification unique configurée, l'utilisateur **cloudowner** peut ajouter des utilisateurs à partir de la source d’identité à vCenter.  
...
* Nouveaux domaine et contrôleurs de domaine Active Directory exécutés dans votre cloud privé AVS
...
2. [Comment configurer une source d’identité sur votre serveur vCenter de cloud privé AVS](#set-up-an-identity-source-on-your-avs-private-cloud-vcenter)
...
## <a name="set-up-an-identity-source-on-your-avs-private-cloud-vcenter"></a>Configurer une source d’identité sur votre serveur vCenter de cloud privé AVS

1. [Réaffectez les privilèges](escalate-private-cloud-privileges.md) pour votre serveur vCenter de cloud privé AVS.
2. Collectez les paramètres de configuration requis pour la configuration de la source d’identité.
    ...
    | **Choisir un certificat** | Si vous souhaitez utiliser LDAPS avec votre serveur LDAP Active Directory ou source d’identité de serveur OpenLDAP, un bouton  **Choisir un certificat** s’affiche une fois que vous avez tapé `ldaps://` dans la zone de texte URL. Une URL secondaire n’est pas nécessaire. |
...
3. Connectez-vous à votre serveur vCenter de cloud privé AVS une fois les privilèges réaffectés.
