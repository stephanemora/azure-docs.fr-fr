---
title: Démarrage rapide – Créer une machine virtuelle VMware sur un cloud privé AVS
description: Décrit comment créer une machine virtuelle VMware sur un cloud privé AVS
author: sharaths-cs
ms.author: b-shsury
ms.date: 08/16/2019
ms.topic: article
...
ms.locfileid: "77020061"
---
# <a name="create-vmware-virtual-machines-on-your-avs-private-cloud"></a>Créer des machines virtuelles VMware sur votre cloud privé AVS

Pour créer des machines virtuelles sur votre cloud privé AVS, commencez par accéder au portail AVS depuis le portail Azure.
...
4. Dans la page **Vue d’ensemble**, cliquez sur **Accéder au portail AVS** pour ouvrir un nouvel onglet de navigateur pour le portail AVS. Si vous y êtes invité, connectez-vous avec vos informations d’identification Azure. 
...
Pour accéder à vCenter, démarrez à partir du portail AVS. Sur la page d’accueil, sous **Tâches courantes**, cliquez sur **Lancer le client vSphere**. Sélectionnez le cloud privé AVS, puis cliquez sur **Lancer le client vSphere** sur le cloud privé AVS.
...
## <a name="upload-an-iso-or-vsphere-template"></a>Charger un modèle ISO ou vSphere
  ...
  > [!WARNING]
  > Pour le chargement des fichiers ISO, utilisez le client HTML5 vSphere. L'utilisation du client Flash peut entraîner une erreur.
...
L'installation de base est maintenant terminée. Vous pouvez commencer à utiliser votre cloud privé AVS de la même manière que vous utilisez votre infrastructure de machines virtuelles locale.

Les sections suivantes contiennent des informations facultatives sur la configuration des serveurs DNS et DHCP pour les charges de travail du cloud privé AVS, ainsi que sur la modification de la configuration réseau par défaut.
...
AVS affecte un compte d’utilisateur vCenter par défaut ayant le nom d’utilisateur `cloudowner@AVS.local`. Aucune autre configuration de compte n'est nécessaire pour commencer. AVS attribue normalement aux administrateurs les privilèges dont ils ont besoin pour effectuer des opérations normales. Configurez votre annuaire actif local ou Azure AD en tant que [source d’identité supplémentaire](set-vcenter-identity.md) sur votre cloud privé AVS.

## <a name="create-a-dns-and-dhcp-server-optional"></a>Créer un serveur DNS et DHCP (facultatif)

Les applications et charges de travail exécutées dans un environnement de cloud privé AVS nécessitent une résolution de nom et des services DHCP pour la recherche et l’attribution d’adresses IP. Une infrastructure DHCP et DNS appropriée est nécessaire pour fournir ces services. Vous pouvez configurer une machine virtuelle dans vCenter pour fournir ces services dans votre environnement de cloud privé AVS.
...
Les pages Network (Réseau) du portail AVS vous permettent de spécifier la configuration des tables de pare-feu et des adresses IP publiques des machines virtuelles.

### <a name="allocate-public-ips"></a>Allouer des adresses IP publiques

1. Accédez à **Network > Public IP** (Réseau > Adresse IP publique) dans le portail AVS.
...
### <a name="default-avs-firewall-rules-for-public-ip"></a>Règles de pare-feu AVS par défaut pour les adresses IP publiques
...
* Trafic interne du cloud privé AVS : Tout le trafic est-ouest entre (de/vers) les réseaux de charges de travail et le réseau de gestion (montré ci-dessus) est autorisé.
* Trafic Internet :
...
* [Configurer des passerelles VPN sur un réseau AVS](vpn-gateway.md)
