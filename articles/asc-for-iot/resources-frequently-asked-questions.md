---
title: Questions fréquentes sur Azure Security Center pour IoT (préversion) | Microsoft Docs
...
Cet article fournit une liste de questions fréquentes et les éponses correspondantes au sujet d’Azure Security Center (ASC) pour IoT. 
...
ASC pour IoT permet aux entreprises d’étendre leur vue de cybersécurité existante à la totalité des solutions IoT. Azure fournit une vue de bout en bout de votre solution d’entreprise, ce qui vous permet de prendre des mesures et décisions professionnelles en fonction de la posture de sécurité de votre entreprise et des données collectées. La sécurité combinée d’Azure IoT, Azure IoT Edge, Azure Sphere, Azure Central et Azure Security Center vous permet de créer la solution que vous souhaitez avec la sécurité dont vous avez besoin.

## <a name="who-is-asc-for-iot-made-for"></a>À qui ASC pour IoT est-il destiné ? 

ASC pour IoT est intégré au sein de la sécurité d’Azure IoT Hub et fournit la gestion des activités quotidiennes pour les opérations de sécurité. ASC pour IoT est également intégré aux fonctionnalités d’Azure Security Center et fournit une vue intégrée pour la surveillance et la gestion de la sécurité IoT dans le cadre de votre solution de sécurité globale.

## <a name="how-does-asc-for-iot-compare-to-the-competition"></a>Comment ASC pour IoT se mesure-t-il par rapport à la concurrence ?
...
## <a name="do-i-have-to-be-an-azure-security-center-asc-customer"></a>Dois-je être un client Azure Security Center (ASC) ?

Non, mais cela est recommandé. Sans ASC, ASC pour IoT reçoit des données de ressources connectées limitées et fournit une analyse limitée de la surface d’attaque potentielle, des menaces et des attaques potentielles. 

## <a name="do-i-have-to-be-an-azure-iot-customer"></a>Dois-je être un client Azure IoT ?

Oui. ASC pour IoT repose sur l’infrastructure et la connectivité d’Azure IoT.

## <a name="do-i-have-to-install-an-agent"></a>Dois-je installer un agent ?

L’installation de l’agent sur vos appareils IoT n’est pas obligatoire pour activer Microsoft ASC pour IoT. Vous pouvez choisir entre les trois options suivantes, pour des niveaux de sécurité et des fonctionnalités de surveillance différentes selon votre choix :

1. Installez l’agent de sécurité ASC pour IoT avec ou sans modifications. Cette option fournit le plus haut niveau d’informations de sécurité améliorées pour le comportement et l’accès à l’appareil.

2. Créez votre propre agent et implémentez le schéma de message de sécurité Microsoft ASC pour IoT. Cette option permet l’utilisation des outils d’analyse de Microsoft ASC pour IoT en plus de l’agent de sécurité de votre appareil.

3. Aucune installation de l’agent de sécurité sur vos appareils IoT. Cette option permet de surveiller les communications d’IoT Hub, avec des fonctionnalités de surveillance et de gestion de la sécurité limitées. 

## <a name="what-does-the-asc-for-iot-agent-do"></a>Que fat l’agent ASC pour IoT ?

L’agent ASC pour IoT couvre les menaces au niveau de l’appareil en matière de configuration, de comportement, d’accès (en analysant la configuration), de processus et de connectivité. L’agent de sécurité ASC pour IoT n’analyse pas les données professionnelles ou d’activité.

## <a name="where-i-can-get-the-asc-for-iot-security-agent"></a>Où puis-je obtenir l’agent de sécurité ASC pour IoT ?

L’agent de sécurité ASC pour IoT est open source et disponible sur GitHub en versions 32 64 bits pour Windows et Linux : https://github.com/Azure/Azure-Security-IoT-Preview

## <a name="where-does-the-asc-for-iot-agent-get-installed"></a>Où l’agent ASC pour IoT est-il installé ? 

Vous trouverez des informations détaillées sur l’installation et le déploiement de l’agent sur GitHub : https://github.com/Azure/Azure-Security-IoT-Preview
...
ASC pour IoT prend en charge un large éventail de plateformes. Consultez les [plateformes d’appareils pris en charge](how-to-deploy-agent.md) pour vérifier la prise en charge de vos appareils spécifiques. 
...
2. Vérifiez que le [service a été activé avec succès](quickstart-onboard-iot-hub.md) sur **Sécurité** dans votre IoT Hub. 

3. Vérifiez que l’appareil est [configuré dans IoT Hub avec le module ASC pour IoT](quickstart-create-security-twin.md).  

Si les activités ou les journaux ne sont toujours pas disponibles, contactez votre partenaire ASC pour IoT pour obtenir une aide supplémentaire.
...
Le module ASC pour IoT est utilisé pour la gestion et la configuration de l’agent.


## <a name="next-steps"></a>Étapes suivantes

Pour en savoir plus sur la prise en main d’ASC pour IoT, consultez les articles suivants :
...
- Comprendre les [alertes de sécurité ASC pour IoT](concept-security-alerts.md)
