---
title: Didacticiel pour créer une application web avec le Cache Redis qui utilise le modèle Cache-Aside | Microsoft Docs
description: Découvrez comment créer une application web avec le Cache Redis qui utilise le modèle Cache-Aside
...
Dans ce didacticiel vous mettrez à jour l’application web ASP.NET *ContosoTeamStats*, créée dans le [démarrage rapide ASP.NET pour Cache Redis Azure](cache-web-app-howto.md), pour inclure un leaderboard qui utilise le [modèle Cache-Aside](https://docs.microsoft.com/azure/architecture/patterns/cache-aside) avec le Cache Redis Azure. L’exemple d’application affiche une liste des statistiques d’équipe d’une base de données et montre les différentes façons d’utiliser le Cache Redis Azure pour stocker et récupérer des données à partir du cache pour améliorer les performances. Lorsque vous avez terminé le didacticiel, vous disposerez d’une application web, optimisée avec le Cache Redis Azure et hébergée dans Azure, effectuant des opérations de lecture et écriture sur une base de données.

Ce tutoriel vous montre comment effectuer les opérations suivantes :

> [!div class="checklist"]
> * Améliorer le débit des données et réduire la charge de la base de données en stockant et en récupérant des données à l’aide du Cache Redis Azure.
...
* Ce didacticiel se poursuit là où vous l’avez laissé dans le [démarrage rapide ASP.NET pour le Cache Redis Azure](cache-web-app-howto.md). Si vous ne l’avez pas encore fait, commencez par suivre ce guide de démarrage rapide.
...
1. Dans Visual Studio, ouvrez la solution *ContosoTeamStats* que vous avez créée dans le [démarrage rapide ASP.NET pour le Cache Redis Azure](cache-web-app-howto.md).
...
1. Ajoutez la section `connectionStrings` suivante dans la section `configuration`. Le nom de la chaîne de connexion doit correspondre au nom de la classe de contexte de base de données Entity Framework, qui est `TeamContext`.
    ...
    Cette chaîne de connexion suppose que vous avez rempli les [conditions préalables](#prerequisites) et installé la base de données locale SQL Server Express, qui fait partie de la charge de travail de *développement de bureau .NET* installée avec Visual Studio 2017.
...
1. Dans la section `body`, ajoutez la nouvelle instruction `Html.ActionLink` suivante pour les *statistiques d’équipe Contoso* juste en dessous du lien pour le *test du Cache Redis Azure*.
...
## <a name="configure-the-app-for-redis-cache"></a>Configurer l’application pour le Cache Redis
...
Dans ce test, l’application et les bases de données s’exécutent localement. Toutefois, le Cache Redis est hébergé à distance dans Azure. Par conséquent, le cache réduira probablement les performances de la base de données. Pour de meilleures performances, l’application cliente et l’instance de Cache Redis Azure doivent se trouver au même emplacement. Dans la section suivante, vous allez déployer toutes les ressources sur Azure pour voir l’amélioration des performances avec l’utilisation d’un cache.
...
1. Sous **Serveur**, cliquez sur **Configurer les paramètres requis** > **Créer un nouveau serveur** et fournissez les informations suivantes, puis cliquez sur le bouton **Sélectionner** :
   ...
   | **Lieu** | *Est des États-Unis* | Sélectionnez la région où vous avez créée le cache et le service d’applications. |
...
> [Mise à l’échelle du cache Azure Redis](./cache-how-to-scale.md)