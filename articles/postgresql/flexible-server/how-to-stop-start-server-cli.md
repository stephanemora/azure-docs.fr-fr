---
title: Arrêter/Démarrer - Azure CLI - Serveur flexible Azure Database pour PostgreSQL
description: Cet article explique comment arrêter/démarrer des opérations dans Azure Database pour PostgreSQL à l’aide d’Azure CLI.
author: mksuni
ms.author: sumuth
...
ms.locfileid: "111026506"
---
# <a name="stopstart-azure-database-for-postgresql---flexible-server-preview-using-azure-cli"></a>Arrêter/démarrer un serveur flexible Azure Database pour PostgreSQL (préversion) à l’aide d’Azure CLI
...
Pour arrêter un serveur, exécutez la commande ```az postgres flexible-server stop```. Si vous utilisez un [contexte local](/cli/azure/config/param-persist), vous n’avez pas besoin de fournir d’arguments.
...
Pour démarrer un serveur, exécutez la commande ```az postgres flexible-server start```. Si vous utilisez un [contexte local](/cli/azure/config/param-persist), vous n’avez pas besoin de fournir d’arguments.
...
- En savoir plus sur les [le redémarrage d’Azure Database pour PostgreSQL - Serveur flexible](./how-to-restart-server-cli.md)
