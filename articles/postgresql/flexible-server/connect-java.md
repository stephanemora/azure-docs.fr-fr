---
title: 'Démarrage rapide : Utiliser Java et JDBC avec un serveur flexible Azure Database pour PostgreSQL'
description: Dans ce guide de démarrage rapide, vous allez voir comment utiliser Java et JDBC avec un serveur flexible Azure Database pour PostgreSQL.
author: mksuni
ms.author: sumuth
...
ms.locfileid: "113288101"
---
# <a name="quickstart-use-java-and-jdbc-with-azure-database-for-postgresql-flexible-server"></a>Démarrage rapide : Utiliser Java et JDBC avec un serveur flexible Azure Database pour PostgreSQL

Cette rubrique montre la création d’un exemple d’application qui utilise Java et [JDBC](https://en.wikipedia.org/wiki/Java_Database_Connectivity) pour stocker et récupérer des informations sur un [serveur flexible Azure Database pour PostgreSQL](./index.yml).
...
> [!NOTE]
> Vous trouverez des informations plus détaillées sur la création de serveurs PostgreSQL dans l’article [Création d’un serveur Azure Database pour PostgreSQL à l’aide du portail Azure](./quickstart-create-server-portal.md).
...
Le serveur PostgreSQL que vous avez créé précédemment contient une base de données vide nommée **postgres**. Pour ce guide de démarrage rapide, nous allons créer une nouvelle base de données nommée `demo` en effectuant les étapes suivantes :

1. Installez l’instance locale de [psql](https://www.postgresql.org/docs/current/static/app-psql.html) pour vous connecter à un serveur Azure PostgreSQL. 
2. Exécutez la commande psql suivante pour vous connecter à la base de données **postgres** de votre serveur PostgreSQL.
...
3. Créez une base de données nommée **demo** à l’invite en tapant la commande suivante :
