---
title: Installer l’extension SQL Server IaaS Agent
...
ms.locfileid: "93286149"
---
# <a name="install-sql-server-iaas-agent-extension"></a>Installer l’extension SQL Server IaaS Agent
[!INCLUDE[appliesto-sqlvm](../../includes/appliesto-sqlvm.md)]

Le fait d’inscrire votre machine virtuelle SQL Server auprès du fournisseur de ressources de machine virtuelle SQL installe l’[extension SQL Server IaaS Agent](sql-server-iaas-agent-extension-automate-management.md). 
...
L’inscription auprès du fournisseur de ressources installe l’extension [SQL Server IaaS Agent](sql-server-iaas-agent-extension-automate-management.md) et crée la _ressource_ de **machine virtuelle SQL** dans votre abonnement ; il s’agit d’une ressource _distincte_ de la ressource de machine virtuelle. Le fait de désinscrire votre machine virtuelle SQL Server du fournisseur de ressources va supprimer la _ressource_ de **machine virtuelle SQL** , mais pas la machine virtuelle elle-même.

Pendant le déploiement d’une image de machine virtuelle SQL Server de la Place de marché Azure via le portail Azure, la machine virtuelle SQL Server est inscrite automatiquement auprès du fournisseur de ressources. Toutefois, si vous choisissez d’installer SQL Server vous-même sur une machine virtuelle Azure, ou de provisionner une machine virtuelle Azure à partir d’un disque dur virtuel personnalisé, vous devez inscrire votre machine virtuelle SQL Server auprès du fournisseur de ressources de machine virtuelle SQL pour installer l’extension SQL IaaS Agent. 
...
Inscrivez votre fournisseur de ressources de machine virtuelle SQL dans votre abonnement Azure en utilisant Azure CLI ou Azure PowerShell. 
...
L’installation de l’extension en mode d’administration complet redémarre le service SQL Server ; il est donc recommandé d’installer tout d’abord l’extension en mode léger, puis de procéder à la [mise à niveau vers le mode complet](#upgrade-to-full) pendant une fenêtre de maintenance. 

### <a name="lightweight-management-mode"></a>Mode de gestion léger

Utilisez l’interface Azure CLI ou Azure PowerShell pour inscrire votre machine virtuelle SQL Server auprès du fournisseur de ressources et installer l’extension SQL IaaS en mode léger. Le service SQL Server ne sera pas redémarré. Vous pouvez ensuite effectuer une mise à niveau vers le mode complet à tout moment ; cependant, comme cela entraîne le redémarrage du service SQL Server, nous vous recommandons d’attendre une fenêtre de maintenance planifiée. 
...
Les instances de SQL Server 2008 et 2008 R2 installées sur Windows Server 2008 ( _non R2_ ) peuvent être inscrites auprès du fournisseur de ressources de machines virtuelles SQL en [mode sans agent](sql-server-iaas-agent-extension-automate-management.md#management-modes). Cette option garantit la conformité et permet de surveiller la machine virtuelle SQL Server dans le portail Azure avec des fonctionnalités limitées.

Spécifiez `AHUB`, `PAYG` ou `DR` pour **sqlLicenseType** et `SQL2008-WS2008` ou `SQL2008R2-WS2008` pour **sqlImageOffer**. 

Pour inscrire votre SQL Server 2008 ou 2008 R2 sur une instance Windows Server 2008, utilisez l’extrait de code Azure PowerShell ou Azure CLI suivant : 
...
Inscrire votre machine virtuelle SQL Server 2008 en mode sans agent avec Azure PowerShell : 
...
Inscrire votre machine virtuelle SQL Server 2008 R2 en mode sans agent avec Azure PowerShell : 
...
Exécutez l’extrait de code Azure PowerShell suivant :
...
Vous pouvez vérifier si votre machine virtuelle SQL Server a déjà été inscrite auprès du fournisseur de ressources de machine virtuelle SQL via le portail Azure, l’interface Azure CLI ou Azure PowerShell. 
...
Pour désinscrire votre machine virtuelle SQL Server auprès du fournisseur de ressources à l’aide d’Azure PowerShell, utilisez la commande [Remove-AzSqlVM](/powershell/module/az.sqlvirtualmachine/remove-azsqlvm). Cela supprimera la *ressource* de machine virtuelle SQL Server, mais ne supprimera pas la machine virtuelle. 
