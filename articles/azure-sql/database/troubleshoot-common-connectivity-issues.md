...
Si votre programme client se connecte à votre base de données dans SQL Database à l’aide de la classe .NET Framework **System.Data.SqlClient.SqlConnection**, utilisez .NET 4.6.1 ou une version ultérieure (ou .NET Core) afin de pouvoir utiliser la fonctionnalité de nouvelle tentative de connexion. Pour plus d’informations sur la fonctionnalité, consultez [cette page web](https://docs.microsoft.com/dotnet/api/system.data.sqlclient.sqlconnection).
