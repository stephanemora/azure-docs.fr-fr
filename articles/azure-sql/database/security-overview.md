...
description: En savoir plus sur la sécurité dans Azure SQL Database et Azure SQL Managed Instance, notamment sur les différences avec SQL Server.
...
ms.locfileid: "84266047"
---
# <a name="an-overview-of-azure-sql-database--sql-managed-instance-security-capabilities"></a>Vue d’ensemble des capacités de sécurité d’Azure SQL Database et SQL Managed Instance

[!INCLUDE[appliesto-sqldb-sqlmi](../includes/appliesto-sqldb-sqlmi.md)]

Cet article décrit les principes de la sécurisation de la couche Données d’une application avec [Azure SQL Database](sql-database-paas-overview.md) et [Azure SQL Managed Instance](../managed-instance/sql-managed-instance-paas-overview.md). La stratégie de sécurité décrite suit une approche en couches de défense en profondeur comme illustré ci-après, et se déplace de l’extérieur vers :

![sql-security-layer.png](./media/security-overview/sql-security-layer.png)

## <a name="network-security"></a>Sécurité du réseau

Microsoft Azure SQL Database et SQL Managed Instance fournissent un service de base de données relationnelle pour les applications cloud et d’entreprise. Pour protéger les données client, les pare-feu empêchent tout accès réseau au serveur, sauf octroi d’accès explicite en fonction de l’adresse IP ou de l’origine du trafic de réseau virtuel Azure.
...
> [!NOTE]
> Le contrôle d’accès avec des règles de pare-feu ne s’applique *pas* à une **instance gérée SQL**. Pour plus d’informations sur la configuration de la mise en réseau requise, consultez [Connexion à une instance gérée SQL](../managed-instance/connect-application-instance.md).
...
SQL Database et SQL Managed Instance sécurisent les données des clients en utilisant le protocole [Transport Layer Security](https://support.microsoft.com/help/3135244/tls-1-2-support-for-microsoft-sql-server) pour chiffrer les données en mouvement.

SQL Database et SQL Managed Instance appliquent en permanence le chiffrement (SSL/TLS) aux connexions. De cette façon, toutes les données sont chiffrées « en transit » entre le client et le serveur, quel que soit le paramètre **Encrypt** ou **TrustServerCertificate** de la chaîne de connexion.
...
[L’évaluation des vulnérabilités ](sql-vulnerability-assessment.md) est un service simple à configurer, qui vous permet de découvrir, suivre et vous aide à corriger des vulnérabilités de base de données potentielles, avec pour objectif d’améliorer de manière proactive la sécurité globale des bases de données. L’évaluation des vulnérabilités (VA) fait partie de l’offre Advanced Data Security, qui est un package unifié de capacités avancées de sécurité SQL. L’évaluation des vulnérabilités est accessible et peut être gérée par le biais du portail SQL Advanced Data Security central.
