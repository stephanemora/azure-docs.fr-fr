...
Une fois le [serveur logique](./logical-servers.md) supprimé, une période minimale de sept jours est observée avant que le nom soit libéré des enregistrements. Au cours de cette période, l’Instance managée SQL portant le même nom ne peut pas être créée. En guise de solution de contournement, vous devez utiliser un nom différent pour l’Instance managée SQL ou créer un ticket de support pour libérer un nom de serveur logique.
