...
ms.locfileid: "84038080"
---
# <a name="business-critical-tier---azure-sql-database--sql-managed-instance"></a>Niveau Critique pour l'entreprise - Azure SQL Database et SQL Managed Instance 
...
Le modèle de niveau de service Premium/Critique pour l’entreprise est basé sur un cluster de processus de moteur de base de données. Ce modèle d'architecture s'appuie sur le fait qu'un quorum de nœuds de moteur de base de données est toujours disponible. Celui-ci n'a qu'un impact minime sur les performances de votre charge de travail, même pendant les activités de maintenance. Le niveau de service Hyperscale n'est actuellement disponible que pour Azure SQL Database (et non pour SQL Managed Instance). Ce niveau, qui offre des performances de calcul et un stockage hautement évolutifs, tire parti de l'architecture Azure pour effectuer un scale-out du stockage et des ressources de calcul d'une instance d'Azure SQL Database bien au-delà des limites disponibles pour les niveaux de service Usage général et Critique pour l'entreprise.
...
-   **Exigences de faible latence pour les E/S** : les charges de travail nécessitant une réponse rapide de la couche de stockage (1 à 2 millisecondes en moyenne) doivent utiliser le niveau Critique pour l'entreprise. 
