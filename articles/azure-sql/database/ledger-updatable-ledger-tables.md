...
description: Cet article fournit des informations sur les tables de registre pouvant être mises à jour, le schéma du registre et les vues du registre dans Azure SQL Database
ms.custom: ''
ms.date: 05/25/2021
...
Les tables de registre pouvant être mises à jour sont les tables avec version système sur lesquelles les utilisateurs peuvent effectuer des mises à jour et des suppressions tout en fournissant également des fonctionnalités de preuve de falsification. Lorsque des mises à jour ou des suppressions se produisent, toutes les versions antérieures d’une ligne sont conservées dans une table secondaire, appelée table d’historique. La table d’historique reflète le schéma de la table de registre pouvant être mise à jour. Lorsqu’une ligne est mise à jour, la dernière version de la ligne reste dans la table de registre, tandis que sa version antérieure est insérée dans la table d’historique par le système, de manière transparente pour l’application. 
...
Vous pouvez utiliser les deux technologies conjointement en créant des tables qui sont à la fois des tables de registre et des tables temporelles pouvant être mises à jour. La création d’une table de registre pouvant être mise à jour peut se faire de deux manières :

- Lors de la création d’une nouvelle base de données dans le portail Azure, en sélectionnant **Activer le registre sur toutes les futures tables de cette base de données** pendant la configuration du registre, ou en spécifiant l’argument `LEDGER = ON` dans votre instruction [CREATE DATABASE (Transact-SQL)](/sql/t-sql/statements/create-database-transact-sql). Cela crée une base de données de registre, ce qui garantit que toutes les futures tables créées dans votre base de données sont des tables de registre pouvant être mises à jour par défaut.
- Lors de la création d’une table dans une base de données où le registre n’est pas activé au niveau de la base de données, en spécifiant l’argument `LEDGER = ON` dans votre instruction [CREATE TABLE (Transact-SQL)](/sql/t-sql/statements/create-table-transact-sql).
...
> [!IMPORTANT]
> Une fois créée, une table de registre ne peut pas être convertie en table qui n’est pas une table de registre. Cela permet de la protéger contre un attaquant qui voudrait temporairement supprimer les fonctionnalités sur une table de registre pour effectuer des changements, puis réactiver les fonctionnalités de registre. 
...
> [!NOTE]
> Si vous ne spécifiez pas les colonnes `GENERATED ALWAYS` requises de la table de registre et de la table d’historique de registre dans l’instruction [CREATE TABLE (Transact-SQL)](/sql/t-sql/statements/create-table-transact-sql?view=azuresqldb-current&preserve-view=true), le système ajoutera automatiquement les colonnes et utilisera les noms par défaut ci-dessous. Pour plus d’informations, consultez nos exemples de [Création d’une table de registre pouvant être mise à jour](/sql/t-sql/statements/create-table-transact-sql?view=azuresqldb-current&preserve-view=true#x-creating-a-updatable-ledger-table).
...
Lorsque vous créez une table de registre pouvant être mise à jour, vous pouvez spécifier le nom du schéma qui doit contenir votre table d’historique et le nom de la table d’historique, ou bien le système génère le nom de la table d’historique et l’ajoute au même schéma que la table de registre. Les tables d’historique avec des noms générés par le système sont appelées des tables d’historique anonymes. La convention d’affectation de noms pour une table d’historique anonyme est `<schema>`.`<updatableledgertablename>`.MSSQL_LedgerHistoryFor_`<GUID>`.

## <a name="ledger-view"></a>Vue du registre

Pour chaque table de registre pouvant être mise à jour, le système génère automatiquement une vue, appelée vue de registre. La vue du registre est une jointure de la table de registre pouvant être mise à jour et de la table d’historique associée. La vue du registre signale toutes les modifications de ligne qui se sont produites dans la table de registre pouvant être mise à jour en joignant les données d’historique dans la table d’historique. Cela permet aux utilisateurs, à leurs partenaires ou aux auditeurs d’analyser toutes les opérations historiques et de détecter les falsifications potentielles. Chaque opération de ligne est accompagnée de l’ID de la transaction d’action, ainsi qu’une indication sur la nature de l’opération, `DELETE` ou `INSERT`. Les utilisateurs peuvent récupérer plus d’informations sur le moment où la transaction a été exécutée et l’identité de l’utilisateur qui l’a exécutée, et les mettre en corrélation avec d’autres opérations effectuées par cette transaction.

Par exemple, si vous souhaitez effectuer le suivi de l’historique des transactions pour un scénario d’opérations bancaires simple, la vue du registre est très utile pour fournir une chronique des transactions dans le temps, au lieu d’avoir à afficher indépendamment la table de registre et les tables d’historique pouvant être mises à jour, ou à créer votre propre vue.
