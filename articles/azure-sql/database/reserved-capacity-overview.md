...
- Pour les abonnements Entreprise, **Add Reserved Instances** (Ajouter des instances réservées) doit être activé dans le [portal EA](https://ea.azure.com). Si ce paramètre est désactivé, vous devez être administrateur EA de l’abonnement. utilisé pour payer la capacité de réserve.
...
La taille de la réservation doit s’appuyer sur le nombre total de calculs utilisés par les bases de données SQL ou instances gérées SQL existantes ou sur le point d’être déployées dans une région spécifique et utilisant le même niveau de performance et la même génération de matériel.
...
4. Renseignez les champs obligatoires. Les bases de données SQL (uniques ou mises en pool) ou instances gérées SQL existantes, correspondant aux attributs que vous sélectionnez, peuvent prétendre à la remise de capacité de réserve. Le nombre réel de vos bases de données SQL ou de vos instances gérées SQL qui obtiennent la remise dépend de l’étendue et de la quantité sélectionnées.
...
| Champ      | Description|
|------------|--------------|
|Abonnement|Abonnement utilisé pour payer la réservation de capacité. Les coûts initiaux de la réservation sont facturés au mode de paiement défini sur l’abonnement. Le type d’abonnement doit être Accord Entreprise (numéros de l’offre : MS-AZR-0017P ou MS-AZR-0148p) ou un accord individuel avec paiement à l’utilisation (numéros de l’offre : MS-AZR-0003P ou MS-AZR-0023P). Pour un abonnement Entreprise, les frais sont déduits du solde d’engagement monétaire de l’inscription ou facturés comme un dépassement. Pour un abonnement individuel avec paiement à l’utilisation, les frais sont facturés sur le mode de paiement par carte de crédit ou par facture sur l’abonnement.|
|Étendue       |L’étendue de la réservation vCore peut couvrir un seul abonnement ou plusieurs abonnements (étendue partagée). Si vous sélectionnez : <br/><br/>**Partagée** : la remise de réservation vCore est appliquée aux bases de données SQL ou instances gérées SQL en cours d’exécution dans tous les abonnements de votre contexte de facturation. Pour les clients Entreprise, l'étendue partagée correspond à l'inscription et inclut tous les abonnements compris dans l'inscription. Pour les clients Paiement à l’utilisation, l’étendue partagée correspond à tous les abonnements Paiement à l’utilisation créés par l’administrateur de compte.<br/><br/>**Abonnement unique** : la remise de réservation vCore est appliquée aux bases de données SQL ou instances gérées SQL incluses dans cet abonnement. <br/><br/>**Groupe de ressources unique** : la remise de réservation est appliquée aux bases de données SQL ou instances gérées SQL incluses dans l’abonnement sélectionné et dans le groupe de ressources sélectionné au sein de cet abonnement.|
...
|Niveau de performances|Niveau de service pour les bases de données SQL ou les instances gérées SQL. |
|Terme        |Une année ou trois ans.|
|Quantité    |Quantité de ressources de calcul achetées au sein de la réservation de capacité. La quantité correspond au nombre de vCores de la région Azure et du niveau de performance sélectionnés qui est réservé et qui bénéficie de la remise de facturation. Par exemple, si vous exécutez ou que vous envisagez d’exécuter plusieurs bases de données SQL avec la capacité de calcul totale de 16 vCores Gen5 dans la région USA Est, vous devez spécifier une quantité de 16 pour optimiser l’avantage pour toutes les bases de données. |
...
La remise de réservation vCore est appliquée automatiquement au nombre de bases de données SQL ou d’instances gérées SQL qui correspondent à l’étendue et aux attributs de la réservation de capacité. Vous pouvez mettre à jour l’étendue de la réservation de capacité par le biais du [portail Azure](https://portal.azure.com), de PowerShell, de CLI ou de l’API.
