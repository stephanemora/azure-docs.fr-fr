...
Dans les étapes suivantes, vous allez exécuter cet exemple de script R dans votre base de données :
...
1. Utilisez l’instruction `SELECT` pour interroger la table.
    ...
    **Résultats**
    ...
    ![Contenu de la table RTestData](./media/r-script-create-quickstart/select-rtestdata.png)
...
1. Exécutez le script R suivant. Il récupère les données de la table en utilisant l’instruction `SELECT`, les transmet via le runtime R et retourne les données sous forme de trame de données. La clause `WITH RESULT SETS` définit le schéma de la table de données retournées pour SQL Database en ajoutant le nom de colonne *NewColName*.
    ...
    **Résultats**
    ...
    ![Sortie du script R qui retourne des données d’une table](./media/r-script-create-quickstart/r-output-rtestdata.png)
...
1. Vous pouvez aussi générer des valeurs en utilisant simplement le script R sans données d’entrée (`@input_data_1` est vide).
    ...
    **Résultats**
    ...
    ![Résultats de la requête en utilisant @script comme entrée](./media/r-script-create-quickstart/r-data-generated-output.png)

## <a name="check-r-version"></a>Vérifier la version de R

Si vous souhaitez savoir quelle version de R est installée dans votre base de données, exécutez le script suivant.
...
Microsoft fournit un nombre de packages R préinstallés avec Machine Learning Services dans votre base de données.
...
![Packages installés dans R](./media/r-script-create-quickstart/r-installed-packages.png)
...
> [Créer et former un modèle prédictif dans R avec Azure SQL Database Machine Learning Services (préversion)](r-train-score-model-create-quickstart.md)
