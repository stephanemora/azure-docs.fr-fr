...
Pour prendre en charge les [déploiements au sein des réseaux virtuels Azure](../../virtual-network/virtual-network-for-azure-services.md) et assurer l’isolation et la sécurité des clients, SQL Managed Instance s’appuie sur des [clusters virtuels](connectivity-architecture-overview.md#high-level-connectivity-architecture). Un cluster virtuel représente un ensemble dédié de machines virtuelles isolées, déployées dans le sous-réseau du réseau virtuel du client. Fondamentalement, chaque déploiement d’instance managée sur un sous-réseau vide génère une nouvelle structure de cluster virtuel.

Les opérations ultérieures sur les instances managées déployées peuvent également avoir des effets sur le cluster virtuel sous-jacent. Elles influent sur la durée des opérations de gestion, car le déploiement de machines virtuelles supplémentaires s’accompagne d’une surcharge qui doit être prise en compte pour planifier de nouveaux déploiements ou des mises à jour d’instances gérées existantes.
...
## <a name="management-operations-duration"></a>Durée des opérations de gestion
...
- **Création de cluster virtuel** :  La création constitue une étape synchrone dans les opérations de gestion d’instance. **90 % des opérations se terminent dans les quatre heures**.
...
- **Attachement de fichiers de base de données à partir du Stockage Azure** :  étape synchrone. Exemples : scale-up ou scale-down du calcul (vCore) ou du stockage dans le niveau de service Usage général. **90 % de ces opérations ne prennent pas plus de cinq minutes**.
...
|Déploiement |Création d’une première instance de quatre vCore, dans un sous-réseau vide ou non|Création de cluster virtuel**|90 % des opérations se terminent dans les 4 heures.|
...
|**Suppression**|Suppression d’instance|Sauvegarde de la fin du journal pour toutes les bases de données|90 % des opérations se terminent en une minute.<br>Remarque : Si la dernière instance du sous-réseau est supprimée, cette opération planifiera la suppression du cluster virtuel au bout de 12 heures.**|
...
\*\* 12 heures correspond à la configuration actuelle, qui est susceptible de changer à l’avenir. N’y attachez pas une dépendance stricte. Si vous devez supprimer un cluster virtuel plus tôt (par exemple pour libérer le sous-réseau), consultez [Supprimer un sous-réseau après avoir supprimé une instance managée](virtual-cluster-delete.md).
...
[![Annuler une opération](./media/management-operations-overview/canceling-operation.png)](./media/management-operations-overview/canceling-operation.png#lightbox)
...
En cas de réussite de la demande d’annulation, l’opération de gestion est annulée en quelques minutes, entraînant un échec.

![Résultat d’opération d’annulation](./media/management-operations-overview/canceling-operation-result.png)
...
Pour annuler l’opération de gestion, vous devez spécifier son nom. Par conséquent, vous devez d’abord utiliser la commande get pour récupérer la liste des opérations, puis annuler l’opération en question.
...
Pour une explication détaillée des commandes, consultez [az sql mi op](https://docs.microsoft.com/cli/azure/sql/mi/op?view=azure-cli-latest).
