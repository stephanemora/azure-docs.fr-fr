...
description: Découvrez comment activer l’accès privé pour les solutions Azure Digital Twins avec une liaison privée
author: baanders
ms.author: baanders
ms.date: 1/25/2021
ms.topic: how-to
...
Vous pouvez activer la liaison privée avec un point de terminaison privé pour une instance Azure Digital Twins dans le cadre de la configuration initiale de l’instance, ou l’activer ultérieurement sur une instance existante. 

L’une ou l’autre de ces méthodes de création donnera les mêmes options de configuration et le même résultat final pour votre instance. Cette section décrit comment appliquer chacune de ces méthodes dans le [portail Azure](https://portal.azure.com).
...
Dans cette section, vous allez utiliser le [portail Azure](https://portal.azure.com) pour activer la liaison privée avec un point de terminaison privé sur une instance Azure Digital Twins en cours de création. Cette section se concentre sur l’étape de mise en réseau du processus de création. Pour une procédure pas à pas complète de création d’une instance Azure Digital Twins, consultez [*Guide pratique : Configurer une instance et l’authentification*](how-to-set-up-instance-portal.md).
...
Dans cette section, vous allez utiliser le [portail Azure](https://portal.azure.com) pour activer la liaison privée avec un point de terminaison privé pour une instance Azure Digital Twins existante.
...
### <a name="option-1-using-the-azure-portal"></a>Option 1 : Utilisation du portail Azure
...
### <a name="option-2-using-the-azure-cli"></a>Option n°2 : Utilisation de l’interface de ligne de commande Azure (CLI)

Avec Azure CLI, cette action est effectuée à l’aide de la commande `az resource update`, comme indiqué ci-dessous. La propriété publicNetworkAccess d’une instance Azure Digital Twins peut être définie sur `disabled` ou `enabled`.

Pour désactiver l’indicateur, utilisez la commande suivante, en remplaçant les espaces réservés par les valeurs de votre instance.
...
Voici une capture d’écran qui montre à quoi ressemble la sortie.

:::image type="content" source="media/how-to-enable-private-link/network-flag-cli.png" alt-text="Capture d’écran d’une fenêtre de terminal exécutant la commande Azure CLI ci-dessus. La sortie contient les détails de l’instance, y compris une propriété appelée publicNetworkAccess avec la valeur Disabled (désactivée)." lightbox="media/how-to-enable-private-link/network-flag-cli.png":::

Pour activer l’indicateur, utilisez la commande similaire suivante.
...
### <a name="option-3-usingarmclient"></a>Option 3 : Utilisation d’ARMClient  
...
Pour désactiver l’accès public au réseau :
  
```cmd/sh
armclient login 
...
Pour activer l’accès public au réseau :  
  
```cmd/sh
armclient login 
