...
Pour pouvoir effectuer cette opération, vous devez être désigné comme propriétaire dans votre abonnement Azure. Vous pouvez vérifier que c’est le cas en exécutant la commande `az role assignment list --assignee <your-Azure-email>` et en vérifiant dans la sortie que *roleDefinitionName* a la valeur *Owner*. Si une autre valeur que *Owner* est définie, y compris *Contributor*, veuillez contacter l’administrateur de votre abonnement qui peut accorder des autorisations dans votre abonnement. Ils peuvent élever votre rôle sur l’intégralité de l’abonnement afin que vous puissiez exécuter la commande suivante, ou un propriétaire peut exécuter la commande suivante en votre nom pour configurer vos autorisations Azure Digital Twins pour vous.

Pour affecter les autorisations « propriétaire » de votre utilisateur dans votre instance Azure Digital Twins, utilisez la commande suivante (doit être exécutée par un propriétaire de l’abonnement Azure) :
...
Vous disposez maintenant d’une instance Azure Digital Twins opérationnelle et des permissions pour la gérer.
