...
| --- | --- |
| Intégration transparente |Utilise le protocole iSCSI pour lier de façon invisible les équipements de stockage de données. Les données stockées dans le cloud, dans un centre de données ou sur des serveurs distants semblent être stockées dans un emplacement unique. |
...
* **Appliance virtuelle StorSimple** : également appelée appareil virtuel StorSimple. Une version logicielle de l’appareil StorSimple qui réplique l’architecture et la plupart des capacités de l’appareil physique de stockage hybride. StorSimple Cloud Appliance s’exécute sur un nœud unique dans une machine virtuelle Azure. Les appareils virtuels Premium, qui tirent parti d’Azure Premium Storage, sont disponibles dans Update 2 et version ultérieure.
...
> [!IMPORTANT]
> Lorsque vous utilisez StorSimple, ne convertissez pas les blobs en archive, même si votre appareil est en phase de mise en attente. Pour récupérer des données de l’appareil, vous devez réhydrater les blobs de l’archivage vers le type chaud ou froid, ce qui entraîne des coûts importants.
...
| Partage de fichiers global <br></br> Collaboration |Talon  |[StorSimple avec Talon](https://www.theinfostride.com/talon-and-microsoft-to-host-azure-storsimple-web-conference-with-capita/) | |
