...
* Si vous cliquez sur **Instantané local** ou sur **Instantané cloud**, le volet **Résultats** affiche les informations suivantes sur chaque instantané de sauvegarde (selon les paramètres de votre **Vue**) :
  
  * **Heure** – Heure à laquelle l'instantané a été pris.
  * **Type** – Indique s'il s'agit d'un instantané local ou d'un instantané cloud.
  * **Propriétaire** – Propriétaire du contenu. 
  * **Disponible** : indique si l'instantané est actuellement disponible. **Vrai** indique que l'instantané est disponible et peut être restauré. **Faux** indique que l'instantané n'est plus disponible. 
...
* Si vous développez **Instantané local** ou **Instantané cloud**, puis que vous cliquez sur un nom d'instantané individuel, le volet **Résultats** affiche les informations suivantes sur l'instantané que vous avez sélectionné :
  
  * **Nom** – Volume identifié par la lettre de lecteur. 
  * **Nom local** : le nom local du lecteur (s'il est disponible). 
...
2. Dans le volet **Étendue**, développez le nœud **Catalogue de sauvegarde**, développez un groupe de volumes, puis cliquez sur **Instantanés locaux** ou **Instantanés sur le cloud**. Une liste d'instantanés de sauvegarde s'affiche dans le volet **Résultats**.
3. Recherchez la sauvegarde à restaurer, cliquez avec le bouton droit sur celle-ci, puis cliquez sur **Restaurer**.
...
5. Vous pouvez suivre la tâche de restauration en cours d’exécution. Dans le volet **Étendue**, développez le nœud **Travaux**, puis cliquez sur **En cours d'exécution**. Les détails des travaux s'affichent dans le volet **Résultats**. À l’issue de la tâche de restauration, les détails de la tâche sont transférés vers la liste **Dernières 24 heures** .
...
1. Cliquez sur l’icône de bureau pour démarrer le Gestionnaire d’instantanés StorSimple.
2. Dans le volet **Étendue**, développez le nœud **Catalogue de sauvegarde**, développez un groupe de volumes, puis cliquez sur **Instantanés sur le cloud**. Une liste de sauvegardes s'affiche dans le volet **Résultats**.
3. Recherchez le volume ou le groupe de volumes à cloner, cliquez avec le bouton droit sur le nom du volume ou du groupe de volumes, puis cliquez sur **Cloner**. La boîte de dialogue **Cloner un instantané sur le cloud** apparaît.
   
    ![Cloner un instantané cloud](./media/storsimple-snapshot-manager-manage-backup-catalog/HCS_SSM_Clone.png) 
4. Remplissez la boîte de dialogue **Cloner un instantané sur le cloud** comme suit : 
   
   1. Dans la zone de texte **Nom**, saisissez un nom pour le volume cloné. C'est le nom qui s'affiche sous le nœud **Volumes**. 
   ...
   4. Cliquez sur **Créer**.
5. Une fois le processus de clonage terminé, vous devez initialiser le volume cloné. Démarrez le Gestionnaire de serveur, puis l’outil Gestion des disques. Pour obtenir des instructions détaillées, consultez la section [Monter les volumes](storsimple-snapshot-manager-manage-volumes.md#mount-volumes). Après initialisation, le volume est répertorié sous le nœud **Volumes** dans le volet **Étendue**. Si le volume n’est pas répertorié, actualisez la liste des volumes (cliquez avec le bouton droit sur le nœud **Volumes**, puis cliquez sur **Actualiser**).
...
1. Cliquez sur l’icône de bureau pour démarrer le Gestionnaire d’instantanés StorSimple.
2. Dans le volet **Étendue**, développez le nœud **Catalogue de sauvegarde**, développez un groupe de volumes, puis cliquez sur **Instantanés locaux** ou **Instantanés sur le cloud**. Une liste d'instantanés s'affiche dans le volet **Résultats**.
3. Cliquez avec le bouton droit sur l'instantané que vous souhaitez supprimer, puis cliquez sur **Supprimer**.
...
3. Recherchez le volume que vous souhaitez cloner, cliquez dessus avec le bouton droit, puis cliquez sur **Cloner**. La boîte de dialogue **Cloner un instantané sur le cloud** apparaît.
...
4. Remplissez la boîte de dialogue **Cloner un instantané sur le cloud** comme suit : 
   ...
   3. (Facultatif) Sélectionnez **Dossier (NTFS)**, puis tapez un chemin de dossier ou cliquez sur **Parcourir** et sélectionnez un emplacement pour le dossier. 
   4. Cliquez sur **Créer**. 
5. Une fois le processus de clonage terminé, vous devez initialiser le volume cloné. Démarrez le Gestionnaire de serveur, puis l’outil Gestion des disques. Pour obtenir des instructions détaillées, consultez la section [Monter les volumes](storsimple-snapshot-manager-manage-volumes.md#mount-volumes). Après initialisation, le volume est répertorié sous le nœud **Volumes** dans le volet **Étendue**. 
...
6. Ouvrez le dossier NTFS qui contient le volume cloné, développez le nœud **Volumes**, puis ouvrez le volume cloné. Recherchez le fichier à récupérer, puis copiez-le dans le volume principal.
