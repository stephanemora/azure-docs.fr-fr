...
### <a name="obtain-a-red-hat-pull-secret-optional"></a>Obtenir un secret d’extraction Red Hat (facultatif)
...
Obtenez votre secret d’extraction en accédant à https://cloud.redhat.com/openshift/install/azure/aro-provisioned et en cliquant sur *Download pull secret* (Télécharger le secret d’extraction).
...
Exécutez la commande suivante pour créer un cluster. Notez les paramètres `apiserver-visibility` et `ingress-visibility`. Vous pouvez facultativement transmettre un secret d’extraction pour permettre au cluster d’accéder à des registres de conteneurs Red Hat et à du contenu supplémentaire. Obtenez votre secret d’extraction en accédant à la page [Red Hat OpenShift Cluster Manager](https://cloud.redhat.com/openshift/install/azure/installer-provisioned) et en cliquant sur Copy Pull Secret (Copier le secret d’extraction).
...
>[!IMPORTANT]
> Pour vous connecter à un cluster Azure Red Hat OpenShift privé, vous devez effectuer l’étape suivante à partir d’un hôte qui se trouve sur le réseau virtuel que vous avez créé ou sur un réseau virtuel [appairé](https://docs.microsoft.com/azure/virtual-network/virtual-network-peering-overview) avec le réseau virtuel sur lequel le cluster a été déployé.
