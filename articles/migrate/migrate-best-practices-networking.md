---
title: Meilleures pratiques de configuration du réseau pour les charges de travail migrées vers Azure | Microsoft Docs
...
- Les réseaux virtuels peuvent contenir les adresses IP privées et publiques définies dans [RFC 1918](https://tools.ietf.org/html/rfc1918), exprimées en notation CIDR. Les adresses IP publiques ne sont pas directement accessibles à partir d’Internet.
...
Lorsque vous planifiez votre topologie de réseau virtuel, vous devez prendre en compte un certain nombre d’aspects, notamment penser à la manière d’organiser les espaces d’adressage IP, comment implémenter un réseau hub-and-spoke, comment segmenter les réseaux virtuels en sous-réseaux, configurer le serveur DNS et implémenter les zones de disponibilité Azure.
...
## <a name="best-practice-implement-a-hub-spoke-network-topology"></a>Meilleure pratique : implémentation d’une topologie de réseau hub-and-spoke

Une topologie de réseau hub-and-spoke consiste à isoler des charges de travail tout en partageant des services tels que l’identité et la sécurité.
...
- Obtenir des recommandations de réseau pour l’exécution de machines virtuelles [Windows](https://docs.microsoft.com/azure/architecture/reference-architectures/n-tier/windows-vm#network-recommendations) et [Linux](https://docs.microsoft.com/azure/architecture/reference-architectures/n-tier/linux-vm#network-recommendations) dans Azure.
...
- Chaque passerelle VPN Azure comprend deux instances dans une configuration de type actif / passif.
    ...
    - En cas de problèmes non planifiés, la récupération de la connexion est plus longue et peut atteindre 1 minute trente dans le pire des cas.
...
- [Obtenir une vue d’ensemble](https://docs.microsoft.com/azure/vpn-gateway/vpn-gateway-about-vpngateway) des passerelles VPN.
...
- Le WAN virtuel vous permet de vous connecter et de configurer des appareils de branche pour communiquer avec Azure. Cela peut se faire manuellement ou à l’aide d’appareils de fournisseur de votre choix via un Virtual WAN partenaire.
...
Le service Azure ExpressRoute vous permet d’étendre votre infrastructure locale dans le cloud Microsoft en créant des connexions privées entre le centre de données virtuel Azure et des réseaux locaux.
...
Le déploiement de connexions ExpressRoute implique généralement la souscription d’un engagement auprès d’un fournisseur de services ExpressRoute. Pour un démarrage rapide, il convient généralement d’utiliser un réseau privé virtuel de site à site pour établir la connectivité entre le centre de données virtuel et les ressources locales, puis d’effectuer une migration vers une connexion ExpressRoute lorsqu’une interconnexion physique avec votre fournisseur de services est établie.

**En savoir plus :**
- [Obtenir une vue d’ensemble](https://docs.microsoft.com/azure/expressroute/expressroute-introduction) d’ExpressRoute.
...
**Problème** Imaginez à présent que vous déployiez un service Azure (par exemple, Azure App Service) dans les régions USA Ouest et USA Est.
...
- Cela fonctionne pour les utilisateurs de la côte est, mais pas pour ceux implantés à l’ouest. Le problème est le suivant :
    ...
    - Votre réseau WAN peut supposer que les deux préfixes sont plus proches de la région USA Est que de la région USA Ouest, et donc acheminer les utilisateurs des deux bureaux vers le circuit ExpressRoute de la région USA Est, ce qui affecte l’expérience des utilisateurs du bureau de Los Angeles.
...
Microsoft partage avec vous la responsabilité de la sécurisation des réseaux virtuels. Microsoft fournit de nombreuses fonctionnalités de mise en réseau, ainsi que des services conçus pour aider à sécuriser les ressources. En matière de sécurité des réseaux virtuels, il est recommandé de respecter un certain nombre de bonnes pratiques, notamment implémenter un réseau de périmètre, utiliser des groupes de sécurité et de filtrage, sécuriser l’accès aux ressources et aux adresses IP, ou encore implémenter un mécanisme de protection contre les attaques.
...
- Il est important d’exploiter les deux formes de protection, de sorte que si un pirate parvient à accéder au trafic de l’extérieur, il puisse être intercepté lorsqu’il tente de se connecter à des ordinateurs situés dans le même sous-réseau.
...
- Les adresses IP publiques sont créées avec une référence SKU de base ou standard, qui présentent certaines différences. Les références SKU standard peut être affectées à n’importe quel service, mais elles sont généralement configurées sur des machines virtuelles, des équilibreurs de charge et des passerelles d’application.
...
## <a name="leverage-azure-security-features-for-networking"></a>Tirer parti des fonctionnalités de sécurité Azure pour la mise en réseau
...
Le Pare-feu Azure est un service de sécurité réseau informatique managé qui protège vos ressources de réseau virtuel. Il s’agit d’un service de pare-feu entièrement avec état, pourvu d’une haute disponibilité intégrée et de l’extensibilité sans limites du cloud.

![Points de terminaison de service](./media/migrate-best-practices-networking/firewall.png)
*Pare-feu Azure*

- Dans un Pare-feu Azure, vous pouvez créer, appliquer et consigner des stratégies de connectivité réseau et d’application de façon centralisée entre les abonnements et les réseaux virtuels.
...
## <a name="best-practice-deploy-azure-web-application-firewall-waf"></a>Meilleure pratique : déploiement d’un pare-feu d’applications web (WAF) Azure
...
Le pare-feu d’applications web (WAF) Azure est une fonctionnalité de la passerelle d’application Azure.
- WAF protège de manière centralisée vos applications web contre les vulnérabilités et exploits courants.
- Cette fonctionnalité assure une protection sans modifier le code du serveur principal.
- Elle peut protéger plusieurs applications web derrière une passerelle d’application.
...
- Les NVA renforcent la sécurité et les fonctions réseau du réseau virtuel. Elles peuvent être déployées pour différentes applications : les pare-feu à haute disponibilité, la prévention des intrusions, la détection d’intrusions, les pare-feu d’applications web (WAF), l’optimisation WAN, le routage, l’équilibrage de charge, le VPN, la gestion des certificats, Active Directory et l’authentification multifacteur.
...
Dans le hub, le réseau de périmètre (disposant d’un accès à Internet) est normalement géré via un Pare-feu Azure, une batterie de pare-feu ou des pare-feu d’applications web (WAF). Voici un comparatif :

**Type de pare-feu** | **Détails**
--- | ---
WAF | Les applications web sont courantes et ont tendance à comporter diverses vulnérabilités face aux attaques potentielles.<br/><br/> Les WAF sont conçues pour détecter les attaques contre les applications web (HTTP/HTTPS), d’une manière plus précise qu’avec un pare-feu générique.<br/><br/> Contrairement à la technologie de pare-feu classique, les WAF intègrent un ensemble de fonctionnalités spécifiques pour protéger les serveurs web internes contre les menaces.
Pare-feu Azure | Comme les batteries de pare-feu NVA, le Pare-feu Azure utilise un mécanisme d’administration commune et un ensemble de règles de sécurité pour protéger les charges de travail hébergées dans les réseaux spoke, et pour contrôler l’accès aux réseaux locaux.<br/><br/> Le Pare-feu Azure dispose d’une évolutivité intégrée.
