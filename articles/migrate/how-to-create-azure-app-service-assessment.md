---
title: Créer une évaluation Azure App Service
description: Découvrez comment évaluer des applications web en vue de leur migration vers Azure App Service
author: rashi-ms
ms.author: rajosh
ms.topic: tutorial
ms.date: 07/28/2021
...
Cet article vous montre comment évaluer les applications web ASP.NET découvertes pour la migration vers Azure App Service, en utilisant l’outil Azure Migrate : Découverte et évaluation.

> [!Note]
> La détection et l’évaluation des applications web ASP.NET s’exécutant dans votre environnement VMware sont actuellement en préversion. Pour bien démarrer, suivez [ce tutoriel](tutorial-discover-vmware.md). Si vous voulez tester cette fonctionnalité dans un projet existant, vérifiez que vous avez rempli les [prérequis](how-to-discover-sql-existing-project.md) indiqués dans cet article.
...
- Pour créer une évaluation, vous devez configurer une appliance Azure Migrate pour [VMware](how-to-set-up-appliance-vmware.md). L’[appliance](migrate-appliance.md) découvre les serveurs locaux et envoie les métadonnées et les données de performances à Azure Migrate. La même appliance découvre les applications web ASP.NET s’exécutant dans votre environnement VMware.

## <a name="azure-app-service-assessment-overview"></a>Présentation de l’évaluation d’Azure App Service
...
[En savoir plus](concepts-azure-webapps-assessment-calculation.md) sur les évaluations Azure App Service.
