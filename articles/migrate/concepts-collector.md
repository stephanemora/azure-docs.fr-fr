...
- L’adresse IP/le nom de domaine complet (FQDN) du serveur proxy doit respecter le format *http:\//AdresseIP* ou *http:\//FQDN*.
...
*.oneget.org:443<br/><br/> *.github.com/oneget/oneget<br/><br/> *.windows.net:443<br/><br/> *.windowsazure.com:443<br/><br/> *.azure.microsoft.com<br/><br/> *.azure.microsoft.com/en-us<br/><br/> *.powershellgallery.com:443<br/><br/> *.msecnd.net:443<br/><br/> *.visualstudio.com:443<br/><br/> *.visualstudio.microsoft.com | Permet de télécharger le module PowerShell vCenter PowerCLI. | L’accès aux URL est obligatoire.<br/><br/> La vérification du prérequis n’échoue pas.<br/><br/> L’installation automatique du module sur la machine virtuelle Collecteur échoue. Vous devrez installer le module manuellement dans une machine disposant d’une connexion Internet, puis copier les modules dans l’appareil. [Apprenez-en plus en accédant à l’étape 4 de ce guide de résolution des problèmes](https://docs.microsoft.com/azure/migrate/troubleshooting-general#error-unhandledexception-internal-error-occurred-systemiofilenotfoundexception).
...
> [!NOTE]
> Les métadonnées découvertes par l’appliance Azure Migrate Collector sont utilisées pour vous aider à adapter la taille de vos applications lors de leur migration vers Azure, à effectuer l’analyse d’adéquation Azure, l’analyse des dépendances des applications et la planification des coûts. Microsoft n’utilise pas ces données dans le cadre d’un audit de conformité des licences.
...
Le collecteur est fourni avec une licence d’évaluation Windows Server 2016 qui est valide pendant 180 jours. Si la période d’évaluation arrive à expiration pour la machine virtuelle du collecteur, nous vous recommandons de télécharger un nouveau fichier OVA et de créer une appliance.
