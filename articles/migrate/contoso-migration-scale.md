---
title: Contoso - Mettre à l'échelle une migration vers Azure | Microsoft Docs
...
Dans cet article, la société Contoso opère une migration à échelle vers Azure. Elle réfléchit à la façon planifier et d’effectuer une migration de plus de 3 000 charges de travail, 8 000 bases de données et plus de 10 000 machines virtuelles.


Cet article fait partie d’une série d’articles qui décrivent la façon dont la société fictive Contoso migre ses ressources locales vers le cloud Microsoft Azure. La série comprend des informations générales et de planification, ainsi que des scénarios de déploiement qui illustrent la manière de configurer une infrastructure de migration, d’évaluer l’adéquation des ressources locales à une migration et d’exécuter différents types de migration. Les scénarios augmentent en complexité. Nous ajouterons des articles à la série au fil du temps.
...
- **Répondre à la croissance** : Contoso étant en pleine croissance, son infrastructure et ses systèmes locaux sont soumis à une charge importante.
...
- **Réduire les coûts de licences** : Contoso souhaite limiter autant que possible les coûts liés au cloud.
...
**Machines virtuelles** | Plus de 10 000 | Les machines virtuelles s’exécutent sur des hôtes VMware et sont gérées par des serveurs vCenter.
...
- **Phase 1 : Évaluation** : identifier les ressources actuelles et déterminer si elles se prêtent à une migration vers Azure.
- **Phase 2 : Migration** : déplacer les ressources vers Azure. La façon dont les applications et les objets seront déplacés vers Azure dépendra des applications elles-mêmes et des objectifs de la société.
- **Phase 3 : Optimisation** : après avoir déplacé les ressources vers Azure, Contoso doit les améliorer et les rationnaliser pour en optimiser les performances et l’efficacité.
- **Phase 4 : Sécurisation et gestion** : maintenant que tout est en place, Contoso tire parti de la sécurité Azure, des ressources et des services de gestion pour régir, sécuriser et superviser ses applications cloud dans Azure.
...
1. Détection : détecter les machines virtuelles VMware locales.
...
1. Évaluation de l’état de préparation : évaluer si les ordinateurs locaux se prêtent à une exécution dans Azure. L’évaluation comprend les éléments suivants :
    - Recommandations de taille : obtenir des recommandations de taille pour les machines virtuelles Azure, en fonction de l’historique des performances des machines virtuelles locales.
    - Estimation des coûts mensuels : obtenir une estimation des coûts d’exécution des ordinateurs locaux dans Azure.
2. Identification des dépendances : visualiser les dépendances des ordinateurs locaux pour créer des groupes d’ordinateurs optimaux pour l’évaluation et la migration.
...
Contoso doit utiliser Azure Migrate pour déterminer avec précision l’échelle de cette migration. 
...
- Les administrateurs de Contoso se documentent sur la façon d’effectuer un [déploiement à l’échelle avec Azure Migrate](how-to-scale-assessment.md).
...
#### <a name="partner-tools"></a>Outils de partenaires
...
## <a name="phase-2-migrate"></a>Phase 2 : Migration
...
Outre la réplication des machines virtuelles, Site Recovery a besoin d’un certain nombre de composants pour la migration de VMware.
...
Azure Database Migration Service (DMS) est un service entièrement géré conçu pour permettre des migrations sans interruption de plusieurs sources de base de données vers des plateformes de données Azure avec un temps d’arrêt minimum.
...
- Pendant la phase de provisionnement du service DMS, Contoso doit vérifier qu’il est correctement dimensionné et configuré pour optimiser les performances de migration des données. Contoso sélectionnera l’option « Niveau critique pour l’entreprise avec 4 vCores », permettant ainsi au service de tirer parti de plusieurs processeurs virtuels pour la parallélisation et de transferts de données plus rapides.
...
- Plusieurs [outils de partenaires](https://azure.microsoft.com/migration/partners/) peuvent aussi être utilisés pour la migration.
...
Pour exploiter au mieux son investissement cloud, Contoso entend utiliser l’outil gratuit Azure Cost Management.

- Cette solution sous licence conçue par Cloudyn, filiale de Microsoft, permettra à Contoso de gérer ses dépenses cloud avec précision et en toute transparence.  Cette solution propose des outils pour surveiller, répartir et réduire les coûts liés au cloud.
...
- Contoso entend exploite le travail réalisé par le service informatique de Microsoft et envisage d’implémenter le kit de ressources d’optimisation des ressources Azure (ARO).
...
Il est possible de mettre à profit des outils de partenaires comme [Hanu](https://hanu.com/insight/) et [Scalr]( https://www.scalr.com/cost-optimization/).


## <a name="phase-4-secure--manage"></a>Phase 4 : Sécurisation et gestion
