---
title: Évaluer des instances de machine virtuelle GCP en vue de leur migration vers Azure avec Azure Migrate Server Assessment
description: Explique comment évaluer des instances de machines virtuelles GVP en vue de leur migration vers Azure à l’aide d’Azure Migrate Server Assessment.
ms.topic: tutorial
ms.date: 09/14/2020
ms.custom: MVC
...
ms.locfileid: "91716367"
---
# <a name="tutorial-assess-google-cloud-platform-gcp-vm-instances-for-migration-to-azure"></a>Tutoriel : Évaluer des instances de machines virtuelles Google Cloud Platform (GCP) en vue de leur migration vers Azure

Dans le cadre de votre migration vers Azure, vous devez évaluer vos charges de travail locales pour déterminer le niveau de préparation du cloud, identifier les risques et estimer les coûts et la complexité de la tâche.

Cet article vous montre comment évaluer des instances de machines virtuelles Google Cloud Platform (GCP) en vue de leur migration vers Azure à l’aide d’Azure Migrate : Server Assessment.
...
- Avant d’effectuer les étapes de ce tutoriel, vous devez suivre le premier tutoriel de cette série afin de [savoir comment découvrir votre inventaire local](tutorial-discover-gcp.md). 


## <a name="decide-which-assessment-to-run"></a>Décider de l’évaluation à exécuter

Déterminez si votre évaluation doit s’appuyer sur des critères de dimensionnement basés sur les données/métadonnées de configuration de machines collectées localement en l’état ou sur des données de performances.
...
**Basée sur les performances** | Pour une évaluation basée sur les données de performances collectées. | La taille recommandée pour la machine virtuelle Azure est basée sur les données d’utilisation du processeur et de la mémoire.<br/><br/> Le type de disque recommandé est basé sur les IOPS et le débit des disques locaux.
...
1. Dans la page **Serveurs** > **Serveurs Windows et Linux**, cliquez sur **Évaluer et migrer des serveurs**.
   ...
   ![Emplacement du bouton Évaluer et migrer des serveurs](./media/tutorial-assess-gcp/assess.png)

2. Dans **Azure Migrate : Server Assessment, cliquez sur **Évaluer**.

    ![Emplacement du bouton Évaluer](./media/tutorial-assess-gcp/assess-servers.png)
...
6. Cliquez sur **Tout afficher** pour passer en revue les propriétés de l’évaluation.
    ...
    ![Emplacement du bouton Tout afficher pour passer en revue les propriétés de l’évaluation](./media/tutorial-assess-gcp/assessment-name.png)
...
7. Dans **Propriétés d’évaluation** > **Propriétés de la cible** :
    ...
    - Dans **Instances réservées**, indiquez si vous souhaitez utiliser des instances réservées pour la machine virtuelle pendant sa migration.
...
10. Si vous avez apporté des modifications, cliquez sur **Enregistrer**.
    ...
    ![Propriétés de l’évaluation](./media/tutorial-assess-gcp/assessment-properties.png)
...
2. Dans **Évaluations**, sélectionnez une évaluation pour l’ouvrir. Voici un exemple (estimations et coûts, à titre d’exemple uniquement) : 
    ...
    ![Récapitulatif de l’évaluation](./media/tutorial-assess-gcp/assessment-summary.png)
...
![Niveau de confiance](./media/tutorial-assess-gcp/confidence-rating.png)
