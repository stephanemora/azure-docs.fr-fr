---
title: (DÉPRÉCIÉ) Principal de service de cluster Azure Kubernetes
...
ms.locfileid: "85504737"
---
# <a name="deprecated-set-up-an-azure-ad-service-principal-for-a-kubernetes-cluster-in-container-service"></a>(DÉPRÉCIÉ) Configurer un principal de service Azure Active Directory pour un cluster Kubernetes dans Container Service

> [!TIP]
> Pour obtenir la version mise à jour de cet article qui utilise Azure Kubernetes Service, consultez [Principaux de service avec Azure Kubernetes Service (AKS)](../../aks/kubernetes-service-principal.md).
...
> [!IMPORTANT]
> Pour créer un principal de service, vous devez disposer des autorisations suffisantes pour inscrire une application auprès de votre client Azure AD et l’affecter à un rôle dans votre abonnement. Si voir si vous disposez des autorisations requises, [procédez à une vérification dans le portail](../../active-directory/develop/howto-create-service-principal-portal.md#permissions-required-for-registering-an-app).
>

## <a name="option-1-create-a-service-principal-in-azure-ad"></a>Option 1 : Créer un principal de service dans Azure AD
...
## <a name="option-2-generate-a-service-principal-when-creating-the-cluster-with-az-acs-create"></a>Option n°2 : générer un principal de service lors de la création du cluster avec `az acs create`
...
Si les informations d’identification de votre principal de service ont expiré, utilisez la commande [az ad sp reset-credentials](/cli/azure/ad/sp) pour mettre à jour les informations d’identification :
