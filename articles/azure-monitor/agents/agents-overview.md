...
- Envoyer les données à des outils tiers à l’aide d’[Azure Event Hubs](../platform/diagnostics-extension-stream-event-hubs.md).
...
* Envoyer les données à un espace de travail Log Analytics pour tirer parti des fonctionnalités prises en charge par les [journaux Azure Monitor](../platform/data-platform-logs.md) comme les [requêtes de journal](../log-query/log-query-overview.md).
* Utiliser [Azure Monitor pour machines virtuelles](../insights/vminsights-overview.md), qui vous permet de superviser vos machines à grande échelle, y compris leurs processus et dépendances vis-à-vis d’autres ressources et de processus externes.  
...
- Envoyer les données aux [métriques Azure Monitor](../platform/data-platform-metrics.md) pour les analyser avec [Metrics Explorer](../platform/metrics-getting-started.md) et tirer parti des fonctionnalités telles que les [alertes de métrique](./../platform/alerts-metric-overview.md) en temps quasi-réel et la [mise à l’échelle automatique](../platform/autoscale-overview.md) (Windows uniquement).
- Envoyer les données à des outils tiers à l’aide d’[Azure Event Hubs](../platform/diagnostics-extension-stream-event-hubs.md).
...
L’[agent InfluxData Telegraf](../platform/collect-custom-metrics-linux-telegraf.md) est utilisé pour collecter les données de performances des ordinateurs Linux sur les métriques Azure Monitor.
...
<sup>1</sup> Nécessite l’installation de Python 3 sur la machine.

<sup>2</sup> Problème connu de collecte d’événements Syslog. Seules les données de performances sont actuellement prises en charge.
...
- [Vue d’ensemble de l’agent Log Analytics](../platform/log-analytics-agent.md)
- [Vue d’ensemble de l’extension Diagnostics Azure](../platform/diagnostics-extension-overview.md)
- [Collecter des métriques personnalisées pour une machine virtuelle Linux avec l’agent InfluxData Telegraf](../platform/collect-custom-metrics-linux-telegraf.md)
