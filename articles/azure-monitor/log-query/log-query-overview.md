...
- **Portails.** Vous pouvez effectuer une analyse interactive des données de journal dans le [portail Azure](../../azure-monitor/log-query/portals.md).  Cela vous permet de modifier votre requête et d’analyser les résultats dans divers formats et visualisations.  
- **Règles d’alerte.** Les [règles d’alerte](../../azure-monitor/platform/alerts-overview.md) identifient de façon proactive les problèmes à partir des données dans votre espace de travail.  Chaque règle d’alerte est basée sur une recherche dans les journaux qui est exécutée automatiquement à intervalles réguliers.  Les résultats sont inspectés pour déterminer si une alerte doit être créée.
...
- **Exportation.**  Quand vous importez des données de l’espace de travail Log Analytics vers Excel ou [Power BI](../../azure-monitor/platform/powerbi.md), vous créez une requête de journal pour définir les données à exporter.
...
- En savoir plus sur les [portails que vous utilisez pour créer et modifier des recherches dans les journaux](../../azure-monitor/log-query/portals.md).
- Consultez un [didacticiel sur l’écriture de requêtes](../../azure-monitor/log-query/get-started-queries.md) à l’aide du nouveau langage de requête.
