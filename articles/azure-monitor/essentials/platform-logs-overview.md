...
- Affichez le journal d’activité dans le portail Azure et accédez aux événements à partir de PowerShell et de l’interface CLI. Pour plus d’informations, consultez [Afficher le journal d’activité](../essentials/activity-log.md#view-the-activity-log). 
...
Vous pouvez envoyer des journaux de plateforme à une ou plusieurs des destinations répertoriées dans le tableau suivant, en fonction de vos besoins d’analyse. Configurez des destinations pour les journaux de plateforme en [créant un paramètre de diagnostic](../essentials/diagnostic-settings.md).
