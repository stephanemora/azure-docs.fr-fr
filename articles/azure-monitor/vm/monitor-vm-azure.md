...
Les machines virtuelles dans Azure génèrent les données suivantes pour l’hôte de machine virtuelle comme pour les autres ressources Azure, comme décrit dans [Données de supervision](../insights/monitor-azure-resource.md#monitoring-data).
...
- [Agent Log Analytics](../agents/agents-overview.md#log-analytics-agent) : disponible pour les machines virtuelles dans Azure, dans d’autres environnements cloud et localement. Collecte les données dans les journaux Azure Monitor. Prend en charge les solutions de supervision et Azure Monitor pour machines virtuelles. Il s’agit du même agent que celui utilisé pour System Center Operations Manager.
- [Dependency Agent](../agents/agents-overview.md#dependency-agent) : collecte les données des processus exécutés sur la machine virtuelle et leurs dépendances. S’appuie sur l’agent Log Analytics pour transmettre des données dans Azure et prend en charge les solutions Azure Monitor pour machines virtuelles, Service Map et Wire Data 2.0.
...
[Azure Monitor pour machines virtuelles](../vm/vminsights-overview.md) est un [insight](../monitor-reference.md) dans Azure Monitor qui est le principal outil de supervision des machines virtuelles dans Azure Monitor. Il fournit la valeur ajoutée suivante par rapport aux fonctionnalités standard d’Azure Monitor.
...
![Affichage du niveau de performance d’Azure Monitor pour machines virtuelles](media/monitor-vm-azure/vminsights-01.png)

![Affichage des mappages Azure Monitor pour machines virtuelles](media/monitor-vm-azure/vminsights-02.png)
...
| Insights | Ouvre [Azure Monitor pour machines virtuelles](../vm/vminsights-overview.md) avec la carte de la machine virtuelle sélectionnée. |
...
Azure Monitor pour machines virtuelles active la collecte d’un ensemble prédéterminé de compteurs de performances écrits dans la table *InsightsMetrics*. Il s’agit de la même table que celle utilisée par [Azure Monitor pour conteneurs](../insights/container-insights-overview.md). 

| Source de données | Spécifications | Tables |
|:---|:---|:---|
| Azure Monitor pour machines virtuelles | Activez-la sur chaque machine virtuelle. | InsightsMetrics<br>VMBoundPort<br>VMComputer<br>VMConnection<br>VMProcess<br>Consultez [Comment interroger les journaux d’activité d’Azure Monitor pour machines virtuelles](../vm/vminsights-log-search.md) pour plus d’informations. |
