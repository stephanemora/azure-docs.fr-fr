...
description: Découvrez les journaux de diagnostic Azure dans Azure Monitor et comment les utiliser pour comprendre les événements qui se produisent au sein d’une ressource Azure.
author: nkiest
...
Les **journaux de diagnostic** fournissent des informations complètes et fréquentes sur le fonctionnement d’une ressource Azure. Azure Monitor propose deux types de journaux de diagnostic :

* **Journaux d’activité de locataire** : ces journaux d’activité proviennent des services au niveau du locataire qui existent en dehors d’un abonnement Azure, tels que les journaux d’activité Azure Active Directory.
* **Journaux d’activité de ressources** : ces journaux d’activité proviennent des services Azure qui déploient des ressources au sein d’un abonnement Azure, tels que les groupes de sécurité réseau ou les comptes de stockage.
...
* [Envoyez-les à **Event Hubs**](diagnostic-logs-stream-event-hubs.md) pour ingestion par un service tiers ou une solution d’analytique personnalisée comme Power BI.
...
* La durée pendant laquelle chaque catégorie de journal doit être conservée dans un compte de stockage
    - Une durée de rétention de zéro jour signifie que les journaux d’activité sont conservés indéfiniment. La valeur peut également être n’importe quel nombre de jours, compris entre 1 et 365.
...
Ces paramètres peuvent être configurés à partir des paramètres de diagnostics du portail Azure, à l’aide des commandes Azure PowerShell et de l’interface CLI, ou à l’aide de l’[API REST Azure Monitor](https://docs.microsoft.com/rest/api/monitor/).
