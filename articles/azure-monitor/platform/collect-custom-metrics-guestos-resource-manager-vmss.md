---
title: Envoyer des métriques de système d’exploitation invité vers le magasin de métriques d’Azure Monitor à l’aide d’un modèle Azure Resource Manager pour un groupe de machines virtuelles identiques Windows
...
1. Créez un nouveau groupe de ressources pour la machine virtuelle déployée. Exécutez la commande suivante : 
   ...
   > [!NOTE]  
   > N’oubliez pas d’utiliser une région Azure dans laquelle les mesures personnalisées sont activées. N’oubliez pas d’utiliser une [région Azure dans laquelle les mesures personnalisées sont activées](https://github.com/MicrosoftDocs/azure-docs-pr/pull/metrics-custom-overview.md#supported-regions).
