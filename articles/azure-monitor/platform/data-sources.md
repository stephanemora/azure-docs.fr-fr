...
L’agent Log Analytics permet de superviser et de gérer en détail vos machines virtuelles ou votre ordinateur physique Windows ou Linux. La machine virtuelle peut s’exécuter dans Azure, dans un autre cloud ou en local et l’agent se connecte à Log Analytics soit directement, soit par l’intermédiaire du System Center Operations Manager et vous permet de collecter des données depuis des [sources de données](../../azure-monitor/platform/agent-data-sources.md) que vous configurez ou depuis des [solutions de supervision](../../azure-monitor/insights/solutions.md) qui fournissent des informations supplémentaires sur les applications exécutées sur la machine virtuelle.

### <a name="dependency-agent"></a>Agent de dépendances
[Service Map](../insights/service-map.md) et [Azure Monitor pour machines virtuelles](../../azure-monitor/insights/vminsights-overview.md) nécessitent une instance de Dependency Agent sur les machines virtuelles Windows et Linux. Celle-ci opère avec l’agent Log Analytics pour collecter les données découvertes sur les processus exécutés sur la machine virtuelle et les dépendances de processus externes. Elle stocke ces données dans Log Analytics et visualise les composants interconnectés découverts.  
