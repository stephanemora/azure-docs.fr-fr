...
L'agent Azure Log Analytics, précédemment appelé Microsoft Monitoring Agent (MMA) ou agent OMS Linux, a été développé fournir une gestion complète sur plusieurs machines locales, ordinateurs surveillés par [System Center Operations Manager](https://docs.microsoft.com/system-center/scom/) et machines virtuelles situées dans un cloud. Les agents Windows et Linux attachement à un Azure Monitor et stocker des données de journal collectées à partir de différentes sources dans votre espace de travail Analytique de journal, ainsi que les journaux uniques ou les métriques tel que défini dans une solution de surveillance. 
...
Avant d’analyser et agir sur les données collectées, vous devez d’abord installer et connecter les agents pour tous les ordinateurs que vous souhaitez envoyer des données vers le service Azure Monitor. Vous pouvez installer des agents sur vos machines virtuelles Azure à l’aide de l’extension de machine virtuelle Log Analytics pour Windows, et pour les machines d'un environnement hybride à l’aide du programme d’installation, de la ligne de commande ou avec DSC (Desired State Configuration) dans Azure Automation. 

L’agent pour Linux et Windows communique en sortie pour le service Azure Monitor sur le port TCP 443 et si l’ordinateur se connecte via un pare-feu ou un serveur proxy pour communiquer sur Internet, passez en revue les conditions requises ci-dessous pour comprendre la configuration du réseau Obligatoire. Si vos stratégies de sécurité n’autorisent pas les ordinateurs sur le réseau pour se connecter à Internet, vous pouvez configurer un [passerelle d’Analytique de journal](gateway.md) , puis configurez l’agent pour se connecter via la passerelle vers les journaux Azure Monitor. L’agent peut recevoir des informations de configuration, puis envoyer les données collectées, selon les données, les règles de collecte et de surveillance des solutions vous avez activé dans votre espace de travail. 

Si vous analysez un ordinateur avec System Center Operations Manager 2012 R2 ou version ultérieure, il peut être multirésident avec le service Azure Monitor pour collecter des données et les transférer au service et toujours être surveillé par [Operations Manager](../../azure-monitor/platform/om-agents.md). Avec des ordinateurs Linux, l’agent n’inclut pas un composant de service de contrôle d’intégrité que l’agent Windows, et informations sont collectées et traite par un serveur d’administration en son nom. Étant donné que les ordinateurs Linux sont analysés différemment avec Operations Manager, ils ne pas recevoir de configuration ou collecter des données directement et transférer via le groupe d’administration, comme le fait d’un système géré par l’agent de Windows. Par conséquent, ce scénario n’est pas pris en charge avec les ordinateurs Linux à Operations Manager.  
...
Pour garantir la sécurité des données en transit vers les journaux Azure Monitor, nous vous encourageons vivement à configurer l’agent à utiliser au moins sécurité TLS (Transport Layer) 1.2. Les versions antérieures de TLS/SSL (Secure Sockets Layer) se sont avérées vulnérables et bien qu’elles fonctionnent encore pour assurer la compatibilité descendante, elles sont **déconseillées**.  Pour plus d’informations, passez en revue [Envoi sécurisé de données via TLS 1.2](../../azure-monitor/platform/data-security.md#sending-data-securely-using-tls-12). 

## <a name="network-firewall-requirements"></a>Configuration requise du pare-feu réseau
Les informations ci-dessous la liste des informations de configuration du pare-feu et de proxy requises pour l’agent Linux et Windows communiquer avec les journaux Azure Monitor.  
...
Connexion des ordinateurs dans votre abonnement Azure ou d’un environnement hybride directement avec les journaux d’Azure Monitor peut être accompli à l’aide de différentes méthodes selon vos besoins. Le tableau suivant décrit chacune d’entre elle, pour vous permettre d’identifier la plus adaptée à votre organisation.
...
| System Center Operations Manager|[Intégration d’Operations Manager à Log Analytics](../../azure-monitor/platform/om-agents.md) | Configurer l’intégration entre Operations Manager et Azure Monitor de journaux à transmettent les données collectées à partir d’ordinateurs Windows à un groupe d’administration.|  
