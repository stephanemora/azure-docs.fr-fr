...
Les **métriques de plateforme** sont créées par des ressources Azure et vous donnent une visibilité sur leur intégrité et leurs performances. Chaque type de ressource crée un [ensemble distinct de métriques](metrics-supported.md) sans aucune configuration requise. 
...
- Utiliser [Metrics Analytics](metrics-charts.md) pour analyser les métriques collectées et les représenter sur un graphique. Suivre les performances d’une ressource (par exemple, machine virtuelle, site web ou application logique) en épinglant des graphiques sur un [tableau de bord Azure](../../azure-portal/azure-portal-dashboards.md).
...
- Utiliser la [mise à l’échelle automatique](autoscale-overview.md) pour augmenter ou diminuer les ressources si une métrique dépasse le seuil défini.
- Acheminer les métriques vers les journaux pour analyser les données métriques avec les données de journal et stocker les valeurs métriques pendant plus de 93 jours. 
- Transmettre en continu les métriques vers un [Event Hub](stream-monitoring-data-event-hubs.md) pour les acheminer vers [Azure Stream Analytics](../../stream-analytics/stream-analytics-introduction.md) ou vers des systèmes externes.
...
- Accéder à des valeurs métriques à partir d’une ligne de commande ou à l’aide d’une application personnalisée avec les [cmdlets PowerShell](https://docs.microsoft.com/powershell/module/azurerm.insights/) ou l’[API REST](rest-api-walkthrough.md).
...
- [Journaux d’activité](collect-activity-logs.md) provenant de ressources Azure qui incluent des informations sur leur configuration et leur intégrité et [journaux de diagnostic](diagnostic-logs-stream-log-store.md) qui fournissent des informations sur leur fonctionnement.
...
- Accéder à des valeurs métriques à partir d’une ligne de commande ou à l’aide d’une application personnalisée avec les [cmdlets PowerShell](https://docs.microsoft.com/powershell/module/azurerm.operationalinsights/) ou l’[API REST](https://dev.loganalytics.io/).
...
Pour une explication de cette fonctionnalité, voir [Créer des alertes de métrique de journaux dans Azure Monitor](alerts-metric-logs.md). La liste des valeurs prises en charge est disponible dans le document [Métriques prises en charge avec Azure Monitor](metrics-supported.md#microsoftoperationalinsightsworkspaces).
...
Pour obtenir des conseils concernant les différents types de données de surveillance, voir [Diffuser des données de surveillance Azure vers un hub d’événements pour les utiliser dans un outil externe](stream-monitoring-data-event-hubs.md).
