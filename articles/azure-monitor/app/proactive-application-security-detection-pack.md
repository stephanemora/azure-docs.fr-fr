...
3. Activité utilisateur suspecte : l’application est accédée à partir de plusieurs pays ou régions par le même utilisateur à peu près au même moment. par exemple, de l’Espagne et des États-Unis au cours de la même heure. Cette détection indique une tentative potentiellement malveillante d’accès à l’application.
...
2. **Portée.** À partir de quels pays/régions les requêtes suspectes provient ? Quel est l’utilisateur le plus suspect ? Ces informations peuvent être obtenues dans la notification.
3. **Diagnostic**. La détection donne la liste des utilisateurs suspects et la liste des pays/régions pour chaque utilisateur, pour vous aider à diagnostiquer le problème.
