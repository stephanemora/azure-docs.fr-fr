...
*J'ai [installé Azure Monitor Application Insights Agent](./status-monitor-v2-overview.md) sur mon serveur web pour surveiller les applications existantes mais je ne vois aucun résultat.*

* Consultez [Résolution des problèmes liés à Status Monitor](./status-monitor-v2-troubleshoot.md).
...
Les données de performances (UC, taux d'E/S, etc.) sont disponibles pour les [services web Java](java-2x-collectd.md), les [applications de bureau Windows](./windows-desktop.md), les [services et applications web IIS si vous installez Application Insights Agent](./status-monitor-v2-overview.md) et [Azure Cloud Services](./app-insights-overview.md). Ces données figurent sous Paramètres, Serveurs.
...
* Windows Server 2008 : vérifiez que vous avez installé les mises à jour suivantes : [KB2468871](https://support.microsoft.com/kb/2468871), [KB2533523](https://support.microsoft.com/kb/2533523), [KB2600217](https://www.microsoft.com/download/details.aspx?id=28936).
...
> [!NOTE]
> À partir de la version 2.14, le package [Microsoft.Aspnet.ApplicationInsights.HostingStartup](https://www.nuget.org/packages/Microsoft.AspNet.ApplicationInsights.HostingStartup) n’est plus nécessaire. Les journaux du SDK sont à présent collectés avec le package [Microsoft.ApplicationInsights](https://www.nuget.org/packages/Microsoft.ApplicationInsights/). Aucun package supplémentaire n’est nécessaire.

1. Modifiez votre fichier applicationinsights.config pour inclure les éléments suivants :
...
2. Redémarrez le processus pour que ces nouveaux paramètres soient récupérés par le kit de développement logiciel (SDK)

3. Dès que vous avez terminé, annulez ces modifications.
...
Les clients peuvent également utiliser un outil .NET Core multiplateforme, [`dotnet-trace`](/dotnet/core/diagnostics/dotnet-trace), pour collecter des journaux qui peuvent contribuer à la résolution des problèmes. Cela peut être particulièrement utile pour les environnements Linux.

Après avoir installé [`dotnet-trace`](/dotnet/core/diagnostics/dotnet-trace), exécutez la commande ci-dessous dans Bash.
...
Découvrez comment supprimer Application Insights dans Visual Studio en suivant les étapes décrites dans l’[article associé](./remove-application-insights.md).
