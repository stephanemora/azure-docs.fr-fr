...
[Voir l’intégralité de cet exemple](https://github.com/MohanGsk/ApplicationInsights-Home/tree/master/Samples/AzureEmailService/MvcWebRole).
...
L’exemple d’initialiseur suivant lit les données depuis [`HttpContext`](/aspnet/core/fundamentals/http-context) et les ajoute à une instance `RequestTelemetry`. `IHttpContextAccessor` est automatiquement fourni via l’injection d'une dépendance de constructeur.
