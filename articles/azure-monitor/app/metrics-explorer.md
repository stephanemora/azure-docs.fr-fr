...
> [!NOTE]
> Cet article décrit l’expérience Metrics Explorer classique, actuellement déconseillée et destinée à être mise hors service. Nous recommandons d'opter pour la nouvelle expérience décrite [cet article](../platform/metrics-charts.md).
...
![Sélectionnez une partie d'un graphique.](./media/metrics-explorer/12-drag.png)
...
![Pointez la souris sur un graphique](./media/metrics-explorer/02-focus.png)
...
![En-tête du panneau.](./media/metrics-explorer/11-grain.png)

Vous pouvez ajuster le niveau de granularité dans le panneau Période :

![En-tête du panneau.](./media/metrics-explorer/grain.png)
...
![Dans Metrics Explorer, sélectionnez Ajouter un graphique](./media/metrics-explorer/04-add.png)

Sélectionnez **Modifier** sur un graphique nouveau ou existant pour modifier ce qui est affiché :

![Sélectionner une ou plusieurs mesures](./media/metrics-explorer/08-select.png)
...
![Activez le regroupement, puis une propriété de regroupement](./media/metrics-explorer/15-segment.png)
...
![Ajustez le curseur.](./media/metrics-explorer/18-height.png)
...
* **%** - versions en pourcentage de chaque agrégation utilisées uniquement avec des graphiques segmentés. Le total équivaut toujours à 100 %, et le graphique montre la contribution relative des différents composants d’un total.
    ...
    ![Agrégation de pourcentage](./media/metrics-explorer/percentage-aggregation.png)

### <a name="change-the-aggregation-type"></a>Modification du type d’agrégation

![Modification du graphique, puis sélection de l’agrégation](./media/metrics-explorer/05-aggregation.png)
...
![Désélectionnez toutes les métriques afin d’afficher les valeurs par défaut](./media/metrics-explorer/06-total.png)
...
![Cliquez sur Paramètres avancés, sélectionnez une plage personnalisée et spécifiez les valeurs minimale et maximale.](./media/metrics-explorer/y-axis-range.png)
...
![Cliquez sur le filtre, développez une propriété et vérifiez les valeurs](./media/metrics-explorer/19-filter.png)
...
[Créez votre propriété](../../azure-monitor/app/api-custom-events-metrics.md#properties). Définissez-la dans un [Initialiseur de télémétrie](../../azure-monitor/app/api-custom-events-metrics.md#defaults) pour qu'elle s'affiche dans toute la télémétrie, notamment la télémétrie standard envoyée par différents modules de kit de développement logiciel (SDK).
...
![Sélectionnez une grille ou un graphique, puis choisissez un type de graphique](./media/metrics-explorer/16-chart-grid.png)
...
![Choisissez Favori](./media/metrics-explorer/21-favorite-save.png)

Pour réafficher le panneau, **allez dans le panneau Vue d'ensemble** et ouvrez Favoris :

![Dans le panneau Vue d'ensemble, cliquez sur Favoris](./media/metrics-explorer/22-favorite-get.png)
...
![Dans les boutons en haut de Metrics Explorer](./media/metrics-explorer/17-reset.png)

## <a name="live-metrics-stream"></a>Flux de métriques temps réel

Pour obtenir une vue bien plus immédiate de votre télémétrie, ouvrez le [Flux temps réel](live-stream.md). La plupart des mesures s’affichent au bout de quelques minutes en raison du processus d’agrégation. En revanche, les métriques temps réel sont optimisées pour avoir une faible latence. 
...
![Dans Metrics Explorer, sélectionnez Règles d'alerte, Ajouter une alerte](./media/metrics-explorer/appinsights-413setMetricAlert.png)

[En savoir plus sur les alertes][alerts].


## <a name="continuous-export"></a>Exportation continue
Si vous souhaitez mettre en place une exportation continue des données pour les traiter en externe, envisagez d’utiliser l [’Exportation continue](../../azure-monitor/app/export-telemetry.md).

### <a name="power-bi"></a>Power BI
Si vous souhaitez obtenir des vues enrichies de vos données, vous pouvez [exporter vers Power BI](https://blogs.msdn.com/b/powerbi/archive/2015/11/04/explore-your-application-insights-data-with-power-bi.aspx).

## <a name="analytics"></a>Analytics
[Analyse](../../azure-monitor/app/analytics.md) est un moyen plus souple d’analyser vos données de télémétrie à l’aide d’un langage de requête puissant. Utilisez l’analyse si vous souhaitez combiner ou calculer les résultats à partir des mesures ou effectuer une exploration approfondie des performances récentes de votre application. 
