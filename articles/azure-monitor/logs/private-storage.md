...
* Coordonnez la version TLS entre les agents et le compte de stockage : il est recommandé d’envoyer des données à Log Analytics à l’aide du protocole TLS 1.2 ou version ultérieure. Passez en revue les [conseils spécifiques à la plateforme](./data-security.md#sending-data-securely-using-tls-12) et, si nécessaire, [configurez vos agents pour utiliser TLS 1.2](../agents/agent-windows.md#configure-agent-to-use-tls-12). Si, pour une raison quelconque, cela n’êtes pas possible, configurez le compte de stockage pour qu’il accepte TLS 1.0.
