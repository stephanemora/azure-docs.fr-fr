...
Consultez [Créer des champs personnalisés dans Azure Monitor](../logs/custom-fields.md) pour plus d'informations sur l'analyse des données lors de la collecte. Cela crée des propriétés personnalisées dans la table, pouvant être utilisées par les requêtes comme n’importe quelle autre propriété.
