...
| **Récupérer** | Accéder aux résultats de requête de journal à partir d’une ligne de commande à l’aide d’[Azure CLI](/cli/azure/monitor/log-analytics).<br>Accéder aux résultats de requête de journal à partir d’une ligne de commande à l’aide de [cmdlets PowerShell](/powershell/module/az.operationalinsights).<br>Accéder aux résultats de requête de journal à partir d’une application personnalisée à l’aide d’une [API REST](https://dev.loganalytics.io/). |
...
Une fois que vous avez créé un espace de travail Log Analytics, vous devez configurer différentes sources pour envoyer leurs données. Aucune donnée n’est collectée automatiquement. Cette configuration diffère d’une source de données à l’autre. Par exemple, [créez des paramètres de diagnostic](../essentials/diagnostic-settings.md) pour envoyer des journaux de ressources à partir de ressources Azure vers l’espace de travail. [Activer VM Insights](../vm/vminsights-enable-overview.md) pour collecter des données de machines virtuelles. Configurez des [sources de données sur l’espace de travail](../agents/data-sources.md) pour collecter des événements et des données de performances supplémentaires.
...
- Consultez [Créer un espace de travail Log Analytics dans le portail Azure](./quick-create-workspace.md) pour créer un espace de travail.
...
- Consultez [Requêtes de journal dans Azure Monitor](./log-query-overview.md) pour savoir exactement où sont utilisées les requêtes de journal et obtenir des références de tutoriels et autres documents pour vous aider à démarrer.
...
- Pour obtenir une description de Log Analytics, consultez [Présentation de Log Analytics dans Azure Monitor](./log-analytics-overview.md). 
- Consultez [Tutoriel Log Analytics](./log-analytics-tutorial.md) pour parcourir les fonctionnalités de Log Analytics afin de créer une requête de journal simple et d’analyser ses résultats.
