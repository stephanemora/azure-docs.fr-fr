---
title: Consulter Azure Monitor pour afficher les journaux des conteneurs en temps réel | Microsoft Docs
description: Cet article décrit la vue en temps réel des journaux de conteneur (stdout/stderr) sans l’utilisation de kubectl avec Azure Monitor pour les conteneurs.
...
Cette fonctionnalité, qui est actuellement en préversion, fournit une vue en temps réel de vos journaux de conteneur Azure Kubernetes Service (AKS) (stdout/stderr) sans nécessiter l’exécution des commandes kubectl. Lorsque vous sélectionnez cette option, un nouveau volet apparaît au-dessous de la table de données de performances de conteneurs sur la vue **Conteneurs**. Il affiche la journalisation dynamique générée par le moteur du conteneur afin de faciliter la résolution des problèmes en temps réel.  
...
Si votre cluster Kubernetes n’est pas configuré avec l’autorisation Kubernetes RBAC ni intégré dans l’authentification unique Azure AD, il est inutile de suivre ces étapes. Étant donné que l’autorisation Kubernetes utilise kube-api, les autorisations en lecture seule sont requises.
...
AKS peut être configuré dans le but d’utiliser Azure Active Directory (AD) pour l’authentification utilisateur. Si vous effectuez cette configuration pour la première fois, consultez [Intégrer Azure Active Directory dans Azure Kubernetes Service](../../aks/aad-integration.md). Au cours des étapes de création de l’[application cliente](../../aks/aad-integration.md#create-client-application) et de spécification de l’**URI de redirection**, vous devez ajouter un autre URI à la liste ** https://ininprodeusuxbase.microsoft.com/***.  
...
## <a name="view-live-logs"></a>Afficher les journaux dynamiques
Lorsque vous affichez **Conteneurs**, vous pouvez **Afficher les journaux du conteneur** ou **Afficher les journaux dynamiques de conteneur**.  Lorsque vous sélectionnez **Afficher les journaux dynamiques de conteneur**, un nouveau volet apparaît au-dessous de la table de données de performances de conteneurs. Il affiche la journalisation dynamique générée par le moteur du conteneur afin de faciliter la résolution des problèmes en temps réel.  
...
4. Sélectionnez la vue **Conteneurs**. Le panneau de propriétés du conteneur sélectionné comporte le lien **Afficher les journaux dynamiques de conteneur**.  
...
Une fois l’authentification réussie, le volet du journal dynamique s’affiche dans la section inférieure du volet central. Si l’indicateur d’état de récupération affiche une coche verte tout à droite du volet, cela signifie que les données peuvent être récupérées.
  ...
  ![Données récupérées sur le volet des journaux dynamiques](./media/container-insights-live-logs/live-logs-pane-01.png)  
...
Dans la barre de recherche, vous pouvez filtrer les résultats par mot clé pour mettre en évidence ce texte dans le journal.   
  ...
  ![Exemple de filtre du volet des journaux dynamiques](./media/container-insights-live-logs/live-logs-pane-filter-01.png)
...
![Affichage dynamique de l’interruption du volet de journaux dynamiques](./media/container-insights-live-logs/live-logs-pane-pause-01.png)
