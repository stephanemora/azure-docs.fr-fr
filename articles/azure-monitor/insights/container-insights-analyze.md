...
Quand vous développez les objets dans la hiérarchie, le volet des propriétés est mis à jour en fonction de l’objet sélectionné. Dans le volet, vous pouvez également afficher les événements Kubernetes avec des recherches dans les journaux prédéfinies en cliquant sur le lien **Afficher les journaux des événements Kubernetes** en haut du volet. Pour plus d’informations sur l’affichage des données des journaux Kubernetes, consultez [Rechercher dans les journaux pour analyser les données](#search-logs-to-analyze-data). Lorsque vous consultez vos conteneurs dans la vue **Conteneurs**, vous pouvez voir les journaux de conteneur en temps réel. Pour plus d’informations sur cette fonctionnalité et la configuration requise pour accorder et contrôler les accès, consultez [Comment afficher les journaux de conteneur en temps réel avec Azure Monitor pour les conteneurs](container-insights-live-logs.md). 
...
Lorsque vous placez la souris sur le graphique à barres sous la colonne **Tendance**, chaque barre indique l’utilisation du processeur ou de la mémoire, selon la métrique sélectionnée, au sein d’une période d’échantillonnage de 15 minutes.  
...
Azure Monitor pour les conteneurs n’inclut pas d’ensemble prédéfini d’alertes que vous pouvez copier et modifier en fonction de vos processus et procédures de prise en charge. En attendant, lisez l’article [Créer des alertes de journal avec Azure Monitor](../../azure-monitor/platform/alerts-log.md?toc=/azure/azure-monitor/toc.json) et découvrez comment créer votre propre ensemble d’alertes.  