...
La solution utilise la fonctionnalité syslog native de l’hôte ESXi pour transmettre des données à une machine virtuelle cible, qui dispose de l’agent Log Analytics. Toutefois, la solution n’écrit pas de fichiers dans syslog sur la machine virtuelle cible. L’agent Log Analytics ouvre le port 1514 et l’écoute. Une fois les données reçues, l’agent Log Analytics envoie (push) les données dans Log Analytics.

## <a name="install-and-configure-the-solution"></a>Installer et configurer la solution
Utilisez les informations suivantes pour installer et configurer la solution.

* Ajoutez la solution VMware Monitoring à votre abonnement à l’aide de la procédure décrite dans [Installer une solution de gestion](../../azure-monitor/insights/solutions.md#install-a-management-solution).
...
Créez une machine virtuelle de système d’exploitation Linux pour recevoir toutes les données Syslog des hôtes ESXi. [L’agent Log Analytics Linux](../../azure-monitor/learn/quick-collect-linux-computer.md) est le point de regroupement de toutes les données syslog de l’hôte ESXi. Vous pouvez utiliser plusieurs hôtes ESXi pour transférer des journaux à un seul serveur Linux, comme dans l’exemple suivant.
...
1. Dans le portail Azure, effectuer une recherche de journal pour `VMware_CL`. Quand Log Analytics collecte les données Syslog, il conserve le format Syslog. Dans le portail, certains des champs sont capturés, tel que *Hostname* et *ProcessName*.  
...
À partir d’ici, vous pouvez modifier la requête de recherche pour l’adapter à un aspect spécifique. Pour plus d’informations sur la création de recherches de journaux, consultez [Trouver des données avec les recherches de journaux dans Log Analytics](../../azure-monitor/log-query/log-query-overview.md).
...
* Utiliser des [recherches de journal](../../azure-monitor/log-query/log-query-overview.md) dans Log Analytics pour afficher des données détaillées de l’hôte VMware.
* [Créer vos propres tableaux de bord](../../azure-monitor/platform/dashboards.md) affichant des données de l’hôte VMware.
* [Créer des alertes](../../azure-monitor/platform/alerts-overview.md) lorsque des événements d’hôte VMware spécifiques se produisent.
