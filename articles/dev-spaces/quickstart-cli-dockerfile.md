---
title: Déployer une application avec un fichier Dockerfile sur Kubernetes à l’aide d’espaces d’Azure Dev Spaces
...
ms.date: 07/08/2019
ms.topic: quickstart
description: Déployer un microservice avec un fichier Dockerfile sur AKS à l’aide d’Azure Dev Spaces
...
ms.locfileid: "67710692"
---
# <a name="quickstart-develop-an-application-with-a-dockerfile-on-kubernetes-using-azure-dev-spaces"></a>Démarrage rapide : Développer une application avec un fichier Dockerfile sur Kubernetes à l’aide d’espaces d’Azure Dev Spaces
...
Dans cet article, vous utilisez l’[exemple d’application Azure Dev Spaces](https://github.com/Azure/dev-spaces) pour illustrer l’utilisation d’Azure Dev Spaces. L’exemple d’application est écrit en Go. Toutefois, vous pouvez utiliser presque n’importe quel langage, tant que vous fournissez un fichier Dockerfile valide pour conteneuriser votre application et l’exécuter sur AKS.

Clonez l’application à partir de GitHub et accédez au dépôt *dev-spaces/samples/golang/getting-started/webfrontend* :
...
L’exemple d’application comprend un fichier Dockerfile valide. Pour générer les ressources du chart Helm en vue d’exécuter l’application dans Kubernetes, utilisez la commande `azds prep` :

```cmd
azds prep --public
```

Vous devez exécuter la commande `prep` à partir du dépôt *dev-spaces/samples/golang/getting-started/webfrontend* pour générer correctement les ressources du chart Helm.
...
Si vous ouvrez l’URL publique et accédez au chemin `/api`, vous pouvez voir le service s’exécuter. L’URL publique s’affiche dans la sortie de la commande `azds up`. Dans cet exemple, l’URL publique est `http://default.webfrontend.1234567890abcdef1234.eus.azds.io/` et vous accéderiez à `http://default.webfrontend.1234567890abcdef1234.eus.azds.io/api`.
...
1. Mettez à jour la [ligne 29 dans `src/app/main.go`](https://github.com/Azure/dev-spaces/blob/master/samples/golang/getting-started/webfrontend/src/app/main.go#L29) de la façon suivante :
