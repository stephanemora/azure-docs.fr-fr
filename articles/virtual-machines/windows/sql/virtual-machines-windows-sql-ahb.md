...
ms.locfileid: "52576981"
---
# <a name="how-to-change-the-licensing-model-for-a-sql-virtual-machine-in-azure"></a>Guide pratique pour changer le modèle de licence d’une machine virtuelle SQL dans Azure
...
## <a name="register-legacy-sql-vm-with-new-resource-provider"></a>Inscrire une machine virtuelle SQL existante auprès du nouveau fournisseur de ressources
La possibilité de passer d’un modèle de licence à l’autre est une fonctionnalité offerte par le nouveau fournisseur de ressources de machine virtuelle SQL (Microsoft.SqlVirtualMachine). À l’heure actuelle, pour pouvoir changer votre modèle de licence, vous devez d’abord inscrire le nouveau fournisseur à votre abonnement, puis inscrire votre machine virtuelle existante auprès du nouveau fournisseur de ressources de machine virtuelle SQL. 
...
Le code suivant inscrit tout d’abord le nouveau fournisseur de ressources SQL à votre abonnement, puis inscrit votre machine virtuelle SQL auprès du nouveau fournisseur de ressources. 
...
1. Accédez à **Abonnements** et sélectionnez l’abonnement qui vous intéresse.  
  ...
  ![Sélectionner votre abonnement](media/virtual-machines-windows-sql-ahb/open-subscriptions.png)
...
Avec cet extrait de code, votre modèle de licence Paiement à l’utilisation se change en modèle BYOL (ou Azure Hybrid Benefit) : 
```powershell
...
Avec cet extrait de code, votre modèle BYOL se change en Paiement à l’utilisation :
```powershell
#example: $SqlVm = Get-AzureRmResource -ResourceType Microsoft.SqlVirtualMachine/SqlVirtualMachines -ResourceGroupName AHBTest -ResourceName AHBTest
$SqlVm = Get-AzureRmResource -ResourceType Microsoft.SqlVirtualMachine/SqlVirtualMachines -ResourceGroupName <resource_group_name> -ResourceName <VM_name>
...
```
  ...
  >[!NOTE]
  > Pour changer de licences, vous devez utiliser le nouveau fournisseur de ressources de machine virtuelle SQL. Si vous essayez d’exécuter ces commandes avant d’inscrire votre machine virtuelle SQL auprès du nouveau fournisseur, vous pouvez rencontrer cette erreur : `Get-AzureRmResource : The Resource 'Microsoft.SqlVirtualMachine/SqlVirtualMachines/AHBTest' under resource group 'AHBTest' was not found. The property 'sqlServerLicenseType' cannot be found on this object. Verify that the property exists and can be set. ` Si vous voyez cette erreur, [inscrivez votre machine virtuelle SQL auprès du nouveau fournisseur de ressources](#register-legacy-SQL-vm-with-new-resource-provider). 
 

## <a name="use-azure-cli"></a>Utiliser Azure CLI
...
```
  ...
  >[!NOTE]
  >Pour changer de licences, vous devez utiliser le nouveau fournisseur de ressources de machine virtuelle SQL. Si vous essayez d’exécuter ces commandes avant d’inscrire votre machine virtuelle SQL auprès du nouveau fournisseur, vous pouvez rencontrer cette erreur : `The Resource 'Microsoft.SqlVirtualMachine/SqlVirtualMachines/AHBTest' under resource group 'AHBTest' was not found. ` Si vous voyez cette erreur, [inscrivez votre machine virtuelle SQL auprès du nouveau fournisseur de ressources](#register-legacy-SQL-vm-with-new-resource-provider). 
