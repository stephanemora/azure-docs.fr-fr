...
## <a name="remarks"></a>Remarques
 ...
 - La possibilité de modifier le modèle de licence est une fonctionnalité du fournisseur de ressources SQL VM. Manipulation d’automatiquement une machine virtuelle SQL Server dans le portail inscrit votre machine virtuelle SQL Server avec le fournisseur de ressources. Toutefois, _certains_ auront peut-être besoin de manuellement [inscrire leur machine virtuelle SQL Server](#register-sql-server-vm-with-the-sql-vm-resource-provider) avec le fournisseur de ressources, telles que :
     - Clients ayant effectué leur machine virtuelle SQL Server à l’aide de PowerShell ou Azure CLI. 
     - Clients ayant installé automatique de SQL Server sur une image non SQL Server. 
     - Clients ayant déploiement leur machine virtuelle à l’aide de disques durs virtuels personnalisés. 
