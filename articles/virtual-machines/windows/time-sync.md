---
title: Synchronisation de l’heure pour les machines virtuelles Windows dans Azure | Microsoft Docs
...
- Il est très difficile de déterminer ce qu’il peut se passer dans un système si les journaux (ou d’autres données) ne sont pas acceptés au bon moment. Un même événement peut avoir l’air de s’être produit à différents moments, rendant la corrélation difficile.
...
### <a name="use-the-default"></a>Utilisez la valeur par défaut
...
Le passage à la synchronisation de l’heure host-only est judicieux si vous rencontrez des problèmes de synchronisation de l’heure avec la configuration par défaut. Essayez la synchronisation host-only pour voir si cela améliore la synchronisation de l’heure sur la machine virtuelle. 

### <a name="external-time-server"></a>Serveur de temps externe

Si vous avez des exigences spécifiques concernant la synchronisation de l’heure, vous pouvez également utiliser des serveurs de temps externes. Les serveurs de temps externes peuvent fournir une heure spécifique, ce qui peut être utile pour les scénarios de test. Ils garantissent l’uniformité de l’heure avec les machines hébergées dans des centres de données non Microsoft, ou gèrent les décalages de secondes d’une façon particulière.
