...
Depuis l'annonce du lancement des grandes instances Azure pour SAP HANA (HLI) en septembre 2016, de nombreux clients ont adopté cette approche « Hardware as-a-service » pour leur plateforme de calcul en mémoire.  Ces dernières années, l'amélioration de l'extensibilité des machines virtuelles Azure combinée à la prise en charge du modèle de déploiement avec montée en puissance parallèle HANA a permis de dépasser la demande de capacité de base de données ERP de la plupart des entreprises clientes.  Des clients commencent à exprimer le désir de migrer leur charge de travail SAP HANA de leurs serveurs physiques vers des machines virtuelles Azure.
Ce guide n'est pas un document de configuration pas à pas. Il décrit les modèles de déploiement courants et fournit des conseils de planification et de migration, en insistant sur les éléments à prendre en compte lors de la préparation afin de réduire les temps d'arrêt liés à la transition.
...
- Toutes les activités de mise à jour/mise à niveau, le cas échéant, doivent être effectuées avant ou après la migration.  Par exemple, conversion de SAP HANA MCOS en déploiement MDC. 
...
- Cela s'applique à la fois aux références SKU Rev3 et Rev4 de HLI.
...
- Compte tenu des différences qui existent entre les serveurs HLI et les machines virtuelles sur le plan commercial, les clients doivent surveiller l'utilisation de leurs machines virtuelles afin de gérer les coûts.
- Les clients reconnaissent et acceptent que HLI est une plateforme de calcul dédiée. Toutefois, les machines virtuelles s'exécutent sur une infrastructure partagée, qui est sécurisée et isolée des autres locataires.
...
- Planifiez le nœud de récupération d'urgence en même temps que le site principal.  Les clients ne pourront pas utiliser le nœud de récupération d'urgence HLI pour le site principal exécuté sur les machines virtuelles après la migration.
- Les clients ont copié les fichiers de sauvegarde requis sur les machines virtuelles cibles, conformément aux exigences de récupération et de conformité de l'entreprise. Cela permet de couvrir les besoins de récupération jusqu'à une date et heure pendant la période de transition.
...
Les modèles de déploiement courants associés aux clients HLI sont résumés dans le tableau suivant.  La migration vers des machines virtuelles Azure est possible pour tous les scénarios HLI.  Quelques scénarios peuvent nécessiter une modification architecturale mineure pour bénéficier des avantages des offres de service Azure actuelles.
...
| 12 | [Réplication HSR à nœud unique vers site de récupération d'urgence (à coût optimisé)](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/hana-supported-scenario#single-node-hsr-to-dr-cost-optimized) | Oui | - |
...
### <a name="sap-hana-housekeeping"></a>Tâches de nettoyage de SAP HANA 
Avant de migrer la base de données HANA, il est recommandé de nettoyer son contenu pour éviter de migrer des données indésirables/obsolètes et des journaux périmés vers la nouvelle base de données.  Les tâches de nettoyage consistent généralement à supprimer ou à archiver des données anciennes, périmées ou inactives.  Par conséquent, ces opérations liées à l'« hygiène des données » doivent être testées en mode hors production afin de confirmer leur validité avant une utilisation en production.
...
Dans le cadre du déploiement HLI d'un client, la connectivité réseau des réseaux virtuels Azure a été établie sur la base des informations décrites dans l'article [Architecture réseau de SAP HANA (grandes instances)](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/hana-network-architecture). En outre, le routage du trafic réseau s'effectue comme décrit à la section « Routage dans Azure ».
1. Lors de la configuration d'une nouvelle machine virtuelle en tant que cible de réplication HSR pour la migration, si la nouvelle machine virtuelle Azure est placée sur le réseau virtuel existant avec des plages d'adresses IP, qui sont déjà autorisées à se connecter au nœud HLI, aucune mise à jour supplémentaire de la connectivité HLI n'est nécessaire.
2. Si la nouvelle machine virtuelle Azure est placée sur un nouveau réseau virtuel (qui peut se trouver dans une autre région) et appairée au réseau virtuel existant, vous pouvez utiliser la clé de service ExpressRoute et l'ID de ressource de l'approvisionnement HLI d'origine pour autoriser l'accès à la plage d'adresses IP de ce nouveau réseau virtuel.  Coordonnez-vous avec Microsoft Service Management pour activer la connectivité entre le réseau virtuel et HLI.  Remarque : pour réduire le temps de réponse du réseau entre les couches d'application et de base de données, les deux couches doivent se trouver sur le même réseau virtuel.  

### <a name="existing-app-layer-availability-set-availability-zones-and-proximity-placement-group"></a>Groupe à haute disponibilité, zones de disponibilité et groupe de placement de proximité de la couche d'application existante
...
- Si le remplacement du serveur SAP HANA s'effectue par résolution de noms en échangeant les adresses IP contre le nom d'hôte de la base de données, il n'y a rien d'autre à faire.  
...
Si la réplication du stockage est utilisée comme solution de récupération d'urgence, la réplication devra être interrompue (sans planification) après la fermeture de l'application SAP et une fois que le dernier catalogue SAP HANA, le dernier fichier journal et les dernières sauvegardes de données auront été répliqués sur les volumes de stockage distants.  Cette action est une tactique de précaution qui permet de disposer d'une base de données actuelle sur le serveur HLI de récupération d'urgence au cas où un problème se produirait pendant la transition physique vers la machine virtuelle Azure.

### <a name="data-backups-preservation-consideration"></a>Considérations relatives à la conservation des sauvegardes de données
Après le basculement vers SAP HANA sur machine virtuelle Azure, les données basées sur des captures instantanées ou les sauvegardes de fichiers journaux figurant sur le serveur HLI sont DIFFICILEMENT accessibles ou restaurables sur une machine virtuelle en cas de besoin. Pendant la période de transition préliminaire, avant que la sauvegarde basée sur Azure n'établisse un historique suffisant pour répondre aux exigences de récupération jusqu'à une date et heure, nous vous recommandons d'effectuer des sauvegardes au niveau des fichiers en plus des captures instantanées sur le serveur HLI au cours des jours/semaines précédant le basculement.  Veillez à ce que ces sauvegardes soient copiées sur un compte de stockage Azure accessible à la nouvelle machine virtuelle SAP HANA. Outre la sauvegarde du contenu du serveur HLI, il est également recommandé de veiller à ce que des sauvegardes complètes du paysage SAP soient facilement accessibles au cas où une restauration serait nécessaire.

### <a name="adjusting-system-monitoring"></a>Ajustement de la surveillance du système 
Les clients utilisent de nombreux outils différents afin de surveiller les systèmes de leur paysage SAP et d'envoyer des notifications d'alerte sur ceux-ci.  Il s'agit ici d'attirer l'attention sur l'action appropriée lors de l'ajout de nouvelles machines virtuelles intégrant un ajustement pour la surveillance et la mise à jour des destinataires des notifications d'alerte si nécessaire.
...
La région de déploiement actuelle du serveur d'applications SAP se trouve généralement à proximité des serveurs HLI associés.  Toutefois, les emplacements disponibles pour les serveurs HLI sont moins nombreux que les régions Azure.  La migration de serveurs HLI physiques vers des machines virtuelles Azure est l'occasion d'affiner la distance de proximité de tous les services associés afin d'optimiser les performances.  Ce faisant, il convient de s'assurer que la région choisie dispose des ressources requises.  Par exemple, la disponibilité de certaines familles de machines virtuelles ou l'offre de zones Azure pour la haute disponibilité.

### <a name="virtual-network"></a>Réseau virtuel 
L'architecte d'infrastructure sera confronté au choix suivant : exécuter la nouvelle base de données HANA sur le réseau virtuel d'applications SAP existant ou en créer un nouveau.  Ce choix dépendra principalement du schéma actuel de mise en réseau du paysage SAP.  Si une modification de l'infrastructure de déploiement est envisagée, par exemple en passant d'un déploiement à une zone à un déploiement à deux zones et en tirant parti du PPG, l'amélioration de la disponibilité imposera un changement architectural. Pour plus d'informations, consultez l'article [Groupes de placement de proximité Azure pour un temps de réponse optimal du réseau avec les applications SAP](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/sap-proximity-placement-scenarios).   

### <a name="security"></a>Sécurité
Que la nouvelle machine virtuelle SAP HANA se trouve sur un réseau/ sous-réseau virtuel nouveau ou existant, elle représente un nouveau service essentiel à l'entreprise qui doit être protégé.  Un contrôle d'accès approprié conforme à la stratégie de sécurité des informations de l'entreprise pour cette nouvelle classe de service doit être évalué et déployé. 
...
Les performances de stockage comptent parmi les facteurs qui influencent l'expérience utilisateur des applications SAP.  Sur la base d'une référence SKU de machine virtuelle donnée, une recommandation de disposition de stockage minimale est publiée : [Configurations du stockage des machines virtuelles SAP HANA Azure](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/hana-vm-operations-storage). Nous vous recommandons de prendre connaissance de ces spécifications minimales et de les comparer aux statistiques du système HLI existant afin de garantir une capacité et des performances d'E/S adéquates pour la nouvelle machine virtuelle HANA.

Si vous configurez un PPG pour la nouvelle machine virtuelle HANA et ses serveurs, soumettez un ticket de support afin d'inspecter et d'assurer la colocalisation du stockage et de la machine virtuelle HANA.  
Dans la mesure où votre solution de sauvegarde peut nécessiter des modifications, le coût de stockage doit également être revu afin d'éviter des dépenses opérationnelles imprévues. 

### <a name="storage-replication-for-disaster-recovery"></a>Réplication du stockage pour la récupération d'urgence
Dans HLI, la réplication du stockage était l'option par défaut pour la réplication de la récupération d'urgence de la base de données HANA. Sur une machine virtuelle Azure, cette fonctionnalité n'est pas l'option par défaut. Envisagez la réplication HSR, la sauvegarde/restauration ou d'autres solutions prises en charge en fonction des besoins de votre entreprise.
...
De nombreux clients utilisent déjà des solutions de sauvegarde tierces pour SAP HANA sur HLI.  Dans ce cas, il suffit de configurer une machine virtuelle et une base de données HANA protégées supplémentaires.  Pour le moment, les tâches de sauvegarde HLI en cours ne sont pas planifiées si la machine est désactivée après la migration.
...
Si vos objectifs de niveau de service permettent un temps de récupération plus long pour une simple sauvegarde sur Stockage Blob et une restauration sur place ou sur une nouvelle machine virtuelle, il s'agit de la stratégie de récupération d'urgence la plus simple et la moins coûteuse.  
Comme sur la plateforme de grande instance où la récupération d'urgence HANA s'effectue généralement à l'aide de la réplication HSR, sur une machine virtuelle Azure, la réplication HSR est également la solution de récupération d'urgence SAP HANA la plus naturelle et la plus native.  Quel que soit le type de déploiement de la source (instance unique, en cluster avec ou sans basculement automatique, ou sur plusieurs nœuds avec montée en puissance parallèle HANA), un réplica HSR cible de l'infrastructure source est requis dans la région de récupération d'urgence. Ce réplica de récupération d'urgence sera établi après la migration du serveur HLI principal vers la machine virtuelle.  L'instance HANA de récupération d'urgence s'enregistrera auprès de l'instance principale SAP HANA sur machine virtuelle en tant que site de réplication secondaire.  
...
L'approche de migration HSR se traduit par un nouvel hôte de base de données HANA et donc par un nouveau nom d'hôte de base de données pour la couche d'application. Les profils SAP doivent être modifiés pour refléter le nouveau nom d'hôte.  Si le changement s'effectue par résolution de noms en conservant le nom d'hôte, aucune modification de profil n'est requise.

### <a name="operating-system"></a>Système d’exploitation
Les images de système d'exploitation du serveur HLI et de la machine virtuelle, même si elles présentent le même niveau de version, SLES 11 SP4 par exemple, ne sont pas identiques. Les clients doivent valider les packages, le noyau et les différents correctifs requis sur le serveur HLI afin qu'ils puissent être installés sur le système d'exploitation de la machine virtuelle cible.  En outre, les clients ont souvent tendance à configurer une version plus récente du système d'exploitation sur la machine virtuelle cible destinée à SAP HSR.  Cette opération est prise en charge conformément à la [note SAP 2763388](https://launchpad.support.sap.com/#/notes/2763388).
...
Pour les machines virtuelles déployées avec des disques Premium ou Ultra, la configuration standard de la réplication du système SAP HANA s'applique et peut être utilisée pour configurer la réplication HSR.  L'[article d'aide SAP](https://help.sap.com/viewer/6b94445c94ae495c83a19646e7c3fd56/2.0.04/099caa1959ce4b3fa1144562fa09e163.html) référencé donne un aperçu des étapes de configuration de la réplication entre deux systèmes, de basculement vers un système secondaire, de restauration sur un système principal et de désactivation de la réplication système.  Dans le cadre de la migration, nous aurons uniquement besoin des étapes de configuration, de basculement et de désactivation de la réplication système sur le serveur HLI source.  
...
Le modèle de déploiement MCOS (Multiple Components in One System) a été choisi par certains de nos clients HLI.  Leur motivation était de contourner la limitation des captures instantanées de stockage MDC (Multiple Databases Container) des versions antérieures de SAP HANA.  Dans le modèle MCOS, plusieurs instances SAP HANA indépendantes sont empilées sur un même panneau HLI.  L'utilisation de la réplication HSR pour la migration fonctionnait bien, ce qui permettait de disposer de plusieurs machines virtuelles HANA dotées d'une base de données locataire chacune.  Cet état final crée un paysage plus animé que celui auquel un client était peut-être habitué.  Dans la mesure où le déploiement par défaut de SAP HANA 2.0 repose sur le modèle MDC, une alternative viable consiste à [déplacer le locataire HANA](https://launchpad.support.sap.com/#/notes/2472478) après la migration HSR.  Ce processus « regroupe » ces bases de données HANA indépendantes en colocataires au sein d'un même conteneur HANA.  
...
Après une migration réussie de la base de données HANA vers une machine virtuelle Azure, aucune transaction commerciale de production ne doit être effectuée sur la base de données HLI.  Cependant, il est recommandé de maintenir le serveur HLI opérationnel pendant une période égale à sa fenêtre locale de conservation des données de sauvegarde afin de pouvoir récupérer plus rapidement les données en cas de besoin.  Ce n'est qu'alors que le panneau HLI doit être désactivé.  Outre l'engagement technique, les clients doivent, dans leur intérêt, conclure des engagements contractuels HLI avec Microsoft.  Les clients doivent contacter leurs représentants Microsoft pour suivre le processus de désactivation HLI.
...
Si un service proxy comme IPTables est utilisé pour acheminer le trafic local vers et depuis le serveur HLI, ce service n'est plus nécessaire après une migration réussie vers une machine virtuelle.  Toutefois, ce service de connectivité doit être conservé aussi longtemps que le panneau HLI reste opérationnel, comme indiqué dans la rubrique consacrée à la désactivation.  Ce service peut être arrêté une fois le panneau HLI entièrement désactivé. 

### <a name="remove-global-reach-for-hli"></a>Supprimer Global Reach pour HLI 
Global Reach est généralement utilisé pour connecter la passerelle ExpressRoute du client à la passerelle ExpressRoute du serveur HLI, ce qui permet au trafic local du client d'atteindre directement le locataire HLI sans avoir recours à un service proxy.  Comme pour le service proxy IPTables, GlobalReach doit être conservé jusqu'à ce que le panneau HLI soit entièrement désactivé.
