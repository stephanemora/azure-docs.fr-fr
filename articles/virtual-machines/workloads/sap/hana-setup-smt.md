...
Les grandes instances SAP HANA n’ont pas de connexion directe à Internet. L’inscription d’une telle unité auprès du fournisseur du système d’exploitation ainsi que le téléchargement et l’application des mises à jour ne sont pas un simples. Avec SUSE Linux, une solution peut être de configurer un serveur SMT sur une machine virtuelle Azure (VM). Hébergez la machine virtuelle dans un réseau virtuel Azure connecté à la grande instance HANA (HLI). Avec un tel serveur SMT, l’unité de grande instance HANA peut inscrire et télécharger des mises à jour. 
...
8. Une fois la machine virtuelle connectée au site SUSE, installez les packages smt. Saisissez la commande PuTTY suivante pour installer les packages smt.
...
10. Vérifiez maintenant si la connexion au SUSE Customer Center fonctionne. Comme le montre la capture d’écran suivante, dans cet exemple, cela a fonctionné.
    ...
    ![Capture d’écran d’un test de connexion à l’Espace client SUSE.](./media/hana-installation/image7_test_connect.png)
...
![Capture d’écran de la défaillance de l’inscription du client.](./media/hana-installation/image13_registration_failed.PNG)
