...
4. **[A]** Ajoutez les entrées de montage pour les systèmes de fichiers /sapmnt/SID et /usr/sap/SID/SYS des systèmes SAP supplémentaires que vous déployez sur le cluster. Dans cet exemple, **NW2** et **NW3**.  
   ...
   * Si vous utilisez Azure NetApp Files, suivez les instructions sur la page [Haute disponibilité des machines virtuelles Azure pour SAP NW sur RHEL avec Azure NetApp File](./high-availability-guide-rhel-netapp-files.md#prepare-for-sap-netweaver-installation)
   * Si vous utilisez le cluster GlusterFS, suivez les instructions sur la page [Haute disponibilité des machines virtuelles Azure pour SAP NW sur SLES](./high-availability-guide-rhel.md#prepare-for-sap-netweaver-installation)
