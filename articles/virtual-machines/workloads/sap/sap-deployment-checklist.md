...
2.  Document de conception technique – qui contient ce qui suit :
    ...
    4.  Les versions détaillées des packs de support SE, DB, Kernel et SAP. Une version de SE prise en charge par SAP NetWeaver ou S/4HANA ne l'est pas nécessairement sur les machines virtuelles Azure. Il en va de même pour les versions des SGBD. Les sources suivantes doivent impérativement être vérifiées pour aligner et, si nécessaire, mettre à niveau les versions des SAP, SGBD ou SE afin de figurer dans une fenêtre prise en charge par SAP et Azure. Vous devez obligatoirement disposer de combinaisons de versions prises en charge par SAP et Azure pour bénéficier d'une prise en charge complète par SAP et Microsoft. Si nécessaire, planifiez la mise à niveau de certains composants logiciels. Vous trouverez plus de détails sur les logiciels SAP, SE et SGBD pris en charge aux emplacements suivants :
    ...
    6.  Architecture de haute disponibilité et de récupération d'urgence
        ...
        2.  Pour une haute disponibilité dans la même zone, vérifiez ce que le SGBD souhaité peut offrir dans Azure. La plupart des SGBD offrent les méthodes synchrones d'un serveur de secours synchrone, ce que nous recommandons pour les systèmes de production.
        ...
        5.  Si vous décidez de ne pas utiliser de systèmes hors production sur le site de récupération d'urgence, envisagez Azure Site Recovery comme méthode viable de réplication de la couche Application SAP dans la région de récupération d'urgence Azure. Voir aussi [Configurer la reprise d'activité pour un déploiement d'application SAP NetWeaver multiniveau](https://docs.microsoft.com/azure/site-recovery/site-recovery-sap). 
        6.  Si vous décidez d'utiliser une configuration combinée haute disponibilité/récupération d'urgence en exploitant les [Zones de disponibilité Azure](https://docs.microsoft.com/azure/availability-zones/az-overview), familiarisez-vous avec les régions Azure dans lesquelles des Zones de disponibilité sont accessibles et où des restrictions peuvent être introduites en cas de latence réseau accrue entre deux Zones de disponibilité.  
...
4.  Conception des services de base - cette conception pourrait comprendre les éléments suivants :
    1.  Conception d'Active Directory et des noms de sources de données.
...
 
## <a name="pilot-phase-optional"></a>Phase pilote (facultatif)
...
4.   Testez vos procédures de haute disponibilité et de récupération d'urgence.
    1.   Simulez des situations de basculement en arrêtant les machines virtuelles ou en mettant les systèmes d'exploitation en mode panique afin de déterminer si vos configurations de basculement fonctionnent comme prévu. 
    ...
    3.   Testez la séquence de sauvegarde et de restauration, et améliorez-la si nécessaire.
...
Il s'agit de la phase au cours de laquelle vous pouvez rassembler l'ensemble des expériences et enseignements tirés de vos déploiements hors production afin de les appliquer aux futurs déploiements de production. En plus des phases précédentes, vous devez également préparer le travail de transfert de données entre votre emplacement d'hébergement actuel et Azure. 
