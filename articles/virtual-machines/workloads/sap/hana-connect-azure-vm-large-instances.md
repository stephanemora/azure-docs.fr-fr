...
Nous allons nous intéresser de plus près à la création d’un réseau virtuel Azure pour grandes instances HANA.
...
Nous allons voir en quoi les définitions des réseaux virtuels sont liées la liste des différentes plages d’adresses IP que nous établissons. La notion d’**espace d’adressage** dont nous parlons désigne l’espace d’adressage que le réseau virtuel Azure est autorisé à utiliser. Il s’agit aussi de la plage d’adresses que le réseau virtuel utilise pour la propagation des itinéraires BGP. Cet **espace d’adressage** est illustré ici :
