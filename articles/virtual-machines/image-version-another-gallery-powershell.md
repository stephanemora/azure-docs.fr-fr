...
Vous devez créer une définition d’image qui correspond au système d’exploitation, à l’état du système d’exploitation et à la génération par Hyper-V de la définition d’image contenant la version de votre image source. Vous pouvez voir toutes les informations dont vous avez besoin pour recréer la définition d’image à l’aide de la cmdlet [Get-AzGalleryImageDefinition](/powershell/module/az.compute/get-azgalleryimagedefinition).
...
Créez une version d’image à l’aide de la cmdlet [New-AzGalleryImageVersion](/powershell/module/az.compute/new-azgalleryimageversion). Vous devez transmettre l’ID de l’image source dans le paramètre `-SourceImageId` pour créer la version d’image dans votre bibliothèque de destination. 
