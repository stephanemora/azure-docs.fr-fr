...
Utilisez la commande suivante pour créer un groupe de ressources `<resourceGroupName>` dans une région. Remplacez `<resourceGroupName>` par le nom de votre choix. Pour ce tutoriel, nous utilisons `East US 2`. Pour plus d’informations, consultez ce [guide de démarrage rapide](../quick-create-cli.md).
