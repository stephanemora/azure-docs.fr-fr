...
Voici différentes méthodes que vous pouvez utiliser pour créer un cluster RDMA Linux avec ou sans planificateur de tâches.
...
* **Disponibilité de taille de machine virtuelle** : vérifiez [les produits disponibles par région](https://azure.microsoft.com/regions/services/) pour connaître la disponibilité de la série H ou d’autres tailles de machine virtuelle compatibles avec RDMA dans les régions Azure.
...
Examinez également les considérations relatives au déploiement pour les [tailles de machine virtuelle Azure prenant en charge RDMA](../sizes-hpc.md?toc=%2fazure%2fvirtual-machines%2flinux%2ftoc.json#rdma-capable-instances).

### <a name="provision-an-sles-12-hpc-vm"></a>Approvisionner une machine virtuelle SLES 12 HPC
...
Pour afficher les images de SLES 12 HPC publiquement disponibles dans Azure, exécutez une commande comme celle qui suit, si votre environnement d’interpréteur de commandes prend en charge **grep** :

    azure vm image list | grep "suse.*hpc"

Configurez une machine virtuelle prenant en charge RDMA avec une image de SLES 12 SP3 HPC en exécutant une commande telle que la suivante :
...
* Un service cloud est créé dans la région Azure spécifiée par l’emplacement. Spécifiez un emplacement dans lequel la taille de machine virtuelle que vous choisissez est disponible, tel que « USA Ouest ».
* Pour la prise en charge de la priorité SUSE (des frais supplémentaires s’appliquent), le nom de l’image de SLES 12 peut actuellement être l’une des options dont le nom contient `priority`, par exemple : 
...
* **Mises à jour** : installez les mises à jour à l’aide de zypper. Vous pouvez également installer les utilitaires NFS.
  ...
  > [!IMPORTANT]
  > Dans une machine virtuelle SLES 12 HPC, nous vous recommandons de ne pas appliquer les mises à jour du noyau, qui peuvent provoquer des problèmes avec les pilotes RDMA Linux.
  >
  >
* **Intel MPI** : terminez l’installation d’Intel MPI sur la machine virtuelle en exécutant la commande suivante :
...
* **Clés SSH pour machines virtuelles SLES** : générez des clés SSH pour établir l’approbation de votre compte utilisateur parmi les nœuds de calcul du cluster SLES lors de l’exécution de travaux MPI. Si vous avez déployé une machine virtuelle HPC basée sur CentOS, ne suivez pas cette étape. Consultez les instructions de la suite de cet article pour définir une confiance SSH sans mot de passe entre les nœuds du cluster une fois que vous avez capturé l’image et déployé le cluster.
  ...
  Pour créer des clés SSH, exécutez la commande `ssh-keygen`. Lorsque vous y êtes invité, appuyez sur **Entrée** pour générer les clés dans l’emplacement par défaut sans définir de mot de passe.
...
Depuis votre ordinateur client, exécutez les commandes CLI Azure suivantes pour capturer l’image. Pour plus d’informations, consultez la page [Capture d’une machine virtuelle Linux classique en tant qu’image](capture-image-classic.md).  
...
Voici un exemple de script `user_authentication.sh` pour activer l’authentification basée sur l’utilisateur sur un cluster HPC basé sur CentOS :
...
Exécutez ce script en procédant comme suit. Vous pouvez également modifier ce script ou utiliser une autre méthode permettant d’établir une authentification SSH sans mot de passe entre les nœuds de calcul du cluster.
...
Exécutez une commande MPI sur l’un des nœuds de calcul pour montrer que MPI est installé correctement et peut communiquer entre au moins deux nœuds de calcul. La commande **mpirun** suivante exécute la commande **hostname** sur deux nœuds. Pour le paramètre `-hosts`, passez les adresses IP des deux nœuds au réseau virtuel Azure (par exemple, 10.32.0.4,10.32.0.5).
...
```
### <a name="sample-mpi-run-script"></a>Exemple de script d’exécution MPI
Voici un exemple de script Bash pour configurer les variables nécessaires à l’exécution d’une application MPI. Modifiez le chemin d’accès à `mpivars.sh` selon les besoins de votre installation d’Intel MPI.
