...
## <a name="issues-with-creating-or-modifying-a-gallery"></a>Problèmes liés à la création ou à la modification d’une galerie ##
...
**Cause**  : le nom donné à la galerie n’est pas conforme aux exigences d’affectation de noms.  
**Solution de contournement**  : choisissez un nom remplissant les conditions suivantes : 1) longueur maximale de 80 caractères, 2) uniquement lettres anglaises, chiffres, traits de soulignement et points, 3) commençant et se terminant par des lettres anglaises ou des chiffres.
...
**Solution de contournement**  : choisissez pour la galerie un nom remplissant les conditions suivantes : 1) longueur maximale de 80 caractères, 2) uniquement lettres anglaises, chiffres, traits de soulignement et points, 3) commençant et se terminant par des lettres anglaises ou des chiffres.
...
**Solution de contournement**  : vérifiez que le nom de la région est correctement orthographié. Vous pouvez exécuter cette commande pour voir les régions auxquelles vous avez accès. Si la région n’est pas répertoriée dans la liste, envoyez [une demande d’accès](/troubleshoot/azure/general/region-access-request-process).
...
**Solution de contournement**  : supprimez toutes les définitions d’images que contient la galerie, puis supprimez la galerie. Si la définition d’image contient des versions d’images, vous devez supprimer celles-ci avant de supprimer la définition d’image.
...
**Cause**  : le groupe de ressources contient déjà une galerie de ce nom, et vous avez tenté d’en créer une autre dans une région différente.  
**Solution de contournement**  : utilisez une galerie ou un groupe de ressources différents.

## <a name="issues-with-creating-or-modifying-image-definitions"></a>Problèmes liés à la création ou à la modification de définitions d’images ##

*La modification de la propriété « galleryImage.properties.<property\> » n’est pas autorisée.*  
**Cause**  : vous avez tenté de modifier le type de système d’exploitation, l’état du système d’exploitation, la génération Hyper-V, l’offre, l’éditeur et la référence SKU. La modification de ces propriétés n’est pas autorisée.  
...
**Cause**  : la même galerie et le même groupe de ressources contiennent déjà une définition d’image du même nom, et vous avez tenté d’en créer une autre dans la même galerie mais dans une région différente.  
...
**Cause**  : le nom donné <imageDefinitionName\> se termine par un point ou un trait de soulignement.  
**Solution de contournement**  : choisissez pour la définition d’image un nom remplissant les conditions suivantes : 1) longueur maximale de 80 caractères, 2) uniquement lettres anglaises, chiffres, tirets, traits de soulignement et points, 3) commençant et se terminant par des lettres anglaises ou des chiffres.

*Le nom d’entité <imageDefinitionName\> n’est pas valide selon sa règle de validation : ^[^\_\\W][\\w-.\_]{0,79}(?<![-.])$"*  
**Cause**  : le nom donné <imageDefinitionName\> se termine par un point ou un trait de soulignement.  
**Solution de contournement**  : choisissez pour la définition d’image un nom remplissant les conditions suivantes : 1) longueur maximale de 80 caractères, 2) uniquement lettres anglaises, chiffres, traits d’union, traits de soulignement et points, 3) commençant et se terminant par des lettres anglaises ou des chiffres.
...
**Cause**  : la valeur donnée d’éditeur, d’offre ou de référence SKU n’est pas conforme aux exigences d’affectation des noms.  
**Solution de contournement**  : choisissez une valeur remplissant les conditions suivantes : 1) longueur maximale de 128 caractères pour l’éditeur ou de 64 caractères pour l’offre et la référence SKU, 2) uniquement lettres anglaises, chiffres, traits d’union, traits de soulignement et points, 3) ne se terminant pas par un point.
...
*L’emplacement fourni, <region\>, n’est pas disponible pour le type de ressource « Microsoft.Compute/galeries ». La liste des régions disponibles pour le type de ressource est ...*  
**Cause**  : l’emplacement <region\> est incorrect ou requiert une demande d’accès.  
**Solution de contournement**  : vérifiez que le nom de la région est correctement orthographié. Vous pouvez exécuter cette commande pour voir les régions auxquelles vous avez accès. Si la région n’est pas répertoriée dans la liste, envoyez [une demande d’accès](/troubleshoot/azure/general/region-access-request-process).
...
**Solution de contournement**  : dans une galerie donnée, toutes les définitions d’images doivent avoir une combinaison unique d’éditeur, d’offre et de référence SKU. Choisissez une combinaison unique ou une nouvelle galerie, puis recréez la définition d’image.
...
**Cause**  : vous avez choisi un type de compte de stockage Standard ZRS dans une région qui ne le prend pas encore en charge.  
...
## <a name="issues-with-creating-or-updating-image-versions"></a>Problèmes liés à la création ou à la mise à jour de versions d’images ##
...
**Solution de contournement**  : vérifiez que <resourceID\> est correct et que la région source de la version d’image de galerie est la même que la région de <resourceID\>.
...
**Solution de contournement**  : assurez-vous que l’ID de source est identique à l’ID de source existant, ou modifiez le numéro de la version d’image.
...
**Cause**  : lors de la création d’une version d’image à l’aide d’une liste de disques et/ou de captures instantanées de disques, au moins deux disques ou captures instantanées de disque ont le même numéro de lun.  
**Solution de contournement**  : supprimez ou modifiez les numéros de lun en double.
...
**Cause**  : Le format du <resourceID\> n’est pas correct.  
**Solution de contournement**  : vérifiez que le resourceID est correct.
...
**Cause**  : la source dans « <resourceID\>» contient un disque de système d’exploitation éphémère. Le service Shared Image Gallery ne prend pas en charge actuellement de disque de système d’exploitation éphémère.  
...
**Solution de contournement**  : essayez de créer la version d’image ultérieurement.
...
**Cause**  : le format valide pour une version d’image est 3 entiers séparés par un point. Le format du nom de version d’image n’est pas valide.  
**Solution de contournement**  : utilisez un nom de version d’image respectant le format Major(int).Minor(int).Patch(int). Par exemple : 1.0.0. ou 2018.12.1.
...
**Cause**  : chiffrement ajouté au disque de données d’une version d’image existante. Impossible d’ajouter un chiffrement à une version d’image existante.  
...
**Cause**  : le resourceID de la source est peut-être incorrect.  
**Solution de contournement**  : assurez-vous que le resourceID de la source est correct.
...
**Cause**  : le numéro de lun spécifié pour le chiffrement ne correspond à aucun numéro de lun de disque attaché à la machine virtuelle.  
**Solution de contournement**  : remplacez le numéro de lun dans le chiffrement par le numéro de lun d’un disque de données présent dans la machine virtuelle.

*Des numéros de lun « number<\> » en double ont été spécifiés dans les encryption.dataDiskImages de la région cible « <region\>  ».*  
**Cause**  : les paramètres de chiffrement utilisés dans la région <region\> spécifient un numéro de lun moins deux fois.  
**Solution de contournement**  : modifiez le numéro de lun dans la région <region\> pour vous assurer que tous les numéros de lun sont uniques dans la région <region\>.

*OSDiskImage et DataDiskImage ne peuvent pas pointer vers le même blob <sourceID\>* .  
**Cause**  : la source du disque de système d’exploitation et au moins un disque de données ne sont pas uniques.  
...
**Cause**  : un appel de mise à jour à la version d’image contient un nouveau disque de données ou un a nouveau numéro de lun pour un disque.  
**Solution de contournement**  : utilisez les numéros de lun et les disques de données de la version d’image existante.
...
## <a name="issues-creating-or-updating-a-vm-or-scale-sets-from-image-version"></a>Problèmes lors de la création ou de la mise à jour d’une machine virtuelle ou de groupes identiques à partir d’une version d’image ##
...
**Solution de contournement**  : Vérifiez que la valeur **OSType** de la version d’image à partir de laquelle vous tentez de créer la machine virtuelle ou le groupe de machines virtuelles identiques est la même que celle de la source utilisée pour créer la version d’image. 

## <a name="issues-creating-a-disk-from-an-image-version"></a>Problèmes lors de la création d’un disque à partir d’une version d’image ##

*La valeur du paramètre imageReference n’est pas valide.*  
**Cause**  : vous avez tenté d’exporter à partir d’une version d’image SIG vers un disque, mais vous avez utilisé une position de lun qui n’existe pas sur l’image.    
