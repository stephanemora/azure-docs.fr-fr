...
Vous devez créer une définition d’image qui correspond au système d’exploitation, à l’état du système d’exploitation et à la génération Hyper-V de la définition d’image contenant la version de votre image source. Vous pouvez voir toutes les informations dont vous avez besoin pour recréer la définition d’image dans votre nouvelle galerie à l’aide de [Get-AzGalleryImageDefinition](/cli/azure/sig/image-definition#az_sig_image_definition_show).
...
Par ailleurs, essayez [Azure Image Builder](./image-builder-overview.md) pour automatiser la création d’une version d’image. Vous pouvez même l’utiliser pour mettre à jour et [créer une nouvelle version de l’image à partir d’une image existante](./linux/image-builder-gallery-update-image-version.md). 
