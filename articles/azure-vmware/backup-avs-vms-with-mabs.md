...
Dans cet article, nous allons suivre les procédures de sauvegarde des machines virtuelles VMware s’exécutant sur Azure VMware Solution à l’aide du serveur de sauvegarde Azure. Avant de commencer, assurez-vous de passer en revue [Configurer le serveur de sauvegarde Microsoft Azure pour Azure VMware Solution](set-up-mabs-for-avs.md).
...
## <a name="add-the-provisioning-ip-address-for-azure-vmware-solution-esxi-hosts-on-azure-backup-server"></a>Ajouter l’adresse IP d’approvisionnement pour les hôtes ESXi Azure VMware Solution sur le serveur de sauvegarde Azure

Au cours de la préversion, Azure VMware Solution ne résout pas l’hôte ESX à partir de la machine virtuelle déployée dans le réseau virtuel. Vous devrez effectuer des étapes supplémentaires pour ajouter l’entrée de fichier hôte sur la machine virtuelle du serveur de sauvegarde Azure.

### <a name="identify-the-ip-address-for-esxi-hosts"></a>Identifier l’adresse IP pour les hôtes ESXi

1. Ouvrez le navigateur et connectez-vous aux URL vCenter. 

   > [!TIP]
   > Vous trouverez les URL dans [Se connecter au vCenter local de votre cloud privé](tutorial-access-private-cloud.md#connect-to-the-local-vcenter-of-your-private-cloud).

1. Dans vSphere Client, sélectionnez le cluster pour lequel vous envisagez d’activer la sauvegarde.

   :::image type="content" source="media/avs-backup/vsphere-client-select-host.png" alt-text="VSphere Client - sélectionner l’hôte":::

1. Sélectionnez **Configurer** > **les adaptateurs VMKernel** > **de mise en réseau**. Dans la liste des appareils, identifiez la carte réseau sur laquelle le rôle d’**approvisionnement** est activé. Prenez note de l’**adresse IP** et du nom d’hôte ESXi.

   :::image type="content" source="media/avs-backup/vmkernel-adapters-provisioning-enabled.png" alt-text="Adaptateurs VMKernel - périphériques d’approvisionnement activés":::

1. Répétez l’étape précédente pour chaque hôte ESXi sous chaque cluster pour lequel vous envisagez d’activer la sauvegarde.

### <a name="update-the-host-file-on-azure-backup-server"></a>Mettre à jour le fichier hôte sur le serveur de sauvegarde Azure

1. Ouvrez le Bloc-notes en tant qu'administrateur.

1. Sélectionnez **Fichier** > **Ouvrir**et recherchez c:\Windows\System32\Drivers\etc\hosts.

1. Ajoutez l’entrée pour chaque hôte ESXi avec l’adresse IP que vous avez identifiée dans la section précédente.

1. Enregistrez vos modifications et fermez le Bloc-notes.
...
1. Dans la boîte de dialogue **Gérer les informations d’identification**, sélectionnez **Ajouter**.
   ...
   ![Boîte de dialogue Gérer les informations d’identification du serveur de sauvegarde Azure](../backup/media/backup-azure-backup-server-vmware/mabs-manage-credentials-dialog.png)
...
1. Sélectionnez **Ajouter** pour ajouter les nouvelles informations d’identification.
   ...
   ![Boîte de dialogue Gérer les informations d’identification du serveur de sauvegarde Azure](../backup/media/backup-azure-backup-server-vmware/new-list-of-mabs-creds.png)
...
1. Spécifiez la durée pendant laquelle vous souhaitez conserver les données sauvegardées sur disque.
   ...
   :::image type="content" source="media/avs-backup/new-protection-group-specify-short-term-goals.png" alt-text="Spécifier vos objectifs à court terme pour la protection sur disque":::
...
1. Dans la page **Vérifier l’allocation du stockage sur disque**, vérifiez l’espace disque fourni pour les sauvegardes de machine virtuelle.
   ...
   :::image type="content" source="media/avs-backup/review-disk-allocation.png" alt-text="Vérifier l'espace disque alloué dans le pool de stockage":::
...
:::image type="content" source="media/avs-backup/monitor-backup-jobs-in-mabs.png" alt-text="Surveiller l’état des travaux de sauvegarde dans le serveur de sauvegarde Azure":::
...
1. Avant la récupération à partir d’un point de récupération en ligne, assurez-vous que l’emplacement intermédiaire contient suffisamment d’espace libre pour héberger la taille non compressée complète de la machine virtuelle que vous souhaitez récupérer. Vous pouvez afficher ou modifier l’emplacement intermédiaire en exécutant l’**Assistant Configurer les paramètres d’abonnement**.
   ...
   :::image type="content" source="media/avs-backup/mabs-recovery-folder-settings.png" alt-text="Paramètres du dossier de récupération du serveur de sauvegarde Azure":::
