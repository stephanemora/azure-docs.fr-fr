...
Cet article vous montre comment sauvegarder des machines virtuelles VMware s’exécutant sur Azure VMware Solution avec le serveur de sauvegarde Azure.  D’abord, passez en revue [Configurer le serveur de sauvegarde Microsoft Azure pour Azure VMware Solution](set-up-backup-server-for-azure-vmware-solution.md).
...
1. Dans la page **Prise en main** de VMware, sélectionnez **Télécharger les certificats d’autorité de certification racine approuvés**.
   ...
   :::image type="content" source="../backup/media/backup-azure-backup-server-vmware/vsphere-web-client.png" alt-text="Capture d’écran montrant la fenêtre de Prise en main du client web vSphere pour accéder à vSphere à distance.":::
...
1. Dans l’**Assistant Importation de certificat**, sélectionnez **Ordinateur local** comme destination du certificat et sélectionnez **Suivant**.
   ...
   :::image type="content" source="../backup/media/backup-azure-backup-server-vmware/certificate-import-wizard1.png" alt-text="Capture d’écran montrant la boîte de dialogue de l’Assistant d’importation de certificat avec l’ordinateur local sélectionné.":::
...
1. Sélectionnez **Placez tous les certificats dans le magasin suivant** et sélectionnez **Parcourir** pour choisir le magasin de certificats.
   ...
   :::image type="content" source="../backup/media/backup-azure-backup-server-vmware/cert-import-wizard-local-store.png" alt-text="Capture d’écran montrant la boîte de dialogue du magasin de certificats avec l’option Placer tous les certificats dans le magasin suivant sélectionné.":::
...
1. Passez en revue les paramètres, puis sélectionnez **Terminer** pour commencer l’importation du certificat.
   ...
   :::image type="content" source="../backup/media/backup-azure-backup-server-vmware/cert-wizard-final-screen.png" alt-text="Capture d’écran montrant l’Assistant d’importation de certificat.":::
...
1. Ouvrez le serveur de sauvegarde Azure et, dans la console du serveur de sauvegarde Azure, sélectionnez **Gestion** > **Serveurs de production** > **Gérer VMware**.
   ...
   :::image type="content" source="../backup/media/backup-azure-backup-server-vmware/add-vmware-credentials.png" alt-text="Capture d’écran montrant la console Sauvegarde Microsoft Azure.":::

1. Dans la boîte de dialogue **Gérer les informations d’identification**, sélectionnez **Ajouter**.

   :::image type="content" source="../backup/media/backup-azure-backup-server-vmware/mabs-manage-credentials-dialog.png" alt-text="Capture d’écran montrant la gestion des informations d’identification au serveur de sauvegarde Azure.":::
...
1. Dans la boîte de dialogue **Ajouter des informations d’identification**, entrez un nom et une description pour les nouvelles informations d’identification, Spécifiez le nom d’utilisateur et le mot de passe que vous avez définis sur le serveur VMware.
   ...
   :::image type="content" source="../backup/media/backup-azure-backup-server-vmware/mabs-add-credential-dialog2.png" alt-text="Capture d’écran montrant les détails des informations d’identification dans serveur de sauvegarde Azure.":::

1. Sélectionnez **Ajouter** pour ajouter les nouvelles informations d’identification.

   :::image type="content" source="../backup/media/backup-azure-backup-server-vmware/new-list-of-mabs-creds.png" alt-text="Capture d’écran montrant la boîte de dialogue Gérer les informations d’identification de serveur de sauvegarde Azure avec les nouvelles informations d’identification affichées.":::
...
1. Dans la console du serveur de sauvegarde Azure, sélectionnez **Gestion** > **Serveurs de production** > **Ajouter**.
   ...
   :::image type="content" source="../backup/media/backup-azure-backup-server-vmware/add-vcenter-to-mabs.png" alt-text="Capture d’écran montrant la console de sauvegarde Microsoft Azure avec le bouton Ajouter sélectionné.":::

1. Sélectionnez **Serveurs VMware**, puis sélectionnez **Suivant**.

   :::image type="content" source="../backup/media/backup-azure-backup-server-vmware/production-server-add-wizard.png" alt-text="Capture d’écran montrant l’option Serveurs VMware sélectionnée de l’Assistant Ajout d’un serveur de production.":::

1. Spécifiez l’adresse IP du vCenter.

   :::image type="content" source="../backup/media/backup-azure-backup-server-vmware/add-vmware-server-provide-server-name.png" alt-text="Capture d’écran montrant l’Assistant Ajout d’un serveur de production montrant comment ajouter un serveur hôte VMware vCenter ou ESXi et ses informations d’identification.":::
...
1. Sélectionnez **Ajouter** pour ajouter le vCenter à la liste des serveurs, puis sélectionnez **Suivant**.
   ...
   :::image type="content" source="../backup/media/backup-azure-backup-server-vmware/add-vmware-server-credentials.png" alt-text="Capture d’écran montrant le serveur VMware et les identifiants définis pour l’Assistant Ajout d’un serveur de production.":::
...
1. Dans la page **Résumé**, sélectionnez **Ajouter** pour ajouter le vCenter au serveur de sauvegarde Azure.
   ...
   :::image type="content" source="../backup/media/backup-azure-backup-server-vmware/tasks-screen.png" alt-text="Capture d’écran montrant le récapitulatif du serveur VMware et les identifiants définis et le bouton Ajouter sélectionné pour l’Assistant Ajout d’un serveur de production.":::

1. Dans la page **Terminer**, vérifiez les paramètres, puis sélectionnez **Fermer**.

   :::image type="content" source="../backup/media/backup-azure-backup-server-vmware/summary-screen.png" alt-text="Capture d’écran montrant le récapitulatif du serveur VMware et les identifiants ajoutés pour l’Assistant Ajout d’un serveur de production.":::
...
1. Dans la console du serveur de sauvegarde Azure, sélectionnez **Protection** > **Nouveau**.
   ...
   :::image type="content" source="../backup/media/backup-azure-backup-server-vmware/open-protection-wizard.png" alt-text="Capture d’écran montrant la console de sauvegarde Microsoft Azure avec le bouton nouveau sélectionné pour créer un nouveau groupe de protection.":::

1. Dans la page d’accueil de l’Assistant **Création d’un nouveau groupe de protection**, sélectionnez **Suivant**.

   :::image type="content" source="../backup/media/backup-azure-backup-server-vmware/protection-wizard.png" alt-text="Capture d’écran montrant l’Assistant groupe de protection.":::
...
1. Dans la page **Sélectionner les membres du groupe**, sélectionnez les machines virtuelles (ou les dossiers de machines virtuelles) que vous souhaitez sauvegarder, puis sélectionnez **Suivant**.
   ...
   :::image type="content" source="../backup/media/backup-azure-backup-server-vmware/server-add-selected-members.png" alt-text="Capture d’écran montrant l’Assistant Créer un nouveau groupe de protection pour sélectionner les membres du groupe.":::
...
1. Définissez la protection à court terme sur **Azure Data Box Disk**, activez la protection en ligne puis sélectionnez **Suivant**.
   ...
   :::image type="content" source="../backup/media/backup-azure-backup-server-vmware/name-protection-group.png" alt-text="Capture d’écran montrant l’Assistant Créer un nouveau groupe de protection pour sélectionner la méthode de protection des données.":::
...
1. Spécifiez la durée pendant laquelle vous souhaitez conserver les données sauvegardées sur disque.
   ...
   :::image type="content" source="media/azure-vmware-solution-backup/new-protection-group-specify-short-term-goals.png" alt-text="Capture d’écran montrant la durée de rétention pour spécifier les objectifs de récupération à court terme pour la protection sur disque.":::
...
1. Dans la page **Vérifier l’allocation du stockage sur disque**, vérifiez l’espace disque fourni pour les sauvegardes de machine virtuelle.
   ...
   :::image type="content" source="media/azure-vmware-solution-backup/review-disk-allocation.png" alt-text="Capture d’écran montrant la boîte de dialogue Vérifier l’allocation de stockage sur disque pour vérifier le stockage cible affecté à chaque source de données.":::
...
1. Dans la page **Choisir la méthode de création d’un réplica**, spécifiez la façon dont vous souhaitez effectuer la sauvegarde initiale, puis sélectionnez **Suivant**.
   ...
   :::image type="content" source="../backup/media/backup-azure-backup-server-vmware/name-protection-group.png" alt-text="Capture d’écran montrant l’Assistant Créer un nouveau groupe de protection pour sélectionner la méthode de création de réplica.":::
...
1. Dans la page **Spécifier les données de protection en ligne**, sélectionnez les machines virtuelles ou les dossiers de machines virtuelles à sauvegarder, puis sélectionnez **Suivant**. 
   ...
   :::image type="content" source="../backup/media/backup-azure-backup-server-vmware/select-data-to-protect.png" alt-text="Capture d’écran montrant l’Assistant Créer un nouveau groupe de protection pour spécifier les données que vous souhaitez que DPM protège en ligne.":::
...
1. Dans la page **Spécifier la planification de sauvegarde en ligne**, indiquez la fréquence à laquelle vous souhaitez sauvegarder les données du stockage local vers Azure. 
   ...
   :::image type="content" source="../backup/media/backup-azure-backup-server-vmware/online-backup-schedule.png" alt-text="Capture d’écran montrant l’Assistant Créer un nouveau groupe de protection pour spécifier la planification de sauvegarde en ligne que DPN utilisera pour générer votre plan de protection.":::
...
1. Dans la page **Spécifier la stratégie de rétention en ligne**, sélectionnez la durée pendant laquelle vous souhaitez conserver les points de récupération créés à partir des sauvegardes dans Azure.
   ...
   :::image type="content" source="../backup/media/backup-azure-backup-server-vmware/retention-policy.png" alt-text="Capture d’écran montrant l’Assistant Créer un nouveau groupe de protection pour spécifier une stratégie de rétention en ligne.":::

1. Dans la page **Résumé**, vérifiez les paramètres puis sélectionnez **Créer un groupe**.

   :::image type="content" source="../backup/media/backup-azure-backup-server-vmware/protection-group-summary.png" alt-text="Capture d’écran montrant l’Assistant Créer un nouveau groupe de protection pour récapituler.":::
...
:::image type="content" source="media/azure-vmware-solution-backup/monitor-backup-jobs.png" alt-text="Capture d’écran montrant les tâches de sauvegarde dans serveur de sauvegarde Azure.":::
...
1. À l’aide du volet **Parcourir**, parcourez ou filtrez pour rechercher la machine virtuelle que vous souhaitez récupérer. Une fois que vous sélectionnez une machine virtuelle ou un dossier, les **Points de récupération du volet affichent les points de récupération disponibles.
   ...
   :::image type="content" source="../backup/media/restore-azure-backup-server-vmware/recovery-points.png" alt-text="Capture d’écran montrant les points de récupération disponibles pour le serveur VMware.":::

1. Dans le volet **Points de récupération pour**, sélectionnez une date à laquelle un point de récupération a été pris. À titre d’exemple, les dates du calendrier en gras ont des points de récupération disponibles. Autrement, vous pouvez faire un clic droit sur la machine virtuelle, sélectionner **Afficher tous les points de récupération**, puis sélectionner le point de récupération dans la liste.
...
1. Avant la récupération à partir d’un point de récupération en ligne, assurez-vous que l’emplacement intermédiaire contient suffisamment d’espace libre pour héberger la taille non compressée complète de la machine virtuelle que vous souhaitez récupérer. Vous pouvez afficher ou modifier l’emplacement intermédiaire en exécutant l’**Assistant Configurer les paramètres d’abonnement**.
   ...
   :::image type="content" source="media/azure-vmware-solution-backup/mabs-recovery-folder-settings.png" alt-text="Capture d’écran montrant l’emplacement du dossier de récupération.":::

1. Sélectionnez **Récupérer** pour ouvrir **l’Assistant Récupération**.

   :::image type="content" source="../backup/media/restore-azure-backup-server-vmware/recovery-wizard.png" alt-text="Capture d’écran montrant la boîte de dialogue de vérification de l’Assistant récupération.":::
...
1. Dans la page **Sélectionner le type de récupération**, choisissez si vous souhaitez récupérer vers l’instance d’origine ou vers un nouvel emplacement.
   ...
   :::image type="content" source="../backup/media/restore-azure-backup-server-vmware/recovery-type.png" alt-text="Capture d’écran montrant l’Assistant de récupération pour sélectionner le type de récupération.":::
...
1. À l’aide du volet **Parcourir**, parcourez ou filtrez pour rechercher la machine virtuelle que vous souhaitez récupérer. Une fois que vous sélectionnez une machine virtuelle ou un dossier, les **Points de récupération du volet affichent les points de récupération disponibles.
   ...
   :::image type="content" source="../backup/media/restore-azure-backup-server-vmware/vmware-rp-disk.png" alt-text="Capture d’écran montrant les points de récupération pour le serveur VMware.":::
...
1. Pour rechercher les fichiers que vous souhaitez récupérer, dans le volet **Chemin**, double-cliquez sur l'élément dans la colonne **Élément récupérable** pour l’ouvrir. Ensuite, sélectionnez le fichier ou les dossiers que vous souhaitez récupérer. Pour sélectionner plusieurs éléments, sélectionnez la touche **Ctrl** tout en sélectionnant chaque élément. Utilisez le volet **Chemin** pour effectuer une recherche dans la liste des fichiers ou des dossiers figurant dans la colonne **Élément récupérable**.
   ...
   :::image type="content" source="../backup/media/restore-azure-backup-server-vmware/vmware-rp-disk-ilr-2.png" alt-text="Capture d’écran montrant la date et l’heure des points de récupération disponibles sélectionnés.":::
...
1. Dans l'écran **Spécifier la destination**, sélectionnez **Parcourir** afin de rechercher un emplacement réseau pour vos fichiers ou dossiers. Le serveur de sauvegarde Azure crée un dossier où tous les éléments récupérés sont copiés. Le nom du dossier a le préfixe MABS_day-month-year. Lorsque vous sélectionnez un emplacement pour les fichiers ou le dossier récupérés, les détails de cet emplacement sont fournis.
   ...
   :::image type="content" source="../backup/media/restore-azure-backup-server-vmware/specify-destination.png" alt-text="Capture d’écran montrant la date et l’heure, la destination et le chemin d’accès de destination pour les points de récupération disponibles sélectionnés.":::
...
- [Gestion du cycle de vie des machines virtuelles du service Azure VMware Solution](./integrate-azure-native-services.md).
