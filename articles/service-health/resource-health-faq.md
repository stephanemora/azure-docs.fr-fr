...
Resource Health [prend en charge](resource-health-alert-arm-template-guide.md) les alertes basées sur le journal d’activité. Les alertes du journal d'activité utilisent des [groupes d'actions](../azure-monitor/alerts/action-groups.md) pour avertir les utilisateurs qu'une alerte a été déclenchée. Les groupes d'actions prennent en charge différents canaux de notification (e-mail, SMS, webhook, actions de gestion des services informatiques, etc.).
