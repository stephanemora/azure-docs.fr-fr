...
- [Modes WAF sur Application Gateway](https://docs.microsoft.com/azure/web-application-firewall/ag/ag-overview#waf-modes)

- [Modes WAF sur Front Door](https://docs.microsoft.com/azure/web-application-firewall/afds/afds-overview#waf-modes)
...
- [Règles et groupes de règles CRS de pare-feu d’applications web](https://docs.microsoft.com/azure/web-application-firewall/ag/application-gateway-crs-rulegroups-rules?tabs=owasp31)
...
- [Stratégie WAF sur Front Door](/cli/azure/network/front-door/waf-policy)
...
- [Vue d’ensemble de la journalisation](https://docs.microsoft.com/azure/web-application-firewall/ag/ag-overview#logging)
...
- [Configuration de la surveillance d'un compte de stockage](https://docs.microsoft.com/azure/storage/common/manage-storage-analytics-logs#configure-logging)
...
- [Guide pratique pour activer les paramètres de diagnostic du journal d’activité Azure](/azure/azure-monitor/essentials/diagnostic-settings-legacy)
...
**Aide** : utilisez des stations de travail disposant d’un accès privilégié (PAW) avec l’authentification multifacteur configurée pour se connecter à Azure Web Application Firewall (WAF) et aux ressources connexes.
...
- [Quelle est la condition d’emplacement dans l’accès conditionnel Azure Active Directory (Azure AD)](../active-directory/reports-monitoring/quickstart-configure-named-locations.md)
...
Simplifiez ce processus en créant des paramètres de diagnostic pour les comptes d’utilisateurs Azure AD et en envoyant les journaux d’audits et les journaux de connexions à un espace de travail Log Analytics. Configurez les alertes souhaitées dans un espace de travail Log Analytics.
...
**Aide** : utilisez les fonctionnalités Risk and Identity Protection d’Azure Active Directory (Azure AD) pour configurer des réponses automatiques aux actions suspectes détectées liées aux identités d’utilisateurs. Ingérez des données dans Azure Sentinel pour approfondir votre examen.
...
**Aide** : contrôlez l’accès à vos ressources Azure comme pour Web Application Firewall avec le contrôle d’accès en fonction du rôle Azure (Azure RBAC).
...
- [Guide pratique pour configurer des clés de chiffrement gérées par le client](https://docs.microsoft.com/azure/storage/common/customer-managed-keys-configure-key-vault?tabs=portal)
...
- [Vue d’ensemble de l’intégration entre Application Gateway et Azure Security Center](https://docs.microsoft.com/azure/security-center/security-center-partner-integration#overview)
...
Accordez ou refusez des autorisations à des utilisateurs spécifiques, à des groupes de sécurité intégrés ou à des groupes définis dans Azure Active Directory (Azure AD), s’ils sont intégrés à Azure DevOps ou à Azure AD, ou s’ils sont intégrés à Team Foundation Server (TFS).
...
- [Utilisation de la suppression réversible d’Azure Key Vault](https://docs.microsoft.com/azure/key-vault/general/key-vault-recovery?tabs=azure-powershell&amp;preserve-view=true)
...
**Aide** : Security Center attribue un niveau de gravité à chaque alerte pour vous aider à classer par ordre de priorité celles devant être examinées en premier. La gravité dépend de la confiance que Security Center accorde à la recherche ou à la métrique utilisées pour émettre l’alerte, ainsi qu’à la conviction quand à l’existence d’une intention malveillante derrière l’activité à l’origine de l’alerte.
