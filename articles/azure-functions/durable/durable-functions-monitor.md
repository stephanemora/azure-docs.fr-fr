...
6. Continue à s’exécuter jusqu’à ce que le paramètre `CurrentUtcDateTime` (.NET) ou `currentUtcDateTime` (JavaScript) passe l’heure d’expiration du moniteur, ou jusqu’à ce qu’une alerte SMS soit envoyée.
