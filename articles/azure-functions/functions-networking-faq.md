...
Avec l’intégration de réseau virtuel (actuellement en préversion), vous pouvez accéder aux ressources sécurisées par des points de terminaison de service à partir d’une fonction en cours d’exécution. Pour plus d’informations, consultez [Préversion de l’intégration de réseau virtuel](functions-networking-options.md#preview-version-of-virtual-network-integration).
