...
2. Modifier le code de méthode `Main` afin d’obtenir la chaîne de connexion de stockage et la clé d’instrumentation Application Insights à partir d’un fichier *appsettings.json*, à l’aide de l’infrastructure de configuration .NET Core.  Voici un exemple :
