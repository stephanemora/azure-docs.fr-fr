...
- [Python à l’aide de pytest](./functions-reference-python.md?tabs=application-level#unit-testing)
...
1. [Créez une nouvelle application Functions](./functions-get-started.md) et nommez-la **Functions**.
2. [Créez une fonction HTTP à partir du modèle](./functions-get-started.md) et nommez-la **MyHttpTrigger**.
...
6. [Faites référence à l’application *Functions*](/visualstudio/ide/managing-references-in-a-project) à partir de l’application *Functions.Tests*.
...
Si vous souhaitez accéder aux paramètres de l’application dans vos tests, vous pouvez [injecter](./functions-dotnet-dependency-injection.md) une instance de `IConfiguration` avec des valeurs de variables d’environnement factices dans votre fonction.
