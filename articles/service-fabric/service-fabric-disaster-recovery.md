...
#### <a name="stateless-services"></a>Services sans état
`InstanceCount` pour le service indique le nombre d’instances souhaitées devant être en cours d’exécution. En cas d’échec d’une ou de la totalité des instances, Service Fabric répond en créant automatiquement des instances de remplacement sur d’autres nœuds. Service Fabric continue à créer des remplacements jusqu’à ce que le service atteigne de nouveau le nombre d’instances souhaité.
Autre exemple, si le paramètre `InstanceCount` du service sans état a pour valeur -1 ; dans ce cas, une instance doit s’exécuter sur chacun des nœuds du cluster. Si certaines de ces instances devaient échouer, Service Fabric détecte que ce service n’est pas à l’état souhaité et essaie de créer les instances sur les nœuds où elles sont manquantes.

#### <a name="stateful-services"></a>Services avec état
Il existe deux types de service avec état :
1. Service avec état persistant
2. Service avec état non persistant (l’état est stocké en mémoire)

La récupération en cas de défaillance du service avec état dépend du type du service avec état et du nombre de réplicas détenus par le service et ayant échoué.

Qu’est-ce que le quorum ?
Dans un service avec état, les données entrantes sont répliquées entre les réplicas (Primary et ActiveSecondary). Si la majorité des réplicas reçoivent les données, celles-ci sont considérées comme étant validées par le quorum (pour 5 réplicas, le _quorum_ est égal à 3). Cela signifie qu’à tout moment, au moins un quorum de réplicas est assuré d’avoir les données les plus récentes. En cas de défaillance d’un ou plusieurs réplicas (par exemple, 2 réplicas sur 5), nous pouvons utiliser la valeur de quorum pour calculer si la récupération est possible (les 3 réplicas restants étant toujours opérationnels, au moins 1 réplica est assuré d’avoir des données complètes).

Qu’est-ce que la perte de quorum ?
En cas de défaillance du quorum des réplicas, la partition est déclarée comme étant en état de perte de quorum. Supposons qu’une partition a 5 réplicas, ce qui signifie qu’au moins 3 sont assurés d’avoir des données complètes. En cas de défaillance du quorum (3 sur 5) de réplicas, Service Fabric ne peut pas déterminer si les réplicas restants (2 sur 5) disposent de suffisamment de données pour restaurer la partition.

Pour déterminer si un incident s’est produit pour un service avec état et pour gérer cet incident, effectuez les trois étapes suivantes :
...
Les réplicas peuvent connaître des échecs par intermittence en raison d’une défaillance temporaire. Patientez un certain temps, car Service Fabric essaie de les récupérer. Si les réplicas ont été inactifs pendant une durée supérieure à la durée attendue, suivez les étapes de résolution des problèmes ci-dessous.
- Les réplicas ont peut-être planté. Consultez les rapports d’intégrité au niveau des réplicas et vos journaux des applications. Collectez les vidages sur incident et prenez les mesures nécessaires pour effectuer la récupération.
- Le processus de réplica n’a peut-être pas répondu. Inspectez vos journaux des applications pour vérifier ce point. Collectez le vidage des processus, puis tuez le processus qui ne répond pas. Service Fabric crée un processus de remplacement et essaie de récupérer le réplica.
- Les nœuds qui hébergent les réplicas peuvent être défaillants. Redémarrez la machine virtuelle sous-jacente pour récupérer les nœuds.

Il peut y avoir des situations où il n’est pas possible de récupérer des réplicas. Par exemple, les lecteurs peuvent avoir échoué ou les ordinateurs ne répondent pas physiquement. Dans ce cas, Service Fabric doit être invité à ne pas attendre la récupération du réplica.
N’utilisez pas ces méthodes si une perte de données potentielle n’est pas acceptable pour mettre le service en ligne ; dans ce cas, tous les efforts doivent être faits pour récupérer les ordinateurs physiques.

Les étapes ci-après pouvant aboutir à des pertes de données, soyez prudent si vous décidez de les suivre :
   
> [!NOTE]
> L’utilisation de ces méthodes autrement que pour cibler certaines partitions n’est _jamais_ sûre. 
>

- Utilisez l’API `Repair-ServiceFabricPartition -PartitionId` ou `System.Fabric.FabricClient.ClusterManagementClient.RecoverPartitionAsync(Guid partitionId)`. Cette API permet de spécifier l’ID de la partition à récupérer de l’état de perte de quorum avec perte de données potentielle.
- Si votre cluster rencontre des défaillances fréquentes qui font passer les services dans l’état de perte de quorum et qu’_une perte de données potentielle est acceptable_, la spécification d’une valeur [QuorumLossWaitDuration](https://docs.microsoft.com/powershell/module/servicefabric/update-servicefabricservice?view=azureservicefabricps) appropriée peut aider vos services à récupérer automatiquement. Service Fabric attend pendant la durée indiquée par QuorumLossWaitDuration (par défaut, durée infinie) avant d’effectuer la récupération. Cette méthode _n’est pas recommandée_ car elle peut entraîner des pertes de données inattendues.
...
Service Fabric utilise le concept de nœud initial. Il s’agit de nœuds qui permettent de maintenir la disponibilité du cluster sous-jacent. Ces nœuds vous aident à garantir que le cluster reste actif en établissant des baux avec d’autres nœuds et en servant de tiebreaker pour certains types de défaillances réseau. Si des défaillances aléatoires suppriment la majorité des nœuds initiaux du cluster, et que ceux-ci ne sont pas restaurés, votre cercle de fédération du cluster se réduit, car vous avez perdu le quorum des nœuds de départ, et le cluster échoue. Dans Azure, le fournisseur de ressources Service Fabric gère les configurations de cluster Service Fabric, et par défaut distribue les nœuds initiaux dans des domaines d’erreur et de mise à niveau de type Nœud principal. Si ce type de nœud principal est marqué comme étant du niveau de durabilité Silver ou Gold, lorsque vous supprimez un nœud initial (soit par la mise à l’échelle dans votre type de nœud principal ou par la suppression manuelle d’un nœud initial), le cluster tente de promouvoir un autre nœud non initial à partir de la capacité disponible du type de nœud principal. Il échoue si vous disposez d’une capacité disponible inférieure au niveau de fiabilité que votre cluster nécessite pour votre type de nœud principal.
