...
Azure Service Fabric prend en charge les clusters qui s’étendent sur des zones de disponibilité en déployant des types de nœuds qui sont épinglés à des zones spécifiques. Les zones de disponibilité sont disponibles uniquement dans les régions sélectionnées. Pour plus d’informations, consultez [Vue d’ensemble des zones de disponibilité](../availability-zones/az-overview.md).
...
La topologie recommandée du type de nœud principal nécessite les ressources suivantes :
...
La solution précédente utilise un type de nœud dans chaque zone de disponibilité. La solution suivante permet aux utilisateurs de déployer trois zones de disponibilité dans le même type de nœud.
...
* Migration à partir d’un type de nœud qui utilise l’équilibrage de charge et les ressources IP de base : ce processus est déjà décrit dans [une section précédente](#migrate-to-availability-zones-from-a-cluster-by-using-a-basic-sku-load-balancer-and-a-basic-sku-ip) pour la solution avec un type de nœud par zone de disponibilité.
