...
* Les événements sont consignés via des canaux standard tels que les journaux des événements ETW/Windows et peuvent être visualisés par n’importe quel outil de surveillance qui les prend en charge, tel que Log Analytics. Par défaut, pour les clusters créés dans le portail, les diagnostics sont activés et l’agent de diagnostics Windows Azure envoie les événements au Stockage Table Azure, mais vous devez encore intégrer cela à votre ressource Log Analytics. Apprenez-en davantage sur la configuration de l’[agent Azure Diagnostics](service-fabric-diagnostics-event-aggregation-wad.md) pour modifier la configuration des diagnostics de votre cluster afin de collecter plus de journaux et de compteurs de performances, et l’[intégration Log Analytics](service-fabric-diagnostics-event-analysis-oms.md).
