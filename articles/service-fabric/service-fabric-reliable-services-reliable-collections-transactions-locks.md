...
* **Isolation** : Les modifications effectuées par des transactions concurrentes doivent être isolées transaction par transaction. Le niveau d'isolation utilisé pour une opération dans le cadre d'une [ITransaction](/dotnet/api/microsoft.servicefabric.data.itransaction) est déterminé par l'état [IReliableState](/dotnet/api/microsoft.servicefabric.data.ireliablestate) qui effectue l'opération.
