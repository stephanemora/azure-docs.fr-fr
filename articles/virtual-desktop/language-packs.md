...
- L’ISO de langue et le disque 1 Feature on Demand (FOD) de la version du système d’exploitation utilisée par l’image. Vous pouvez les télécharger ici : 
...
Pour créer le référentiel de contenu destiné aux modules linguistiques et fonctionnalités à la demande :

1. Sur une machine virtuelle Azure, téléchargez les images Windows 10 multilingue ISO et FOD pour Windows 10 Entreprise multisession, versions 1903, 1909 et 2004, à partir des liens disponibles dans [Conditions préalables](#prerequisites).
...
4. Connectez-vous au référentiel de partage de fichiers de module linguistique et FOD et montez-le sur un lecteur à lettre (par exemple, le lecteur E).
...
2. Arrêtez la machine virtuelle, puis capturez-la dans une image managée en suivant les instructions disponibles dans [Créer une image managée d’une machine virtuelle généralisée dans Azure](../virtual-machines/windows/capture-image-resource.md).
...
Enfin, vous devez ajouter la langue à la liste des langues de chaque utilisateur afin de pouvoir sélectionner la langue de votre choix dans le menu Paramètres.

Pour permettre à vos utilisateurs de sélectionner les langues que vous avez installées, connectez-vous en tant qu’utilisateur, puis exécutez la cmdlet PowerShell suivante afin d’ajouter les modules linguistiques installés au menu Langues. Vous pouvez également configurer ce script en tant que tâche automatisée qui s’active lorsque l’utilisateur se connecte à sa session.
