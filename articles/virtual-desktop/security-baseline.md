---
title: Base de référence de sécurité Azure pour Azure Virtual Desktop
description: La ligne de base de sécurité pour Azure Virtual Desktop fournit des instructions pratiques et des ressources pour l’implémentation des suggestions de sécurité spécifiées dans le point de référence de sécurité Azure.
author: msmbaldwin
...
Cette ligne de base de sécurité applique l’aide de la [version 2.0 Point de référence de sécurité Azure](../security/benchmarks/overview.md) à Azure Virtual Desktop. Le benchmark de sécurité Azure fournit des recommandations sur la façon dont vous pouvez sécuriser vos solutions cloud sur Azure. Le contenu est regroupé par les **contrôles de sécurité** définis par le point de référence de sécurité Azure et l’aide associée applicable à Azure Virtual Desktop. Les **contrôles** non applicables à Azure Virtual Desktop ont été exclus.

Le service Azure Virtual Desktop comprend le service lui-même, la référence SKU Windows 10 Entreprise pour multi-session ainsi que FSLogix. Pour les recommandations de sécurité liées à FSLogix, consultez la [base de référence de la sécurité pour le stockage](../storage/common/security-baseline.md). Le service possède un agent qui s’exécute sur les machines virtuelles, mais celles-ci étant contrôlées entièrement par le client, suivez les [recommandations de sécurité pour le calcul](../virtual-machines/windows/security-baseline.md)

Pour voir comment Azure Virtual Desktop est entièrement mappé au point de référence de sécurité Azure, consultez le [fichier de mappage complet de la ligne de base de sécurité Azure Virtual Desktop](https://github.com/MicrosoftDocs/SecurityBenchmarks/tree/master/Azure%20Offer%20Security%20Baselines).
...
**Aide** : vous devez créer ou utiliser un réseau virtuel existant lorsque vous déployez des machines virtuelles à inscrire auprès d’Azure Virtual Desktop. Vérifiez que tous les réseaux virtuels Azure suivent un principe de segmentation d’entreprise qui s’aligne sur les risques métier. Tout système qui peut entraîner des risques plus élevés pour l’organisation doit être isolé sur son propre réseau virtuel et être suffisamment sécurisé à l’aide d’un groupe de sécurité réseau ou du Pare-feu Azure.
...
Pour les groupes de sécurité réseau associés aux sous-réseaux de votre machine virtuelle (qui font partie d’Azure Virtual Desktop), vous devez autoriser le trafic sortant vers des points de terminaison spécifiques. 

- [Découvrez quelles URL doivent être autorisées à accéder à Azure Virtual Desktop](safe-url-list.md)
...
**Aide** : Utilisez le Pare-feu Azure pour protéger les applications et les services contre le trafic potentiellement malveillant provenant d’Internet et d’autres emplacements externes. Protégez vos ressources Azure Virtual Desktop contre les attaques de réseaux externes, notamment les attaques par déni de service distribué (DDoS), les attaques spécifiques aux applications et le trafic Internet non sollicité et potentiellement malveillant. Protégez vos ressources contre les attaques par déni de service distribué en activant la protection DDoS Standard sur vos réseaux virtuels Azure. Utilisez Azure Security Center pour détecter les risques de configuration incorrecte liés aux ressources associées à votre réseau.

Azure Virtual Desktop n’est pas destiné à exécuter des applications web et ne nécessite pas de configuration de paramètres supplémentaires ni de déploiement de services réseau supplémentaires pour le protéger des attaques de réseau externe qui ciblent les applications web.
...
**Aide** : Azure Virtual Desktop utilise Azure Active Directory (Azure AD) en tant que service de gestion des identités et des accès par défaut. Vous devez normaliser Azure AD pour régir la gestion des identités et des accès de votre organisation dans :
...
**Aide** : Azure Virtual Desktop utilise Azure Active Directory (Azure AD) pour fournir la gestion des identités et des accès aux ressources Azure, aux applications cloud et aux applications locales. Cela inclut les identités d’entreprise, comme les employés, ainsi que les identités externes, comme les partenaires et les fournisseurs. Cela permet à l’authentification unique de gérer et sécuriser l’accès aux données et ressources de votre organisation localement et dans le cloud. Connectez l’ensemble de vos utilisateurs, applications et appareils à Azure AD pour un accès transparent et sécurisé ainsi qu’une visibilité et un contrôle accrus.
...
**Aide** : Azure Virtual Desktop utilise Azure Active Directory (Azure AD) qui prend en charge des contrôles d’authentification forts avec l’authentification multifacteur et des méthodes sans mot de passe fortes.
...
**Aide** : Azure Virtual Desktop est intégré à Azure Active Directory (Azure AD) qui fournit les sources de données suivantes :
...
**Aide** : Azure Virtual Desktop prend en charge l’accès conditionnel avec Azure Active Directory (Azure AD) pour un contrôle d’accès granulaire en fonction des conditions définies par l’utilisateur. Par exemple, les connexions utilisateur de certaines plages d’adresses IP doivent utiliser l’authentification multifacteur pour valider l’accès. 
...
- [Les informations de configuration de l’accès conditionnel spécifique à Azure Virtual Desktop sont disponibles ici](set-up-mfa.md)
...
**Aide** : Azure Virtual Desktop utilise des comptes Azure Active Directory (Azure AD) pour gérer ses ressources ainsi que pour analyser régulièrement les comptes d’utilisateur et l’affectation d’accès afin de vérifier la validité des comptes et des accès correspondants.
...
- [Rôles intégrés pour Azure Virtual Desktop](rbac.md)
...
**Aide** : Azure Virtual Desktop utilise Azure Active Directory (Azure AD) pour gérer ses ressources. Pour empêcher le verrouillage accidentel de votre organisation Azure AD, configurez un compte d’accès d’urgence pour conserver un accès lorsque les comptes administratifs normaux ne peuvent pas être utilisés. Les comptes d’accès d’urgence sont généralement hautement privilégiés et ne doivent pas être attribués à des personnes spécifiques. Les comptes d’accès d’urgence sont limités à des cas d’urgence ou à des scénarios « de secours » où il est impossible d’utiliser des comptes d’administration normaux.
...
**Aide** : Azure Virtual Desktop est intégré à Azure Active Directory (Azure AD) pour gérer ses ressources. Utilisez les fonctionnalités de gestion des droits d’utilisation d’Azure AD pour automatiser les workflows de demandes d’accès, notamment les attributions, les révisions et l’expiration des accès. En outre, les approbations à double ou à plusieurs niveaux sont également prises en charge.
...
**Aide** : dans le cadre des scénarios de support où Microsoft a besoin d’accéder aux données client, Azure Virtual Desktop prend en charge Customer Lockbox pour fournir une interface qui vous permet de consulter et d’approuver ou de refuser les demandes d’accès aux données client.
...
**Aide** : Non applicable. Azure Virtual Desktop ne peut pas être utilisé pour garantir la sécurité des ressources dans un processus de gestion de cycle de vie. Il incombe au client de gérer les attributs et les configurations réseau des ressources considérées comme ayant un impact élevé. 
...
**Aide** : utilisez la fonctionnalité de détection des menaces intégrée Azure Security Center et activez Azure Defender (officiellement Azure Advanced Threat Protection) pour vos ressources Azure Virtual Desktop. Azure Defender pour Azure Virtual Desktop offre une couche supplémentaire d’intelligence de la sécurité qui détecte les tentatives d’accès ou d’exploitation inhabituelles et potentiellement dangereuses de vos ressources Azure Virtual Desktop.

Transférez tous les journaux d’Azure Virtual Desktop vers votre solution d’informations de sécurité et gestion d'événements (SIEM) qui peut être utilisée pour configurer des détections de menaces personnalisées. Veillez à surveiller les différents types de ressources Azure pour identifier les menaces et anomalies potentielles. Concentrez-vous sur l’obtention d’alertes de haute qualité afin de réduire les faux positifs que les analystes doivent trier. Les alertes peuvent provenir de données de journal, d’agents ou d’autres données.
...
**Aide** : Azure Virtual Desktop ne génère ni ne traite les journaux de requête DNS (Domain Name System). Toutefois, les ressources qui sont inscrites auprès du service peuvent générer des journaux de flux.
...
- [Environnement Azure Virtual Desktop](environment-setup.md)
...
**Aide** : Azure Virtual Desktop vous permet de déployer vos propres machines virtuelles et de les inscrire auprès du service, ainsi que d’exécuter la base de données SQL dans l’environnement.

Azure Virtual Desktop peut utiliser une solution tierce pour effectuer des évaluations de vulnérabilité sur les périphériques réseau et les applications web. Lors de l’exécution d’analyses à distance, n’utilisez pas un compte d’administration unique et perpétuel. Envisagez d’implémenter une méthodologie d’approvisionnement JIT pour le compte d’analyse. Les informations d’identification du compte d’analyse doivent être protégées, surveillées et utilisées uniquement pour l’analyse des vulnérabilités.
...
**Aide** : Azure Virtual Desktop n’utilise ni ne nécessite aucun logiciel tiers. Cependant, Azure Virtual Desktop vous permet de déployer vos propres machines virtuelles et de les inscrire auprès du service.
...
- [Configurer Microsoft Endpoint Configuration Manager pour Azure Virtual Desktop](configure-automatic-updates.md)
...
**Aide** : Azure Virtual Desktop ne permet pas aux clients d’effectuer leurs propres tests d’intrusion sur leurs ressources Azure Virtual Desktop.
...
**Aide** : Azure Virtual Desktop ne fournit aucune fonctionnalité spécifique pour les processus de détection des points de terminaison et réponse (EDR). Toutefois, les ressources inscrites auprès du service peuvent bénéficier de fonctionnalités de détection des points de terminaison et réponse. 
...
**Aide** : protégez vos ressources Azure Virtual Desktop avec une solution de protection contre les programmes malveillants de point de terminaison moderne et gérée de manière centralisée qui prend en charge l’analyse périodique en temps réel.
...
- [Comment configurer un plan de continuité d’activité et reprise d’activité dans Azure Virtual Desktop](disaster-recovery.md)
...
- [Guide pratique pour sauvegarder des clés de coffre de clés dans Azure](/powershell/module/azurerm.keyvault/backup-azurekeyvaultkey?preserve-view=true&view=azurermps-6.13.0)
...
- [Implémentation de la sécurité](../backup/backup-azure-restore-files-from-vm.md#security-implementations)
...
- [Benchmark de sécurité Azure - Gestion des identités](../automation/update-management/overview.md)
