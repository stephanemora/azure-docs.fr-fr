...
Si vous utilisez Azure SQL Database avec des points de terminaison de service de réseau virtuel/Managed Instance (en préversion) qui joint un réseau vous pour héberger SSISDB, vous devez aussi joindre votre runtime d’intégration Azure-SSIS au même réseau virtuel. Azure Data Factory version 2 (préversion) vous permet de joindre votre runtime d’intégration SSIS Azure à un réseau virtuel. Pour en savoir, consultez [Joindre un runtime d’intégration Azure-SSIS à un réseau virtuel](join-azure-ssis-integration-runtime-virtual-network.md).
