...
:::image type="content" source="media/data-migration-guidance-s3-to-azure-storage/performance.png" alt-text="Le diagramme illustre plusieurs partitions de fichiers dans un magasin A W S S3 avec des actions de copie associées à Stockage Blob Azure A D L S Gen2.":::
...
:::image type="content" source="media/data-migration-guidance-s3-to-azure-storage/solution-architecture-public-network.png" alt-text="Le diagramme illustre une migration sur Internet par H T T P entre un magasin A W S S3, via Azure Integration Runtime dans A D F Azure, et le service Stockage Azure. Le runtime dispose d'un canal de contrôle auprès de Data Factory.":::
...
:::image type="content" source="media/data-migration-guidance-s3-to-azure-storage/solution-architecture-private-network.png" alt-text="Le diagramme illustre une migration sur une connexion de Peering privé entre un magasin A W S S3, via un runtime d'intégration auto-hébergé sur des machines virtuelles Azure accédant à des points de terminaison de service de réseau virtuel, et le service Stockage Azure. Le runtime dispose d'un canal de contrôle auprès de Data Factory.":::
...
:::image type="content" source="media/data-migration-guidance-s3-to-azure-storage/pricing-pipeline.png" alt-text="Le diagramme illustre un pipeline de migration des données, avec un déclencheur manuel dirigeant vers Lookup, vers ForEach, vers un sous-pipeline pour chaque partition contenant Copy, puis vers Procédure stockée. En dehors du pipeline, Procédure stockée se dirige vers Azure SQL D B, qui se dirige vers Lookup, et A W S S3 se dirige vers Copy, qui se dirige vers Stockage Blob.":::
...
:::image type="content" source="media/data-migration-guidance-s3-to-azure-storage/pricing-table.png" alt-text="Capture d'écran d'un tableau contenant une estimation de prix.":::
