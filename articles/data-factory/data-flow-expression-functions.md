...
ms.locfileid: "56409002"
---
# <a name="mapping-data-flow-expression-functions"></a>Fonctions d’expression des flux de données de mappage
...
Selon la condition, applique une valeur ou une autre. La valeur « Other » est considérée comme NULL si elle n’est pas spécifiée. Les deux valeurs doivent être compatibles (numérique, chaîne...) * ``iif(custType == 'Premium', 10, 4.5)``
* ``iif(amount > 100, 'High')``
...
Décale la chaîne sur la droite selon la marge intérieure fournie, jusqu’à ce qu’elle atteigne une certaine longueur. Si la chaîne est supérieure ou égale à cette longueur, il s’agit d’un no-op * ``rpad('great', 10, '-') -> 'great-----'``
* ``rpad('great', 4, '-') -> 'great'``
* ``rpad('great', 8, '<>') -> 'great<><'``
*********************************
...
Décale la chaîne sur la gauche selon la marge intérieure fournie, jusqu’à ce qu’elle atteigne une certaine longueur. Si la chaîne est supérieure ou égale à cette longueur, il s’agit d’un no-op * ``lpad('great', 10, '-') -> '-----great'``
* ``lpad('great', 4, '-') -> 'great'``
* ``lpad('great', 8, '<>') -> '<><great'``
*********************************
<code>left</code>
...
Le modèle est une chaîne avec une correspondance littérale, à l’exception des caractères spéciaux suivants : « _ » correspond à n’importe quel caractère de l’entrée (comme le caractère « . » dans les expressions régulières posix) ; « % » correspond à zéro, un ou plusieurs caractères de l’entrée (comme les caractères « .* » dans les expressions régulières posix).
...
Convertit un type de données primitif en une chaîne. Vous pouvez spécifier un format pour les nombres et les dates. Si aucun format n’est spécifié, la valeur système par défaut est utilisée. Le format décimal Java est utilisé pour les nombres. Le format de date par défaut est AAAA-MM-JJ * ``toString(10) -> '10'``
...
Arrondit un nombre donné selon une mise à l’échelle et un mode d’arrondi facultatifs. Si la mise à l’échelle est omise, la valeur par défaut de 0 est utilisée.  Si le mode est omis, la valeur par défaut ROUND_HALF_UP(5) est utilisée. Les valeurs d’arrondi incluent 1 - ROUND_UP 2 - ROUND_DOWN 3 - ROUND_CEILING 4 - ROUND_FLOOR 5 - ROUND_HALF_UP 6 - ROUND_HALF_DOWN 7 - ROUND_HALF_EVEN 8 - ROUND_UNNECESSARY * ``round(100.123) -> 100.0``
* ``round(2.5, 0) -> 3.0``
...
Convertit une chaîne en une date selon un format de date facultatif. Si le format de date est omis, les combinaisons suivantes sont acceptées. [ aaaa, aaaa-[M]M, aaaa-[M]M-[j]j, aaaa-[M]M-[j]j, aaaa-[M]M-[j]j, aaaa-[M]M-[j]jT* ] * ``toDate('2012-8-8') -> 2012-8-8``
* ``toDate('12/12/2012', 'MM/dd/yyyy') -> 2012-12-12``
*********************************
<code>toTimestamp</code>
==============================
<code><b>toTimestamp(<i>&lt;string&gt;</i> : any, [<i>&lt;timestamp format&gt;</i> : string], [<i>&lt;time zone&gt;</i> : string]) => timestamp</b></code><br/><br/>
Convertit une chaîne en une date selon un format d’horodatage facultatif. Si l’horodatage est omis, le modèle par défaut est utilisé : aaaa-[M]M-[j]j hh:mm:ss[.f...] est utilisé * ``toTimestamp('2016-12-31 00:12:00') -> 2012-8-8T00:12:00``
...
Vérifie si la ligne est marquée pour l’insertion. Les transformations qui acceptent plusieurs flux d’entrée peuvent passer l’index (de base 1) du flux. La valeur par défaut pour l’index de flux est 1 * ``isInsert() -> true``
* ``isInsert(1) -> false``
*********************************
<code>isUpdate</code>
==============================
<code><b>isUpdate([<i>&lt;value1&gt;</i> : integer]) => boolean</b></code><br/><br/>
Vérifie si la ligne est marquée pour la mise à jour. Les transformations qui acceptent plusieurs flux d’entrée peuvent passer l’index (de base 1) du flux. La valeur par défaut pour l’index de flux est 1 * ``isUpdate() -> true``
* ``isUpdate(1) -> false``
*********************************
<code>isDelete</code>
==============================
<code><b>isDelete([<i>&lt;value1&gt;</i> : integer]) => boolean</b></code><br/><br/>
Vérifie si la ligne est marquée pour la suppression. Les transformations qui acceptent plusieurs flux d’entrée peuvent passer l’index (de base 1) du flux. La valeur par défaut pour l’index de flux est 1 * ``isDelete() -> true``
* ``isDelete(1) -> false``
*********************************
<code>isMatch</code>
==============================
<code><b>isMatch([<i>&lt;value1&gt;</i> : integer]) => boolean</b></code><br/><br/>
Vérifie si la ligne correspond à la recherche. Les transformations qui acceptent plusieurs flux d’entrée peuvent passer l’index (de base 1) du flux. La valeur par défaut pour l’index de flux est 1 * ``isMatch() -> true``
* ``isMatch(1) -> false``
*********************************
<code>isError</code>
==============================
<code><b>isError([<i>&lt;value1&gt;</i> : integer]) => boolean</b></code><br/><br/>
Vérifie si la ligne est marquée comme comprenant une erreur. Les transformations qui acceptent plusieurs flux d’entrée peuvent passer l’index (de base 1) du flux. La valeur par défaut pour l’index de flux est 1 * ``isError() -> true``
* ``isError(1) -> false``
*********************************
<code>isIgnore</code>
==============================
<code><b>isIgnore([<i>&lt;value1&gt;</i> : integer]) => boolean</b></code><br/><br/>
Vérifie si la ligne est marquée comme devant être ignorée. Les transformations qui acceptent plusieurs flux d’entrée peuvent passer l’index (de base 1) du flux. La valeur par défaut pour l’index de flux est 1 * ``isIgnore() -> true``
* ``isIgnore(1) -> false``
*********************************
<code>sum</code>
==============================
...
Obtient la valeur des n lignes évaluées dans le premier paramètre avant la ligne actuelle. Le deuxième paramètre correspond au nombre de lignes précédentes à examiner. La valeur par défaut est 1. S’il n’y a pas autant de lignes, la valeur Null est retournée, sauf si une valeur par défaut est spécifiée. * ``lag(amount, 2) -> 60``
* ``lag(amount, 2000, 100) -> 100``
*********************************
<code>lead</code>
==============================
<code><b>lead(<i>&lt;value&gt;</i> : any, [<i>&lt;number of rows to look after&gt;</i> : number], [<i>&lt;default value&gt;</i> : any]) => any</b></code><br/><br/>
Obtient la valeur des n lignes évaluées dans le premier paramètre après la ligne actuelle. Le deuxième paramètre correspond au nombre de lignes suivantes à examiner. La valeur par défaut est 1. S’il n’y a pas autant de lignes, la valeur Null est retournée, sauf si une valeur par défaut est spécifiée. * ``lead(amount, 2) -> 60``
...
Affecte un numérotage séquentiel aux lignes d’une fenêtre en commençant par 1 * ``rowNumber() -> 1``
