...
| OPTION 2 : Préfixe S3<br>- prefix | Le préfixe pour le nom de la clé S3 sous le compartiment donné configuré dans le jeu de données pour filtrer les fichiers S3 sources. Les clés S3 dont le nom commence par `bucket_in_dataset/this_prefix` sont sélectionnées. C’est le filtre côté service de S3, qui offre de meilleures performances que le filtre de caractères génériques, qui est utilisé. | Non |
