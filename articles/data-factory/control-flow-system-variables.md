...
description: Cet article décrit les variables système prises en charge par Azure Data Factory et Azure Synapse Analytics. Vous pouvez utiliser ces variables dans des expressions lors de la définition d’entités dans l’un ou l’autre service.
author: chez-charlie
ms.author: chez
...
| @pipeline()?TriggeredByPipelineName | Nom du pipeline qui déclenche l’exécution du pipeline. Applicable lorsque l’exécution du pipeline est déclenchée par une activité ExecutePipeline. Donne la valeur _Null_ lorsqu’elle est utilisée dans d’autres circonstances. Notez le point d’interrogation après @pipeline() |
...
>[!NOTE]
>Le service s’attend à ce que les événements personnalisés soient mis en forme avec [le schéma d’événement Azure Event Grid](../event-grid/event-schema.md).
