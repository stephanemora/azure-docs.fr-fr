...
description: Découvrez comment configurer une transformation de la source dans un flux de données de mappage dans les pipelines Azure Data Factory ou Azure Synapse Analytics.
author: kromerm
ms.author: makromer
...
Dans les espaces de travail Azure Synapse, une option supplémentaire, ```Workspace DB```, est disponible pour les transformations des sources de flux de données. Cette option vous permet de sélectionner directement une base de données d’espace de travail de n’importe quel type disponible comme données sources, vous évitant ainsi d’avoir à utiliser des services liés ou jeux de données supplémentaires.
...
Les pipelines Azure Data Factory et Synapse ont accès à plus de [90 connecteurs natifs](connector-overview.md). Pour inclure dans votre flux de données des données provenant de ces autres sources, utilisez l’outil Copier l’activité pour charger ces données dans l’une des zones de transit prises en charge.
...
**Dérive de schéma** : La [dérive de schéma](concepts-data-flow-schema-drift.md) est la capacité du service à gérer nativement des schémas flexibles dans vos flux de données sans avoir besoin de définir explicitement des changements de colonnes.
...
* La sélection de l’option **Déduire les types de colonnes dérivés** ordonne au service de détecter et de définir les types de données pour chaque nouvelle colonne découverte. Si cette fonctionnalité est désactivée, toutes les colonnes dérivées seront de type chaîne.
