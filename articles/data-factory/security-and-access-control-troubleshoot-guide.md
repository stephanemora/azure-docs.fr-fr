...
description: Découvrez comment résoudre les problèmes de sécurité et de contrôle d'accès dans Azure Data Factory et Synapse Analytics.
author: lrtoyou1223
...
ms.locfileid: "124814958"
---
# <a name="troubleshoot-azure-data-factory-and-synapse-analytics-security-and-access-control-issues"></a>Résoudre les problèmes de sécurité et de contrôle d'accès dans Azure Data Factory et Synapse Analytics
...
Une fois que vous désactivez l'accès au réseau public pour le service, le runtime d'intégration auto-hébergé génère l'erreur suivante : « La clé d'authentification n'est pas valide ou vide. »
...
1. Vérifiez que vous avez créé la machine virtuelle Azure dans le même réseau virtuel que le point de terminaison privé du service.

2. Exécutez PsPing et Ping à partir de la machine virtuelle Azure dans le nom de domaine complet du service :
...
- En option, nous vous proposons d'ajouter manuellement une « liaison de réseau virtuel » sous la « zone DNS de liaison privée » du service. Pour plus d'informations, consultez l'article [Azure Private Link](./data-factory-private-link.md#dns-changes-for-private-endpoints). L'instruction consiste à configurer la zone DNS privée ou le serveur DNS privé pour résoudre le nom de domaine complet du service en adresse IP privée. 
...
- Toutefois, si vous ne voulez pas configurer la zone DNS privée ou le serveur DNS privé, essayez la solution temporaire suivante :
  ...
  1. Modifiez le fichier hôte dans Windows et mappez l'adresse IP privée (le point de terminaison privé du service) au nom de domaine complet du service.
     ...
     :::image type="content" source="media/self-hosted-integration-runtime-troubleshoot-guide/add-mapping-to-host.png" alt-text="Capture d’écran du mappage de l’adresse IP privée à l’hôte.":::
...
Pour résoudre le problème, accédez à [Azure Private Link](./data-factory-private-link.md).
...
:::image type="content" source="media/self-hosted-integration-runtime-troubleshoot-guide/enable-public-network-access.png" alt-text="Capture d'écran du contrôle 'Activé' pour 'Autoriser l'accès au réseau public' dans le volet Mise en réseau.":::

# <a name="synapse-analytics"></a>[Synapse Analytics](#tab/synapse-analytics)
:::image type="content" source="media/self-hosted-integration-runtime-troubleshoot-guide/enable-public-network-access-synapse.png" alt-text="Capture d'écran du contrôle 'Activé' pour 'Autoriser l'accès au réseau public' dans le volet Mise en réseau.":::

---

### <a name="service-private-dns-zone-overrides-azure-resource-manager-dns-resolution-causing-not-found-error"></a>La zone DNS privée du service remplace la résolution DNS d'Azure Resource Manager, entraînant une erreur « introuvable »
...
:::image type="content" source="media/security-access-control-troubleshoot-guide/private-dns-zones.png" alt-text="Capture d’écran de la recherche de zones DNS privées.":::
2. Vérifiez s’il existe un enregistrement A **adf**.
:::image type="content" source="media/security-access-control-troubleshoot-guide/a-record.png" alt-text="Capture d’écran d’un enregistrement A.":::
3.  Accédez à **Liens de réseau virtuel**, puis supprimez tous les enregistrements.
:::image type="content" source="media/security-access-control-troubleshoot-guide/virtual-network-link.png" alt-text="Capture d’écran de lien de réseau virtuel.":::
4.  Accédez à votre service dans le portail Azure et recréez le point de terminaison privé pour le portail.
:::image type="content" source="media/security-access-control-troubleshoot-guide/create-private-endpoint.png" alt-text="Capture d’écran de la recréation d’un point de terminaison privé.":::
5.  Revenez à Zones DNS privées et vérifiez s’il existe une nouvelle zone DNS privée **privatelink.adf.azure.com**.
:::image type="content" source="media/security-access-control-troubleshoot-guide/check-dns-record.png" alt-text="Capture d’écran de nouvel enregistrement DNS.":::
...
### <a name="internal-error-while-trying-to-delete-a-data-factory-or-synapse-workspace-with-customer-managed-key-cmk-and-user-assigned-managed-identity-ua-mi"></a>Erreur interne lors de la tentative de suppression d'une fabrique de données ou d'un espace de travail Synapse avec une clé gérée par le client (CMK) et une identité managée attribuée par l'utilisateur (UA-MI)
...
* Vous avez révoqué l'accès du service au coffre de clés dans lequel la clé CMK a été stockée. Vous pouvez réattribuer l'accès avec les autorisations suivantes : **Obtenir, Ne pas inclure la clé et Inclure la clé**. Ces autorisations sont requises pour activer des clés gérées par le client. Veuillez consulter [Révoquer l'accès aux clés gérées par le client](enable-customer-managed-key.md#grant-data-factory-access-to-azure-key-vault). Une fois l'autorisation fournie, vous devez pouvoir supprimer le service.
...
* L'identité managée attribuée par l'utilisateur (UA-MI) a été supprimée avant le service. Vous pouvez effectuer une récupération à l’aide d’appels d’API REST. Pour ce faire, vous pouvez utiliser le client http de votre choix dans n’importe quel langage de programmation. Si vous n’avez pas encore configuré d’appels d’API REST avec l’authentification Azure, le plus simple consiste à utiliser POSTMAN/Fiddler. Procédez comme suit.

   1.  Effectuer un appel GET à l'aide de la méthode : GET Url like   `https://management.azure.com/subscriptions/YourSubscription/resourcegroups/YourResourceGroup/providers/Microsoft.DataFactory/factories/YourFactoryName?api-version=2018-06-01`
   ...
   4.  Effectuez un appel PUT vers la même URL transmettant le nouveau corps. Il est très important que vous transmettiez ce que vous avez obtenu dans la réponse GET et ne modifiiez que l’identité. Dans le cas contraire, ces éléments remplaceront d’autres paramètres. 
