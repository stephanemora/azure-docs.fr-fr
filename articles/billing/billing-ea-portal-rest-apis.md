...
Si vous êtes un partenaire ou un développeur sans inscription Entreprise Azure et que vous souhaitez accéder à l’API, utilisez l’inscription Test. Le nom de l’inscription est _EnrollmentNumber 100_. Cette inscription vous permet d’[effectuer un test avec la clé d’accès d’inscription](https://automaticbillingspec.blob.core.windows.net/spec/TestEnrollmentUsageApiKey.txt) pour appeler l’API et voir des exemples de données.
