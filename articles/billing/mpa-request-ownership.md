...
Pour fournir une seule facture consolidée pour les services managés et la consommation Azure, le fournisseur de solutions Cloud (CSP) peut reprendre la propriété de facturation des abonnements Azure de leurs clients ayant un Contrat Entreprise (EA).
...
L’accès accordé aux utilisateurs, groupes ou principaux de service existants à l’aide du [contrôle d’accès en fonction du rôle (RBAC) Azure](../role-based-access-control/overview.md) est conservé durant le transfert. Le partenaire n’obtient pas de nouvel accès RBAC aux abonnements.  
