---
title: Comprendre votre facture de Contrat Partenaire Microsoft dans Azure
description: Découvrez comment lire et comprendre votre facture de Contrat Partenaire Microsoft dans Azure
author: bandersmsft
manager: jureid
tags: billing
ms.service: billing
...
ms.locfileid: "72512534"
---
# <a name="terms-in-your-microsoft-partner-agreement-invoice"></a>Termes de votre facture de Contrat Partenaire Microsoft

Cet article s’applique à un compte de facturation Azure associé à un Contrat Partenaire Microsoft. [Vérifiez si vous avez accès à un Contrat Partenaire Microsoft](#check-access-to-a-microsoft-partner-agreement).
...
Vous bénéficiez d’une facturation mensuelle. Les frais qui s’appliquent entre la fin de la période de facturation et la date de facturation sont inclus dans la facture du mois prochain puisqu’elles se trouvent dans la période de facturation suivante. Pour chaque facture, les dates de début et de fin de la période de facturation sont répertoriées dans la facture au format PDF située au-dessus de **Récapitulatif de facturation**.
...
## <a name="billing-details-by-product"></a>Détails de facturation par produit

La section **Détails de facturation par produit** indique le montant total des frais facturés pour chaque produit associé à ce profil de facturation. Vous pouvez voir le détail quotidien des frais de chaque produit dans le fichier CSV Utilisation et frais Azure. Pour savoir comment rapprocher la facture PDF avec le fichier CSV Utilisation et frais, consultez [Comprendre votre facture de Contrat Partenaire Microsoft](billing-mpa-understand-your-bill.md).

## <a name="how-to-pay"></a>Comment acheter

En bas de la facture, vous trouverez des instructions de paiement. Vous pouvez payer par chèque ou par virement dans un délai de 60 jours à compter de la date de votre facture.
