...
Selon la façon dont vous avez déployé Azure Stack, commencez par créer un principal de service. Ce document décrit la création d’un principal de service à la fois pour [Azure Active Directory (Azure AD)](#create-service-principal-for-azure-ad) et pour [Active Directory Federation Services (AD FS)](#create-service-principal-for-ad-fs). Une fois que vous avez créé le principal de service, une série d’étapes communes à AD FS et à Azure Active Directory permet de [déléguer des autorisations](#assign-role-to-service-principal) à ce rôle.     
...
Lorsque c’est fait, [attribuez un rôle à votre application](#assign-role-to-service-principal).
...
1. Ouvrez une session Windows PowerShell avec élévation de privilèges et exécutez les commandes suivantes :
   ...
   > [!NOTE]
   > Cet exemple crée un certificat auto-signé. Quand vous exécutez ces commandes dans un déploiement de production, utilisez [Get-Certificate](/powershell/module/pkiclient/get-certificate) pour récupérer l’objet de certificat pour le certificat que vous voulez utiliser.
...
Une fois le principal de service créé, vous devez [lui attribuer un rôle](#assign-role-to-service-principal).
