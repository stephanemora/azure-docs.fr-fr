...
* Deux serveurs et d’autres prérequis conformes aux exigences matérielles ASDK, comme décrit dans [Démarrage rapide : Évaluer le Kit de développement Azure Stack](azure-stack-deploy-overview.md).
...
> [!NOTE]
> Suivez les [instructions de déploiement](azure-stack-run-powershell-script.md) pour chacun des environnements ASDK que vous déployez. Dans cet article, les environnements ASDK sont appelés **POC1** et **POC2**.
