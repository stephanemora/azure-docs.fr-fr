...
L’horodateur dans les noms d’objets blob dans le dossier `PT=Time` correspond à l’heure d’arrivée des données dans TSI (ce n’est pas l’horodateur des événements).

Les données du dossier `PT=TsId` seront optimisées pour les requêtes au fil du temps et ne sont pas statiques. Durant le repartitionnement, les mêmes événements peuvent être présents dans plusieurs objets blob. Par ailleurs, le nommage des objets blob peut être amené à changer.
