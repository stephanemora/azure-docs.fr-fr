...
L’article suivant décrit comment inscrire un serveur au service de synchronisation de stockage et le gérer. Consultez [Comment déployer Azure File Sync](file-sync-deployment-guide.md) pour plus d’informations sur comment déployer Azure File Sync de bout en bout.
...
* Votre appareil doit exécuter une version prise en charge de Windows Server. Pour plus d’informations, consultez [Configuration requise et interopérabilité d’Azure File Sync](file-sync-planning.md#windows-file-server-considerations).
* Vérifiez qu’un service de synchronisation de stockage a été déployé. Pour plus d’informations sur comment déployer un service de synchronisation de stockage, consultez [Comment déployer Azure File Sync](file-sync-deployment-guide.md).
