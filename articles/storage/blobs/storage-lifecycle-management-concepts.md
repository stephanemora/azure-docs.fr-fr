...
| prefixMatch | Un ensemble de chaînes pour les préfixes à mettre en correspondance. Chaque règle peut définir jusqu’à 10 préfixes sensibles à la casse. Une chaîne de préfixe doit commencer par un nom de conteneur. Par exemple, si vous souhaitez faire correspondre tous les objets blob sous `https://myaccount.blob.core.windows.net/container1/foo/...` pour une règle, prefixMatch est `container1/foo`. | Si vous ne définissez pas prefixMatch, la règle s’applique à tous les objets blob au sein du compte de stockage. | Non |
...
> [!NOTE]
> Pour en savoir plus sur la fonctionnalité d’index d’objets Blob ainsi que sur les problèmes et limitations connus, consultez [Gérer et rechercher des données dans Stockage Blob Azure avec un index d’objets Blob](storage-manage-find-blobs.md).
...
L’option **Dernier accès** est disponible en préversion dans toutes les régions publiques.
...
Cela peut prendre jusqu’à 24 heures avant que la stratégie mise à jour ne soit appliquée. Une fois la stratégie en vigueur, cela peut prendre jusqu’à 24 heures pour que les actions s’exécutent. Par conséquent, l’exécution des actions de la stratégie peut prendre jusqu’à 48 heures. Si la mise à jour consiste à désactiver ou à supprimer une règle et que enableAutoTierToHotFromCool a été utilisé, la hiérarchisation automatique vers le niveau Chaud se produit toujours. Par exemple, définissez une règle incluant enableAutoTierToHotFromCool basée sur le dernier accès. Si la règle est désactivée/supprimée et qu’un objet Blob est actuellement à froid, puis accédé, il repassera à Chaud, car il sera appliqué à l’accès en dehors de la gestion du cycle de vie. L’objet Blob ne passe pas de Chaud à Froid étant donné que la règle de gestion du cycle de vie est désactivée/supprimée.  La seule façon d’empêcher autoTierToHotFromCool consiste à désactiver le suivi de l’heure du dernier accès. 
