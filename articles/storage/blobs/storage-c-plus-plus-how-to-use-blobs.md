...
> [!NOTE]
> Ce guide cible la bibliothèque cliente Azure Storage pour C++ version 1.0.0 et les versions ultérieures, jusqu’à la version 12.x. Microsoft recommande d’utiliser la dernière version de la bibliothèque cliente pour C++, disponible via [GitHub](https://github.com/Azure/azure-sdk-for-cpp/tree/master/sdk/storage). Pour plus d’informations, consultez le [guide de démarrage rapide](quickstart-blobs-c-plus-plus.md) le plus récent.
...
Pour tester votre application sur votre ordinateur Windows local, vous pouvez utiliser [l’émulateur de stockage Azurite](../common/storage-use-azurite.md). Azurite est un utilitaire qui simule sur votre ordinateur de développement local les services Blob, File d’attente et Table disponibles dans Azure. L’exemple suivant vous montre comment déclarer un champ statique pour qu’il contienne une chaîne de connexion vers votre émulateur de stockage local :
