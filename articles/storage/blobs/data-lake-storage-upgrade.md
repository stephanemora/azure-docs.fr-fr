---
title: Mettre à niveau vos solutions d’analytique de Big Data d’Azure Data Lake Storage Gen1 vers Azure Data Lake Storage Gen2 (préversion)
description: Mettre à niveau votre solution pour utiliser Azure Data Lake Storage Gen2 (préversion)
services: storage
author: normesta
ms.topic: conceptual
ms.author: normesta
...
ms.locfileid: "53098540"
---
# <a name="upgrade-your-big-data-analytics-solutions-from-azure-data-lake-storage-gen1-to-azure-data-lake-storage-gen2-preview"></a>Mettre à niveau vos solutions d’analytique de Big Data d’Azure Data Lake Storage Gen1 vers Azure Data Lake Storage Gen2 (préversion)

Si Azure Data Lake Storage Gen1 fait partie de vos solutions d’analytique de Big Data, ce guide vous permet de mettre à jour ces solutions pour utiliser Azure Data Lake Storage Gen2 (préversion). Vous pouvez utiliser ce document pour évaluer les dépendances entre votre solution et Data Lake Storage Gen1. Ce guide vous explique également comment planifier et effectuer la mise à niveau.
...
La fonctionnalité de stockage de la **Figure1** est fournie par Data Lake Storage Gen1 (**Figure 2**). Notez comment les différents composants du flux de données interagissent avec Data Lake Storage Gen1 via l’API REST ou le SDK Java. Notez également la façon dont les composants de fonctionnalités transversales interagissent avec Data Lake Storage Gen1. Le composant d’approvisionnement utilise des modèles de ressources Azure, tandis que le composant de surveillance s’appuyant sur Log Analytics utilise les données opérationnelles provenant de Data Lake Storage Gen1.
...
|-----------------------|-----------------|----------------------|----------------------------------|
| Organisation des données| Prend en charge les données stockées en tant que fichiers et dossiers | Prend en charge les données stockées en tant qu’objets/objets blob, ainsi que les dossiers et fichiers - [Lien](https://docs.microsoft.com/azure/storage/data-lake-storage/namespace) | Prend en charge les données stockées en tant que dossiers et fichiers – *Déjà disponible* Prend en charge les données stockées en tant qu’objets/objets blob - *Pas encore disponible* |
| Espace de noms| Espace de noms hiérarchique| Espace de noms plat et espaces de noms hiérarchiques | Espace de noms plat : *Déjà disponible*  |
| API  | API REST sur HTTPS | API REST sur HTTP/HTTPS| *Déjà disponible* |
| API côté serveur| [API REST compatible WebHDFS](https://msdn.microsoft.com/library/azure/mt693424.aspx) | API REST du service Blob Azure [API REST de Data Lake Storage Gen2](https://docs.microsoft.com/rest/api/storageservices/data-lake-storage-gen2) | *API REST de Data Lake Storage Gen2 – Déjà disponible API REST du service Blob Azure – Pas encore disponible*       |
| Client de système de fichiers Hadoop | Oui ([Azure Data Lake Storage](https://hadoop.apache.org/docs/current/hadoop-azure-datalake/index.html)) | Oui ([ABFS](https://jira.apache.org/jira/browse/HADOOP-15407))  | *Déjà disponible*  |  | [Clé partagée](https://docs.microsoft.com/rest/api/storageservices/authorize-with-shared-key), [Identités Azure Active Directory](https://docs.microsoft.com/azure/active-directory/active-directory-authentication-scenarios), [Signatures d’accès partagé (SAP)](https://docs.microsoft.com/azure/storage/common/storage-dotnet-shared-access-signature-part-1?toc=%2fazure%2fstorage%2fqueues%2ftoc.json) | *Déjà disponible*  |
...
| Opérations de données - Journaux  | Oui | Requêtes uniques de journaux pour une durée spécifique avec intégration Azure Monitoring des tickets de support | Requêtes uniques de journaux pour une durée spécifique avec Intégration Azure Monitoring – *Pas encore disponible* des tickets de support - *Déjà disponible* |
| Chiffrement des données au repos | Transparent, côté serveur, avec clés gérées par le service et clés gérées par le client dans Azure KeyVault | Transparent, côté serveur, avec clés gérées par le service et clés gérées par le client dans Azure KeyVault | Clés gérées par le service – *Déjà disponible* Clés gérées par le client – *Déjà disponible*  |
| Réseau virtuel  | [Intégration de réseau virtuel (préversion)](https://docs.microsoft.com/azure/data-lake-store/data-lake-store-network-security)  | [Point de terminaison de service](https://docs.microsoft.com/azure/storage/common/storage-network-security?toc=%2fazure%2fstorage%2fblobs%2ftoc.json) | *Déjà disponible* | Opérations de gestion (par exemple, créer un compte) | [Le contrôle d’accès en fonction du rôle](https://docs.microsoft.com/azure/role-based-access-control/overview) (RBAC) fournit par Azure pour la gestion des comptes ; | [Le contrôle d’accès en fonction du rôle](https://docs.microsoft.com/azure/role-based-access-control/overview) (RBAC) fournit par Azure pour la gestion des comptes ; | *Déjà disponible*| Kits de développement logiciel pour développeur | .NET, Java, Python, Node.js  | .NET, Java, Python, Node.js, C++, Ruby, PHP, Go, Android, iOS| *Pas encore disponible* | Optimisation des performances pour les charges de travail d’analyses parallèles. Débit et nombre d’E/S par seconde élevés. | Optimisation des performances pour les charges de travail d’analyses parallèles. Débit et nombre d’E/S par seconde élevés. | *Déjà disponible* |
...
| Gestion des données                             | Expiration des fichiers                                                                                                                                        | Stratégies de cycle de vie                                                                                                                                                                                                                                                                                                                                                                                                          | *Pas encore disponible*                                                                                                                       |

### <a name="programming-interfaces"></a>Interfaces de programmation

Ce tableau décrit les ensembles d’API disponibles pour vos applications personnalisées. Pour plus de clarté, nous avons réparti ces ensembles d’API en 2 types : API de gestion et API de système de fichiers.

Les API de gestion permettent de gérer les comptes, tandis que les API de système de fichiers vous permettent d’effectuer des opérations sur les fichiers et les dossiers.
...
|----------------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------|
| SDK .NET - gestion                  | [Lien](https://docs.microsoft.com/dotnet/api/overview/azure/datalakestore/management?view=azure-dotnet)                                                                                                                                                                                                                 | Non pris en charge                                                      | *Déjà disponible -* [Lien](https://docs.microsoft.com/rest/api/storageservices/operations-on-the-account--blob-service-)                                    |
| SDK .NET – système de fichiers                  | [Lien](https://docs.microsoft.com/dotnet/api/overview/azure/datalakestore/client?view=azure-dotnet)                                                                                                                                                                                                                     | *Pas encore disponible*                                                | *Pas encore disponible*                                                                                                                                             |
| SDK Java- gestion                  | [Lien](https://docs.microsoft.com/java/api/overview/azure/datalakestore/management)                                                                                                                                                                                                                                     | Non pris en charge                                                      | *Déjà disponible -* [Lien](https://docs.microsoft.com/java/api/com.microsoft.azure.storage.blob?view=azure-java-stable)                                     |
| SDK Java – système de fichiers                  | [Lien](https://docs.microsoft.com/java/api/overview/azure/datalakestore/client)                                                                                                                                                                                                                                         | *Pas encore disponible*                                                | *Pas encore disponible*                                                                                                                                             |
| Node.js - gestion                   | [Lien](https://www.npmjs.com/package/azure-arm-datalake-store)                                                                                                                                                                                                                                                                | Non pris en charge                                                      | *Déjà disponible -* [Lien](http://azure.github.io/azure-storage-node/)                                                                                            |
| Node.js - système de fichiers                   | [Lien](https://www.npmjs.com/package/azure-arm-datalake-store)                                                                                                                                                                                                                                                                | *Pas encore disponible*                                                | *Pas encore disponible*                                                                                                                                             |
| Python - gestion                    | [Lien](https://docs.microsoft.com/python/api/overview/azure/datalakestore/management?view=azure-python)                                                                                                                                                                                                                 | Non pris en charge                                                      | *Déjà disponible -* [Lien](https://docs.microsoft.com/python/api/overview/azure/storage/management?view=azure-python)                                       |
| Python - système de fichiers                    | [Lien](http://azure-datalake-store.readthedocs.io/en/latest/)                                                                                                                                                                                                                                                                 | *Pas encore disponible*                                                | *Pas encore disponible*                                                                                                                                             |
| API REST - gestion                  | [Lien](https://docs.microsoft.com/rest/api/datalakestore/accounts)                                                                                                                                                                                                                                                      | Non pris en charge                                                      | *Déjà disponible -*                                                                                                                                               |
| API REST - système de fichiers                  | [Lien](https://docs.microsoft.com/rest/api/datalakestore/webhdfs-filesystem-apis)                                                                                                                                                                                                                                       | *Déjà disponible*                                                    | *Déjà disponible -* [Lien](https://docs.microsoft.com/rest/api/storageservices/data-lake-storage-gen2)                                                      |
| PowerShell - gestion et système de fichiers | [Lien](https://docs.microsoft.com/powershell/module/azurerm.datalakestore/?view=azurermps-6.3.0)                                                                                                                                                                                                                        | Gestion – Pas pris en charge Système de fichiers - *Pas encore disponible*        | Gestion – *Déjà disponible -* [Lien](https://docs.microsoft.com/powershell/module/azure.storage/?view=azurermps-6.13.0) Système de fichiers - *Pas encore disponible* |
| CLI – gestion                       | [Lien](https://docs.microsoft.com/cli/azure/dls/account?view=azure-cli-latest)                                                                                                                                                                                                                                          | Non pris en charge                                                      | *Déjà disponible -* [Lien](https://docs.microsoft.com/cli/azure/storage?view=azure-cli-latest)                                                              |
| CLI - système de fichiers                       | [Lien](https://docs.microsoft.com/cli/azure/dls/fs?view=azure-cli-latest)                                                                                                                                                                                                                                               | *Pas encore disponible*                                                | *Pas encore disponible*                                                                                                                                             |
| Modèles Azure Resource Manager - gestion             | [Template1](https://azure.microsoft.com/resources/templates/101-data-lake-store-no-encryption/)  [Template2](https://azure.microsoft.com/resources/templates/101-data-lake-store-encryption-adls/)  [Template3](https://azure.microsoft.com/resources/templates/101-data-lake-store-encryption-key-vault/)  | Non pris en charge                                                      | *Déjà disponible -* [Lien](https://docs.microsoft.com/azure/templates/microsoft.storage/2018-07-01/storageaccounts)                                         |
...
|                      | [HDInsight ](https://docs.microsoft.com/azure/data-lake-store/data-lake-store-hdinsight-hadoop-use-portal)                                                               | [HDInsight](https://docs.microsoft.com/azure/storage/data-lake-storage/quickstart-create-connect-hdi-cluster) 3.6 - *Déjà disponible* HDInsight 4.0 - *Pas encore disponible*      | HDInsight 3.6 ESP – *Pas encore disponible* HDInsight 4.0 ESP - *Pas encore disponible*                                                                 |
|                      | Databricks Runtime 3.1 et versions ultérieures                                                                                                                                               | [Databricks Runtime 4.2 et versions ultérieures](https://docs.azuredatabricks.net/spark/latest/data-sources/azure/azure-datalake-gen2.html#azure-data-lake-storage-gen2) *- Déjà disponible* | Databricks Runtime 5.1 et versions ultérieures – *Déjà disponible*                                                                                              |
...
|                      | [SQL Server Integration Services ](https://docs.microsoft.com/sql/integration-services/connection-manager/azure-data-lake-store-connection-manager?view=sql-server-2017) | *Pas encore disponible*                                                                                                                                                          | *Pas encore disponible*                                                                                                                             |
...
|                      | [Logic Apps ](https://docs.microsoft.com/connectors/azuredatalake/)                                                                                                      | *Pas encore disponible*                                                                                                                                                          | *Pas encore disponible*                                                                                                                             |
| IoT                  | [Event Hubs – Capture ](https://docs.microsoft.com/azure/data-lake-store/data-lake-store-archive-eventhub-capture)                                                       | *Pas encore disponible*                                                                                                                                                          | *Pas encore disponible*                                                                                                                             |
|                      | [Stream Analytics ](https://docs.microsoft.com/azure/data-lake-store/data-lake-store-stream-analytics)                                                                   | *Pas encore disponible*                                                                                                                                                          | *Pas encore disponible*                                                                                                                             |
| Consommation          | [PowerBI Desktop  ](https://docs.microsoft.com/azure/data-lake-store/data-lake-store-power-bi)                                                                           | *Pas encore disponible*                                                                                                                                                          | *Pas encore disponible*                                                                                                                             |
...
| Productivité         | [Portail Azure](https://docs.microsoft.com/azure/data-lake-store/data-lake-store-get-started-portal)                                                                      | *Non pris en charge*                                                                                                                                                              | Gestion des comptes *– Déjà disponible* Opérations de données *–* *Pas encore disponible*                                                                   |
...
|---------------------|--------------|----------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------|
| Infrastructure d’analyse | Cloudera     | CDH                  | [Lien](https://www.cloudera.com/documentation/enterprise/5-11-x/topics/admin_adls_config.html)                                                                                            | *Pas encore disponible*                                                                                                  | *Pas encore disponible*                                                                                                  |
|                     | Cloudera     | Altus                | [Lien](https://www.cloudera.com/more/news-and-blogs/press-releases/2017-09-27-cloudera-introduces-altus-data-engineering-microsoft-azure-hybrid-multi-cloud-data-analytic-workflows.html) | N/D                                                                                                                   | *Pas encore disponible*                                                                                                  |
...
| Journaux d’activité                                                                                          | [Lien](https://docs.microsoft.com/azure/data-lake-store/data-lake-store-diagnostic-logs#audit-logs)   | Requêtes uniques de journaux pour une durée spécifique avec Intégration Azure Monitoring – *Pas encore disponible* des tickets de support - *Déjà disponible* |
| Journaux de diagnostic                                                                                        | [Lien](https://docs.microsoft.com/azure/data-lake-store/data-lake-store-diagnostic-logs#request-logs) | Requêtes uniques de journaux pour une durée spécifique avec Intégration Azure Monitoring – *Pas encore disponible* des tickets de support - *Déjà disponible* |
...
|------------------------------------|-----------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **Lift-and-shift**                 | [Azure Data Factory](https://docs.microsoft.com/azure/data-factory/load-azure-data-lake-storage-gen2-from-gen1) | Service cloud managé                                                                                                                | Ne copie que les données. Les listes de contrôle d’accès ne peuvent pas être copiées actuellement.                                                                                                                                                                                                                                                                      |
...
| **Copie ponctuelle et copie incrémentielle** | Azure Data Factory                                                                                                    | Service cloud managé                                                                                                                | Pour la prise en charge de la copie incrémentielle dans ADF, les données doivent être organisées en séries chronologiques. Le plus court intervalle entre les copies incrémentielles est de [15 minutes](https://docs.microsoft.com/azure/data-factory/how-to-create-tumbling-window-trigger). Pour des intervalles plus courts, ADF ne fonctionne pas. Les listes de contrôle d’accès ne peuvent pas être copiées actuellement. |
| **Adoption parallèle**              | [WANdisco](http://docs.wandisco.com/bigdata/wdfusion/adls/)                                                           | Prise en charge la réplication cohérente. En cas d’utilisation d’un environnement 100 % Hadoop connecté à Azure Data Lake Storage, prise en charge de la réplication bidirectionnelle | Si vous n’utilisez pas d’environnement 100 % Hadoop, il peut y avoir un délai dans la réplication.                                                                                                                                                                                                                                                  |

Notez que certains produits tiers peuvent gérer la mise à niveau de Data Lake Storage Gen1 vers Data Lake Storage Gen2 sans passer par les outils de copie de données/métadonnées présentés ci-dessus (par exemple : [Cloudera](http://blog.cloudera.com/blog/2017/08/use-amazon-s3-with-cloudera-bdr/)). Ils offrent une expérience centralisée qui permet la migration des données ainsi que la migration des charges de travail. Vous devrez peut-être effectuer une mise à niveau hors-bande pour les outils en dehors de leur écosystème.
...
* Data Lake Storage Gen2 prend uniquement en charge les fichiers de 5 To maximum. Pour vous mettre à niveau vers Data Lake Storage Gen2, vous devrez peut-être redimensionner les fichiers supérieurs à cette taille dans Data Lake Storage Gen1.
...
La tâche principale ici consiste à transformer l’URI adl:// utilisé dans les charges de travail existantes en URI abfss://.
...
Le schéma d’URI permettant d’accéder aux fichiers Data Lake Storage Gen2 est expliqué [ici](https://docs.microsoft.com/azure/storage/data-lake-storage/use-hdi-cluster?branch=release-preview-abfs) en détail, mais de manière générale, il suit le format *abfss://\<NOM_SYSTÈME_FICHIERS\>\@\<NOM_COMPTE\>.dfs.core.windows.net/\<CHEMIN_ACCÈS\>.*
...
Une fois la transition terminée, les étapes finales impliquent une vérification approfondie de la mise à niveau. Cela implique notamment de vérifier que les données ont été copiées de manière fiable, que les listes de contrôle d’accès ont été configurées correctement, que les pipelines E2E fonctionnent, etc. Une fois les vérifications terminées, vous pouvez arrêter vos anciens pipelines, supprimer vos comptes source Data Lake Storage Gen1 et adopter pleinement vos solutions basées sur Data Lake Storage Gen2.
