...
> [!IMPORTANT]
> Le stockage immuable pour le stockage blob Azure dans les comptes pour lesquels la fonctionnalité d’espace de noms hiérarchique activée est actuellement en PRÉVERSION.
...
**Puis-je utiliser la suppression conditionnelle en même temps que les stratégies blob immuables ?**

Oui, si vos exigences de conformité autorisent l’activation de la suppression réversible. [La suppression conditionnelle pour le Stockage blob Azure](./soft-delete-blob-overview.md) s’applique à tous les conteneurs d’un compte de stockage, indépendamment de la conservation légale ou de la stratégie de rétention limitée dans le temps en vigueur. Microsoft recommande d’activer la suppression conditionnelle pour une protection supplémentaire avant d’appliquer des stratégies immuables. Si la suppression conditionnelle est activée sur un conteneur, puis qu’une stratégie d’immuabilité est ajoutée au conteneur, tous les blobs qui ont déjà été supprimés conditionnellement seront définitivement supprimés une fois la stratégie de rétention de la suppression conditionnelle expirée. Les blobs supprimés de manière conditionnelle peuvent être restaurés au cours de la période de rétention de la suppression conditionnelle. Les blobs qui n’ont pas encore été supprimés de manière conditionnelle sont protégés par la stratégie d’immuabilité et ne peuvent pas être supprimés de manière conditionnelle tant que la stratégie immuable n’a pas expiré (pour la rétention basée sur le temps) ou n’a pas été supprimée (pour les conservations légales).
