---
title: Autoriser l’accès à des données de blob à l’aide d’une identité managée
titleSuffix: Azure Storage
description: Utilisez des identités managées pour les ressources Azure afin d’autoriser l’accès à des données de blob à partir d’applications s’exécutant dans des machines virtuelles Azure, des applications de fonction, etc.
services: storage
author: tamram
ms.service: storage
ms.topic: how-to
...
ms.locfileid: "129859650"
---
# <a name="authorize-access-to-blob-data-with-managed-identities-for-azure-resources"></a>Autoriser l’accès à des données de blob à l’aide d’identités managées pour les ressources Azure

Stockage Blob Azure prend en charge l’authentification Azure Active Directory (Azure AD) avec des [identités managées pour les ressources Azure](../../active-directory/managed-identities-azure-resources/overview.md). Les identités managées pour les ressources Azure autorisent l’accès à des données de blob en utilisant les informations d’identification Azure AD d’applications s’exécutant dans des machines virtuelles Azure, d’applications de fonction, de groupes de machines virtuelles identiques et d’autres services. En utilisant des identités managées pour ressources Azure et Azure AD Authentication, vous pouvez éviter de stocker des informations d’identification avec les applications qui s’exécutent dans le cloud.

Cet article montre comment autoriser l’accès à des données de blob à partir d’une machine virtuelle Azure à l’aide d’identités managées pour les ressources Azure.

## <a name="enable-managed-identities-on-a-vm"></a>Activer les identités managées sur une machine virtuelle

Avant de pouvoir utiliser des identités managées pour les ressources Azure pour autoriser l’accès à des blobs à partir de votre machine virtuelle, vous devez activer les identités managées pour les ressources Azure sur la machine virtuelle. Pour savoir comment activer des identités managées pour ressources Azure, consultez un de ces articles :
...
## <a name="use-a-managed-identity-to-create-a-block-blob-in-net"></a>Utiliser une identité managée pour créer un objet blob de blocs dans .NET
...
L’exemple de code suivant montre comment obtenir un jeton d’accès et l’utiliser pour créer un objet client de service, puis comment utiliser le client de service pour charger un nouveau blob :
...
> [!NOTE]
> Pour autoriser les requêtes sur des données de blob avec Azure AD, vous devez utiliser le protocole HTTPS pour ces requêtes.
