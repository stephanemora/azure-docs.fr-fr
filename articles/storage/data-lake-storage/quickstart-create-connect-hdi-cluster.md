...
Data Lake Storage est utilisé comme couche données dans ce guide de démarrage rapide. Avec son service d’espace de noms hiérarchique et un [pilote Hadoop](abfs-driver.md), Data Lake Storage est optimisé pour l’analyse et le traitement distribués. Les données stockées dans Data Lake Storage persistent même après la suppression d’un cluster HDInsight.
...
![hdinsight options de création personnalisée création rapide](media/quickstart-create-connect-hdi-cluster/hdinsight-creation-options.png)
...
* Utilisateur SSH (clusters Linux) : il est utilisé pour se connecter au cluster à l'aide de SSH. Pour en savoir plus, voir [Utilisation de SSH avec Hadoop Linux sur HDInsight depuis Linux, Unix ou OS X](../../hdinsight/hdinsight-hadoop-linux-use-ssh-unix.md).
...
| --- | --- | --- |
| Hadoop |Nœud principal (2), nœud de données (1+) |![Nœuds de cluster Hadoop HDInsight](media/quickstart-create-connect-hdi-cluster/hdinsight-hadoop-cluster-type-nodes.png) |
| hbase |Serveur principal (2), serveur de région (1+), nœud principal/ZooKeeper (3) |![Nœuds de cluster HDInsight HBase](media/quickstart-create-connect-hdi-cluster/hdinsight-hbase-cluster-type-setup.png) |
| Storm |Nœud Nimbus (2), serveur supervisor (1+), nœud ZooKeeper (3) |![Nœuds de cluster HDInsight Storm](media/quickstart-create-connect-hdi-cluster/hdinsight-storm-cluster-type-setup.png) |
| Spark |Nœud principal (2), nœud worker (1+), nœud ZooKeeper (3) (gratuits pour les machines virtuelles ZooKeeper A1) |![Nœuds de cluster HDInsight Spark](media/quickstart-create-connect-hdi-cluster/hdinsight-spark-cluster-type-setup.png) |
...
![Tailles de nœuds de machine virtuelle HDInsight](media/quickstart-create-connect-hdi-cluster/hdinsight-node-sizes.png)
...
- [Pilote de système de fichiers ABFS Hadoop pour Azure Data Lake Storage Gen2](abfs-driver.md)
- [Tutoriel : Extraire, transformer et charger des données à l’aide d’Apache Hive sur Azure HDInsight](tutorial-extract-transform-load-hive.md)
