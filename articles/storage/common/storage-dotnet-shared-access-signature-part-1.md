...
* **SAP avec stratégie d’accès stockée :** une stratégie d’accès stockée est définie dans un conteneur de ressources (conteneur d’objets blob, table, file d’attente ou partage de fichiers) et permet de gérer les contraintes d’une ou de plusieurs signatures d’accès partagé. Quand vous associez une signature d’accès partagé à une stratégie d’accès stockée, la signature hérite des contraintes (heure de début, heure d’expiration et autorisations) définies pour la stratégie.
