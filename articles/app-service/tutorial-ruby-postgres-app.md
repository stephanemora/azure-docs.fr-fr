...
1. Ouvrez la fenêtre de terminal et exécutez `psql` pour vous connecter à votre serveur Postgres local.
...
1. Saisissez `\q` pour quitter le client Postgres. 

1. Créez un utilisateur Postgres qui peut créer des bases de données en exécutant la commande suivante, à l’aide de votre nom d’utilisateur de connexion à Linux.
...
1. Exécutez les [migrations de Rails](https://guides.rubyonrails.org/active_record_migrations.html#running-migrations) pour créer les tables requises par l’application. Pour identifier les tables créées dans les migrations, consultez le répertoire _db/migrate_ dans le dépôt Git.
...
1. Dans un navigateur, accédez à `http://localhost:3000`. Ajoutez quelques tâches dans la page.

    ![Ruby on Rails se connecte correctement à Postgres](./media/tutorial-ruby-postgres-app/postgres-connect-success.png)

1. Pour arrêter le serveur Rails, tapez `Ctrl + C` dans le terminal.
...
1. De retour dans le terminal local, définissez les variables d’environnement suivantes :
...
1. Exécutez les migrations de base de données Rails avec les valeurs de production que vous venez de configurer pour créer les tables dans votre base de données Postgres dans Azure Database pour PostgreSQL.

    ```bash
    rake db:migrate RAILS_ENV=production
    ```

1. Durant l’exécution dans l’environnement de production, l’application Rails nécessite des ressources précompilées. Générez les ressources nécessaires à l’aide de la commande suivante :

    ```bash
    rake assets:precompile
    ```

1. L’environnement de production Rails utilise également des secrets pour gérer la sécurité. Générez une clé secrète.

    ```bash
    rails secret
    ```

1. Enregistrez la clé secrète dans les variables respectives utilisées par l’environnement de production Rails. Pour plus de commodité, utilisez la même clé pour les deux variables.
...
1. Activez l’environnement de production Rails pour traiter les fichiers CSS et JavaScript.

    ```bash
    export RAILS_SERVE_STATIC_FILES=true
    ```

1. Exécutez l’exemple d'application dans l'environnement de production.

    ```bash
    rails server -e production
    ```

1. Accédez à `http://localhost:3000`. Si la page se charge sans erreur, l’application Ruby on Rails se connecte à la base de données Postgres dans Azure.
...
1. Exécutez les commandes Git suivantes pour valider vos modifications :
...
1. Dans le terminal local, [générez un nouveau secret](configure-language-ruby.md#set-secret_key_base-manually) pour l’environnement de production Rails dans Azure.

    ```bash
    rails secret
    ```

1. Dans la commande Cloud Shell suivante, remplacez les deux espaces réservés _&lt;output-of-rails-secret>_ par la nouvelle clé secrète que vous avez générée dans le terminal local.
...
1. Étant donné que vous déployez la branche `main`, vous devez définir la branche de déploiement par défaut pour votre application App Service sur `main` (voir [Modifier la branche de déploiement](deploy-local-git.md#change-deployment-branch)). Dans Cloud Shell, définissez le paramètre d’application `DEPLOYMENT_BRANCH` à l’aide de la commande [`az webapp config appsettings set`](/cli/azure/webapp/config/appsettings#az_webapp_config_appsettings_set). 

    ```azurecli-interactive
    az webapp config appsettings set --name <app-name> --resource-group myResourceGroup --settings DEPLOYMENT_BRANCH='main'
    ```

1. Dans la fenêtre du terminal local, ajoutez un référentiel distant Azure à votre dépôt Git local.

    ```bash
    git remote add azure <paste-copied-url-here>
    ```

1. Effectuez une transmission de type push vers le référentiel distant Azure pour déployer l’application Ruby on Rails. Le mot de passe que vous avez fourni précédemment dans le cadre de la création de l’utilisateur du déploiement vous est demandé.
...
1. Dans le terminal, accédez à la racine du référentiel Git.

1. Générez une nouvelle migration qui ajoute une colonne booléenne appelée `Done` à la table `Tasks` :
...
1. Dans le terminal, exécutez les migrations de base de données Rails pour apporter la modification dans la base de données locale.
...
1. Ouvrez le fichier *app/controllers/tasks_controller.rb*. Ajoutez la ligne suivante à la fin du fichier :

    ```rb
    params.require(:task).permit(:Description)
    ```

1. Modifiez cette ligne pour inclure le nouveau paramètre `Done`.
...
1. Ouvrez le fichier *app/views/tasks/_form.html.erb*, qui est le formulaire d’édition.

1. Recherchez la ligne `<%=f.error_span(:Description) %>` et insérez le code suivant juste en dessous :
...
1. Ouvrez le fichier *app/views/tasks/show.html.erb*, qui est la page de vue à enregistrement unique. 
...
1. Dans le même fichier, recherchez la ligne `<td><%= task.Description %></td>` et insérez le code suivant juste en dessous :
...
1. Dans le terminal local, exécutez le serveur Rails.

    ```bash
    rails server
    ```

1. Pour afficher le changement d’état de la tâche, accédez à `http://localhost:3000`, puis ajoutez ou modifiez des éléments.
...
1. Dans le terminal, exécutez les migrations de base de données Rails pour l’environnement de production afin d’apporter la modification dans la base de données Azure.
