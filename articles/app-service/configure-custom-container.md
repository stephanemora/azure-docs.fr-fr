...
Lorsque votre application s’exécute, les paramètres d’application App Service sont automatiquement injectés dans le processus en tant que variables d’environnement. 
...
Par défaut, le stockage persistant est *activé* et le paramètre n’est pas exposé dans les paramètres d’application. Pour le désactiver, définissez le paramètre d’application `WEBSITES_ENABLE_APP_SERVICE_STORAGE` via le [Cloud Shell](https://shell.azure.com). Dans Bash :
...
Activez le stockage persistant en définissant le paramètre d’application `WEBSITES_ENABLE_APP_SERVICE_STORAGE`, à l’aide de la commande [az webapp config appsettings set](/cli/azure/webapp/config/appsettings?view=azure-cli-latest#az-webapp-config-appsettings-set) dans le [Cloud Shell](https://shell.azure.com).
