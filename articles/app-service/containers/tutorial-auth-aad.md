...
Enregistrez vos modifications. De retour dans la _fenêtre du terminal local_, exécutez les commandes suivantes afin de valider vos modifications dans le référentiel Git.
...
> [!NOTE]
> Ne vous inquiétez pas au sujet du partage de ce code entre les applications frontales et principales. Cela n’a aucune incidence CORS sur l’application frontale.
> 
...
Pour plus d’informations sur la façon de configurer cela pour les autres fournisseurs, consultez [Refresh access tokens](../app-service-authentication-how-to.md?toc=%2fazure%2fapp-service%2fcontainers%2ftoc.json#refresh-identity-provider-tokens) (Actualiser des jetons d’accès).
...
Votre application frontale a désormais l’autorisation requise et ajoute également l’ID d’application du serveur principal aux paramètres de connexion. Par conséquent, elle peut obtenir un jeton d’accès pour l’authentification avec l’application principale. App Service fournit ce jeton dans votre code du serveur en injectant un en-tête `X-MS-TOKEN-AAD-ACCESS-TOKEN` à chaque requête authentifiée (consultez [Retrieve tokens in app code](../app-service-authentication-how-to.md?toc=%2fazure%2fapp-service%2fcontainers%2ftoc.json#retrieve-tokens-in-app-code) (Récupérer des jetons dans le code d’application)).
