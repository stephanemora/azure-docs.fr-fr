...
Dans Cloud Shell, créez une [application web](overview.md#app-service-on-linux) multiconteneur dans le plan App Service `myAppServicePlan` avec la commande [az webapp create](/cli/azure/webapp#az_webapp_create). N’oubliez pas de remplacer _\<app_name>_ par un nom d’application unique (les caractères valides sont `a-z`, `0-9` et `-`).
