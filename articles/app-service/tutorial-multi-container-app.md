...
Dans Cloud Shell, créez une [application web](overview.md) multiconteneur dans le plan App Service `myAppServicePlan` avec la commande [az webapp create](/cli/azure/webapp?view=azure-cli-latest#az-webapp-create). N’oubliez pas de remplacer _\<app-name>_ par un nom d’application unique.
...
Dans Cloud Shell, reconfigurez votre [application web](overview.md) multiconteneur avec la commande [az webapp config container set](/cli/azure/webapp/config/container?view=azure-cli-latest#az-webapp-config-container-set). N’oubliez pas de remplacer _\<app-name>_ par un nom d’application unique.
