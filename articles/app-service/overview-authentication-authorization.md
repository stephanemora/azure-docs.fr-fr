...
[Architecture des fonctionnalités](#feature-architecture)
...
### <a name="feature-architecture"></a>Architecture des fonctionnalités
...
Le module s’exécute séparément de votre code d’application et peut être configuré à l’aide des paramètres d’Azure Resource Manager ou à l’aide d’[un fichier de configuration](configure-authentication-file-based.md). Aucun Kit de développement logiciel (SDK), aucun langage de programmation spécifique ni aucune modification de votre code d’application ne sont nécessaires. 
...
Le module d’authentification et d’autorisation s’exécute comme un [module IIS](/iis/get-started/introduction-to-iis/iis-modules-overview) natif, dans le même bac à sable que votre application. Lorsqu’il est activé, chaque requête HTTP entrante le traverse avant d’être gérée par votre application.
...
Les appels entre une application de navigateur approuvée dans App Service et une autre API REST dans App Service ou [Azure Functions](../azure-functions/functions-overview.md) peuvent être authentifiés à l’aide du flux dirigé vers le serveur. Pour plus d’informations, consultez [Personnaliser les connexions et les déconnexions](configure-authentication-customize-sign-in-out.md).
...
| 2. Post-authentification | Le fournisseur redirige le client vers `/.auth/login/<provider>/callback`. | Le code client [publie un jeton du fournisseur](configure-authentication-customize-sign-in-out.md#client-directed-sign-in) vers `/.auth/login/<provider>` pour validation. |
...
Cette option assure un traitement plus souple des requêtes anonymes. Par exemple, il permet de [présenter plusieurs fournisseurs de connexion](configure-authentication-customize-sign-in-out.md#use-multiple-sign-in-providers) aux utilisateurs. Vous devez cependant écrire du code.
...
Cette option évite d’avoir à écrire du code d’authentification dans l’application. Une autorisation plus fine, par exemple propre au rôle, peut être gérée en examinant les revendications de l’utilisateur (consultez la section [Accéder aux revendications utilisateur](configure-authentication-user-identities.md)).
...
Il est en général nécessaire d’écrire du code pour recueillir, stocker et actualiser ces jetons dans votre application. Avec le magasin de jetons, il suffit de [récupérer les jetons](configure-authentication-oauth-tokens.md#retrieve-tokens-in-app-code) en temps utile et de [demander à App Service de les actualiser](configure-authentication-oauth-tokens.md#refresh-auth-tokens) lorsqu’ils ne sont plus valides. 
...
- [Personnaliser les connexions et les déconnexions](configure-authentication-customize-sign-in-out.md)
<!-- - [Authenticate native client apps](configure-authentication-client-apps.md) -->
- [Utiliser des jetons et des sessions OAuth](configure-authentication-oauth-tokens.md)
- [Revendications d’utilisateur et d’application](configure-authentication-user-identities.md)
- [Configuration basée sur des fichiers](configure-authentication-file-based.md)
