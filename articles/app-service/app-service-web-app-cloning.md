...
Scénario : Vous souhaitez cloner le contenu d’une application web de la région USA Centre Sud vers une nouvelle application web de la région USA Centre Nord. Cela peut être effectué à l’aide de la version Azure Resource Manager de l’applet de commande PowerShell pour créer une application web avec l’option `-SourceWebApp`.
...
Scénario : Vous souhaitez cloner le contenu d’une application web de la région USA Centre Sud vers une nouvelle application web d’un environnement App Service Environment (ASE) existant.
