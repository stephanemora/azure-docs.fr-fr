...
Azure Cache pour Redis offre une haute disponibilité intégrée.  Une architecture de haute disponibilité est utilisée pour garantir le fonctionnement de votre instance Redis managée, même lorsque des pannes affectent les machines virtuelles sous-jacentes, qu’il s’agisse d’interruptions planifiées et non planifiées. Elle offre des taux de pourcentage bien supérieurs à ce qui est réalisable en hébergeant Redis sur une seule machine virtuelle.

Azure Cache pour Redis implémente la haute disponibilité en utilisant plusieurs machines virtuelles, appelées *nœuds*, pour un cache. Les nœuds sont configurés afin que la réplication des données et le basculement se fassent de manière coordonnée. La haute disponibilité facilite également les opérations de maintenance, telles que les mises à jour correctives du logiciel Redis. Diverses options de haute disponibilité sont disponibles dans les niveaux Standard, Premium et Entreprise :
...
Les niveaux Entreprise prennent en charge une forme plus avancée de géoréplication. Nous l’appelons la [géoréplication active](cache-how-to-active-geo-replication.md). Utilisant des types de données répliqués sans conflit, le logiciel Redis Enterprise prend en charge les écritures sur plusieurs instances de cache, et s’occupe de la fusion des modifications et de la résolution des conflits. Vous pouvez joindre plusieurs instances de cache de niveau Entreprise situées dans différentes régions Azure afin de former un cache géorépliqué actif. 

Une application qui utilise ce type de cache peut lire et écrire des données dans les instances de cache géodistribuées via les points de terminaison correspondants. Le cache doit utiliser ce qui se trouve le plus près de chaque instance de calcul, pour vous assurer le temps de latence le plus court. L’application doit également superviser les instances de cache et basculer vers une autre région quand l’une des instances devient indisponible. Pour plus d’informations sur le fonctionnement de la géoréplication active, consultez [Géodistribution active-active (CRDT)](https://redislabs.com/redis-enterprise/technology/active-active-geo-distribution/).
