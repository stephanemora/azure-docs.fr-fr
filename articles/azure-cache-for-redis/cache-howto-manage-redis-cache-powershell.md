---
title: Gérer le Cache Azure pour Redis avec Azure PowerShell | Microsoft Docs
...
Cette rubrique décrit comment effectuer des tâches courantes telles que la création, la mise à jour et la mise à l’échelle de vos instances du Cache Azure pour Redis, comment régénérer les clés d'accès et comment afficher des informations sur vos caches. Pour obtenir une liste complète des cmdlets PowerShell du Cache Redis Azure, consultez [Cmdlets du Cache Azure pour Redis](https://docs.microsoft.com/powershell/module/azurerm.rediscache/?view=azurermps-6.6.0).
...
Le tableau suivant contient les propriétés et les descriptions pour les paramètres fréquemment utilisés lors de la création et de la gestion de vos instances du Cache Azure pour Redis avec Azure PowerShell.
...
Les nouvelles instances du Cache Azure pour Redis sont créées à l’aide de la cmdlet [New-AzureRmRedisCache](https://docs.microsoft.com/powershell/module/azurerm.rediscache/new-azurermrediscache?view=azurermps-6.6.0) .
...
Le paramètre `databases` ne peut être configuré qu’au moment de la création du cache. L’exemple suivant crée un cache premium P3 (26 Go) avec 48 bases de données à l’aide de l’applet de commande [New-AzureRmRedisCache](https://docs.microsoft.com/powershell/module/azurerm.rediscache/New-AzureRmRedisCache?view=azurermps-6.6.0) .
...
`Set-AzureRmRedisCache` peut être utilisé pour mettre à l’échelle une instance du Cache Azure pour Redis lorsque les propriétés `Size`, `Sku` ou `ShardCount` sont modifiées. 
...
Vous pouvez récupérer des informations sur un cache à l’aide de l’applet de commande [Get-AzureRmRedisCache](https://docs.microsoft.com/powershell/module/azurerm.rediscache/get-azurermrediscache?view=azurermps-6.6.0) .
...
Pour récupérer les clés d'accès de votre cache, vous pouvez utiliser l’applet de commande [Get-AzureRmRedisCacheKey](https://docs.microsoft.com/powershell/module/azurerm.rediscache/Get-AzureRmRedisCacheKey?view=azurermps-6.6.0) .
...
Pour régénérer les clés d’accès de votre cache, vous pouvez utiliser l’applet de commande [New-AzureRmRedisCacheKey](https://docs.microsoft.com/powershell/module/azurerm.rediscache/New-AzureRmRedisCacheKey?view=azurermps-6.6.0) .
...
Pour supprimer un Cache Azure pour Redis, utilisez la cmdlet [Remove-AzureRmRedisCache](https://docs.microsoft.com/powershell/module/azurerm.rediscache/remove-azurermrediscache?view=azurermps-6.6.0) .
...
Vous pouvez importer des données dans une instance du Cache Azure pour Redis à l’aide de la cmdlet `Import-AzureRmRedisCache` .

> [!IMPORTANT]
> L’importation/exportation est uniquement disponible pour les caches de niveau [Premium](cache-premium-tier-intro.md) . Pour plus d’informations sur l’importation/exportation, voir [Importer et exporter des données dans le Cache Azure pour Redis](cache-how-to-import-export-data.md).
> 
> 
...
Vous pouvez exporter des données depuis une instance du Cache Azure pour Redis à l’aide de la cmdlet `Export-AzureRmRedisCache`.
...
Vous pouvez redémarrer votre Cache Azure pour Redis à l’aide de la cmdlet `Reset-AzureRmRedisCache`.
...
* [Documentation relative à la cmdlet du Cache Azure pour Redis sur MSDN](https://docs.microsoft.com/powershell/module/azurerm.rediscache/?view=azurermps-6.6.0)
