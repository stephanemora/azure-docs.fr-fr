...
description: Découvrez les réponses aux questions les plus fréquentes, les modèles et les meilleures pratiques pour le Cache Azure pour Redis
services: cache
documentationcenter: ''
author: yegu-ms
manager: jhubbard
...
* **Mémoire** : Les niveaux De base et Standard offrent respectivement 250 Mo et 53 Go. Le niveau Premium offre jusqu’à 530 Go. Pour plus d’informations, consultez [Tarification du Cache Azure pour Redis](https://azure.microsoft.com/pricing/details/cache/).
* **Performances réseau** : Si vous avez une charge de travail qui nécessite un débit élevé, le niveau Premium offre davantage de bande passante par rapport au niveau Standard ou De base. En outre, à chaque niveau les caches de taille supérieure offrent davantage de bande passante en raison de la machine virtuelle sous-jacente qui héberge le cache. Pour plus d’informations, consultez le [le tableau suivant](#cache-performance).
...
* **Persistance des données Redis** : Le niveau Premium vous permet de conserver les données du cache dans un compte de stockage Azure. Dans un cache De base/Standard, toutes les données sont stockées uniquement dans la mémoire. Sous-jacente des problèmes d’infrastructure peut entraîner une perte de données. Nous vous recommandons d’utiliser la fonctionnalité de persistance des données Redis disponible au niveau Premium pour accroître la résilience contre la perte de données. Le Cache Azure pour Redis propose des options RDB et AOF (à venir) de persistance Redis. Pour plus d’informations, consultez [Comment configurer la persistance pour un Cache Azure pour Redis Premium](cache-how-to-premium-persistence.md).
* **Cluster Redis** : Pour créer des caches de plus de 53 Go ou partager des données parmi plusieurs nœuds Redis, vous pouvez utiliser le clustering Redis disponible au niveau Premium. Chaque nœud se compose d’une paire de caches principal/réplica pour la haute disponibilité. Pour plus d’informations, consultez [Comment configurer le clustering Redis pour un Cache Azure pour Redis Premium](cache-how-to-premium-clustering.md).
...
* Le débit des caches de taille identique est plus élevé dans le niveau Premium que dans le niveau Standard. Par exemple, avec un Cache de 6 Go, débit de P1 est de 180 000 demandes par seconde (RPS) par rapport à 100 000 RPS de C3.
* Avec le clustering Redis, le débit augmente de façon linéaire à mesure que vous augmentez le nombre de partitions (nœuds) dans le cluster. Par exemple, si vous créez un cluster P4 de 10 partitions, le débit disponible est de 400 000 * 10 = 4 millions de demandes par seconde.
* Pour les tailles de clé supérieures, le débit du niveau Premium est plus élevé que celui du niveau Standard.

| Niveau tarifaire | Taille | Cœurs d’unité centrale | Bande passante disponible | Taille de la valeur de 1 Ko | Taille de la valeur de 1 Ko |
| --- | --- | --- | --- | --- | --- |
...
Oui, le Cache Azure Redis est disponible dans le Cloud Azure Government, Azure China 21Vianet Cloud et Microsoft Azure Germany. Les URL d’accès et de gestion du Cache Azure pour Redis diffèrent de celles du cloud public Azure.
...
- [Azure China 21Vianet Cloud - Cache Azure pour Redis](https://www.azure.cn/home/features/redis-cache/)
- [Microsoft Azure Germany](https://azure.microsoft.com/overview/clouds/germany/)

Pour plus d’informations sur l’utilisation du Cache Azure pour Redis avec PowerShell dans le Cloud Azure Government, Azure China 21Vianet Cloud et Microsoft Azure Germany, consultez [comment se connecter à d’autres clouds - Cache Azure pour PowerShell Redis](cache-howto-manage-redis-cache-powershell.md#how-to-connect-to-other-clouds).
...
* **Valeurs de délai d’expiration**
  ...
  * Utilisez plusieurs instances `ConnectionMultiplexer` pour les charges de travail personnalisées.
      * Vous pouvez suivre ce modèle si vous savez que la charge de votre application est variable. Exemple :
...
Le serveur Redis ne prend pas en charge SSL de façon native, contrairement au Cache Azure pour Redis. Si vous vous connectez à un Cache Azure pour Redis et que votre client est compatible SSL, par exemple StackExchange.Redis, vous devez utiliser SSL.

>[!NOTE]
>Le port non SSL est désactivé par défaut pour les nouvelles instances Cache Azure pour Redis. Si votre client ne prend pas en charge SSL, vous devez activer le port non-SSL en suivant les instructions de la section [Ports d’accès](cache-configure.md#access-ports) de l’article [Configuration d’un cache dans le Cache Azure pour Redis](cache-configure.md).
>
>
...
* Utilisez au moins la valeur par défaut connectTimeout de 5 secondes. Cet intervalle donne StackExchange.Redis suffisamment de temps pour rétablir la connexion en cas de blocage du réseau.
...
* Évitez d’utiliser certaines commandes Redis qui prennent un certain temps, à moins que vous comprenez l’impact de ces commandes. Par exemple, n’exécutez pas le [clés](https://redis.io/commands/keys) commande en production. Selon le nombre de clés, il peut prendre beaucoup de temps à retourner. Redis est un serveur à thread unique qui traite une commande à la fois. Si vous avez émis d’autres commandes après KEYS, elles ne sont pas traitées tant que Redis traite la commande KEYS. Le [site redis.io](https://redis.io/commands/) fournit plus d’informations sur la complexité de temps de chaque opération prise en charge. Cliquez sur chaque commande pour afficher la complexité de chaque opération.
* Taille des clés : dois-je utiliser des petites clés/valeurs ou de grandes clés/valeurs ? Cela dépend du scénario. Si votre scénario requiert des clés plus volumineuses, vous pouvez ajuster le ConnectionTimeout, puis valeurs des nouvelles tentatives et adapter votre logique de nouvelle tentative. À partir d’une perspective de serveur Redis, plus petites valeurs offrent de meilleures performances.
...
Le pool de threads CLR comporte deux types de threads : « Worker » et « Port de terminaison d’e/s » (IOCP) threads.
...
* Les threads IOCP sont utilisés lorsque les e/s asynchrone se produit, par exemple quand lire à partir du réseau.
...
Une fois que le nombre de threads existants (occupés) atteint le nombre « minimal » de threads, le pool de threads limite le taux d’injection des nouveaux threads à hauteur d’un thread toutes les 500 millisecondes. En règle générale, si votre système reçoit une rafale de tâches nécessitant un thread IOCP, elle sera traitée rapidement. Toutefois, si le nombre de tâches est supérieur au paramètre « Minimum » configuré, le traitement de certaines tâches sera retardé car le ThreadPool attendra l’une ou l’autre des conditions suivantes :

1. Un thread existant se libère pour traiter la tâche.
2. Aucun thread existant ne se libère pendant 500 ms, donc un nouveau thread est créé.

En fait, cela signifie que lorsque le nombre de threads occupés est supérieur au nombre minimal de threads, vous devrez probablement subir un délai de 500 ms avant que le trafic réseau est traité par l’application. Il est également important de noter que lorsqu’un thread existant reste inactif pendant plus de 15 secondes (d’après mes souvenirs), il sera nettoyé et ce cycle de croissance et de diminution peut se répéter.
...
Dans l’exemple précédent, vous pouvez voir que pour le thread IOCP six threads sont occupés et le système est configuré pour autoriser les quatre threads minimale. Dans ce cas, le client subirait probablement deux retards de 500 ms, car 6 > 4.
...
Par conséquent, nous recommandons fortement aux clients de définir la valeur de configuration minimale pour les threads IOCP et WORKER sur une valeur supérieure à la valeur par défaut. Nous ne pouvons pas donner à recommander ce que cette valeur doit être, car la valeur correcte pour une application sera probablement trop élevée ou faible pour une autre application. Ce paramètre peut également avoir un impact sur les performances d’autres composants d’applications complexes, ce qui signifie que chaque client doit ajuster ce paramètre en fonction de ses besoins spécifiques. Le mieux est de commencer à 200 ou 300, puis de tester et d’ajuster cette valeur en fonction des besoins.

Configuration de ce paramètre :

* Nous vous recommandons de modifier ce paramètre par programmation à l’aide de la [ThreadPool.SetMinThreads (...) ](/dotnet/api/system.threading.threadpool.setminthreads#System_Threading_ThreadPool_SetMinThreads_System_Int32_System_Int32_) méthode dans `global.asax.cs`. Exemple :
...
```
  ...
  > [!NOTE]
  > La valeur spécifiée par cette méthode est un paramètre global qui affectent l’AppDomain entier. Par exemple, si vous disposez d’un ordinateur 4 cœurs et que vous souhaitez définir *minWorkerThreads* et *minIoThreads* 50 par UC pendant l’exécution, vous utiliseriez **ThreadPool.SetMinThreads (200, 200)**.

* Il est également possible de spécifier le minimal de threads à l’aide de la définition de la [ *minIoThreads* ou *minWorkerThreads* paramètre de configuration](https://msdn.microsoft.com/library/vstudio/7w2sway1(v=vs.100).aspx) sous le `<processModel>` élément de configuration dans `Machine.config`, généralement situé dans `%SystemRoot%\Microsoft.NET\Framework\[versionNumber]\CONFIG\`. **Définition du nombre minimal de threads de cette manière est généralement pas recommandé, car il s’agit d’un paramètre de l’échelle du système.**

  > [!NOTE]
  > La valeur spécifiée dans cet élément de configuration est un paramètre applicable *par cœur*. Par exemple, si vous avez un ordinateur 4 cœurs et que vous souhaitez que votre *minIoThreads* définissant 200 lors de l’exécution, vous utiliseriez `<processModel minIoThreads="50"/>`.
  >
...
Ces outils vous permettent de surveiller l’intégrité de vos instances Cache Azure pour Redis et vous aident à gérer vos applications de mise en cache. Pour plus d’informations, consultez la section « Paramètres de support et dépannage » de l’article [Configuration de Cache Azure pour Redis](cache-configure.md).
...
* Causes côté client
  ...
  * L’application cliente a effectué une opération de mise à l’échelle.
    * Dans le cas des Services de Cloud ou applications Web, cela peut être en raison d’une mise à l’échelle.
