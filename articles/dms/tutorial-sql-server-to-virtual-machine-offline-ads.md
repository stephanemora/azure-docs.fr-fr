---
title: 'Tutoriel : Migrer SQL Server vers SQL Server sur les machines virtuelles Azure hors connexion avec Azure Data Studio'
titleSuffix: Azure Database Migration Service
description: Migrer SQL Server vers SQL Server sur les machines virtuelles Azure hors connexion avec Azure Data Studio et Azure Database Migration Service (préversion)
services: dms
author: kbarlett001
ms.author: kebarlet
manager: ''
...
Dans ce tutoriel, vous allez migrer la base de données **Adventureworks** d’une instance locale de SQL Server vers SQL Server sur les machines virtuelles Azure hors connexion à l’aide d’Azure Data Studio et d’Azure Database Migration Service.
...
Cet article décrit une migration hors connexion de SQL Server vers SQL Server sur les machines virtuelles Azure. Pour une migration en ligne, consultez [Migrer SQL Server vers SQL Server sur les machines virtuelles Azure en ligne à l’aide d’Azure Data Studio et de DMS](tutorial-sql-server-to-virtual-machine-online-ads.md).
...
* Le certificat de l’instance source de SQL Server provenant d’une base de données protégée par un chiffrement TDE (Transparent Data Encryption) doit être migré vers SQL Server sur les machines virtuelles Azure avant la migration des données. Pour en savoir plus, consultez [Déplacer une base de données protégée par TDE vers un autre serveur SQL Server](/sql/relational-databases/security/encryption/move-a-tde-protected-database-to-another-sql-server).
    > [!TIP]
...
1. Sélectionnez SQL Server sur les machines virtuelles Azure en tant que cible.
    :::image type="content" source="media/tutorial-sql-server-to-virtual-machine-offline-ads/assessment-complete-target-selection.png" alt-text="Confirmation de l’évaluation":::
...
1. Sélectionnez le mode de migration **Migration hors connexion**.
    > [!NOTE]
    > En mode de migration hors connexion, la base de données SQL Server source n’est pas disponible pour l’activité d’écriture pendant que les fichiers de sauvegarde de base de données sont restaurés sur la base de données Azure SQL cible. Le temps d’arrêt de l’application persiste du démarrage jusqu’à la fin du processus de migration.
...
1. Sélectionnez **Tester la connexion** dans l’écran **Créer une instance d’Azure Database Migration Service** au sein d’Azure Data Studio pour vérifier que l’instance de DMS créée est connectée au runtime d’intégration autohébergé récemment inscrit, puis sélectionnez **Terminé**.
    :::image type="content" source="media/tutorial-sql-server-to-virtual-machine-offline-ads/test-connection-integration-runtime-complete.png" alt-text="Tester le runtime d’intégration de la connexion":::
...
1. Dans l’**État de la migration de base de données**, vous pouvez suivre les migrations en cours, les migrations effectuées et les migrations non réussies (le cas échéant).
    ...
    :::image type="content" source="media/tutorial-sql-server-to-virtual-machine-offline-ads/monitor-migration-dashboard.png" alt-text="tableau de bord de supervision de la migration":::
...
Une fois que toutes les sauvegardes de base de données ont été restaurées sur SQL Server sur les machines virtuelles Azure, un basculement automatique de la migration est lancé par Azure DMS pour garantir que la base de données migrée est prête à l’emploi. De plus, l’état de la migration passe de *en cours* à *opération réussie*.

## <a name="next-steps"></a>Étapes suivantes

* Pour accéder à un tutoriel montrant comment migrer une base de données vers SQL Server sur les machines virtuelles Azure à l’aide de la commande T-SQL RESTORE, consultez [Migrer une base de données SQL Server vers SQL Server sur les machines virtuelles](../azure-sql/virtual-machines/windows/migrate-to-vm-from-sql-server.md).
