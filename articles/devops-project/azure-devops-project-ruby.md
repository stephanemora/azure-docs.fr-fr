...
Configurez l’intégration continue (CI) et la livraison continue (CD) pour votre application Ruby on Rails avec **Azure DevOps Projects**.  Le projet Azure DevOps simplifie la configuration initiale d’un pipeline de mise en production et de build VSTS.
...
1. Créez un **nouveau** compte VSTS gratuit ou choisissez un compte **existant**.  Choisissez un **nom** pour votre projet VSTS.  Sélectionnez votre **abonnement Azure**, **l’emplacement**, puis le **nom** de votre application.  Une fois que vous avez terminé, sélectionnez **Terminé**.
    ...
    ![Entrer les informations VSTS](_img/azure-devops-project-ruby/vstsazureinfo.png)
