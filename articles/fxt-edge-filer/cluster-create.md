...
* Installez vos systèmes matériels Azure FXT Edge Filer dans votre centre de données.
  ...
  Il n’avez besoin que d’un seul nœud pour créer le cluster, mais vous devez [ajouter au moins deux nœuds supplémentaires](add-nodes.md) pour pouvoir le configurer et le rendre prêt à l’emploi.

* Branchez les câbles d’alimentation et réseau appropriés sur le système.  
* Mettez sous tension au moins un nœud Azure FXT Edge Filer et [définissez son mot de passe racine](node-password.md).
...
Si vous ne l’avez pas déjà fait, mettez sous tension au moins l’un des nœuds FXT de votre cluster, puis vérifiez qu’il a une connexion réseau et une adresse IP. Vous devez définir un nouveau mot de passe racine pour activer le nœud, alors suivez les étapes décrites dans [Définir des mots de passe pour le matériel](node-password.md) si vous ne l’avez pas déjà fait.

Pour vérifier la connexion réseau, assurez-vous que les LED de liaison réseau du nœud sont allumées (et, si nécessaire, les indicateurs situés sur le commutateur réseau auquel il est raccordé). Les LED sont décrites dans [Superviser l’état du matériel Azure FXT Edge Filer](monitor.md).
...
* **DNS server(s)** (Serveur(s) DNS) : entrez l’adresse IP d’un ou plusieurs serveurs DNS (Domain Name System).
  ...
  Pour obtenir des performances optimales, configurez le serveur DNS du cluster à des fins d’équilibrage de charge par tourniquet (round-robin) comme décrit dans [Configurer un serveur DNS pour le cluster Azure FXT Edge Filer](configure-network.md#configure-dns-for-load-balancing).
...
* Ajouter des nœuds au cluster
  ...
  Consultez [Ajouter des nœuds de cluster](add-nodes.md) pour apprendre à ajouter d’autres unités Azure FXT Edge Filer à votre cluster et pour activer la haute disponibilité.

* Spécifier un stockage back-end

  Ajoutez des définitions de *système de stockage principal* pour chaque système de stockage back-end que le cluster va utiliser. Consultez [Ajouter du stockage back-end et configurer un espace de noms virtuel](add-storage.md#about-back-end-storage) pour en savoir plus.
...
* Configurer l’accès client et l’espace de noms virtuel
  ...
  Consultez [Configurer l’espace de noms](add-storage.md#configure-the-namespace) pour plus d’informations. Cette étape inclut :
...
* [Ajuster les paramètres réseau](configure-network.md)

  Il existe plusieurs paramètres réseau à vérifier ou personnaliser pour un nouveau cluster. Consultez [Ajuster les paramètres réseau](configure-network.md) pour plus d’informations sur ces éléments :
...
> [Ajouter des nœuds de cluster](add-nodes.md)
