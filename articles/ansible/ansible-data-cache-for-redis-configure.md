---
title: Tutoriel - Configurer des mises en cache dans Azure Cache pour Redis à l’aide d’Ansible | Microsoft Docs
description: Guide pratique pour utiliser Ansible afin de créer, mettre à l’échelle et redémarrer Azure Cache pour Redis
...
[Azure Cache pour Redis](/azure/azure-cache-for-redis/) est un service open source compatible qui vous permet de générer des applications réactives en fournissant un accès rapide aux données. 
...
> * Redémarrer un cache
> * Ajouter une règle de pare-feu à un cache
> * Supprimer un cache
...
Créez une instance Azure Cache pour Redis dans un nouveau groupe de ressources.
...
Le provisionnement d’un cache peut prendre plusieurs minutes. Le code suivant indique à Ansible d’attendre la fin de l’opération :
...
Pendant le long processus de provisionnement, plusieurs messages d’erreur s’afficheront. Vous pouvez les ignorer sans risque. Le message important est le dernier message. Dans l’exemple suivant, plusieurs messages d’erreur apparaissent avant le message final (« ok »).
...
## <a name="scale-the-cache"></a>Mettre à l’échelle le cache

Azure Cache pour Redis offre différents types de cache en fonction des besoins de votre application. Ces options de cache offrent une flexibilité dans le choix de la taille et des fonctionnalités du cache. Si les besoins de votre application changent après la création du cache, vous pouvez mettre à l’échelle le cache selon ces besoins. Pour plus d’informations sur la mise à l’échelle, consultez la page [Mise à l’échelle du cache Azure pour Redis](/azure/azure-cache-for-redis/cache-how-to-scale).

L’exemple de code suivant met à l’échelle le cache sur **Standard** :
...
La mise à l'échelle d’un cache peut prendre plusieurs minutes. Le code suivant indique à Ansible d’attendre la fin de l’opération :
...
Comme pour la tâche de provisionnement d’Azure Cache pour Redis, un résultat similaire au message suivant est normal :

```Ouput
**FAILED - RETRYING: Get facts (100 retries left)** is normal.
```

## <a name="reboot-the-cache"></a>Redémarrer le cache

Le code suivant redémarre le cache créé dans les sections précédentes.
...
### <a name="add-firewall-rule"></a>Ajouter une règle de pare-feu

Le code suivant ajoute une règle de pare-feu au cache :
...
Le code ci-dessous supprime le cache :
