---
title: Exemples d’interfaces de ligne de commande Azure pour Traffic Manager | Microsoft Docs
description: En savoir plus sur un script Azure CLI que vous pouvez utiliser pour diriger le trafic dans plusieurs régions pour une haute disponibilité des applications.
services: virtual-network
documentationcenter: virtual-network
author: duongau
manager: twooley
ms.service: virtual-network
ms.devlang: na
ms.topic: article
ms.tgt_pltfrm: ''
ms.workload: infrastructure
ms.date: 10/23/2018
ms.author: duau
ms.custom: devx-track-azurecli
ms.openlocfilehash: f547305dd492f8e8587514474e4602f2214009cb
ms.sourcegitcommit: 5a3b9f35d47355d026ee39d398c614ca4dae51c6
ms.translationtype: HT
ms.contentlocale: fr-FR
ms.lasthandoff: 09/02/2020
ms.locfileid: "89400832"
---
# <a name="azure-cli-samples-for-traffic-manager"></a>Exemples d’interfaces de ligne de commande Azure pour Traffic Manager

Le tableau suivant contient des liens vers des scripts Bash pour Traffic Manager créé à l’aide d’Azure CLI.

|Intitulé  |Description |
|---------|---------|
|[Diriger le trafic dans plusieurs régions pour une haute disponibilité des applications](./scripts/traffic-manager-cli-websites-high-availability.md)   |    Crée deux plans App Service, deux applications web, un profil Traffic Manager et deux points de terminaison Traffic Manager.     |
|    |         |


