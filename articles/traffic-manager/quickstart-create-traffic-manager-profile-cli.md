...
:::image type="content" source="./media/quickstart-create-traffic-manager-profile/environment-diagram.png" alt-text="Diagramme de l’environnement de déploiement Traffic Manager utilisant CLI." border="false":::
...
Créez un profil Traffic Manager en utilisant la commande [az network traffic-manager profile create](/cli/azure/network/traffic-manager/profile#az_network_traffic_manager_profile_create) qui dirige le trafic utilisateur en fonction de la priorité du point de terminaison.
...
Créez des plans App Service web en utilisant la commande [az appservice plan create](/cli/azure/appservice/plan#az_appservice_plan_create) pour les deux instances de l’application web que vous allez déployer dans deux régions Azure distinctes.
...
Créez deux instances de l’application web en utilisant la commande [az webapp create](/cli/azure/webapp#az_webapp_create) dans les plans App Service dans les régions Azure *USA Est* et *Europe Ouest*.
...
Ajoutez les deux applications web en tant que points de terminaison Traffic Manager en utilisant la commande [az network traffic-manager endpoint create](/cli/azure/network/traffic-manager/endpoint#az_network_traffic_manager_endpoint_create) dans le profil Traffic Manager comme suit :
...
Déterminez le nom DNS du profil Traffic Manager en utilisant la commande [az network traffic-manager profile show](/cli/azure/network/traffic-manager/profile#az_network_traffic_manager_profile_show).
...
2. Pour voir le basculement de Traffic Manager en action, désactivez votre site principal en utilisant la commande [az network traffic-manager endpoint update](/cli/azure/network/traffic-manager/endpoint#az_network_traffic_manager_endpoint_update).
...
Une fois que vous avez terminé, supprimez les groupes de ressources, les applications web et toutes les ressources associées en utilisant la commande [az group delete](/cli/azure/group#az_group_delete).
