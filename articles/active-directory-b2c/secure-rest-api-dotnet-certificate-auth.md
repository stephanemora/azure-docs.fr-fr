...
Dans un article connexe, vous [créez un service RESTful](rest-api-claims-exchange-dotnet.md) qui interagit avec Azure Active Directory B2C (Azure AD B2C).
...
>[!NOTE]
>Vous pouvez également sécuriser votre service RESTful [à l’aide de l’authentification de base HTTP](secure-rest-api-dotnet-basic-auth.md). Toutefois, l’authentification de base HTTP est considérée comme moins sécurisée qu’un certificat client. Notre recommandation est de sécuriser le service RESTful à l’aide de l’authentification par certificat client comme décrit dans cet article.
...
>[!NOTE]
>Assurez-vous que votre plan Azure App Service est Standard ou supérieur. Pour plus d’informations, consultez la rubrique [Présentation détaillée des plans Azure App Service](https://docs.microsoft.com/azure/app-service/overview-hosting-plans).
...
## <a name="step-2-upload-your-certificate-to-azure-ad-b2c-policy-keys"></a>Étape 2 : Charger votre certificat sur les clés de stratégie Azure AD B2C
...
7. Dans la zone **Mot de passe**, entrez le mot de passe du certificat.
    ...
    ![Chargez la clé de stratégie dans la page Créer une clé du Portail Azure](./media/secure-rest-api-dotnet-certificate-auth/rest-api-netfw-secure-client-cert-upload.png)
...
5. Immédiatement après l’élément `<Metadata>` fermant, ajoutez l’extrait de code XML suivant :
    ...
    ![Définir les éléments XML de l’authentification ClientCertificate (Certificat client)](./media/secure-rest-api-dotnet-certificate-auth/rest-api-netfw-secure-client-cert-tech-profile.png)

## <a name="step-4-upload-the-policy-to-your-tenant"></a>Étape 4 : Charger la stratégie sur un client
...
1. Ouvrez **Paramètres Azure AD B2C**, puis sélectionnez **Infrastructure d’expérience d’identité**.
    ...
    >[!NOTE]
    >L’option Exécuter maintenant nécessite la préinscription d’au moins une application sur le locataire. Pour découvrir comment inscrire des applications, consultez les articles sur Azure AD B2C [Bien démarrer](tutorial-create-tenant.md) et [Inscription des applications](tutorial-register-applications.md).
...
3. Testez le processus en tapant **Test** dans la zone **Prénom**.
    ...
    ![Zone de texte Prénom mise en surbrillance et erreur de validation d’entrée affichée](./media/secure-rest-api-dotnet-certificate-auth/rest-api-netfw-test.png)
...
![Ajout d’un code de validation de certificat](./media/secure-rest-api-dotnet-certificate-auth/rest-api-netfw-secure-client-code.png)
...
Si vous avez des problèmes avec cette étape, consultez [Collecter les journaux d’activité à l’aide d’Application Insights](troubleshoot-with-application-insights.md).
