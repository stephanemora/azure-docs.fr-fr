...
Cet article se concentre sur la façon de personnaliser l’interface utilisateur Azure AD B2C avec du *contenu dynamique* à l’aide de stratégies personnalisées. Pour commencer, consultez [Personnalisation de l’interface utilisateur dans une stratégie personnalisée](custom-policy-ui-customization.md).

>[!NOTE]
>L’article Azure AD B2C, [Configurer la personnalisation de l’interface utilisateur dans une stratégie personnalisée](custom-policy-ui-customization.md), explique les notions de base suivantes :
...
Dans l’article [Azure Active Directory B2C : Configurer la personnalisation de l’interface utilisateur dans une stratégie personnalisée](custom-policy-ui-customization.md), vous avez chargé des fichiers HTML5 vers le stockage Blob Azure. Ces fichiers HTML5 sont statiques et affichent le même contenu HTML pour chaque requête.
...
3. Nommez l’application (par exemple, *Contoso.AADB2C.UI*), puis sélectionnez **OK**.
    ...
    ![Créer un projet Visual Studio](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-create-project1.png)

4. Sélectionnez le modèle **Application web**.

5. Définissez l’authentification sur **Aucune authentification**.

    ![Sélectionnez le modèle Application web.](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-create-project2.png)
...
1. Cliquez avec le bouton droit sur le dossier Vues/Accueil, puis sur **Ajouter** > **Nouvel élément**.
    ...
    ![Ajouter un nouvel élément dans Visual Studio](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-add-view1.png)
...
5. Sélectionnez **Ajouter**.
    ...
    ![Boîte de dialogue Ajouter un nouvel élément dans Visual Studio avec Page de vue MVC mis en évidence](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-add-view2.png)
...
10. Copiez le contenu du fichier et collez-le sous la définition de la disposition. Votre code doit ressembler à ce qui suit :
    ...
    ![Fichier unified.cshtml après avoir ajouté le modèle HTML5](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-edit-view1.png)
...
![élément img avec la valeur personnalisée src background_background_image](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-add-static-background.png)
...
1. Ouvrez **Controllers\HomeController.cs** et ajoutez la méthode suivante :
    ...
    ![Changer le contrôleur pour restituer la vue](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-controller-view.png)
...
1. Dans **l’Explorateur de solutions**, cliquez avec le bouton droit sur le projet **Contoso.AADB2C.UI**, puis sélectionnez **Publier**.
    ...
    ![Publier dans Microsoft Azure App Service](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-publish1.png)

2. Sélectionnez la vignette **Microsoft Azure App Service**, puis cliquez sur **Publier**.

    ![Créer un Microsoft Azure App Service](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-publish2.png)
...
4. Sélectionnez **Créer** pour commencer à créer les ressources Azure.
    ...
    ![Fournir les propriétés App Service](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-publish3.png)
...
1. Dans le [portail Azure](https://portal.azure.com/), sélectionnez **App Services**, puis sélectionnez le nom de votre application API.
    ...
    ![Sélectionner l’application API dans le portail Azure](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-CORS1.png)

2. Dans la section **Paramètres**, sous la section **API**, sélectionnez **CORS**.

    ![Élément de menu CORS mis en évidence dans le menu App Service sur le portail Azure](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-CORS2.png)
...
4. Sélectionnez **Enregistrer**.
    ...
    ![Page de paramètres CORS avec un astérisque mis en évidence dans Origines autorisées](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-CORS3.png)
...
6. Modifiez la valeur de `LoadUri` en remplaçant _~/tenant/default/unified_ par _https://<nom_app>.azurewebsites.net/home/unified_.
    ...
    ![Exemple d’extrait XML avec l’élément LoadUri mis en évidence](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-content-definition.png)
...
2. Ouvrez **B2C_1A_signup_signin**, la stratégie personnalisée de partie de confiance que vous avez chargée, puis sélectionnez **Exécuter maintenant**.
    ...
    ![Votre stratégie d’inscription ou de connexion](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-demo1.png)
...
2. Recherchez l’élément `<img>` avec l’ID `background_background_image` et remplacez la valeur `src` par `@ViewData["background"]`.
    ...
    ![élément img avec la valeur src mise en évidence ](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-add-dynamic-background.png)
...
5. Ajoutez le paramètre de chaîne de requête _campaignId_ à l’URI. Par exemple, ajoutez `&campaignId=hawaii`, comme illustré dans l’image suivante :
    ...
    ![URI avec le paramètre de chaîne de requête campaignId mis en évidence](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-campaignId-param.png)

6. Appuyez sur **Entrée** pour afficher l’image d’arrière-plan d’Hawaï.

    ![Page S’inscrire ou se connecter avec l’image d’arrière-plan personnalisé Hawaii](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-demo2.png)

7. Remplacez la valeur par *Tokyo* et appuyez sur **Entrée**.
    Le navigateur affiche l’arrière-plan de Tokyo.

    ![Page S’inscrire ou se connecter avec l’image d’arrière-plan personnalisé Tokyo](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-demo3.png)
...
* Une fois que vous avez [pris en main les stratégies personnalisées](custom-policy-get-started.md), nous vous recommandons de créer votre scénario à l’aide de vos propres fichiers de stratégie personnalisée. Des [exemples de fichiers de stratégie](https://github.com/Azure-Samples/active-directory-b2c-custom-policy-starterpack/tree/master/scenarios/aadb2c-ief-ui-customization) sont à votre disposition pour référence.
