...
- **Langue spécifiée par le paramètre ui_locales** : après avoir activé la personnalisation de la langue, votre parcours utilisateur est traduit dans la langue indiquée ici.
- **Langue requise par le navigateur** : si aucun paramètre `ui_locales` n’a été spécifié, votre parcoursutilisateur est traduit dans la langue requise par le navigateur, *si la langue est prise en charge*.
- **Langue par défaut de la stratégie** : si le navigateur ne spécifie aucune langue ou s’il en spécifie une qui n’est pas prise en charge, le parcours utilisateur est traduit dans la langue par défaut de la stratégie.
...
Les stratégies créées avant la disponibilité générale de la personnalisation de la langue doivent d’abord activer cette fonctionnalité. La personnalisation de la langue est activée par défaut dans les stratégies créées après. 

En activant la personnalisation de la langue sur une stratégie, vous pouvez contrôler la langue du parcours utilisateur en ajoutant le paramètre `ui_locales`.
1. [Accédez à la page des fonctionnalités B2C sur le portail Azure](https://docs.microsoft.com/azure/active-directory-b2c/active-directory-b2c-app-registration#navigate-to-b2c-settings).
2. Accédez à une stratégie pour laquelle vous souhaitez activer la traduction.
3. Sélectionnez **Personnalisation de la langue**.  
...
5. Sélectionnez **Enregistrer** dans la partie supérieure du volet.
...
2. Dans la page **Modifier une stratégie**, sélectionnez **Personnalisation de la langue**.
3. Sélectionnez la langue dans laquelle vous voulez traduire.
4. Sélectionnez la page pour laquelle vous souhaitez fournir des traductions.
5. Sélectionnez l’icône de dossier et sélectionnez le fichier JSON à charger.
 
Ces changements sont automatiquement enregistrés dans votre stratégie.
