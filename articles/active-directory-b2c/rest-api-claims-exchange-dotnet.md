---
title: Intégrer les échanges de revendications de l’API REST dans un parcours utilisateur
...
* Envoyer les revendications d’entrée et les lire dans votre code
...
1. Dans la zone **Nom**, tapez un nom pour l’application (par exemple, *Contoso.AADB2C.API*), puis sélectionnez **OK**.

    ![Création d’un projet Visual Studio dans Visual Studio](./media/rest-api-claims-exchange-dotnet/aadb2c-ief-rest-api-netfw-create-project.png)

1. Dans la fenêtre **Nouvelle application web ASP.NET**, sélectionnez un modèle **API web** ou **Application API Azure**.

    ![Sélection d’un modèle d’API web dans Visual Studio](./media/rest-api-claims-exchange-dotnet/aadb2c-ief-rest-api-netfw-select-web-api.png)
...
1. Dans l’Explorateur de solutions, cliquez avec le bouton droit sur le dossier **Modèles**, sélectionnez **Ajouter**, puis **Classe**.

    ![Ajouter un élément de menu de classe sélectionné dans Visual Studio](./media/rest-api-claims-exchange-dotnet/aadb2c-ief-rest-api-netfw-add-model.png)

1. Nommez la classe `InputClaimsModel`, puis ajoutez les propriétés suivantes à la classe `InputClaimsModel` :
...
1. Créez un modèle, `OutputClaimsModel`, puis ajoutez les propriétés suivantes à la classe `OutputClaimsModel` :
...
1. Créez un autre modèle, `B2CResponseContent`, qui vous permet de générer des messages d’erreur de validation d’entrée. Ajoutez les propriétés suivantes à la classe `B2CResponseContent`, indiquez les références manquantes, puis enregistrez le fichier :
...
1. Dans l’Explorateur de solutions, cliquez avec le bouton droit sur le dossier **Contrôleurs**, sélectionnez **Ajouter**, puis **Contrôleur**.
    ...
    ![Ajout d’un nouveau contrôleur dans Visual Studio](./media/rest-api-claims-exchange-dotnet/aadb2c-ief-rest-api-netfw-add-controller-1.png)

1. Dans la fenêtre **Ajouter une structure**, cliquez sur **Web API Controller - Empty** (Contrôleur d’API web - Vide), puis sélectionnez **Ajouter**.

    ![Sélection du contrôleur d’API web 2 - Vide dans Visual Studio](./media/rest-api-claims-exchange-dotnet/aadb2c-ief-rest-api-netfw-add-controller-2.png)

1. Dans la fenêtre **Ajouter un contrôleur**, nommez le contrôleur **IdentityController**, puis sélectionnez **Ajouter**.

    ![Saisie du nom du contrôleur dans Visual Studio](./media/rest-api-claims-exchange-dotnet/aadb2c-ief-rest-api-netfw-add-controller-3.png)

    La génération de modèles automatiques crée un fichier nommé *IdentityController.cs* dans le dossier *Controllers*.

1. Si le fichier *IdentityController.cs* n’est pas déjà ouvert, double-cliquez dessus, puis remplacez le code dans le fichier par le code suivant :
...
1. Dans l’Explorateur de solutions, cliquez avec le bouton droit sur le projet **Contoso.AADB2C.API**, puis sélectionnez **Publier**.
    ...
    ![Publier dans Microsoft Azure App Service avec Visual Studio](./media/rest-api-claims-exchange-dotnet/aadb2c-ief-rest-api-netfw-publish-to-azure-1.png)

1. Dans la fenêtre **Publier**, sélectionnez **Microsoft Azure App Service**, puis cliquez sur **Publier**.

    ![Créer dans Microsoft Azure App Service avec Visual Studio](./media/rest-api-claims-exchange-dotnet/aadb2c-ief-rest-api-netfw-publish-to-azure-2.png)
    ...
    > [!TIP]
    > Pour plus d’informations sur la publication, consultez : [Création d’une application web ASP.NET dans Azure](../app-service/app-service-web-get-started-dotnet-framework.md).

1. Dans la zone **Nom de l’application web**, tapez un nom d’application unique (les caractères autorisés sont les plages a-z, 0-9, et les traits d’union (-)). L’URL de l’application web est http://<nom_app>.azurewebsites.NET, où *nom_app* est le nom de votre application web. Vous pouvez accepter le nom généré automatiquement, qui est unique.

    ![Configuration des propriétés App Service](./media/rest-api-claims-exchange-dotnet/aadb2c-ief-rest-api-netfw-publish-to-azure-3.png)

1. Pour commencer à créer des ressources Azure, sélectionnez **Créer**.
    Une fois l’application web ASP.NET créée, l’Assistant la publie sur Azure, puis la démarre dans le navigateur par défaut.

1. Copiez l’URL de l’application web.
...
1. Répétez l’étape précédente avec le fichier SignUpOrSignIn.xml.
...
1. Sélectionnez **Paramètres Azure AD B2C**, puis accédez à **Infrastructure d’expérience d’identité**.
    ...
    > [!NOTE]
    > **Exécuter maintenant** nécessite la préinscription d’au moins une application sur le locataire. Pour découvrir comment inscrire des applications, consultez les articles sur Azure AD B2C [Bien démarrer](tutorial-create-tenant.md) et [Inscription des applications](tutorial-register-applications.md).
...
2. Ouvrez **B2C_1A_signup_signin**, la stratégie personnalisée de partie de confiance que vous avez chargée, puis sélectionnez **Exécuter maintenant**.
    ...
    ![Page de stratégie personnalisée B2C_1A_signup_signin dans le Portail Azure](./media/rest-api-claims-exchange-dotnet/aadb2c-ief-rest-api-netfw-run.png)

3. Testez le processus en tapant **Test** dans la zone **Prénom**.
    Azure AD B2C affiche un message d’erreur en haut de la fenêtre.

    ![Test de la validation d’entrée du prénom sur la page de connexion d’inscription](./media/rest-api-claims-exchange-dotnet/aadb2c-ief-rest-api-netfw-test.png)
...
* Une fois que vous avez [pris en main les stratégies personnalisées](custom-policy-get-started.md), nous vous recommandons de créer votre scénario à l’aide de vos propres fichiers de stratégie personnalisée. Des [exemples de fichiers de stratégie](https://github.com/Azure-Samples/active-directory-b2c-custom-policy-starterpack/tree/master/scenarios/aadb2c-ief-rest-api-netfw) sont à votre disposition pour référence.
...
La tâche suivante consiste à sécuriser votre API RESTful à l’aide de l’authentification de base ou de certificat client. Pour en savoir plus sur la sécurisation de vos API, consultez les articles suivants :
...
Pour plus d’informations sur l’ensemble des éléments disponibles dans un profil technique RESTful, consultez la page [Référence : Profil technique RESTful](restful-technical-profile.md).
