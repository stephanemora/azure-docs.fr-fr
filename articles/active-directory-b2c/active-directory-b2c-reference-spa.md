---
title: Page unique et connectez-vous à l’aide d’un flux implicite - Azure Active Directory B2C | Microsoft Docs
description: Découvrez comment ajouter une inscription de page unique en utilisant le flux implicite OAuth 2.0 avec Azure Active Directory B2C.
...
ms.locfileid: "66508971"
---
# <a name="single-page-sign-in-using-the-oauth-20-implicit-flow-in-azure-active-directory-b2c"></a>Connexion de page unique utilisant le flux implicite OAuth 2.0 dans Azure Active Directory B2C

De nombreuses applications modernes ont un frontal application à page unique écrit principalement en JavaScript. Souvent, l’application est écrite en utilisant une infrastructure comme AngularJS, Ember.js ou Durandal.js. Les applications à page unique et d’autres applications JavaScript qui s’exécutent principalement dans un navigateur présentent certaines problématiques supplémentaires pour l’authentification :

- Les caractéristiques de sécurité de ces applications sont différents à partir d’applications web basées sur serveur traditionnelles.
...
- Redirections de navigateur pleine page à partir de l’application peuvent être INVASIVES sur l’expérience utilisateur.

Pour prendre en charge ces applications, Azure Active Directory B2C (Azure AD B2C) utilise le flux implicite OAuth 2.0. Le flux d’autorisation implicite OAuth 2.0 est décrit dans la [section 4.2 de la spécification OAuth 2.0](https://tools.ietf.org/html/rfc6749) . Dans un flux implicite, l’application reçoit des jetons directement du point de terminaison d’autorisation Azure AD, sans aucun échange de serveur à serveur. Tous les de logique d’authentification et de gestion de session sont effectués entièrement dans le client JavaScript, sans redirections de page supplémentaire.

Azure AD B2C étend le flux implicite OAuth 2.0 standard au-delà de la simple authentification et de la simple autorisation. Azure AD B2C introduit le [paramètre de stratégie](active-directory-b2c-reference-policies.md). Avec le paramètre de stratégie, vous pouvez utiliser OAuth 2.0 pour ajouter des stratégies à votre application, telles que des flux d’utilisateur d’inscription, de connexion et de gestion des profils. Dans les demandes HTTP d’exemple dans cet article, **fabrikamb2c.onmicrosoft.com** est utilisé comme exemple. Vous pouvez remplacer `fabrikamb2c` avec le nom de votre client si vous en avez pas et que vous avez créé un flux utilisateur.
...
| p | Oui | Stratégie à exécuter. Il s’agit du nom d’une stratégie (flux d’utilisateur) qui est créée dans votre locataire Azure AD B2C. La valeur du nom de la stratégie doit commencer par **b2c\_1\_** . |
| prompt | Non | Type d’interaction utilisateur demandée. Actuellement, la seule valeur possible est `login`. Ce paramètre oblige l’utilisateur à entrer leurs informations d’identification lors de cette requête. L’authentification unique ne prendre effet. |

À ce stade, il est demandé à l’utilisateur d’effectuer le flux de travail de la stratégie. L’utilisateur peut avoir à entrer son nom d’utilisateur et le mot de passe, inscrivez-vous avec une identité sociale, l’authentification pour le répertoire ou un nombre illimité d’étapes. Les actions de l’utilisateur dépendent de la façon dont le flux d’utilisateur est défini.
...
| Paramètre | Description |
| --------- | ----------- |
| error | Un code utilisé pour classer les types d’erreurs qui se produisent. |
...
La réception d’un jeton d’ID n’est pas suffisante pour authentifier l’utilisateur. Valider la signature du jeton d’ID et vérifier les revendications du jeton selon les besoins de votre application. Azure AD B2C utilise les [jetons Web JSON (JWT)](https://self-issued.info/docs/draft-ietf-oauth-json-web-token.html) et le chiffrement de clés publiques pour signer les jetons et vérifier leur validité.
...
Après la validation de la signature du jeton d’ID, plusieurs revendications nécessitent une vérification. Exemple :
...
| domain_hint |Obligatoire |Peut être `consumers` ou `organizations`.  Pour actualiser et obtenir des jetons dans un iframe masqué, inclut la `domain_hint` valeur dans la demande.  Extrayez la revendication `tid` du jeton d’ID d’une connexion précédente pour déterminer la valeur à utiliser.  Si la valeur de la revendication `tid` est `9188040d-6c67-4c5b-b112-36a304b66dad`, utilisez `domain_hint=consumers`.  Sinon, utilisez `domain_hint=organizations`. |
...
### <a name="successful-response"></a>Réponse correcte
Une réponse correcte à l’aide de `response_mode=fragment` ressemble à ceci :
...
### <a name="error-response"></a>Réponse d’erreur
Les réponses d’erreur peuvent également être envoyées à l’URI de redirection, pour que l’application puisse les traiter de façon appropriée.  Pour `prompt=none`, une erreur attendue se présente comme dans cet exemple :
...
Lorsque vous souhaitez déconnecter l’utilisateur de l’application, redirigez l’utilisateur vers Azure AD pour effectuer cette déconnexion. Si vous ne rediriger l’utilisateur, il peuvent être en mesure de s’authentifier de nouveau à votre application sans entrer à nouveau leurs informations d’identification, car ils ont une session unique valide l’authentification auprès d’Azure AD.

Vous pouvez simplement rediriger l’utilisateur vers le `end_session_endpoint` qui est répertorié dans le même document de métadonnées OpenID Connect décrit dans [Validation du jeton d’ID](#validate-the-id-token). Exemple :
