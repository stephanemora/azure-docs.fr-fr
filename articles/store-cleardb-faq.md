...
>
> Pour plus d’informations, également concernant les étapes suivantes, consultez [Modifications apportées aux plans de service ClearDB](https://www.navisite.com/blog/news/update-change-of-billing-notice-for-all-azure-cleardb-service-plans/).
...
* [Cluster MySQL s’exécutant sur une machine virtuelle Azure](https://github.com/azure/azure-quickstart-templates/tree/master/application-workloads/mysql/mysql-replication)
...
L’option de base de données par défaut est Titan, qui coûte 3,50 USD. Nous n'indiquons pas le coût lors de la création de la base de données, et vous pouvez acheter par erreur une base de données que vous n'aviez pas l'intention d'acquérir. Nous essayons de trouver un moyen d’améliorer l’expérience, mais en attendant, vous devez vérifier tous vos niveaux tarifaires sélectionnés pour l’application web et la base de données avant de cliquer sur **Créer** et commencer le déploiement des ressources.
...
## <a name="i-cant-see-my-cleardb-database-in-azure-portal"></a>Ma base de données ClearDB n'apparaît pas sur le portail Azure.
