...
>|----|----|----|------------|
>| TomcatErrorCount<br><br>Erreur globale Tomcat (déconseillé) | tomcat.global.error | Count | Nombre d’erreurs de requêtes traitées |
...
>| OldGenMemoryPoolBytes<br><br>Taille des données d’ancienne génération (déconseillé) | jvm.gc.live.data.size | Octets | Taille du pool de mémoire d’ancienne génération après un GC complet. |
>| jvm.gc.live.data.size | jvm.gc.live.data.size | Octets | Taille du pool de mémoire d’ancienne génération après un GC complet. |
>| OldGenPromotedBytes<br><br>Promouvoir à la taille des données d’ancienne génération (déconseillé) | jvm.gc.memory.promoted | Octets | Nombre d’augmentations positives de la taille du pool de mémoire d’ancienne génération avant l’application du GC jusqu’au terme de cette application. |
...
>| YoungGenPromotedBytes<br><br>Promouvoir à la taille des données de nouvelle génération (déconseillé) | jvm.gc.memory.allocated | Octets | Incrémenté pour une augmentation de la taille du pool de mémoire de nouvelle génération après un GC avant le suivant. |
...
>| GCPauseTotalCount<br><br>Nombre d’interruptions du GC (déconseillé) | jvm.gc.pause (nombre total) | Count | Nombre total de GC après le démarrage de cette JMV, y compris les GC de nouvelle et d’ancienne génération. |
...
>| GCPauseTotalTime<br><br>Durée totale de pause du GC (déconseillé) | jvm.gc.pause (durée totale) | Millisecondes | Durée totale du GC utilisée après le démarrage de cette JMV, y compris les GC de nouvelle et d’ancienne génération. |
...
>| Nom | Nom de l’indicateur de performance Spring Actuator | Unité | Détails |
>|----|----|----|------------|
>| TomcatSentBytes<br><br>Nombre total d’octets envoyés par Tomcat (déconseillé) | tomcat.global.sent | Octets | Quantité de données envoyées par le serveur web Tomcat |
>| tomcat.global.sent | tomcat.global.sent | Octets | Quantité de données envoyées par le serveur web Tomcat |
>| TomcatReceivedBytes<br><br>Nombre total d’octets reçus par Tomcat (déconseillé) | tomcat.global.received | Octets | Quantité de données reçues par le serveur web Tomcat |
>| tomcat.global.received | tomcat.global.received | Octets | Quantité de données reçues par le serveur web Tomcat |
>| TomcatRequestTotalTime<br><br>Durée totale des requêtes Tomcat (déconseillé) | tomcat.global.request (durée totale) | Millisecondes | Durée totale de traitement des requêtes par le serveur web Tomcat |
>| TomcatRequestTotalCount<br><br>Nombre total des requêtes Tomcat (déconseillé) | tomcat.global.request (nombre total) | Count | Nombre total de requêtes traitées par le serveur web Tomcat |
...
>| TomcatRequestMaxTime<br><br>Durée maximale des requêtes Tomcat (déconseillé) | tomcat.global.request.max | Millisecondes | Durée maximale de traitement d’une requête par le serveur web Tomcat |
...
>| Nom | Nom de l’indicateur de performance Spring Actuator | Unité | Détails |
>|----|----|----|------------|
>| TomcatSessionActiveMaxCount<br><br>Nombre maximal de sessions actives Tomcat (déconseillé) | tomcat.sessions.active.max | Count | Nombre maximal de sessions actives simultanément |
>| tomcat.sessions.active.max | tomcat.sessions.active.max | Count | Nombre maximal de sessions actives simultanément |
>| TomcatSessionAliveMaxTime<br><br>Durée maximale des sessions actives Tomcat (déconseillé) | tomcat.sessions.alive.max | Millisecondes | Durée maximale (en secondes) pendant laquelle une session ayant expiré a été active |
...
>| TomcatSessionCreatedCount<br><br>Nombre de sessions Tomcat créées (déconseillé) | tomcat.sessions.created | Count | Nombre de sessions créées |
>| tomcat.sessions.created | tomcat.sessions.created | Count | Nombre de sessions créées |
>| TomcatSessionExpiredCount<br><br>Nombre de sessions Tomcat ayant expiré (déconseillé) | tomcat.sessions.expired | Count | Nombre de sessions qui ont expiré |
>| tomcat.sessions.expired | tomcat.sessions.expired | Count | Nombre de sessions qui ont expiré |
>| TomcatSessionRejectedCount<br><br>Nombre de sessions Tomcat rejetées (déconseillé) | tomcat.sessions.rejected | Count | Nombre de sessions qui n’ont pas été créées parce que le nombre maximal de sessions actives a été atteint. |
