...
>[!Note]
> Azure Spring Cloud est actuellement en préversion.  Ces commandes peuvent être modifiées ou supprimées dans une version ultérieure.
...
| Paramètres facultatifs | |
| --- | ---: |
| --defer | Stocker temporairement l’objet dans le cache local au lieu de l’envoyer à Azure.  Utilisez `az cache` pour afficher/effacer. |
