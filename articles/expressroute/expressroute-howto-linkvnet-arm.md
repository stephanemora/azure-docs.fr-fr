...
La prise en charge de FastPath pour l’appairage de réseaux virtuels est désormais en préversion publique.

### <a name="fastpath-and-virtual-network-peering"></a>FastPath et appairage de réseaux virtuels

Avec FastPath et l’appairage de réseaux virtuels, vous pouvez activer la connectivité ExpressRoute directement aux machines virtuelles dans un réseau virtuel local ou appairé, en contournant la passerelle de réseau virtuel ExpressRoute dans le chemin des données.

Pour vous inscrire à cette préversion, exécutez la commande Azure PowerShell suivante dans l’abonnement Azure cible :
