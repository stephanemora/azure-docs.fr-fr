---
title: Arguments de ligne de commande de Microsoft OPC Publisher
description: Cet article donne une vue d’ensemble des Arguments de ligne de commande de Microsoft OPC Publisher
author: jehona-m
ms.author: jemorina
...
ms.locfileid: "104787441"
---
# <a name="command-line-arguments"></a>Arguments de ligne de commande

Vous avez à votre disposition plusieurs arguments de ligne de commande pour définir des paramètres globaux pour OPC Publisher.

## <a name="opc-publisher-command-line-arguments-for-version-25-and-below"></a>Arguments de ligne de commande d’OPC Publisher version 2.5 et versions antérieures

* Utilisation : opcpublisher.exe \<applicationname> [\<iothubconnectionstring>] [\<options>]
    
* ApplicationNamea : nom de l’application agent utilisateur OPC à utiliser, obligatoire                        Le nom de l’application est également utilisé pour inscrire le serveur de publication sous ce nom dans le                        registre des appareils IoT Hub.
    
* iothubconnectionstring : chaîne de connexion du propriétaire de l’IoT Hub, facultative. En règle générale, vous spécifiez la chaîne de connexion du propriétaire de l’IoT Hub uniquement au premier démarrage de l’application. La chaîne de connexion est chiffrée et stockée dans le magasin de certificats de la plateforme.
Lors des appels suivants, elle est lue et réutilisée à partir de là. Si vous spécifiez la chaîne de connexion à chaque démarrage, l’appareil créé pour l’application dans le registre des appareils IoT Hub est supprimé et recréé à chaque fois.
    
Il existe plusieurs variables d’environnement utilisables pour contrôler l’application :
```
...
## <a name="opc-publisher-command-line-arguments-for-version-26-and-above"></a>Arguments de ligne de commande d’OPC Publisher pour la version 2.6 et versions ultérieures
```
      --pf, --publishfile=VALUE
                                       the filename to configure the nodes  to publish.
...
Vous trouverez des ressources supplémentaires dans les dépôts GitHub :
