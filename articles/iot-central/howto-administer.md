...
- Utiliser des jetons d’accès pour les outils de développement
- Personnaliser l’interface utilisateur de votre application
- Personnaliser les liens d’aide dans l’application
- Gérer IoT Central par programmation
...
Si votre administrateur a créé un thème personnalisé pour votre application, cette page inclut une option pour masquer le **nom de l’Application** dans l’interface utilisateur. Cela est utile si le logo d’application dans le thème personnalisé inclut le nom de l’application. Pour plus d’informations, consultez [personnaliser Azure IoT Central UI](./howto-customize-ui.md).
...
Sélectionnez **copie**. Dans la boîte de dialogue, entrez les détails de la nouvelle application avec paiement à l’utilisation. Puis sélectionnez **copie** pour confirmer que vous souhaitez continuer. En savoir plus sur les champs de ce formulaire dans le guide de démarrage rapide [Créer une application](quick-deploy-iot-central.md).

![Page Paramètres de l’application](media/howto-administer/appcopy2.png)

Une fois l’opération de copie réussit, vous pouvez accéder à la nouvelle application à l’aide de la liaison.
...
Utilisez le bouton **Supprimer** pour supprimer définitivement votre application IoT Central. Cette action supprime définitivement toutes les données qui sont associé à l’application.
...
### <a name="application-builder"></a>Générateur d'application

Les utilisateurs dotés du rôle de **Concepteur d’applications** peuvent tout faire dans une application, sauf l’administrer. Générateurs peuvent créer, modifier, supprimer des appareils et des modèles de périphériques, gérer les ensembles d’appareils et exécuter les tâches et analytique. Les concepteurs n’auront pas accès à la section **Administration** de l’application.

### <a name="application-operator"></a>Opérateur d'application

Les utilisateurs dans le rôle d’**Opérateur d’application** ne peuvent ni apporter des modifications aux modèles d’appareils, ni administrer l’application. Opérateurs peuvent ajouter et supprimer des appareils, gérer les ensembles d’appareils et exécuter les tâches et analytique. Les opérateurs n’auront pas accès aux pages **Concepteur d’application** et **Administration**.
...
- **Version d’évaluation** applications sont gratuites pendant sept jours avant leur expiration. Elles peuvent être passées en paiement à l’utilisation à tout moment avant leur expiration.
- **Paiement à l’utilisation** applications sont facturées par périphérique, avec les cinq premiers appareils gratuits.
...
## <a name="export-data"></a>Exporter les données
...
Générer des jetons d’accès pour les utiliser dans les outils de développeurs. Actuellement le seul développeur disponible est l’Explorateur IoT Central pour les messages de dispositif de surveillance et les modifications dans les propriétés et les paramètres. En savoir plus sur l’[explorateur IoT Central](howto-use-iotc-explorer.md).

## <a name="customize-your-application"></a>Personnaliser votre application

Pour plus d’informations sur la modification des couleurs et des icônes dans votre application, consultez [personnaliser Azure IoT Central UI](./howto-customize-ui.md).

## <a name="customize-help"></a>Personnaliser l’aide

Pour plus d’informations sur l’ajout de liens d’aide personnalisé dans votre application, consultez [personnaliser Azure IoT Central UI](./howto-customize-ui.md).

## <a name="manage-programatically"></a>Gérer par programmation

Des packages de kit de développement logiciel (SDK) Azure Resource Manager d’IoT Central sont disponibles pour Node, Python, C#, Ruby, Java et Go. Vous pouvez utiliser ces packages à créer, répertorier, mettre à jour ou supprimer les applications IoT Central. Les packages incluent les programmes d’assistance pour gérer l’authentification et la gestion des erreurs.
...
Pour plus d’informations, consultez les packages et les référentiels GitHub suivants :

| Langue | Dépôt | Package |
| ---------| ---------- | ------- |
...
| Accéder à | [https://github.com/Azure/azure-sdk-for-go](https://github.com/Azure/azure-sdk-for-go) | [https://github.com/Azure/azure-sdk-for-go](https://github.com/Azure/azure-sdk-for-go)
