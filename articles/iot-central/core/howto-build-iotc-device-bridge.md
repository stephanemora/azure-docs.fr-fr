...
Le pont d’appareil IoT Central est une solution open source dans GitHub. Il utilise un modèle Azure Resource Manager personnalisé pour déployer plusieurs ressources sur votre abonnement Azure, notamment une application de fonction dans Azure Functions.
...
* Assurez-vous que votre application inclut une fonction de décodeur qui convertit automatiquement la charge utile de vos messages d'appareil au format JSON avant qu'elle ne soit envoyée à la fonction : **Application > Fonctions de charge utile > Décodeur**.
...
Une fois l'intégration définie, ajoutez le code suivant avant l'appel à `handleMessage` sur la ligne 21 du fichier *IoTCIntegration/index.js* de votre application de fonction. Ce code convertit le corps de votre intégration HTTP au format attendu.
