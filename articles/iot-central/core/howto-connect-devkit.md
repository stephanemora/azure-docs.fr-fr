...
description: En tant que développeur d’appareils, découvrez comment connecter un appareil DevKit IoT MXChip à votre application Azure IoT Central en utilisant IoT Plug-and-Play (préversion).
author: liydu
ms.author: liydu
ms.date: 12/03/2019
ms.topic: how-to
...
Cet article vous montre comment connecter un appareil MXChip IoT DevKit (DevKit) à une application Azure IoT Central. L’appareil utilise le modèle certifié IoT Plug-and-Play (préversion) pour l’appareil DevKit afin de configurer sa connexion à IoT Central.
...
- Une application IoT Central. Vous pouvez suivre les étapes de la section [Créer une application IoT Central](./quick-deploy-iot-central.md).

## <a name="get-device-connection-details"></a>Obtenir des informations relatives à la connexion de l’appareil

1. Dans votre application Azure IoT Central, sélectionnez l’onglet **Modèles d’appareil**, puis sélectionnez **+ Nouveau**. Dans la section **Utiliser un modèle d’appareil préconfiguré**, sélectionnez **MXChip IoT DevKit**.

    ![Modèle d’appareil pour MXChip IoT DevKit](media/howto-connect-devkit/device-template.png)

1. Sélectionnez **Suivant : Personnaliser**, puis **Créer**.

1. Sélectionnez l’onglet **Appareils**. Dans la liste des appareils, sélectionnez **MXChip IoT DevKit**, puis **+ Nouveau** pour créer un appareil à partir d’un modèle.

    ![Nouvel appareil](media/howto-connect-devkit/new-device.png)

1. Dans la fenêtre contextuelle, entrez **l’ID de l’appareil** en tant que `SampleDevKit` et le**nom de l’appareil** en tant que `MXChip IoT DevKit - Sample`. Vérifiez que l’option **Simulé** est bien désactivée. Sélectionnez ensuite **Créer**.

    ![ID et nom de l’appareil](media/howto-connect-devkit/device-id-name.png)

1. Sélectionnez l’appareil que vous venez de créer, puis sélectionnez **Connecter**. Notez **l’étendue de l’ID**, **l’ID de l’appareil** et la **Clé primaire**. Vous aurez besoin de ces valeurs plus loin dans cet article.
...
1. Téléchargez la dernière version du [microprogramme Azure IoT Central Plug and Play (préversion) prédéfini](https://github.com/Azure-Samples/mxchip-iot-devkit-pnp/raw/master/bin/iotc_devkit.bin) pour l’appareil DevKit à partir de GitHub.
