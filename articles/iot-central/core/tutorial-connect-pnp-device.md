...
> [!div class="checklist"]
> * Utilisez Visual Studio Code pour créer un appareil IoT Plug-and-Play (préversion) avec un modèle de fonctionnalité d’appareil.
...
1. Connectez-vous à l’application IoT Central que vous avez créée avec le modèle **Application personnalisée > Application en préversion** dans le guide de démarrage rapide [Créer une application Azure IoT Central](./quick-deploy-iot-central.md).
...
Dans ce tutoriel, vous avez découvert comment connecter un appareil IoT Plug-and-Play (préversion) qui a été généré à partir d’un modèle de fonctionnalité d’appareil du dépôt de modèles publics.
