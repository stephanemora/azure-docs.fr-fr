...
Vous pouvez générer des insights métier hautement personnalisés, comme déterminer l’évolution de l’efficacité des machines ou prédire l’utilisation future de l’énergie dans un atelier de fabrication en créant des pipelines d’analyse personnalisés pour traiter vos données de télémétrie IoT brutes et en stockant le résultat final. Vous pouvez créer des exportations de données dans votre application IoT Central pour exporter les données de télémétrie, les propriétés et le cycle de vie des appareils, ainsi que les informations sur les modifications des modèles d’appareil vers d’autres services Azure. De cette manière, vous pouvez analyser, stocker et visualiser les données dans les outils qui vous intéressent le plus. Apprenez-en plus sur l’exportation de données ici.

## <a name="build-custom-iot-solutions-and-integrations-with-apis"></a>Générer des solutions et des intégrations IoT personnalisées avec des API
Vous pouvez générer des solutions IoT comme des applications auxiliaires mobiles capables de contrôler à distance des appareils et configurer de nouveaux appareils, ou des intégrations personnalisées avec des applications métier existantes permettant d’interagir avec vos appareils IoT et des données adaptées à votre entreprise. Vous pouvez utiliser IoT Central comme pierre de voûte de la modélisation, de l’intégration, de la gestion et de l’accès aux données des appareils. Découvrez plus en détail l’API publique dans ce module de formation.
