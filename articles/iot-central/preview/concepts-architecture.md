...
Pour plus d’informations sur la façon dont les appareils se connectent à votre application Azure IoT Central, consultez [Connectivité des appareils](overview-iot-central-get-connected.md).
...
- Modèles d’appareils pour décrire les fonctionnalités d'un appareil IoT Edge, par exemple :
  ...
  Pour plus d'informations, voir le tutoriel [Créer un modèle d’appareil IoT Edge](./tutorial-define-edge-device-type.md).
...
Pour plus d’informations sur la connectivité des appareils dans Azure IoT Central, consultez [Connectivité des appareils](overview-iot-central-get-connected.md).
...
Dans une application Azure IoT Central, vous pouvez [créer et exécuter des travaux](../core/howto-run-a-job.md?toc=/azure/iot-central/preview/toc.json&bc=/azure/iot-central/preview/breadcrumb/toc.json) pour gérer des appareils connectés. Ces travaux vous permettent d’effectuer des mises à jour en bloc sur les propriétés ou les paramètres des appareils, ou d’exécuter des commandes. Par exemple, vous pouvez créer un travail pour augmenter la vitesse du ventilateur pour plusieurs distributeurs réfrigérés.
...
Maintenant que vous avez découvert l’architecture d’Azure IoT Central, l’étape suivante suggérée est de découvrir la [connectivité des appareils](overview-iot-central-get-connected.md) dans Azure IoT Central.
