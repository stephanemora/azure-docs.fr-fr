...
Vous pouvez rapidement commencer à créer votre solution à l’aide d’un appareil IoT Plug-and-Play certifié. Consultez la liste dans le [catalogue d’appareils Azure IOT](https://catalog.azureiotsolutions.com/alldevices). IoT Central s’intègre au catalogue d’appareils ; ainsi, vous pouvez importer un modèle de capacité d’appareil parmi les appareils IoT Plug-and-Play certifiés. Pour créer un modèle d’appareil à partir de l’un de ces appareils dans IoT Central :

1. Accédez à la page **Modèles d’appareil** dans votre application IoT Central.
1. Sélectionnez **+ Nouveau**, puis sélectionnez les appareils certifiés IoT Plug-and-Play dans le catalogue. IoT Central crée un modèle d’appareil basé sur ce modèle de capacité d’appareil.
...
Ce tutoriel vous explique, en tant que constructeur, comment utiliser un modèle d’appareil de passerelle pour définir un nouveau type d’appareil IoT dans votre application IoT Central. 
...
* Envoie la télémétrie, par exemple les données de détection de température et de présence
...
Voici comment créer des modèles d’appareil IoT : 

1. Dans le volet de navigation gauche, sélectionnez **Modèles d’appareil**. Ensuite, sélectionnez **+ Nouveau**, puis sélectionnez la vignette **Appareil IoT** et la vignette du capteur de présence. Sélectionnez **Suivant : Personnaliser**.

   ![Capture d’écran de la page Modèles d’appareils, avec mise en évidence de certaines options](./media/tutorial-define-iot-device-type/gateway-downstream-new.png)

1. Dans la page **Vérifier**, sélectionnez **Créer**. 

   ![Capture d’écran de la page Vérifier](./media/tutorial-define-iot-device-type/gateway-downstream-review.png)

1. Un modèle d’appareil est créé. 

   ![Capture d’écran du nouveau modèle d’appareil](./media/tutorial-define-iot-device-type/occupancy-sensor.png)

Voici comment créer un modèle d’appareil pour le capteur S1 :

1. Dans le volet de navigation gauche, sélectionnez **Modèles d’appareil**. Ensuite, sélectionnez **+ Nouveau**, puis sélectionnez la vignette **Appareil IoT** et sélectionnez la vignette du capteur de présence. Sélectionnez **Suivant : Personnaliser**.

   ![Capture d’écran de la page Modèles d’appareils, avec mise en évidence de certaines options](./media/tutorial-define-iot-device-type/s1-sensor.png)

1. Dans la page **Vérifier**, sélectionnez **Créer**. 

   ![Capture d’écran de la page Vérifier](./media/tutorial-define-iot-device-type/s1-review.png)

1. Un modèle d’appareil est créé. 

   ![Capture d’écran du nouveau modèle d’appareil](./media/tutorial-define-iot-device-type/s1-template.png)

## <a name="create-an-iot-gateway-device-template"></a>Créer un modèle d’appareil de passerelle IoT

Vous pouvez choisir de créer un modèle d’appareil de passerelle IoT. L’appareil de passerelle a des relations avec les appareils en aval qui se connectent à IoT Central via l’appareil de passerelle. 
...
Les appareils IoT peuvent se connecter à un appareil de passerelle IoT.

![Diagramme de la relation entre l’appareil de passerelle et les appareils en aval](./media/tutorial-define-iot-device-type/gatewaypattern.png)

En tant que constructeur, vous pouvez créer et modifier des modèles d’appareil de passerelle IoT dans votre application. Après avoir publié un modèle d’appareil, vous pouvez connecter des appareils réels qui implémentent le modèle d’appareil.

### <a name="select-a-device-template-type"></a>Sélectionner un type de modèle d’appareil 

Pour ajouter un nouveau modèle d’appareil à votre application :

1. Dans le volet de gauche, sélectionnez l’onglet **Modèles d’appareil**.

   ![Capture d’écran de la page Modèles d’appareils](./media/tutorial-define-iot-device-type/devicetemplate.png)

1. Sélectionnez **+ Nouveau** pour créer un modèle d’appareil.

   ![Capture d’écran de la page Modèles d’appareils, avec mise en évidence de Nouveau](./media/tutorial-define-iot-device-type/devicetemplatenew.png)

   ![Capture d’écran de la page Personnaliser l’appareil](./media/tutorial-define-iot-device-type/gateway-review.png)

1. Dans la page **Sélectionner un type de modèle**, sélectionnez **Azure IoT**, puis sélectionnez **Suivant : Personnaliser**.

   ![Capture d’écran de la page Sélectionner un type de modèle](./media/tutorial-define-iot-device-type/gateway-customize.png)

1. Cochez la case de la passerelle, puis sélectionnez **Créer**.

   ![Capture d’écran de la page Personnaliser l’appareil, avec mise en évidence de la passerelle](./media/tutorial-define-iot-device-type/gateway-review.png)

1. Dans la page Vérifier, sélectionnez **Créer**. 

1. Entrez le nom du modèle de passerelle, **Modèle de passerelle Smart Building**. Sélectionnez la vignette **Personnalisé**.

1. Ajoutez une interface standard **Informations sur l’appareil**.

### <a name="add-relationships"></a>Ajouter des relations

Vous pouvez ajouter des relations en aval aux modèles de capacité d’appareil pour les appareils que vous connectez à un appareil de passerelle.

Créez des relations aux modèles de fonctionnalité d’appareil en aval. Sélectionnez **Enregistrer**.

![Capture d’écran du Modèle de passerelle Smart Building, avec mise en évidence de différentes options](./media/tutorial-define-iot-device-type/gateway-occupancy-s1-rel.png)
...
Si vous sélectionnez **Générer les vues par défaut**, vous pouvez générer les tableaux de bord **Vue d’ensemble** et **À propos de**. 
...
## <a name="create-a-gateway-simulated-device"></a>Créer un appareil simulé de passerelle

À partir de l’Explorateur d’appareils, créez une passerelle smart building simulée. 

![Capture d’écran de la boîte de dialogue Créer un appareil](./media/tutorial-define-iot-device-type/smartbuildingdevice.png)
...
![Capture d’écran de la boîte de dialogue Créer un appareil](./media/tutorial-define-iot-device-type/occupancydevice.png)

À partir de l’Explorateur d’appareils, créez un capteur S1 simulé. 

![Capture d’écran de la boîte de dialogue Créer un appareil](./media/tutorial-define-iot-device-type/s1device.png)

## <a name="add-downstream-devices-relationships-to-a-gateway-device"></a>Ajouter des relations d’appareils en aval à un appareil de passerelle

Sélectionnez S1 Sensor (Capteur S1) et Occupancy Sensor (Capteur de présence), puis sélectionnez **Connecter à la passerelle**. 

![Capture d’écran du capteur de présence, avec mise en évidence de l’élément Connecter à la passerelle](./media/tutorial-define-iot-device-type/connecttogateway.png)
