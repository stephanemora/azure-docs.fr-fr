...
* présente diverses propriétés d’appareil, telles que la version du microprogramme et le numéro de série ;
* comporte divers paramètres, comme la température de la cible.
...
2. Entrez l’adresse e-mail et le mot de passe que vous utilisez pour accéder à votre abonnement Azure :
    ...
    ![Entrer votre compte d’entreprise](./media/tutorial-define-device-type-experimental/sign-in.png)

3. Pour commencer à créer une application Azure IoT Central, cliquez sur **Nouvelle application** :

    ![Page du Gestionnaire d’applications d’Azure IoT Central](./media/tutorial-define-device-type-experimental/iotcentralhome.png)
...
4. Pour créer une nouvelle application Azure IoT Central :
    ...
    * Choisissez **Version d’évaluation**. Vous n’avez pas besoin d’un abonnement Azure pour créer une application d’évaluation.
       ...
       Pour plus d’informations sur les répertoires et les abonnements, consultez le guide de démarrage rapide [Créer une application](quick-deploy-iot-central-experimental.md?toc=/azure/iot-central-experimental/toc.json&bc=/azure/iot-central-experimental/breadcrumb/toc.json).
    ...
    * Cliquez sur **Créer**.
    ...
    ![Page de création d’une application Azure IoT Central](./media/tutorial-define-device-type-experimental/iotcentralcreate.png)

    Pour plus d’informations, consultez [Créer une application](quick-deploy-iot-central-experimental.md?toc=/azure/iot-central-experimental/toc.json&bc=/azure/iot-central-experimental/breadcrumb/toc.json).
...
Pour ajouter un nouveau modèle d’appareil à votre application, vous devez accéder à la page **Modèles d’appareil**. Pour cela, cliquez sur **Modèles d’appareil** dans le menu de navigation de gauche.

![Page Modèles d’appareil](./media/tutorial-define-device-type-experimental/devicetemplates.png)
...
1. Dans la page **Modèles d’appareil**, cliquez sur **+ Nouveau** :

    ![Page Modèles d’appareil, Créer un modèle d’appareil](./media/tutorial-define-device-type-experimental/newtemplate.png)

3. Dans la page **Modèle d’appareil personnalisé**, entrez **Climatiseur connecté** comme nom pour votre appareil, puis cliquez sur **Créer**. Vous pouvez également charger une image de votre appareil qui est visible par les opérateurs dans Device Explorer :

    ![Appareil personnalisé](./media/tutorial-define-device-type-experimental/createcustomdevice.png)
...
4. Dans le modèle d’appareil **Climatiseur connecté**, vérifiez que vous vous trouvez sous l’onglet **Mesures** où vous définissez la télémétrie. Chaque modèle d’appareil que vous définissez comporte différents onglets dans lesquels vous pouvez :
    ...
    * Personnaliser le _tableau de bord_ de l’appareil pour vos opérateurs.
    ...
    ![Mesures de climatiseur](./media/tutorial-define-device-type-experimental/airconmeasurements.png)

    > [!NOTE]
    > Pour renommer le modèle d’appareil, cliquez sur son nom en haut de la page.

5. Pour ajouter la mesure de télémétrie de température, cliquez sur **+ Nouvelle mesure**. Puis choisissez **Telemetry** (Télémétrie) comme type de mesure :

    ![Mesures de climatiseur connecté](./media/tutorial-define-device-type-experimental/airconmeasurementsnew.png)

6. Chaque type de télémétrie que vous définissez pour un modèle d’appareil inclut des [options de configuration](howto-set-up-template-experimental.md?toc=/azure/iot-central-experimental/toc.json&bc=/azure/iot-central-experimental/breadcrumb/toc.json) telles que :
    ...
    * les paramètres de simulation.
    ...
    Vous pouvez également choisir une couleur pour l’affichage de la télémétrie. Pour enregistrer la définition de la télémétrie, cliquez sur **Enregistrer** :

    ![Configuration de la simulation de température](./media/tutorial-define-device-type-experimental/temperaturesimulation.png)
...
7. Après un court instant, l’onglet **Mesures** affiche un graphique de la télémétrie de température de votre climatiseur connecté simulé. Utilisez les contrôles pour gérer la visibilité ou l’agrégation, ou pour modifier la définition de télémétrie :
    ...
    ![Visualisation de la simulation de température](./media/tutorial-define-device-type-experimental/viewsimulation.png)

8. Vous pouvez également personnaliser le graphique à l’aide des contrôles **Ligne**, **Empilé** et **Modifier l’intervalle de temps** :

    ![Personnalisation du graphique](./media/tutorial-define-device-type-experimental/customizechart.png)
...
1. Pour ajouter la mesure d’événement **Erreur du moteur du ventilateur**, cliquez sur **+ Nouvelle mesure**. Puis choisissez **Event** (Événement) comme type de mesure :

    ![Mesures de climatiseur connecté](./media/tutorial-define-device-type-experimental/eventnew.png)

2. Chaque type d’événement que vous définissez pour un modèle d’appareil inclut des [options de configuration](howto-set-up-template-experimental.md?toc=/azure/iot-central-experimental/toc.json&bc=/azure/iot-central-experimental/breadcrumb/toc.json) telles que :
   ...
   * la gravité.
    ...
    Pour enregistrer la définition de l’événement, cliquez sur **Enregistrer** :

    ![Configuration de la mesure d’événement](./media/tutorial-define-device-type-experimental/eventconfiguration.png)
...
3. Après un court instant, l’onglet **Mesures** affiche un graphique des événements générés aléatoirement par votre climatiseur connecté simulé. Utilisez les contrôles pour gérer la visibilité ou pour modifier la définition d’événement :
    ...
    ![Visualisation de la simulation d’événement](./media/tutorial-define-device-type-experimental/eventview.png)

1. Pour voir des détails supplémentaires sur l’événement, cliquez sur l’événement dans le graphique :

    ![Visualisation des détails de l’événement](./media/tutorial-define-device-type-experimental/eventviewdetail.png)
...
1. Pour ajouter une mesure d’état **Mode du ventilateur**, cliquez sur **+ Nouvelle mesure**. Puis choisissez **État** comme type de mesure :

    ![Mesures d’état de climatiseur connecté](./media/tutorial-define-device-type-experimental/statenew.png)

2. Chaque type d’état que vous définissez pour un modèle d’appareil inclut des [options de configuration](howto-set-up-template-experimental.md?toc=/azure/iot-central-experimental/toc.json&bc=/azure/iot-central-experimental/breadcrumb/toc.json) telles que :
   ...
   * la couleur de chaque valeur.
    ...
    Pour enregistrer la définition de la mesure d’état, cliquez sur **Enregistrer** :

    ![Configuration de la mesure d’état](./media/tutorial-define-device-type-experimental/stateconfiguration.png)
...
* Les _propriétés_ vous permettent de définir les métadonnées qui sont associées à votre appareil. Il existe deux catégories de propriété :
    ...
    * Les _propriétés d’appareil_ vous permettent d’autoriser un appareil à envoyer des valeurs de propriété à votre application. Ces propriétés sont uniquement modifiables par l’appareil. Les propriétés d’appareil sont en lecture seule pour un opérateur. Dans le cas d’un climatiseur connecté, la version du microprogramme et le numéro de série de l’appareil sont des propriétés d’appareil signalées par ce dernier.
    ...
    Pour plus d’informations, consultez la section [Propriétés](howto-set-up-template-experimental.md#properties) dans le guide pratique de configuration d’un modèle d’appareil.
...
2. Vous pouvez créer différents types de paramètres, tels que des valeurs numériques ou du texte. Cliquez sur **Nombre** pour ajouter un paramètre de type valeur numérique à votre appareil.
...
3. Pour configurer votre paramètre **Température réglée**, utilisez les informations du tableau suivant :
    ...
    ![Configuration du paramètre Température réglée](./media/tutorial-define-device-type-experimental/configuresetting.png)
...
4. Vous pouvez personnaliser la disposition de l’onglet **Paramètres** en déplaçant et en redimensionnant les vignettes de paramètres :
    ...
    ![Personnalisation de la disposition des paramètres](./media/tutorial-define-device-type-experimental/settingslayout.png)
...
1. Vous pouvez créer différents types de propriétés d’appareil, comme des nombres ou du texte. Pour ajouter une propriété d’emplacement à votre modèle d’appareil, choisissez **Emplacement**. Pour configurer votre propriété d’emplacement, utilisez les informations du tableau suivant :
    ...
    ![Configuration des propriétés de l’appareil](./media/tutorial-define-device-type-experimental/configureproperties.png)
...
1. Pour configurer votre propriété de dernière date de service, utilisez les informations du tableau suivant :
    ...
    ![Configuration des propriétés de l’appareil](./media/tutorial-define-device-type-experimental/configureproperties2.png)
...
1. Pour configurer votre version du microprogramme, utilisez les informations du tableau suivant :
    ...
    ![Configurer la version du microprogramme](./media/tutorial-define-device-type-experimental/configureproperties3.png)
...
1. Pour configurer le numéro de série, utilisez les informations du tableau suivant :
    ...
    ![Configurer le numéro de série](./media/tutorial-define-device-type-experimental/configureproperties4.png)
...
1. Pour configurer votre nouvelle commande, utilisez les informations du tableau suivant :
    ...
    ![Préparation de l’ajout d’un paramètre](./media/tutorial-define-device-type-experimental/commandsecho1.png)
...
1. Cliquez sur **Graphique en courbes** pour ajouter ce composant au **tableau de bord**.
...
1. Configurez le composant **Graphique en courbes** en utilisant les informations du tableau suivant :
    ...
    | Mesures     | Température (cliquez sur **Visibilité** à côté de **Température**) |

    ![Paramètres de graphique en courbes](./media/tutorial-define-device-type-experimental/linechartsettings.png)

    Cliquez ensuite sur **Enregistrer**.

1. Cliquez sur le composant **Historique des événements** en utilisant les informations du tableau suivant :

    | Paramètre      | Valeur       |
    | ------------ | ----------- |
    | Intitulé        | Événements du moteur de ventilateur |
    | Intervalle de temps   | 30 dernières minutes |
    | Mesures     | Erreur du moteur du ventilateur (cliquez sur **Visibilité** à côté de **Erreur du moteur du ventilateur**) |

    ![Paramètres de graphique en courbes](./media/tutorial-define-device-type-experimental/dashboardeventchartsetting.png)
...
1. Configurez le composant **Historique des états** en utilisant les informations du tableau suivant :
    ...
    | Intervalle de temps   | 30 dernières minutes |
    | Mesures | Mode du ventilateur (cliquez sur **Visibilité** à côté de **Mode du ventilateur**) |

    ![Paramètres de graphique en courbes](./media/tutorial-define-device-type-experimental/dashboardstatechartsetting.png)

    Cliquez ensuite sur **Enregistrer**.

1. Pour ajouter les paramètres et propriétés de l’appareil au tableau de bord, choisissez **Paramètres et propriétés**. Cliquez sur **Ajouter/Supprimer** pour ajouter les paramètres ou les propriétés que vous aimeriez voir dans le tableau de bord.
...
1. Configurez le composant **Settings and Properties** (Paramètres et propriétés) en utilisant les informations du tableau suivant :
    ...
    ![Définir des paramètres de propriété de température](./media/tutorial-define-device-type-experimental/propertysettings4.png)
...
1. Vous voyez maintenant des données simulées pour votre climatiseur connecté dans le tableau de bord. Vous pouvez modifier les vignettes et la disposition dans le tableau de bord :
    ...
    ![Afficher le tableau de bord](./media/tutorial-define-device-type-experimental/dashboard.png)
...
* [Personnaliser la vue de l’opérateur](tutorial-customize-operator-experimental.md?toc=/azure/iot-central-experimental/toc.json&bc=/azure/iot-central-experimental/breadcrumb/toc.json)