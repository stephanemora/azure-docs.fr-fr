...
## <a name="trigger-a-workflow-when-a-rule-is-fired"></a>Déclencher un flux de travail quand une règle est déclenchée
...
1. Commencez par [créer une règle dans IoT Central](howto-create-telemetry-rules.md). Après avoir enregistré les conditions de la règle, choisissez **l’action Microsoft Flow**  comme nouvelle action. Un nouvel onglet ou une nouvelle fenêtre doit s’ouvrir dans votre navigateur, vous redirigeant dans Microsoft Flow.
...
1. Cliquez sur **Continuer**. Vous accédez au concepteur Microsoft Flow pour créer votre flux de travail. Le flux de travail comporte un déclencheur IoT Central où votre application et votre règle sont déjà renseignées.
...
1. (Facultatif) Renseignez les autres champs comme vous le souhaitez pour la création des appareils. Par exemple, choisissez si l’appareil est simulé ou non.
...
1. Choisissez votre application dans la liste déroulante. Vous avez maintenant besoin de l’ID d’appareil de l’appareil existant que vous voulez mettre à jour. Vous pouvez obtenir l’ID d’appareil auprès d’IoT Central dans **l’Explorateur d’appareils**.

    ![ID d’appareil dans l’Explorateur d’appareils d’IoT Central](./media/howto-add-microsoft-flow/iotcdeviceid.png)

1. À ce stade, vous pouvez mettre à jour le nom de l’appareil, et indiquer s’il s’agit ou non d’un appareil simulé. Pour mettre à jour des propriétés et des paramètres de l’appareil, vous devez sélectionner le modèle d’appareil de l’appareil que vous voulez mettre à jour dans la liste déroulante **Modèle d’appareil**. La vignette de l’action se développe pour montrer tous les paramètres et propriétés que vous pouvez mettre à jour.
...
1. Sélectionnez les propriétés et les paramètres que vous voulez mettre à jour. Dans le volet Contenu dynamique, sélectionnez l’entrée correspondante dans le déclencheur. Dans cet exemple, la valeur de Location (Emplacement) est propagée vers le bas pour mettre à jour la propriété Location de l’appareil.
    ...
    ![Volet de l’action de mise à jour dynamique d’un appareil dans Flow](./media/howto-add-microsoft-flow/flowupdatedevice.PNG)
...
2. Si vous recevez une erreur lors de l’utilisation d’un compte AAD, essayez en ouvrant Windows PowerShell et en exécutant les applets de commande suivantes en tant qu’administrateur.
    ``` PowerShell
