...
En savoir plus sur [l’exportation vers le Stockage Blob](howto-export-data-blob-storage.md?toc=/azure/iot-central-experimental/toc.json&bc=/azure/iot-central-experimental/breadcrumb/toc.json).
...
En savoir plus sur [l’exportation vers Event Hubs et Service Bus](howto-export-data-event-hubs-service-bus.md?toc=/azure/iot-central-experimental/toc.json&bc=/azure/iot-central-experimental/breadcrumb/toc.json).
...
2. Dans le menu de gauche, cliquez sur **Exportation de données continue**.
    ...
    ![Créer un Event Hub d’exportation de données continue](media/howto-export-data-experimental/export_menu.png)

3. Cliquez sur le bouton **+ Nouveau** dans le coin supérieur droit. Choisissez **Stockage Blob Azure**, **Azure Event Hubs** ou **Azure Service Bus** comme destination de l’exportation. 

    > [!NOTE] 
    > Le nombre maximal d’exportations par application est de cinq. 

    ![Créer une exportation de données continue](media/howto-export-data-experimental/export_new.png)
...
4. Dans la zone de liste déroulante, sélectionnez votre **compte de stockage/espace de noms Event Hubs/espace de noms Service Bus**. Vous pouvez également choisir la dernière option de la liste : **Entrer une chaîne de connexion**. 
    ...
    ![Créer un Event Hub d’exportation de données continue](media/howto-export-data-experimental/export_create.png)
...
5. (Facultatif) Si vous avez choisi **Entrer une chaîne de connexion**, une nouvelle zone vous permettant de coller votre chaîne de connexion s’affiche. Pour obtenir votre chaîne de connexion :
    - Pour votre compte de stockage, accédez à ce compte dans le Portail Azure.
        - Sous **Paramètres**, cliquez sur **Clés d’accès**
    ...
    - Pour Event Hubs ou Service Bus, accédez à l’espace de noms correspondant dans le Portail Azure.
        - Sous **Paramètres**, cliquez sur **Stratégies d’accès partagé**
...
6. Pour activer l’exportation des données continue, assurez-vous que l’option **Exportation de données** est définie sur **Activé**. Sélectionnez **Enregistrer**.
  ...
  ![Configurer l’exportation de données continue](media/howto-export-data-experimental/export_list.png)
...
> [Exporter des données vers le Stockage Blob Azure](howto-export-data-blob-storage.md?toc=/azure/iot-central-experimental/toc.json&bc=/azure/iot-central-experimental/breadcrumb/toc.json)

> [!div class="nextstepaction"]
> [Exporter des données vers Azure Event Hubs et Azure Service Bus](howto-export-data-event-hubs-service-bus.md?toc=/azure/iot-central-experimental/toc.json&bc=/azure/iot-central-experimental/breadcrumb/toc.json)

> [!div class="nextstepaction"]
> [Guide pratique pour visualiser vos données dans Power BI](howto-connect-powerbi.md?toc=/azure/iot-central-experimental/toc.json&bc=/azure/iot-central-experimental/breadcrumb/toc.json)
