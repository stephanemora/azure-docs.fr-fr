...
La section [Simulation](iot-accelerators-remote-monitoring-device-schema.md#simulation) du schéma du modèle d’appareil définit l’état interne d’un appareil simulé :
...
La section [CloudToDeviceMethods](iot-accelerators-remote-monitoring-device-schema.md#cloudtodevicemethods) du schéma du modèle d’appareil définit les méthodes auxquelles un appareil simulé répond.
...
L’article [Créer un appareil simulé](iot-accelerators-remote-monitoring-test.md) vous explique comment exécuter le service de simulation d’appareil en local. L’exécution du service en local facilite le débogage de vos appareils simulés avant de les déployer dans le cloud.
