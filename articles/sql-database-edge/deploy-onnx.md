---
title: Déployer et effectuer des prédictions avec ONNX dans la préversion de SQL Database Edge
description: Découvrez comment effectuer l’apprentissage d’un modèle, le convertir en ONNX, le déployer sur la préversion de SQL Database Edge, puis exécuter une prédiction native sur des données à l’aide du modèle ONNX chargé.
...
* Si vous n’avez pas déployé de module Azure SQL Database Edge, procédez de la manière décrite dans [Déployer SQL Database Edge en préversion à l’aide du Portail Azure](deploy-portal.md).
...
* Ouvrez Azure Data Studio et procédez comme suit pour installer les packages nécessaires pour ce démarrage rapide :
    ...
    1. Cliquez sur **Gérer les packages** puis, sous **Ajouter nouveau**, recherchez **sklearn**, puis installez le package scikit-Learn. 
    1. Installez également les packages **onnxmltools**, **onnxruntime**, **skl2onnx**et **SQLAlchemy**.
...
Stockez le modèle dans Azure SQL Database Edge, dans une table `models` d’une de base de données `onnx`. Dans la chaîne de connexion, spécifiez l’**adresse du serveur**, de **nom d’utilisateur** et le **mot de passe**.
...
Chargez les données dans Azure SQL Database Edge.
...
Avec le modèle dans Azure SQL Database Edge, exécutez une prédiction native (PREDICT) sur les données à l’aide du modèle ONNX chargé.
