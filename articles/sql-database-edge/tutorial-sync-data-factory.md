---
title: Synchroniser des données Azure SQL Database Edge à l’aide d’Azure Data Factory | Microsoft Docs
...
Si vous n’avez pas encore créé de base de données ou de table dans votre déploiement Azure SQL Database Edge, utilisez l’une de ces méthodes pour en créer une :

* Utiliser [SQL Server Management Studio](/sql/ssms/download-sql-server-management-studio-ssms/) ou [Azure Data Studio](/sql/azure-data-studio/download/) pour vous connecter à SQL Database Edge. Exécutez un script SQL pour créer la base de données et la table.
...
* Utilisez SQLPackage.exe pour déployer un fichier de package DAC sur le conteneur SQL Database Edge. Vous pouvez automatiser ce processus en spécifiant l’URI du fichier SqlPackage dans le cadre de la configuration des propriétés souhaitées du module. Vous pouvez également utiliser directement l’outil client SqlPackage.exe pour déployer un package DAC sur SQL Database Edge.
...
Exécutez ces commandes sur l’instance SQL Database Edge :
...
Dans cette section, vous créez un pipeline Azure Data Factory pour synchroniser des données entre une table dans Azure SQL Database Edge et le stockage Blob Azure.
...
7. Pour **Service lié**, sélectionnez **Nouveau**, puis effectuez ces étapes :
    ...
    2. Sous **Nom du serveur**, entrez les détails de votre serveur SQL Database Edge.
...
13. Dans la fenêtre **Nouveau jeu de données**, sélectionnez **Instance SQL Database Edge**, puis **Continuer**.
