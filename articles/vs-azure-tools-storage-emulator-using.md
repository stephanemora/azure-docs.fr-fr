...
Il est possible de lancer automatiquement l’émulateur de stockage quand vous exécutez ou déboguez votre service dans Visual Studio. Dans l’Explorateur de solutions, ouvrez le menu contextuel de votre projet **Azure**, puis choisissez **Propriétés**. Sous l’onglet **Développement**, dans la liste **Démarrer l’émulateur de stockage Microsoft Azure**, choisissez **Vrai** (si cette valeur n’est pas déjà définie).  Certains types de projets n’ont pas l’onglet **Développement**. Dans ce cas, vous pouvez activer ou désactiver le démarrage de l’émulateur de stockage en définissant l’élément `StartDevelopmentStorage` dans le fichier projet. Définissez-le sur **True** pour l’activer ou sur **False** pour le désactiver.  Par exemple, dans un projet Azure Functions, ouvrez le fichier projet pour le modifier et modifiez le code XML comme suit :
